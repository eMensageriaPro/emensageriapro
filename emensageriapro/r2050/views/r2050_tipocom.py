...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2050_tipocom')
    ...
    if r2050_tipocom_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['r2050_tipocom_apagar'] = 0
            dict_permissoes['r2050_tipocom_editar'] = 0
    ...
    return render(request, 'r2050_tipocom_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class r2050tipoComList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2050tipoCom.objects.using(db_slug).all()
    serializer_class = r2050tipoComSerializer
    # permission_classes = (IsAdminUser,)


class r2050tipoComDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2050tipoCom.objects.using(db_slug).all()
    serializer_class = r2050tipoComSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2050_tipocom_lista = r2050tipoCom.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(r2050_tipocom_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='r2050_tipocom_listar.html',
            ...
            response = render_to_response('r2050_tipocom_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2050_tipocom_csv.html', context)
            filename = "r2050_tipocom.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2050_tipocom_id:
            r2050_tipocom_form = form_r2050_tipocom(request.POST or None, instance = r2050_tipocom,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            r2050_tipocom_form = form_r2050_tipocom(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
        ...
        for field in r2050_tipocom_form.fields.keys():
        ...
        if r2050_tipocom_id:
            ...
            r2050_tipocom = None
        #r2050_tipocom_salvar_custom_variaveis#
        tabelas_secundarias = []
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r2050_tipocom_id, tabela='r2050_tipocom').all()
        context = {
