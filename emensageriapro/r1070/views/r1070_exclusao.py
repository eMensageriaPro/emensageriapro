...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r1070_exclusao')
    ...
    r1070_exclusao = get_object_or_404(r1070exclusao.objects.using( db_slug ), excluido = False, id = r1070_exclusao_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            r1070exclusao.objects.using( db_slug ).filter(id = r1070_exclusao_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r1070_exclusao_lista = r1070exclusao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(r1070_exclusao_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r1070_exclusao_csv.html', context)
            filename = "r1070_exclusao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r1070_exclusao_id:
            r1070_exclusao_form = form_r1070_exclusao(request.POST or None, instance = r1070_exclusao, slug = db_slug)
        else:
            r1070_exclusao_form = form_r1070_exclusao(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r1070_exclusao_form.is_valid():
                ...
                if r1070_exclusao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r1070_exclusao_campos_multiple_passo1
                        r1070exclusao.objects.using(db_slug).filter(id=r1070_exclusao_id).update(**dados)
                        obj = r1070exclusao.objects.using(db_slug).get(id=r1070_exclusao_id)
                        #r1070_exclusao_editar_custom
                        #r1070_exclusao_campos_multiple_passo2
                    ...
                    #r1070_exclusao_cadastrar_campos_multiple_passo1
                    obj = r1070exclusao(**dados)
                    obj.save(using = db_slug)
                    #r1070_exclusao_cadastrar_custom
                    #r1070_exclusao_cadastrar_campos_multiple_passo2
        ...
        r1070_exclusao_form = disabled_form_fields(r1070_exclusao_form, permissao.permite_editar)
        if r1070_exclusao_id:
            if dados_evento['status'] != 0:
                r1070_exclusao_form = disabled_form_fields(r1070_exclusao_form, 0)
        #r1070_exclusao_campos_multiple_passo3

        for field in r1070_exclusao_form.fields.keys():
            r1070_exclusao_form.fields[field].widget.attrs['ng-model'] = 'r1070_exclusao_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r1070_exclusao_id, tabela='r1070_exclusao').all()
        context = {
