...
def apagar(request, hash):
    ...
    dados_evento = {}
    if s2300_brasil_id:
        dados_evento = s2300_brasil.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2300_brasil_apagar'] = 0
            dict_permissoes['s2300_brasil_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2300_brasil), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2300_brasil', s2300_brasil_id, usuario_id, 3)
        else:
...
class s2300brasilList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2300brasil.objects.using(db_slug).all()
    serializer_class = s2300brasilSerializer
    # permission_classes = (IsAdminUser,)


class s2300brasilDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2300brasil.objects.using(db_slug).all()
    serializer_class = s2300brasilSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2300_brasil_listar.html',
                filename="s2300_brasil.pdf",
                context=context,
            ...
            response = render_to_response('s2300_brasil_listar.html', context)
            filename = "s2300_brasil.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2300_brasil_csv.html', context)
            filename = "s2300_brasil.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2300_brasil_id:
            s2300_brasil_form = form_s2300_brasil(request.POST or None, instance = s2300_brasil,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2300_brasil_form = form_s2300_brasil(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2300_brasil_form.is_valid():

                dados = s2300_brasil_form.cleaned_data
                obj = s2300_brasil_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2300_brasil_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2300_brasil', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2300_brasil), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2300_brasil', s2300_brasil_id, usuario_id, 2)
        ...
        if s2300_brasil_id:
            if dados_evento['status'] != 0:
                s2300_brasil_form = disabled_form_fields(s2300_brasil_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2300_brasil_id, tabela='s2300_brasil').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2300_brasil_salvar.html',
                filename="s2300_brasil.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2300_brasil_salvar.html', context)
            filename = "s2300_brasil.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
