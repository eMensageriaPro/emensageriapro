...
def apagar(request, hash):
    ...
    if s2300_cargofuncao_id:
        dados_evento = s2300_cargofuncao.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2300_cargofuncao_apagar'] = 0
            dict_permissoes['s2300_cargofuncao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2300_cargofuncao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2300_cargofuncao', s2300_cargofuncao_id, usuario_id, 3)
        else:
...
class s2300cargoFuncaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2300cargoFuncao.objects.using(db_slug).all()
    serializer_class = s2300cargoFuncaoSerializer
    # permission_classes = (IsAdminUser,)


class s2300cargoFuncaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2300cargoFuncao.objects.using(db_slug).all()
    serializer_class = s2300cargoFuncaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2300_cargofuncao_listar.html',
                filename="s2300_cargofuncao.pdf",
                context=context,
            ...
            response = render_to_response('s2300_cargofuncao_listar.html', context)
            filename = "s2300_cargofuncao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2300_cargofuncao_csv.html', context)
            filename = "s2300_cargofuncao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2300_cargofuncao_id:
            s2300_cargofuncao_form = form_s2300_cargofuncao(request.POST or None, instance = s2300_cargofuncao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2300_cargofuncao_form = form_s2300_cargofuncao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2300_cargofuncao_form.is_valid():

                dados = s2300_cargofuncao_form.cleaned_data
                obj = s2300_cargofuncao_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2300_cargofuncao_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2300_cargofuncao', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2300_cargofuncao), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2300_cargofuncao', s2300_cargofuncao_id, usuario_id, 2)
        ...
        if s2300_cargofuncao_id:
            if dados_evento['status'] != 0:
                s2300_cargofuncao_form = disabled_form_fields(s2300_cargofuncao_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2300_cargofuncao_id, tabela='s2300_cargofuncao').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2300_cargofuncao_salvar.html',
                filename="s2300_cargofuncao.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2300_cargofuncao_salvar.html', context)
            filename = "s2300_cargofuncao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
