...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2300exterior.objects.using( db_slug ).filter(id = s2300_exterior_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2300_exterior_id:
            s2300_exterior_form = form_s2300_exterior(request.POST or None, instance = s2300_exterior, slug = db_slug)
        else:
            s2300_exterior_form = form_s2300_exterior(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s2300_exterior_form.is_valid():
                ...
                if s2300_exterior_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2300_exterior_campos_multiple_passo1
                        s2300exterior.objects.using(db_slug).filter(id=s2300_exterior_id).update(**dados)
                        obj = s2300exterior.objects.using(db_slug).get(id=s2300_exterior_id)
                        #s2300_exterior_editar_custom
                        #s2300_exterior_campos_multiple_passo2
                    ...
                    #s2300_exterior_cadastrar_campos_multiple_passo1
                    obj = s2300exterior(**dados)
                    obj.save(using = db_slug)
                    #s2300_exterior_cadastrar_custom
                    #s2300_exterior_cadastrar_campos_multiple_passo2
