...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2300_exterior = get_object_or_404(s2300exterior, id=pk)
        evento_dados = s2300_exterior.evento()

    if request.user.has_perm('s2300.can_see_s2300exterior'):
        
        if pk:
        
            s2300_exterior_form = form_s2300_exterior(
                request.POST or None, 
                instance=s2300_exterior)
                                         
        else:
        
            s2300_exterior_form = form_s2300_exterior(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2300_exterior_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's2300_exterior_apagar', 
                    's2300_exterior_salvar', 
                    's2300_exterior'):
        ...
        s2300_exterior_form = disabled_form_fields(
            s2300_exterior_form, 
            request.user.has_perm('s2300.change_s2300exterior'))
        ...
        if tab or 's2300_exterior' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2300_exterior_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2300_exterior').all()
