...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2300_infoestagiario = get_object_or_404(s2300infoEstagiario, id=pk)
        evento_dados = s2300_infoestagiario.evento()

    if request.user.has_perm('s2300.can_see_s2300infoEstagiario'):
        ...
        if request.method == 'POST':
            ...
            if s2300_infoestagiario_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2300_infoestagiario), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2300_infoestagiario_apagar', 
                    's2300_infoestagiario_salvar', 
                    's2300_infoestagiario'):
        ...
        s2300_infoestagiario_form = disabled_form_fields(
            s2300_infoestagiario_form, 
            request.user.has_perm('s2300.change_s2300infoEstagiario'))
        ...
        if pk:
            ...
            s2300_ageintegracao_form = form_s2300_ageintegracao(
                initial={ 's2300_infoestagiario': s2300_infoestagiario })
            ...
            s2300_ageintegracao_lista = s2300ageIntegracao.objects.\
                filter(s2300_infoestagiario_id=s2300_infoestagiario.id).all()
        ...
        if tab or 's2300_infoestagiario' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2300_infoestagiario_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2300_infoestagiario').all()
