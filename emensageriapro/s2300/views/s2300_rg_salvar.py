...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2300_rg = get_object_or_404(s2300RG, id=pk)
        evento_dados = s2300_rg.evento()

    if request.user.has_perm('s2300.can_see_s2300RG'):
        ...
        if request.method == 'POST':
            ...
            if s2300_rg_form.is_valid():
                ...
                obj = s2300_rg_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                 
                if 's2300-rg' not in request.session['return']:
        ...
        s2300_rg_form = disabled_form_fields(
            s2300_rg_form,
            request.user.has_perm('s2300.change_s2300RG'))
        ...
        #if tab or 's2300_rg' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2300_rg_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2300_rg').all()
