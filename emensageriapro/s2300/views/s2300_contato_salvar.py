...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2300_contato = get_object_or_404(s2300contato, id=pk)
        evento_dados = s2300_contato.evento()

    if request.user.has_perm('s2300.can_see_s2300contato'):
        ...
        if request.method == 'POST':
            ...
            if s2300_contato_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's2300_contato_apagar', 
                    's2300_contato_salvar', 
                    's2300_contato'):
        ...
        s2300_contato_form = disabled_form_fields(
            s2300_contato_form, 
            request.user.has_perm('s2300.change_s2300contato'))
        ...
        if tab or 's2300_contato' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2300_contato_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2300_contato').all()
