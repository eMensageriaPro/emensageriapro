...
def apagar(request, hash):
    ...
    if s3000_idetrabalhador_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s3000_idetrabalhador_apagar'] = 0
            dict_permissoes['s3000_idetrabalhador_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s3000_idetrabalhador), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's3000_idetrabalhador', s3000_idetrabalhador_id, usuario_id, 3)
        else:
...
class s3000ideTrabalhadorList(generics.ListCreateAPIView):
    db_slug = 'default'
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s3000_idetrabalhador_listar.html',
                filename="s3000_idetrabalhador.pdf",
                context=context,
            ...
            response = render_to_response('s3000_idetrabalhador_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s3000_idetrabalhador_csv.html', context)
            filename = "s3000_idetrabalhador.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s3000_idetrabalhador_id:
            s3000_idetrabalhador_form = form_s3000_idetrabalhador(request.POST or None, instance = s3000_idetrabalhador,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s3000_idetrabalhador_form = form_s3000_idetrabalhador(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s3000_idetrabalhador_form.is_valid():
                ...
                if not s3000_idetrabalhador_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's3000_idetrabalhador', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s3000_idetrabalhador), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's3000_idetrabalhador', s3000_idetrabalhador_id, usuario_id, 2)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s3000_idetrabalhador_id, tabela='s3000_idetrabalhador').all()
        context = {
