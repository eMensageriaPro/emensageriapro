...
def apagar(request, hash):
    ...
    if s1035_alteracao_novavalidade_id:
        dados_evento = s1035_alteracao_novavalidade.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1035_alteracao_novavalidade_apagar'] = 0
            dict_permissoes['s1035_alteracao_novavalidade_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1035_alteracao_novavalidade), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1035_alteracao_novavalidade', s1035_alteracao_novavalidade_id, usuario_id, 3)
        else:
...
class s1035alteracaonovaValidadeList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1035alteracaonovaValidade.objects.using(db_slug).all()
    serializer_class = s1035alteracaonovaValidadeSerializer
    # permission_classes = (IsAdminUser,)


class s1035alteracaonovaValidadeDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1035alteracaonovaValidade.objects.using(db_slug).all()
    serializer_class = s1035alteracaonovaValidadeSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1035_alteracao_novavalidade_listar.html',
                filename="s1035_alteracao_novavalidade.pdf",
                context=context,
            ...
            response = render_to_response('s1035_alteracao_novavalidade_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1035_alteracao_novavalidade_csv.html', context)
            filename = "s1035_alteracao_novavalidade.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1035_alteracao_novavalidade_id:
            s1035_alteracao_novavalidade_form = form_s1035_alteracao_novavalidade(request.POST or None, instance = s1035_alteracao_novavalidade,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1035_alteracao_novavalidade_form = form_s1035_alteracao_novavalidade(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1035_alteracao_novavalidade_form.is_valid():
                ...
                if not s1035_alteracao_novavalidade_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1035_alteracao_novavalidade', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1035_alteracao_novavalidade), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1035_alteracao_novavalidade', s1035_alteracao_novavalidade_id, usuario_id, 2)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1035_alteracao_novavalidade_id, tabela='s1035_alteracao_novavalidade').all()
        context = {
