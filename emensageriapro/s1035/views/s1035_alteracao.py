...
def apagar(request, hash):
    ...
    if s1035_alteracao_id:
        dados_evento = s1035_alteracao.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1035_alteracao_apagar'] = 0
            dict_permissoes['s1035_alteracao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1035_alteracao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1035_alteracao', s1035_alteracao_id, usuario_id, 3)
        else:
...
class s1035alteracaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1035alteracao.objects.using(db_slug).all()
    serializer_class = s1035alteracaoSerializer
    # permission_classes = (IsAdminUser,)


class s1035alteracaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1035alteracao.objects.using(db_slug).all()
    serializer_class = s1035alteracaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1035_alteracao_listar.html',
                filename="s1035_alteracao.pdf",
                context=context,
            ...
            response = render_to_response('s1035_alteracao_listar.html', context)
            filename = "s1035_alteracao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1035_alteracao_csv.html', context)
            filename = "s1035_alteracao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1035_alteracao_id:
            s1035_alteracao_form = form_s1035_alteracao(request.POST or None, instance = s1035_alteracao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1035_alteracao_form = form_s1035_alteracao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1035_alteracao_form.is_valid():

                dados = s1035_alteracao_form.cleaned_data
                obj = s1035_alteracao_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s1035_alteracao_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1035_alteracao', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1035_alteracao), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1035_alteracao', s1035_alteracao_id, usuario_id, 2)
        ...
        if s1035_alteracao_id:
            if dados_evento['status'] != 0:
                s1035_alteracao_form = disabled_form_fields(s1035_alteracao_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1035_alteracao_id, tabela='s1035_alteracao').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1035_alteracao_salvar.html',
                filename="s1035_alteracao.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1035_alteracao_salvar.html', context)
            filename = "s1035_alteracao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
