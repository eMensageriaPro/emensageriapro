...
def salvar(request, hash):
    ...
    if s1035_inclusao_id:
        s1035_inclusao = get_object_or_404(s1035inclusao.objects.using( db_slug ), excluido = False, id = s1035_inclusao_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1035_inclusao_id:
            s1035_inclusao_form = form_s1035_inclusao(request.POST or None, instance = s1035_inclusao, slug = db_slug)
        else:
            s1035_inclusao_form = form_s1035_inclusao(request.POST or None, slug = db_slug, initial={})
            ...
            if s1035_inclusao_form.is_valid():
                ...
                if s1035_inclusao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1035_inclusao_campos_multiple_passo1
                        s1035inclusao.objects.using(db_slug).filter(id=s1035_inclusao_id).update(**dados)
                        obj = s1035inclusao.objects.using(db_slug).get(id=s1035_inclusao_id)
                        #s1035_inclusao_editar_custom
                        #s1035_inclusao_campos_multiple_passo2
                    ...
                    #s1035_inclusao_cadastrar_campos_multiple_passo1
                    obj = s1035inclusao(**dados)
                    obj.save(using = db_slug)
                    #s1035_inclusao_cadastrar_custom
                    #s1035_inclusao_cadastrar_campos_multiple_passo2
        ...
        for field in s1035_inclusao_form.fields.keys():
            s1035_inclusao_form.fields[field].widget.attrs['ng-model'] = 's1035_inclusao_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1035_inclusao_id, tabela='s1035_inclusao').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1035_inclusao), indent=4, sort_keys=True, default=str)
            s1035inclusao.objects.using( db_slug ).filter(id = s1035_inclusao_id).delete()
            #s1035_inclusao_apagar_custom
            #s1035_inclusao_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1035_inclusao', s1035_inclusao_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's1035_inclusao_salvar':
            return redirect('s1035_inclusao', hash=request.session['retorno_hash'])
        else:
            return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
    context = {
        'usuario': usuario,
