...
def salvar(request, hash):
    ...
    if s2400_fimbeneficio_id:
        dados_evento = s2400_fimbeneficio.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s2400_fimbeneficio_apagar'] = 0
            dict_permissoes['s2400_fimbeneficio_editar'] = 0
    ...
    if permissao.permite_visualizar:
            ...
            if s2400_fimbeneficio_form.is_valid():
                ...
                if s2400_fimbeneficio_id:
                    if dados_evento['status'] == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s2400_fimbeneficio), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's2400_fimbeneficio', s2400_fimbeneficio_id, usuario_id, 2)
                    else:
                    ...
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2400_fimbeneficio', obj.id, usuario_id, 1)
        ...
        if s2400_fimbeneficio_id:
            if dados_evento['status'] != 0:
                s2400_fimbeneficio_form = disabled_form_fields(s2400_fimbeneficio_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2400_fimbeneficio_id, tabela='s2400_fimbeneficio').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2400_fimbeneficio_salvar.html',
                filename="s2400_fimbeneficio.pdf",
                context=context,
            ...
            response = render_to_response('s2400_fimbeneficio_salvar.html', context)
            filename = "s2400_fimbeneficio.xls"
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2400_fimbeneficio), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2400_fimbeneficio', s2400_fimbeneficio_id, usuario_id, 3)
        else:
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2400_fimbeneficio_listar.html',
                filename="s2400_fimbeneficio.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2400_fimbeneficio_listar.html', context)
            filename = "s2400_fimbeneficio.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2400_fimbeneficio_csv.html', context)
            filename = "s2400_fimbeneficio.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
