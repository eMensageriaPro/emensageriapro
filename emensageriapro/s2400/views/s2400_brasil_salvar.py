...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2400_brasil = get_object_or_404(s2400brasil, id=pk)
        evento_dados = s2400_brasil.evento()

    if request.user.has_perm('s2400.can_see_s2400brasil'):
        ...
        if request.method == 'POST':
            ...
            if s2400_brasil_form.is_valid():
                ...
                if 's2400-brasil' not in request.session['return']:
        ...
        s2400_brasil_form = disabled_form_fields(
            s2400_brasil_form,
            request.user.has_perm('s2400.change_s2400brasil'))
        ...
        #if tab or 's2400_brasil' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2400_brasil_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2400_brasil').all()
