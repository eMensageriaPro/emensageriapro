...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2400_exterior = get_object_or_404(s2400exterior, id=pk)
        evento_dados = s2400_exterior.evento()
    ...
    if request.user.has_perm('s2400.can_see_s2400exterior'):
        ...
        if request.method == 'POST':
            ...
            if s2400_exterior_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's2400_exterior_apagar', 
                    's2400_exterior_salvar', 
                    's2400_exterior'):
        ...
        s2400_exterior_form = disabled_form_fields(
            s2400_exterior_form, 
            request.user.has_perm('s2400.change_s2400exterior'))
        ...
        if tab or 's2400_exterior' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2400_exterior_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2400_exterior').all()
