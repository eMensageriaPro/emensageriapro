...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1250_infoprocjud_id:
            s1250_infoprocjud_form = form_s1250_infoprocjud(request.POST or None, instance = s1250_infoprocjud, slug = db_slug)
        else:
            s1250_infoprocjud_form = form_s1250_infoprocjud(request.POST or None, slug = db_slug, initial={})
            ...
            if s1250_infoprocjud_form.is_valid():
                ...
                if s1250_infoprocjud_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1250_infoprocjud_campos_multiple_passo1
                        s1250infoProcJud.objects.using(db_slug).filter(id=s1250_infoprocjud_id).update(**dados)
                        obj = s1250infoProcJud.objects.using(db_slug).get(id=s1250_infoprocjud_id)
                        #s1250_infoprocjud_editar_custom
                        #s1250_infoprocjud_campos_multiple_passo2
                    ...
                    #s1250_infoprocjud_cadastrar_campos_multiple_passo1
                    obj = s1250infoProcJud(**dados)
                    obj.save(using = db_slug)
                    #s1250_infoprocjud_cadastrar_custom
                    #s1250_infoprocjud_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1250infoProcJud.objects.using( db_slug ).filter(id = s1250_infoprocjud_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1250_ideprodutor_lista = s1250ideProdutor.objects.using( db_slug ).filter(excluido = False).all()
