...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1250_nfs = get_object_or_404(s1250nfs, id=pk)
        evento_dados = s1250_nfs.evento()

    if request.user.has_perm('s1250.can_see_s1250nfs'):
        ...
        if request.method == 'POST':
            ...
            if s1250_nfs_form.is_valid():
                ...
                obj = s1250_nfs_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                 
                if 's1250-nfs' not in request.session['return']:
        ...
        s1250_nfs_form = disabled_form_fields(
            s1250_nfs_form,
            request.user.has_perm('s1250.change_s1250nfs'))
        ...
        #if tab or 's1250_nfs' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's1250_nfs_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1250_nfs').all()
