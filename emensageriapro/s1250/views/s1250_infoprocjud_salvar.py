...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1250_infoprocjud = get_object_or_404(s1250infoProcJud, id=pk)
        evento_dados = s1250_infoprocjud.evento()

    if request.user.has_perm('s1250.can_see_s1250infoProcJud'):
        ...
        if request.method == 'POST':
            ...
            if s1250_infoprocjud_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1250_infoprocjud), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1250_infoprocjud_apagar', 
                    's1250_infoprocjud_salvar', 
                    's1250_infoprocjud'):
        ...
        s1250_infoprocjud_form = disabled_form_fields(
            s1250_infoprocjud_form, 
            request.user.has_perm('s1250.change_s1250infoProcJud'))
        ...
        if tab or 's1250_infoprocjud' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1250_infoprocjud_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1250_infoprocjud').all()
