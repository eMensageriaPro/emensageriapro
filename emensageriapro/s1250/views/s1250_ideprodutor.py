...
class s1250ideProdutorList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1250ideProdutor.objects.using(db_slug).all()
    serializer_class = s1250ideProdutorSerializer
    # permission_classes = (IsAdminUser,)


class s1250ideProdutorDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1250ideProdutor.objects.using(db_slug).all()
    serializer_class = s1250ideProdutorSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1250_ideprodutor_id:
            s1250_ideprodutor_form = form_s1250_ideprodutor(request.POST or None, instance = s1250_ideprodutor,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1250_ideprodutor_form = form_s1250_ideprodutor(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
