...
class s1250tpAquisList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1250tpAquis.objects.using(db_slug).all()
    serializer_class = s1250tpAquisSerializer
    # permission_classes = (IsAdminUser,)


class s1250tpAquisDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1250tpAquis.objects.using(db_slug).all()
    serializer_class = s1250tpAquisSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1250_tpaquis_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1250_tpaquis_id:
            s1250_tpaquis_form = form_s1250_tpaquis(request.POST or None, instance = s1250_tpaquis,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1250_tpaquis_form = form_s1250_tpaquis(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1250_tpaquis_form.is_valid():
                ...
                if not s1250_tpaquis_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1250_tpaquis', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1250_tpaquis), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1250_tpaquis', s1250_tpaquis_id, usuario_id, 2)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1250_tpaquis_id, tabela='s1250_tpaquis').all()
        context = {
