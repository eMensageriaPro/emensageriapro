...
<a href="http://www.emensageriapro.adm.br/%s/">www.emensageriapro.adm.br/%s/</a><br>
...
def alterar_senha(request):
    db_slug = 'default'
    try: usuario_id = request.session['usuario_id']
    ...
    if request.method == 'POST':
        if request.POST.get('senha') == request.POST.get('senha_repetir'):
            if request.POST.get('senha'):
                ...
                Usuarios.objects.using(db_slug).filter(id=usuario_id).update(senha=hashed_password)
