...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2299_infoperant_ideadc = get_object_or_404(s2299infoPerAntideADC, id=pk)
        evento_dados = s2299_infoperant_ideadc.evento()

    if request.user.has_perm('s2299.can_see_s2299infoPerAntideADC'):
        ...
        if request.method == 'POST':
            ...
            if s2299_infoperant_ideadc_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2299_infoperant_ideadc), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2299_infoperant_ideadc_apagar', 
                    's2299_infoperant_ideadc_salvar', 
                    's2299_infoperant_ideadc'):
        ...
        s2299_infoperant_ideadc_form = disabled_form_fields(
            s2299_infoperant_ideadc_form, 
            request.user.has_perm('s2299.change_s2299infoPerAntideADC'))
        ...
        if tab or 's2299_infoperant_ideadc' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2299_infoperant_ideadc_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2299_infoperant_ideadc').all()
