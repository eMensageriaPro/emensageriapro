            ...
            if s2299_infoperant_detverbas_form.is_valid():
                ...
                if s2299_infoperant_detverbas_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2299_infoperant_detverbas_campos_multiple_passo1
                        s2299infoPerAntdetVerbas.objects.using(db_slug).filter(id=s2299_infoperant_detverbas_id).update(**dados)
                        obj = s2299infoPerAntdetVerbas.objects.using(db_slug).get(id=s2299_infoperant_detverbas_id)
                        #s2299_infoperant_detverbas_editar_custom
                        #s2299_infoperant_detverbas_campos_multiple_passo2
                    ...
                    #s2299_infoperant_detverbas_cadastrar_campos_multiple_passo1
                    obj = s2299infoPerAntdetVerbas(**dados)
                    obj.save(using = db_slug)
                    #s2299_infoperant_detverbas_cadastrar_custom
                    #s2299_infoperant_detverbas_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2299infoPerAntdetVerbas.objects.using( db_slug ).filter(id = s2299_infoperant_detverbas_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2299_infoperant_ideestablot_lista = s2299infoPerAntideEstabLot.objects.using( db_slug ).filter(excluido = False).all()
