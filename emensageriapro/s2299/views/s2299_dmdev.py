...
def apagar(request, hash):
    ...
    dados_evento = {}
    if s2299_dmdev_id:
        dados_evento = s2299_dmdev.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s2299_dmdev_apagar'] = 0
            dict_permissoes['s2299_dmdev_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2299_dmdev), indent=4, sort_keys=True, default=str)
...
class s2299dmDevList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2299dmDev.objects.using(db_slug).all()
    serializer_class = s2299dmDevSerializer
    # permission_classes = (IsAdminUser,)


class s2299dmDevDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2299dmDev.objects.using(db_slug).all()
    serializer_class = s2299dmDevSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2299_dmdev_listar.html',
                filename="s2299_dmdev.pdf",
                context=context,
            ...
            response = render_to_response('s2299_dmdev_listar.html', context)
            filename = "s2299_dmdev.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2299_dmdev_csv.html', context)
            filename = "s2299_dmdev.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if s2299_dmdev_form.is_valid():
                ...
                obj = s2299_dmdev_form.save(request=request)
                messages.success(request, 'Salvo com sucesso!')

                if not s2299_dmdev_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2299_dmdev', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2299_dmdev), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2299_dmdev', s2299_dmdev_id, usuario_id, 2)
        ...
        if s2299_dmdev_id:
            if dados_evento['status'] != 0:
                s2299_dmdev_form = disabled_form_fields(s2299_dmdev_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2299_dmdev_id, tabela='s2299_dmdev').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2299_dmdev_salvar.html',
                filename="s2299_dmdev.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2299_dmdev_salvar.html', context)
            filename = "s2299_dmdev.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
