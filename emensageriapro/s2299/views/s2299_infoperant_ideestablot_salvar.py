...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2299_infoperant_ideestablot = get_object_or_404(s2299infoPerAntideEstabLot, id=pk)
        evento_dados = s2299_infoperant_ideestablot.evento()

    if request.user.has_perm('s2299.can_see_s2299infoPerAntideEstabLot'):

        if pk:

            s2299_infoperant_ideestablot_form = form_s2299_infoperant_ideestablot(
                request.POST or None,
                instance=s2299_infoperant_ideestablot)
                     
        else:

            s2299_infoperant_ideestablot_form = form_s2299_infoperant_ideestablot(request.POST or None)
                     
        if request.method == 'POST':

            if s2299_infoperant_ideestablot_form.is_valid():

                obj = s2299_infoperant_ideestablot_form.save(request=request)
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2299_infoperant_ideestablot),
                #            indent=4,
                #            sort_keys=True,
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2299_infoperant_ideestablot_apagar',
                    's2299_infoperant_ideestablot_salvar',
                    's2299_infoperant_ideestablot'):
        ...
        s2299_infoperant_ideestablot_form = disabled_form_fields(
            s2299_infoperant_ideestablot_form,
            request.user.has_perm('s2299.change_s2299infoPerAntideEstabLot'))
        ...
        if pk:
            ...
            s2299_infoperant_detverbas_form = form_s2299_infoperant_detverbas(
                initial={ 's2299_infoperant_ideestablot': s2299_infoperant_ideestablot })
            ...
            s2299_infoperant_detverbas_lista = s2299infoPerAntdetVerbas.objects.\
                filter(s2299_infoperant_ideestablot_id=s2299_infoperant_ideestablot.id).all()

            s2299_infoperant_infoagnocivo_form = form_s2299_infoperant_infoagnocivo(
                initial={ 's2299_infoperant_ideestablot': s2299_infoperant_ideestablot })
        ...
        if tab or 's2299_infoperant_ideestablot' in request.session['return_page']:

            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2299_infoperant_ideestablot_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2299_infoperant_ideestablot').all()
        ...
        context = {
            ...
            'modulos': ['s2299', ],
            'paginas': ['s2299_infoperant_ideestablot', ],
