...
def apagar(request, hash):
    ...
    if s2299_transftit_id:
        dados_evento = s2299_transftit.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2299_transftit_apagar'] = 0
            dict_permissoes['s2299_transftit_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2299_transftit), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2299_transftit', s2299_transftit_id, usuario_id, 3)
        else:
...
class s2299transfTitList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2299transfTit.objects.using(db_slug).all()
    serializer_class = s2299transfTitSerializer
    # permission_classes = (IsAdminUser,)


class s2299transfTitDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2299transfTit.objects.using(db_slug).all()
    serializer_class = s2299transfTitSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2299_transftit_listar.html',
                filename="s2299_transftit.pdf",
                context=context,
            ...
            response = render_to_response('s2299_transftit_listar.html', context)
            filename = "s2299_transftit.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2299_transftit_csv.html', context)
            filename = "s2299_transftit.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2299_transftit_id:
            s2299_transftit_form = form_s2299_transftit(request.POST or None, instance = s2299_transftit,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2299_transftit_form = form_s2299_transftit(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2299_transftit_form.is_valid():

                dados = s2299_transftit_form.cleaned_data
                obj = s2299_transftit_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2299_transftit_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2299_transftit', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2299_transftit), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2299_transftit', s2299_transftit_id, usuario_id, 2)
        ...
        if s2299_transftit_id:
            if dados_evento['status'] != 0:
                s2299_transftit_form = disabled_form_fields(s2299_transftit_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2299_transftit_id, tabela='s2299_transftit').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2299_transftit_salvar.html',
                filename="s2299_transftit.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2299_transftit_salvar.html', context)
            filename = "s2299_transftit.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
