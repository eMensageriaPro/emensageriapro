...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2299_infoperant_detverbas = get_object_or_404(s2299infoPerAntdetVerbas, id=pk)
        evento_dados = s2299_infoperant_detverbas.evento()
    ...
    if request.user.has_perm('s2299.can_see_s2299infoPerAntdetVerbas'):
        ...
        if request.method == 'POST':
            ...
            if s2299_infoperant_detverbas_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2299_infoperant_detverbas), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2299_infoperant_detverbas_apagar', 
                    's2299_infoperant_detverbas_salvar', 
                    's2299_infoperant_detverbas'):
        ...
        s2299_infoperant_detverbas_form = disabled_form_fields(
            s2299_infoperant_detverbas_form, 
            request.user.has_perm('s2299.change_s2299infoPerAntdetVerbas'))
        ...
        if tab or 's2299_infoperant_detverbas' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2299_infoperant_detverbas_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2299_infoperant_detverbas').all()
