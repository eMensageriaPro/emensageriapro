...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2299_infoperant_infoagnocivo = get_object_or_404(s2299infoPerAntinfoAgNocivo, id=pk)
        evento_dados = s2299_infoperant_infoagnocivo.evento()

    if request.user.has_perm('s2299.can_see_s2299infoPerAntinfoAgNocivo'):
        ...
        if request.method == 'POST':
            ...
            if s2299_infoperant_infoagnocivo_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2299_infoperant_infoagnocivo),
                #            indent=4,
                #            sort_keys=True,
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2299_infoperant_infoagnocivo_apagar',
                    's2299_infoperant_infoagnocivo_salvar',
                    's2299_infoperant_infoagnocivo'):
        ...
        s2299_infoperant_infoagnocivo_form = disabled_form_fields(
            s2299_infoperant_infoagnocivo_form,
            request.user.has_perm('s2299.change_s2299infoPerAntinfoAgNocivo'))
        ...
        if tab or 's2299_infoperant_infoagnocivo' in request.session['return_page']:

            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2299_infoperant_infoagnocivo_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2299_infoperant_infoagnocivo').all()
