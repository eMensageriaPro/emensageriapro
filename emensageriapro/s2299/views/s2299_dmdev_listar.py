...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2299_dmdev_lista = s2299dmDev.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/s2299_dmdev_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/s2299_dmdev.csv', context)
