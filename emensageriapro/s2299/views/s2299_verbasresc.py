...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2299_verbasresc')
    ...
    if s2299_verbasresc_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['s2299_verbasresc_apagar'] = 0
            dict_permissoes['s2299_verbasresc_editar'] = 0
    ...
    if permissao.permite_visualizar:
            ...
            if s2299_verbasresc_form.is_valid():
                ...
                if s2299_verbasresc_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2299_verbasresc_campos_multiple_passo1
                        s2299verbasResc.objects.using(db_slug).filter(id=s2299_verbasresc_id).update(**dados)
                        obj = s2299verbasResc.objects.using(db_slug).get(id=s2299_verbasresc_id)
                        #s2299_verbasresc_editar_custom
                        #s2299_verbasresc_campos_multiple_passo2
                    ...
                    #s2299_verbasresc_cadastrar_campos_multiple_passo1
                    obj = s2299verbasResc(**dados)
                    obj.save(using = db_slug)
                    #s2299_verbasresc_cadastrar_custom
                    #s2299_verbasresc_cadastrar_campos_multiple_passo2
        ...
        s2299_verbasresc_form = disabled_form_fields(s2299_verbasresc_form, permissao.permite_editar)
        if s2299_verbasresc_id:
            if dados_evento['status'] != 0:
                s2299_verbasresc_form = disabled_form_fields(s2299_verbasresc_form, 0)
        #s2299_verbasresc_campos_multiple_passo3

        for field in s2299_verbasresc_form.fields.keys():
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2299_verbasresc_id, tabela='s2299_verbasresc').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2299_verbasresc), indent=4, sort_keys=True, default=str)
            s2299verbasResc.objects.using( db_slug ).filter(id = s2299_verbasresc_id).delete()
            #s2299_verbasresc_apagar_custom
            #s2299_verbasresc_apagar_custom
    ...
    return render(request, 's2299_verbasresc_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2299verbasRescList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2299verbasResc.objects.using(db_slug).all()
    serializer_class = s2299verbasRescSerializer
    permission_classes = (IsAdminUser,)


class s2299verbasRescDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2299verbasResc.objects.using(db_slug).all()
    serializer_class = s2299verbasRescSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2299_verbasresc_lista = s2299verbasResc.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2299_verbasresc_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2299_verbasresc_listar.html',
                filename="s2299_verbasresc.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2299_verbasresc_listar.html', context)
            filename = "s2299_verbasresc.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2299_verbasresc_csv.html', context)
            filename = "s2299_verbasresc.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
