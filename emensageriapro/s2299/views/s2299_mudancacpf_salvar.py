...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2299_mudancacpf = get_object_or_404(s2299mudancaCPF, id=pk)
        evento_dados = s2299_mudancacpf.evento()
    ...
    if request.user.has_perm('s2299.can_see_s2299mudancaCPF'):
        ...
        if pk:
            ...
            s2299_mudancacpf_form = form_s2299_mudancacpf(
                ...
                instance=s2299_mudancacpf)
                                         
        else:
        
            s2299_mudancacpf_form = form_s2299_mudancacpf(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s2299_mudancacpf_form.is_valid():
            
                obj = s2299_mudancacpf_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2299_mudancacpf), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2299_mudancacpf_apagar', 
                    's2299_mudancacpf_salvar', 
                    's2299_mudancacpf'):
        ...
        s2299_mudancacpf_form = disabled_form_fields(
            s2299_mudancacpf_form, 
            request.user.has_perm('s2299.change_s2299mudancaCPF'))
        ...
        if tab or 's2299_mudancacpf' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2299_mudancacpf_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2299_mudancacpf').all()
