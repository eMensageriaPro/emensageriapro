...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2299_infotrabinterm_infomv')
    ...
    s2299_infotrabinterm_infomv = get_object_or_404(s2299infoTrabInterminfoMV.objects.using( db_slug ), excluido = False, id = s2299_infotrabinterm_infomv_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s2299_infotrabinterm_infomv), indent=4, sort_keys=True, default=str)
            s2299infoTrabInterminfoMV.objects.using( db_slug ).filter(id = s2299_infotrabinterm_infomv_id).delete()
            #s2299_infotrabinterm_infomv_apagar_custom
            #s2299_infotrabinterm_infomv_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2299_infotrabinterm_infomv', s2299_infotrabinterm_infomv_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
        
        if request.session['retorno_pagina']== 's2299_infotrabinterm_infomv_salvar':
            return redirect('s2299_infotrabinterm_infomv', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's2299_infotrabinterm_infomv_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s2299_infotrabinterm_infomv_form.is_valid():
                ...
                if s2299_infotrabinterm_infomv_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2299_infotrabinterm_infomv_campos_multiple_passo1
                        s2299infoTrabInterminfoMV.objects.using(db_slug).filter(id=s2299_infotrabinterm_infomv_id).update(**dados)
                        obj = s2299infoTrabInterminfoMV.objects.using(db_slug).get(id=s2299_infotrabinterm_infomv_id)
                        #s2299_infotrabinterm_infomv_editar_custom
                        #s2299_infotrabinterm_infomv_campos_multiple_passo2
                    ...
                    #s2299_infotrabinterm_infomv_cadastrar_campos_multiple_passo1
                    obj = s2299infoTrabInterminfoMV(**dados)
                    obj.save(using = db_slug)
                    #s2299_infotrabinterm_infomv_cadastrar_custom
                    #s2299_infotrabinterm_infomv_cadastrar_campos_multiple_passo2
        ...
        for field in s2299_infotrabinterm_infomv_form.fields.keys():
            s2299_infotrabinterm_infomv_form.fields[field].widget.attrs['ng-model'] = 's2299_infotrabinterm_infomv_'+field
        ...
        if s2299_infotrabinterm_infomv_id:
            ...
            s2299_infotrabinterm_remunoutrempr_form = form_s2299_infotrabinterm_remunoutrempr(initial={ 's2299_infotrabinterm_infomv': s2299_infotrabinterm_infomv }, slug=db_slug)
            s2299_infotrabinterm_remunoutrempr_form.fields['s2299_infotrabinterm_infomv'].widget.attrs['readonly'] = True
            s2299_infotrabinterm_remunoutrempr_lista = s2299infoTrabIntermremunOutrEmpr.objects.using( db_slug ).filter(excluido = False, s2299_infotrabinterm_infomv_id=s2299_infotrabinterm_infomv.id).all()
        else:
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2299_infotrabinterm_infomv_id, tabela='s2299_infotrabinterm_infomv').all()
        context = {
...
class s2299infoTrabInterminfoMVList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2299infoTrabInterminfoMV.objects.using(db_slug).all()
    serializer_class = s2299infoTrabInterminfoMVSerializer
    permission_classes = (IsAdminUser,)


class s2299infoTrabInterminfoMVDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2299infoTrabInterminfoMV.objects.using(db_slug).all()
    serializer_class = s2299infoTrabInterminfoMVSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2299_infotrabinterm_infomv_lista = s2299infoTrabInterminfoMV.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 's2299_infotrabinterm_infomv'
        context = {
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2299_infotrabinterm_infomv_csv.html', context)
