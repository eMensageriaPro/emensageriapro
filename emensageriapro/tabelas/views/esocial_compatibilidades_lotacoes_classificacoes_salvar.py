...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='esocial_compatibilidades_lotacoes_classificacoes')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if esocial_compatibilidades_lotacoes_classificacoes_id:
        esocial_compatibilidades_lotacoes_classificacoes = get_object_or_404(eSocialCompatibilidadesLotacoesClassificacoes, id = esocial_compatibilidades_lotacoes_classificacoes_id)
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if esocial_compatibilidades_lotacoes_classificacoes_form.is_valid():
                ...
                obj = esocial_compatibilidades_lotacoes_classificacoes_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not esocial_compatibilidades_lotacoes_classificacoes_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'esocial_compatibilidades_lotacoes_classificacoes', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(esocial_compatibilidades_lotacoes_classificacoes), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'esocial_compatibilidades_lotacoes_classificacoes', esocial_compatibilidades_lotacoes_classificacoes_id, usuario_id, 2)
