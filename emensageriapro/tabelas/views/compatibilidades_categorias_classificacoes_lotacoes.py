...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='compatibilidades_categorias_classificacoes_lotacoes_salvar.html',
                filename="compatibilidades_categorias_classificacoes_lotacoes.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('compatibilidades_categorias_classificacoes_lotacoes_salvar.html', context)
            filename = "compatibilidades_categorias_classificacoes_lotacoes.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
