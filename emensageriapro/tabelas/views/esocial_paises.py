...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='esocial_paises')
    ...
    esocial_paises = get_object_or_404(eSocialPaises.objects.using( db_slug ), excluido = False, id = esocial_paises_id)
    if request.method == 'POST':
        eSocialPaises.objects.using( db_slug ).filter(id = esocial_paises_id).update(excluido = True)
        #esocial_paises_apagar_custom
        #esocial_paises_apagar_custom
        messages.success(request, 'Apagado com sucesso!')
        if request.session['retorno_pagina']== 'esocial_paises_salvar':
            return redirect('esocial_paises', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'esocial_paises_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if esocial_paises_id:
            esocial_paises_form = form_esocial_paises(request.POST or None, instance = esocial_paises, slug = db_slug)
        else:
            esocial_paises_form = form_esocial_paises(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if esocial_paises_form.is_valid():
                ...
                if esocial_paises_id:
                    ...
                    #esocial_paises_campos_multiple_passo1
                    eSocialPaises.objects.using(db_slug).filter(id=esocial_paises_id).update(**dados)
                    obj = eSocialPaises.objects.using(db_slug).get(id=esocial_paises_id)
                    #esocial_paises_editar_custom
                    #esocial_paises_campos_multiple_passo2
                    ...
                    #esocial_paises_cadastrar_campos_multiple_passo1
                    obj = eSocialPaises(**dados)
                    obj.save(using = db_slug)
                    #esocial_paises_cadastrar_custom
                    #esocial_paises_cadastrar_campos_multiple_passo2
        ...
        for field in esocial_paises_form.fields.keys():
            esocial_paises_form.fields[field].widget.attrs['ng-model'] = 'esocial_paises_'+field
...
class eSocialPaisesList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = eSocialPaises.objects.using(db_slug).all()
    serializer_class = eSocialPaisesSerializer
    permission_classes = (IsAdminUser,)


class eSocialPaisesDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = eSocialPaises.objects.using(db_slug).all()
    serializer_class = eSocialPaisesSerializer
    permission_classes = (IsAdminUser,)
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialPaises.objects.using(db_slug).filter(excluido = False).filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialPaises.objects.using(db_slug).filter(excluido = False).filter(query).all()
    else:
        lista = eSocialPaises.objects.using(db_slug).filter(excluido=False).all()
    lista_esocial_paises = []
    for a in lista:
        dic = {}
        dic['key'] = a.codigo
        dic['value'] = '%s' % (a)
        lista_esocial_paises.append(dic)
    dicionario['esocial_paises'] = lista_esocial_paises
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        esocial_paises_lista = eSocialPaises.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(esocial_paises_lista) > 100:
            filtrar = True
            esocial_paises_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')
    
        #esocial_paises_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'esocial_paises'
        context = {
            'esocial_paises_lista': esocial_paises_lista,
        ...
        if for_print in (0,1):
            return render(request, 'esocial_paises_listar.html', context)
        elif for_print == 2:
            #return render_to_pdf('tables/s1000_evtinfoempregador_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(
                request=request,
                template='esocial_paises_listar.html',
                filename="esocial_paises.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('esocial_paises_listar.html', context)
            filename = "esocial_paises.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/esocial_paises_csv.html', context)
            filename = "esocial_paises.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
