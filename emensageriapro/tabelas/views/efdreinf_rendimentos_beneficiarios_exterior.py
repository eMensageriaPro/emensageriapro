...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if efdreinf_rendimentos_beneficiarios_exterior_id:
            efdreinf_rendimentos_beneficiarios_exterior_form = form_efdreinf_rendimentos_beneficiarios_exterior(request.POST or None, instance = efdreinf_rendimentos_beneficiarios_exterior,
                                         slug = db_slug,
                                         initial = {'excluido': False})
        else:
            efdreinf_rendimentos_beneficiarios_exterior_form = form_efdreinf_rendimentos_beneficiarios_exterior(request.POST or None, slug = db_slug,
                                         initial = {'excluido': False})
        if request.method == 'POST':
            if efdreinf_rendimentos_beneficiarios_exterior_form.is_valid():
                ...
                obj = efdreinf_rendimentos_beneficiarios_exterior_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not efdreinf_rendimentos_beneficiarios_exterior_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 'efdreinf_rendimentos_beneficiarios_exterior', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(efdreinf_rendimentos_beneficiarios_exterior), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     'efdreinf_rendimentos_beneficiarios_exterior', efdreinf_rendimentos_beneficiarios_exterior_id, usuario_id, 2)
