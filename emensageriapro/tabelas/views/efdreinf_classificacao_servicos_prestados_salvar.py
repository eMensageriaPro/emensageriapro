...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='efdreinf_classificacao_servicos_prestados')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if efdreinf_classificacao_servicos_prestados_id:
        efdreinf_classificacao_servicos_prestados = get_object_or_404(EFDReinfClassificacaoServicosPrestados, id = efdreinf_classificacao_servicos_prestados_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if efdreinf_classificacao_servicos_prestados_id:
            efdreinf_classificacao_servicos_prestados_form = form_efdreinf_classificacao_servicos_prestados(request.POST or None, instance = efdreinf_classificacao_servicos_prestados, 
        ...
        if request.method == 'POST':
            if efdreinf_classificacao_servicos_prestados_form.is_valid():
                ...
                obj = efdreinf_classificacao_servicos_prestados_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not efdreinf_classificacao_servicos_prestados_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'efdreinf_classificacao_servicos_prestados', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(efdreinf_classificacao_servicos_prestados), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'efdreinf_classificacao_servicos_prestados', efdreinf_classificacao_servicos_prestados_id, usuario_id, 2)
