...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            post = True
            dict_fields = { 
                'classificacao__icontains': 'classificacao__icontains',
        ...
        efdreinf_regras_pagamentos_codigos_lista = EFDReinfRegrasPagamentosCodigos.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/efdreinf_regras_pagamentos_codigos_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/efdreinf_regras_pagamentos_codigos.csv', context)
