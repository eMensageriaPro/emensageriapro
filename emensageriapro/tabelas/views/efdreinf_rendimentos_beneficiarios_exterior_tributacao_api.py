...
class EFDReinfRendimentosBeneficiariosExteriorTributacaoList(generics.ListCreateAPIView):
    ...
    queryset = EFDReinfRendimentosBeneficiariosExteriorTributacao.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = EFDReinfRendimentosBeneficiariosExteriorTributacao.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = EFDReinfRendimentosBeneficiariosExteriorTributacao.objects.filter(query).all()
    else:
        lista = EFDReinfRendimentosBeneficiariosExteriorTributacao.objects.all()
