...
class eSocialBeneficiosPrevidenciariosTiposList(generics.ListCreateAPIView):
    ...
    queryset = eSocialBeneficiosPrevidenciariosTipos.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialBeneficiosPrevidenciariosTipos.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialBeneficiosPrevidenciariosTipos.objects.filter(query).all()
    else:
        lista = eSocialBeneficiosPrevidenciariosTipos.objects.all()
