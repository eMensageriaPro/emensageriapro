...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        CodigosAtividadesProdutosServicosCPRB.objects.using( db_slug ).filter(id = codigos_atividades_produtos_servicos_cprb_id).update(excluido = True)
        #codigos_atividades_produtos_servicos_cprb_apagar_custom
        #codigos_atividades_produtos_servicos_cprb_apagar_custom
        messages.success(request, 'Apagado com sucesso!')
        if request.session['retorno_pagina']== 'codigos_atividades_produtos_servicos_cprb_salvar':
            return redirect('codigos_atividades_produtos_servicos_cprb', hash=request.session['retorno_hash'])
        else:
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = CodigosAtividadesProdutosServicosCPRB.objects.using(db_slug).filter(excluido = False).filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = CodigosAtividadesProdutosServicosCPRB.objects.using(db_slug).filter(excluido = False).filter(query).all()
    else:
        lista = CodigosAtividadesProdutosServicosCPRB.objects.using(db_slug).filter(excluido=False).all()
    lista_codigos_atividades_produtos_servicos_cprb = []
    for a in lista:
        dic = {}
        dic['key'] = a.codigo
        dic['value'] = '%s' % (a)
        lista_codigos_atividades_produtos_servicos_cprb.append(dic)
    dicionario['codigos_atividades_produtos_servicos_cprb'] = lista_codigos_atividades_produtos_servicos_cprb
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        codigos_atividades_produtos_servicos_cprb_lista = CodigosAtividadesProdutosServicosCPRB.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(codigos_atividades_produtos_servicos_cprb_lista) > 100:
            filtrar = True
            codigos_atividades_produtos_servicos_cprb_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        #codigos_atividades_produtos_servicos_cprb_listar_custom
        request.session["retorno_hash"] = hash
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='codigos_atividades_produtos_servicos_cprb_listar.html',
            ...
            response = render_to_response('codigos_atividades_produtos_servicos_cprb_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/codigos_atividades_produtos_servicos_cprb_csv.html', context)
            filename = "codigos_atividades_produtos_servicos_cprb.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
