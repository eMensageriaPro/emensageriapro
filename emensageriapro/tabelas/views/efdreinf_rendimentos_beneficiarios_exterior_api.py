...
class EFDReinfRendimentosBeneficiariosExteriorList(generics.ListCreateAPIView):
    ...
    queryset = EFDReinfRendimentosBeneficiariosExterior.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = EFDReinfRendimentosBeneficiariosExterior.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = EFDReinfRendimentosBeneficiariosExterior.objects.filter(query).all()
    else:
        lista = EFDReinfRendimentosBeneficiariosExterior.objects.all()
