...
class EFDReinfClassificacaoServicosPrestadosList(generics.ListCreateAPIView):
    ...
    queryset = EFDReinfClassificacaoServicosPrestados.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = EFDReinfClassificacaoServicosPrestados.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = EFDReinfClassificacaoServicosPrestados.objects.filter(query).all()
    else:
        lista = EFDReinfClassificacaoServicosPrestados.objects.all()
