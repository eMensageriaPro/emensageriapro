...
class eSocialAtividadesPericulosasInsalubresEspeciaisList(generics.ListCreateAPIView):
    ...
    queryset = eSocialAtividadesPericulosasInsalubresEspeciais.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialAtividadesPericulosasInsalubresEspeciais.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialAtividadesPericulosasInsalubresEspeciais.objects.filter(query).all()
    else:
        lista = eSocialAtividadesPericulosasInsalubresEspeciais.objects.all()
