...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='esocial_inscricoes_tipos')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if esocial_inscricoes_tipos_id:
        esocial_inscricoes_tipos = get_object_or_404(eSocialInscricoesTipos, id = esocial_inscricoes_tipos_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if esocial_inscricoes_tipos_id:
            esocial_inscricoes_tipos_form = form_esocial_inscricoes_tipos(request.POST or None, instance = esocial_inscricoes_tipos, 
                                         initial = {'excluido': False})
        else:
            esocial_inscricoes_tipos_form = form_esocial_inscricoes_tipos(request.POST or None,
                                         initial = {'excluido': False})
        if request.method == 'POST':
            if esocial_inscricoes_tipos_form.is_valid():
            
                dados = esocial_inscricoes_tipos_form.cleaned_data
                obj = esocial_inscricoes_tipos_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not esocial_inscricoes_tipos_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'esocial_inscricoes_tipos', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(esocial_inscricoes_tipos), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'esocial_inscricoes_tipos', esocial_inscricoes_tipos_id, usuario_id, 2)
