...
class EFDReinfCodigosAtividadesProdutosServicosCPRBList(generics.ListCreateAPIView):
    ...
    queryset = EFDReinfCodigosAtividadesProdutosServicosCPRB.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = EFDReinfCodigosAtividadesProdutosServicosCPRB.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = EFDReinfCodigosAtividadesProdutosServicosCPRB.objects.filter(query).all()
    else:
        lista = EFDReinfCodigosAtividadesProdutosServicosCPRB.objects.all()
