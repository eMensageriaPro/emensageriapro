...
class eSocialCompatibilidadesLotacoesClassificacoesList(generics.ListCreateAPIView):
    ...
    queryset = eSocialCompatibilidadesLotacoesClassificacoes.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialCompatibilidadesLotacoesClassificacoes.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialCompatibilidadesLotacoesClassificacoes.objects.filter(query).all()
    else:
        lista = eSocialCompatibilidadesLotacoesClassificacoes.objects.all()
