...
class eSocialCompatibilidadesCategoriasClassificacoesLotacoesList(generics.ListCreateAPIView):
    ...
    queryset = eSocialCompatibilidadesCategoriasClassificacoesLotacoes.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialCompatibilidadesCategoriasClassificacoesLotacoes.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialCompatibilidadesCategoriasClassificacoesLotacoes.objects.filter(query).all()
    else:
        lista = eSocialCompatibilidadesCategoriasClassificacoesLotacoes.objects.all()
