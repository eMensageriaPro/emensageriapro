...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='municipios')
    ...
    if request.method == 'POST':
        ...
        if request.session['retorno_pagina']== 'municipios_salvar':
            return redirect('municipios', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'municipios_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if municipios_id:
            municipios_form = form_municipios(request.POST or None, instance = municipios, slug = db_slug)
        else:
        ...
        for field in municipios_form.fields.keys():
            municipios_form.fields[field].widget.attrs['ng-model'] = 'municipios_'+field
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = Municipios.objects.using(db_slug).filter(excluido = False).filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = Municipios.objects.using(db_slug).filter(excluido = False).filter(query).all()
    else:
        lista = Municipios.objects.using(db_slug).filter(excluido=False).all()
    lista_municipios = []
    for a in lista:
        dic = {}
        dic['key'] = a.codigo
        dic['value'] = '%s' % (a)
        lista_municipios.append(dic)
    dicionario['municipios'] = lista_municipios
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        municipios_lista = Municipios.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(municipios_lista) > 100:
            filtrar = True
            municipios_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')
    
        #municipios_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'municipios'
        context = {
        ...
        if for_print in (0,1):
            return render(request, 'municipios_listar.html', context)
        elif for_print == 2:
            #return render_to_pdf('tables/s1000_evtinfoempregador_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(
                request=request,
                template='municipios_listar.html',
                filename="municipios.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('municipios_listar.html', context)
            filename = "municipios.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/municipios_csv.html', context)
