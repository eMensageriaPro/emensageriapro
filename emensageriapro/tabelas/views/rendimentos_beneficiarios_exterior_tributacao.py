...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='rendimentos_beneficiarios_exterior_tributacao_salvar.html',
                filename="rendimentos_beneficiarios_exterior_tributacao.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('rendimentos_beneficiarios_exterior_tributacao_salvar.html', context)
            filename = "rendimentos_beneficiarios_exterior_tributacao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
