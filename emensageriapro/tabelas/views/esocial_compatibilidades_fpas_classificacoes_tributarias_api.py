...
class eSocialCompatibilidadesFPASClassificacoesTributariasList(generics.ListCreateAPIView):
    ...
    queryset = eSocialCompatibilidadesFPASClassificacoesTributarias.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialCompatibilidadesFPASClassificacoesTributarias.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialCompatibilidadesFPASClassificacoesTributarias.objects.filter(query).all()
    else:
        lista = eSocialCompatibilidadesFPASClassificacoesTributarias.objects.all()
