...
class eSocialAgentesCausadoresAcidentesTrabalhoList(generics.ListCreateAPIView):
    ...
    queryset = eSocialAgentesCausadoresAcidentesTrabalho.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialAgentesCausadoresAcidentesTrabalho.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialAgentesCausadoresAcidentesTrabalho.objects.filter(query).all()
    else:
        lista = eSocialAgentesCausadoresAcidentesTrabalho.objects.all()
