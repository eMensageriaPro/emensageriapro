...
class eSocialCodificacoesAcidenteTrabalhoList(generics.ListCreateAPIView):
    ...
    queryset = eSocialCodificacoesAcidenteTrabalho.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialCodificacoesAcidenteTrabalho.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialCodificacoesAcidenteTrabalho.objects.filter(query).all()
    else:
        lista = eSocialCodificacoesAcidenteTrabalho.objects.all()
