...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='efdreinf_codigos_atividades_produtos_servicos_cprb')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if efdreinf_codigos_atividades_produtos_servicos_cprb_id:
        efdreinf_codigos_atividades_produtos_servicos_cprb = get_object_or_404(EFDReinfCodigosAtividadesProdutosServicosCPRB, id = efdreinf_codigos_atividades_produtos_servicos_cprb_id)
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if efdreinf_codigos_atividades_produtos_servicos_cprb_form.is_valid():
                ...
                obj = efdreinf_codigos_atividades_produtos_servicos_cprb_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not efdreinf_codigos_atividades_produtos_servicos_cprb_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'efdreinf_codigos_atividades_produtos_servicos_cprb', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(efdreinf_codigos_atividades_produtos_servicos_cprb), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'efdreinf_codigos_atividades_produtos_servicos_cprb', efdreinf_codigos_atividades_produtos_servicos_cprb_id, usuario_id, 2)
