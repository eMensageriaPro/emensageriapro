...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='efdreinf_classificacao_tributaria')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if efdreinf_classificacao_tributaria_id:
        efdreinf_classificacao_tributaria = get_object_or_404(EFDReinfClassificacaoTributaria, id = efdreinf_classificacao_tributaria_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if efdreinf_classificacao_tributaria_id:
            efdreinf_classificacao_tributaria_form = form_efdreinf_classificacao_tributaria(request.POST or None, instance = efdreinf_classificacao_tributaria, 
                                         initial = {'excluido': False})
        else:
            efdreinf_classificacao_tributaria_form = form_efdreinf_classificacao_tributaria(request.POST or None,
                                         initial = {'excluido': False})
