...
class eSocialCodigoAliquotasFPASTerceirosList(generics.ListCreateAPIView):
    ...
    queryset = eSocialCodigoAliquotasFPASTerceiros.objects.all()
...
def json_search(request, search):
    ...
    if search.strip():
        try:
            ...
            lista = eSocialCodigoAliquotasFPASTerceiros.objects.filter(query).all()
        except:
            query = reduce(operator.and_, ((Q(descricao__icontains=item) | Q(codigo__icontains=item)) for item in lista))
            lista = eSocialCodigoAliquotasFPASTerceiros.objects.filter(query).all()
    else:
        lista = eSocialCodigoAliquotasFPASTerceiros.objects.all()
