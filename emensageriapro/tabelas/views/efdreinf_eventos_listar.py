...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        efdreinf_eventos_lista = EFDReinfEventos.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/efdreinf_eventos_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/efdreinf_eventos.csv', context)
