...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5003_dpsperapur = get_object_or_404(s5003dpsPerApur, id=pk)
        evento_dados = s5003_dpsperapur.evento()
    ...
    if request.user.has_perm('s5003.can_see_s5003dpsPerApur'):
        ...
        if request.method == 'POST':
            ...
            if s5003_dpsperapur_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5003_dpsperapur), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5003_dpsperapur_apagar', 
                    's5003_dpsperapur_salvar', 
                    's5003_dpsperapur'):
        ...
        s5003_dpsperapur_form = disabled_form_fields(
            s5003_dpsperapur_form, 
            request.user.has_perm('s5003.change_s5003dpsPerApur'))
        ...
        if tab or 's5003_dpsperapur' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5003_dpsperapur_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5003_dpsperapur').all()
