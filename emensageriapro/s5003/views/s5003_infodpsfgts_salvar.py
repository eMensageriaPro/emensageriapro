...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5003_infodpsfgts = get_object_or_404(s5003infoDpsFGTS, id=pk)
        evento_dados = s5003_infodpsfgts.evento()

    if request.user.has_perm('s5003.can_see_s5003infoDpsFGTS'):
        
        if pk:
        
            s5003_infodpsfgts_form = form_s5003_infodpsfgts(
                request.POST or None, 
                instance=s5003_infodpsfgts)
                                         
        else:
        
            s5003_infodpsfgts_form = form_s5003_infodpsfgts(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s5003_infodpsfgts_form.is_valid():
            
                obj = s5003_infodpsfgts_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5003_infodpsfgts), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5003_infodpsfgts_apagar', 
                    's5003_infodpsfgts_salvar', 
                    's5003_infodpsfgts'):
        ...
        s5003_infodpsfgts_form = disabled_form_fields(
            s5003_infodpsfgts_form, 
            request.user.has_perm('s5003.change_s5003infoDpsFGTS'))
        ...
        if pk:
            ...
            s5003_infotrabdps_form = form_s5003_infotrabdps(
                initial={ 's5003_infodpsfgts': s5003_infodpsfgts })
            ...
            s5003_infotrabdps_lista = s5003infoTrabDps.objects.\
                filter(s5003_infodpsfgts_id=s5003_infodpsfgts.id).all()
        ...
        if tab or 's5003_infodpsfgts' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5003_infodpsfgts_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5003_infodpsfgts').all()
