...
def listar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='s5003_baseperante')
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            post = True
            dict_fields = { 
                's5003_infobaseperante__icontains': 's5003_infobaseperante__icontains',
                'tpvalore__icontains': 'tpvalore__icontains',
                'remfgtse__icontains': 'remfgtse__icontains', }
        ...
        s5003_baseperante_lista = s5003basePerAntE.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/s5003_baseperante_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/s5003_baseperante.csv', context)
