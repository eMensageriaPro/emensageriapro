...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s5003basePerAntE.objects.using( db_slug ).filter(id = s5003_baseperante_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5003_baseperante_id:
            s5003_baseperante_form = form_s5003_baseperante(request.POST or None, instance = s5003_baseperante, slug = db_slug)
        else:
            s5003_baseperante_form = form_s5003_baseperante(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s5003_baseperante_form.is_valid():
                ...
                if s5003_baseperante_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s5003_baseperante_campos_multiple_passo1
                        s5003basePerAntE.objects.using(db_slug).filter(id=s5003_baseperante_id).update(**dados)
                        obj = s5003basePerAntE.objects.using(db_slug).get(id=s5003_baseperante_id)
                        #s5003_baseperante_editar_custom
                        #s5003_baseperante_campos_multiple_passo2
                    ...
                    #s5003_baseperante_cadastrar_campos_multiple_passo1
                    obj = s5003basePerAntE(**dados)
                    obj.save(using = db_slug)
                    #s5003_baseperante_cadastrar_custom
                    #s5003_baseperante_cadastrar_campos_multiple_passo2
