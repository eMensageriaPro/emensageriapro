...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2241_altinsalperic_id:
            s2241_altinsalperic_form = form_s2241_altinsalperic(request.POST or None, instance = s2241_altinsalperic, slug = db_slug)
        else:
            s2241_altinsalperic_form = form_s2241_altinsalperic(request.POST or None, slug = db_slug, initial={})
            ...
            if s2241_altinsalperic_form.is_valid():
                ...
                if s2241_altinsalperic_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2241_altinsalperic_campos_multiple_passo1
                        s2241altInsalPeric.objects.using(db_slug).filter(id=s2241_altinsalperic_id).update(**dados)
                        obj = s2241altInsalPeric.objects.using(db_slug).get(id=s2241_altinsalperic_id)
                        #s2241_altinsalperic_editar_custom
                        #s2241_altinsalperic_campos_multiple_passo2
                    ...
                    #s2241_altinsalperic_cadastrar_campos_multiple_passo1
                    obj = s2241altInsalPeric(**dados)
                    obj.save(using = db_slug)
                    #s2241_altinsalperic_cadastrar_custom
                    #s2241_altinsalperic_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2241altInsalPeric.objects.using( db_slug ).filter(id = s2241_altinsalperic_id).delete()
