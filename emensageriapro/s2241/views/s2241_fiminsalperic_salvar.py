...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s2241_fiminsalperic.evento()

    if request.user.has_perm('s2241.can_see_s2241fimInsalPeric'):
        
        if pk:
        
            s2241_fiminsalperic_form = form_s2241_fiminsalperic(
                request.POST or None, 
                instance=s2241_fiminsalperic)
                                         
        else:
        
            s2241_fiminsalperic_form = form_s2241_fiminsalperic(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s2241_fiminsalperic_form.is_valid():
            
                obj = s2241_fiminsalperic_form.save(request=request)
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2241_fiminsalperic), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2241_fiminsalperic_apagar', 
                    's2241_fiminsalperic_salvar', 
                    's2241_fiminsalperic'):
        ...
        s2241_fiminsalperic_form = disabled_form_fields(
            s2241_fiminsalperic_form, 
            request.user.has_perm('s2241.change_s2241fimInsalPeric'))
        ...
        if pk:
            ...
            s2241_fiminsalperic_infoamb_form = form_s2241_fiminsalperic_infoamb(
                initial={ 's2241_fiminsalperic': s2241_fiminsalperic })
            ...
            s2241_fiminsalperic_infoamb_lista = s2241fimInsalPericinfoAmb.objects.\
                filter(s2241_fiminsalperic_id=s2241_fiminsalperic.id).all()
        ...
        if tab or 's2241_fiminsalperic' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2241_fiminsalperic_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2241_fiminsalperic').all()
