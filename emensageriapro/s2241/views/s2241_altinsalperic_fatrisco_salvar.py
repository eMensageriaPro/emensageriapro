...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2241_altinsalperic_fatrisco = get_object_or_404(s2241altInsalPericfatRisco, id=pk)
        evento_dados = s2241_altinsalperic_fatrisco.evento()

    if request.user.has_perm('s2241.can_see_s2241altInsalPericfatRisco'):
        ...
        if request.method == 'POST':
            ...
            if s2241_altinsalperic_fatrisco_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2241_altinsalperic_fatrisco), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2241_altinsalperic_fatrisco_apagar', 
                    's2241_altinsalperic_fatrisco_salvar', 
                    's2241_altinsalperic_fatrisco'):
        ...
        s2241_altinsalperic_fatrisco_form = disabled_form_fields(
            s2241_altinsalperic_fatrisco_form, 
            request.user.has_perm('s2241.change_s2241altInsalPericfatRisco'))
        ...
        if tab or 's2241_altinsalperic_fatrisco' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2241_altinsalperic_fatrisco_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2241_altinsalperic_fatrisco').all()
        ...
        context = {
            ...
            'modulos': ['s2241', ],
            'paginas': ['s2241_altinsalperic_fatrisco', ],
