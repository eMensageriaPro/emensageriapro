...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2241_aposentesp = get_object_or_404(s2241aposentEsp, id=pk)
        evento_dados = s2241_aposentesp.evento()
    ...
    if request.user.has_perm('s2241.can_see_s2241aposentEsp'):
        ...
        if request.method == 'POST':
            ...
            if s2241_aposentesp_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2241_aposentesp), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2241_aposentesp_apagar', 
                    's2241_aposentesp_salvar', 
                    's2241_aposentesp'):
        ...
        s2241_aposentesp_form = disabled_form_fields(
            s2241_aposentesp_form, 
            request.user.has_perm('s2241.change_s2241aposentEsp'))
        ...
        if tab or 's2241_aposentesp' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2241_aposentesp_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2241_aposentesp').all()
