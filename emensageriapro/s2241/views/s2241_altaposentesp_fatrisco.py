...
def apagar(request, hash):
    ...
    if s2241_altaposentesp_fatrisco_id:
        dados_evento = s2241_altaposentesp_fatrisco.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2241_altaposentesp_fatrisco_apagar'] = 0
            dict_permissoes['s2241_altaposentesp_fatrisco_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2241_altaposentesp_fatrisco), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2241_altaposentesp_fatrisco', s2241_altaposentesp_fatrisco_id, usuario_id, 3)
        else:
...
class s2241altAposentEspfatRiscoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2241altAposentEspfatRisco.objects.using(db_slug).all()
    serializer_class = s2241altAposentEspfatRiscoSerializer
    # permission_classes = (IsAdminUser,)


class s2241altAposentEspfatRiscoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2241altAposentEspfatRisco.objects.using(db_slug).all()
    serializer_class = s2241altAposentEspfatRiscoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2241_altaposentesp_fatrisco_listar.html',
                filename="s2241_altaposentesp_fatrisco.pdf",
                context=context,
            ...
            response = render_to_response('s2241_altaposentesp_fatrisco_listar.html', context)
            filename = "s2241_altaposentesp_fatrisco.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2241_altaposentesp_fatrisco_csv.html', context)
            filename = "s2241_altaposentesp_fatrisco.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2241_altaposentesp_fatrisco_id:
            s2241_altaposentesp_fatrisco_form = form_s2241_altaposentesp_fatrisco(request.POST or None, instance = s2241_altaposentesp_fatrisco,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2241_altaposentesp_fatrisco_form = form_s2241_altaposentesp_fatrisco(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2241_altaposentesp_fatrisco_form.is_valid():

                dados = s2241_altaposentesp_fatrisco_form.cleaned_data
                obj = s2241_altaposentesp_fatrisco_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2241_altaposentesp_fatrisco_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2241_altaposentesp_fatrisco', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2241_altaposentesp_fatrisco), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2241_altaposentesp_fatrisco', s2241_altaposentesp_fatrisco_id, usuario_id, 2)
        ...
        if s2241_altaposentesp_fatrisco_id:
            if dados_evento['status'] != 0:
                s2241_altaposentesp_fatrisco_form = disabled_form_fields(s2241_altaposentesp_fatrisco_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2241_altaposentesp_fatrisco_id, tabela='s2241_altaposentesp_fatrisco').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2241_altaposentesp_fatrisco_salvar.html',
                filename="s2241_altaposentesp_fatrisco.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2241_altaposentesp_fatrisco_salvar.html', context)
            filename = "s2241_altaposentesp_fatrisco.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
