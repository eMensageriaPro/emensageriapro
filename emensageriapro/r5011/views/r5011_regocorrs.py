...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r5011_regocorrs')
    ...
    r5011_regocorrs = get_object_or_404(r5011regOcorrs.objects.using( db_slug ), excluido = False, id = r5011_regocorrs_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            r5011regOcorrs.objects.using( db_slug ).filter(id = r5011_regocorrs_id).delete()
            #r5011_regocorrs_apagar_custom
            #r5011_regocorrs_apagar_custom
...
class r5011regOcorrsList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r5011regOcorrs.objects.using(db_slug).all()
    serializer_class = r5011regOcorrsSerializer
    permission_classes = (IsAdminUser,)


class r5011regOcorrsDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r5011regOcorrs.objects.using(db_slug).all()
    serializer_class = r5011regOcorrsSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r5011_regocorrs_lista = r5011regOcorrs.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(r5011_regocorrs_lista) > 100:
            filtrar = True
            r5011_regocorrs_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')
    
        #r5011_regocorrs_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'r5011_regocorrs'
        context = {
        ...
        if for_print in (0,1):
            return render(request, 'r5011_regocorrs_listar.html', context)
        elif for_print == 2:
            #return render_to_pdf('tables/s1000_evtinfoempregador_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(
                request=request,
                template='r5011_regocorrs_listar.html',
            ...
            response = render_to_response('r5011_regocorrs_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r5011_regocorrs_csv.html', context)
            filename = "r5011_regocorrs.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r5011_regocorrs_id:
            r5011_regocorrs_form = form_r5011_regocorrs(request.POST or None, instance = r5011_regocorrs, slug = db_slug)
        else:
            r5011_regocorrs_form = form_r5011_regocorrs(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r5011_regocorrs_form.is_valid():
                ...
                if r5011_regocorrs_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r5011_regocorrs_campos_multiple_passo1
                        r5011regOcorrs.objects.using(db_slug).filter(id=r5011_regocorrs_id).update(**dados)
                        obj = r5011regOcorrs.objects.using(db_slug).get(id=r5011_regocorrs_id)
                        #r5011_regocorrs_editar_custom
                        #r5011_regocorrs_campos_multiple_passo2
                    ...
                    #r5011_regocorrs_cadastrar_campos_multiple_passo1
                    obj = r5011regOcorrs(**dados)
                    obj.save(using = db_slug)
                    #r5011_regocorrs_cadastrar_custom
                    #r5011_regocorrs_cadastrar_campos_multiple_passo2
        ...
        r5011_regocorrs_form = disabled_form_fields(r5011_regocorrs_form, permissao.permite_editar)
        if r5011_regocorrs_id:
            if dados_evento['status'] != 0:
                r5011_regocorrs_form = disabled_form_fields(r5011_regocorrs_form, 0)
        #r5011_regocorrs_campos_multiple_passo3

        for field in r5011_regocorrs_form.fields.keys():
            r5011_regocorrs_form.fields[field].widget.attrs['ng-model'] = 'r5011_regocorrs_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r5011_regocorrs_id, tabela='r5011_regocorrs').all()
        context = {
