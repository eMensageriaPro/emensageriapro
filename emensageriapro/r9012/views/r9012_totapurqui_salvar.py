...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        r9012_totapurqui = get_object_or_404(r9012totApurQui, id=pk)
        evento_dados = r9012_totapurqui.evento()
    ...
    if request.user.has_perm('r9012.can_see_r9012totApurQui'):
        ...
        if request.method == 'POST':
            ...
            if r9012_totapurqui_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(r9012_totapurqui), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    'r9012_totapurqui_apagar', 
                    'r9012_totapurqui_salvar', 
                    'r9012_totapurqui'):
        ...
        r9012_totapurqui_form = disabled_form_fields(
            r9012_totapurqui_form, 
            request.user.has_perm('r9012.change_r9012totApurQui'))
        ...
        if tab or 'r9012_totapurqui' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 'r9012_totapurqui_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='r9012_totapurqui').all()
