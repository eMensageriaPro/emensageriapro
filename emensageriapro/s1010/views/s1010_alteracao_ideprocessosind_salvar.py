...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s1010_alteracao_ideprocessosind.evento()

    if request.user.has_perm('s1010.can_see_s1010alteracaoideProcessoSIND'):
        ...
        if request.method == 'POST':
            ...
            if s1010_alteracao_ideprocessosind_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s1010_alteracao_ideprocessosind), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1010_alteracao_ideprocessosind_apagar', 
                    's1010_alteracao_ideprocessosind_salvar', 
                    's1010_alteracao_ideprocessosind'):
        ...
        s1010_alteracao_ideprocessosind_form = disabled_form_fields(
            s1010_alteracao_ideprocessosind_form, 
            request.user.has_perm('s1010.change_s1010alteracaoideProcessoSIND'))
        ...
        if tab or 's1010_alteracao_ideprocessosind' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1010_alteracao_ideprocessosind_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1010_alteracao_ideprocessosind').all()
