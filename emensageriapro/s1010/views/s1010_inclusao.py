...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1010_inclusao')
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1010_inclusao), indent=4, sort_keys=True, default=str)
            s1010inclusao.objects.using( db_slug ).filter(id = s1010_inclusao_id).delete()
            #s1010_inclusao_apagar_custom
            #s1010_inclusao_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1010_inclusao', s1010_inclusao_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 's1010_inclusao_salvar':
            return redirect('s1010_inclusao', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's1010_inclusao_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1010inclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1010inclusao.objects.using(db_slug).all()
    serializer_class = s1010inclusaoSerializer
    permission_classes = (IsAdminUser,)


class s1010inclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1010inclusao.objects.using(db_slug).all()
    serializer_class = s1010inclusaoSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1010_inclusao_lista = s1010inclusao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        request.session["retorno_pagina"] = 's1010_inclusao'
        context = {
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1010_inclusao_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1010_inclusao_id:
            s1010_inclusao_form = form_s1010_inclusao(request.POST or None, instance = s1010_inclusao, slug = db_slug)
        else:
            s1010_inclusao_form = form_s1010_inclusao(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s1010_inclusao_form.is_valid():
                ...
                if s1010_inclusao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1010_inclusao_campos_multiple_passo1
                        s1010inclusao.objects.using(db_slug).filter(id=s1010_inclusao_id).update(**dados)
                        obj = s1010inclusao.objects.using(db_slug).get(id=s1010_inclusao_id)
                        #s1010_inclusao_editar_custom
                        #s1010_inclusao_campos_multiple_passo2
                    ...
                    #s1010_inclusao_cadastrar_campos_multiple_passo1
                    obj = s1010inclusao(**dados)
                    obj.save(using = db_slug)
                    #s1010_inclusao_cadastrar_custom
                    #s1010_inclusao_cadastrar_campos_multiple_passo2
        ...
        for field in s1010_inclusao_form.fields.keys():
            s1010_inclusao_form.fields[field].widget.attrs['ng-model'] = 's1010_inclusao_'+field
        ...
        if s1010_inclusao_id:
            ...
            s1010_inclusao_ideprocessocp_form = form_s1010_inclusao_ideprocessocp(initial={ 's1010_inclusao': s1010_inclusao }, slug=db_slug)
            s1010_inclusao_ideprocessocp_form.fields['s1010_inclusao'].widget.attrs['readonly'] = True
            s1010_inclusao_ideprocessocp_lista = s1010inclusaoideProcessoCP.objects.using( db_slug ).filter(excluido = False, s1010_inclusao_id=s1010_inclusao.id).all()
            s1010_inclusao_ideprocessoirrf_form = form_s1010_inclusao_ideprocessoirrf(initial={ 's1010_inclusao': s1010_inclusao }, slug=db_slug)
            s1010_inclusao_ideprocessoirrf_form.fields['s1010_inclusao'].widget.attrs['readonly'] = True
            s1010_inclusao_ideprocessoirrf_lista = s1010inclusaoideProcessoIRRF.objects.using( db_slug ).filter(excluido = False, s1010_inclusao_id=s1010_inclusao.id).all()
            s1010_inclusao_ideprocessofgts_form = form_s1010_inclusao_ideprocessofgts(initial={ 's1010_inclusao': s1010_inclusao }, slug=db_slug)
            s1010_inclusao_ideprocessofgts_form.fields['s1010_inclusao'].widget.attrs['readonly'] = True
            s1010_inclusao_ideprocessofgts_lista = s1010inclusaoideProcessoFGTS.objects.using( db_slug ).filter(excluido = False, s1010_inclusao_id=s1010_inclusao.id).all()
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1010_inclusao_id, tabela='s1010_inclusao').all()
        context = {
