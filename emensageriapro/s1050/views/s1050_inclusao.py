...
def apagar(request, hash):
    ...
    if s1050_inclusao_id:
        dados_evento = s1050_inclusao.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1050_inclusao_apagar'] = 0
            dict_permissoes['s1050_inclusao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1050_inclusao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1050_inclusao', s1050_inclusao_id, usuario_id, 3)
        else:
...
class s1050inclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1050inclusao.objects.using(db_slug).all()
    serializer_class = s1050inclusaoSerializer
    # permission_classes = (IsAdminUser,)


class s1050inclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1050inclusao.objects.using(db_slug).all()
    serializer_class = s1050inclusaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1050_inclusao_listar.html',
            ...
            response = render_to_response('s1050_inclusao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1050_inclusao_csv.html', context)
            filename = "s1050_inclusao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1050_inclusao_id:
            s1050_inclusao_form = form_s1050_inclusao(request.POST or None, instance = s1050_inclusao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1050_inclusao_form = form_s1050_inclusao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1050_inclusao_form.is_valid():
                ...
                if not s1050_inclusao_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1050_inclusao', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1050_inclusao), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1050_inclusao', s1050_inclusao_id, usuario_id, 2)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1050_inclusao_id, tabela='s1050_inclusao').all()
        context = {
