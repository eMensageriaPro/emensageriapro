...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s1050_inclusao_horariointervalo.evento()

    if request.user.has_perm('s1050.can_see_s1050inclusaohorarioIntervalo'):
        ...
        if request.method == 'POST':
            ...
            if s1050_inclusao_horariointervalo_form.is_valid():
                ...
                obj = s1050_inclusao_horariointervalo_form.save(request=request)
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s1050_inclusao_horariointervalo), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1050_inclusao_horariointervalo_apagar', 
                    's1050_inclusao_horariointervalo_salvar', 
                    's1050_inclusao_horariointervalo'):
        ...
        s1050_inclusao_horariointervalo_form = disabled_form_fields(
            s1050_inclusao_horariointervalo_form, 
            request.user.has_perm('s1050.change_s1050inclusaohorarioIntervalo'))
        ...
        if tab or 's1050_inclusao_horariointervalo' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1050_inclusao_horariointervalo_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1050_inclusao_horariointervalo').all()
