...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            ...
            obj = r4020infoProcJud.objects.get(id = r4020_infoprocjud_id)
            ...
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r4020_infoprocjud', r4020_infoprocjud_id, usuario_id, 3)
        else:
            messages.error(request, u'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
            
        if request.session['retorno_pagina']== 'r4020_infoprocjud_salvar':
            return redirect('r4020_infoprocjud', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'r4020_infoprocjud_apagar.html', context)