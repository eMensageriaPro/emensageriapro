...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('s1000.can_see_s1000inclusaosoftwareHouse'):
        ...
        if request.method == 'POST':
            ...
            if s1000_inclusao_softwarehouse_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1000_inclusao_softwarehouse), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1000_inclusao_softwarehouse_apagar', 
        ...
        if tab or 's1000_inclusao_softwarehouse' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1000_inclusao_softwarehouse_salvar'
