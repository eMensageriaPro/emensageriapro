...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1000_inclusao_dadosisencao = get_object_or_404(s1000inclusaodadosIsencao, id=pk)
        evento_dados = s1000_inclusao_dadosisencao.evento()

    if request.user.has_perm('s1000.can_see_s1000inclusaodadosIsencao'):
        ...
        if request.method == 'POST':
            ...
            if s1000_inclusao_dadosisencao_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1000_inclusao_dadosisencao), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1000_inclusao_dadosisencao_apagar', 
                    's1000_inclusao_dadosisencao_salvar', 
                    's1000_inclusao_dadosisencao'):
        ...
        s1000_inclusao_dadosisencao_form = disabled_form_fields(
            s1000_inclusao_dadosisencao_form, 
            request.user.has_perm('s1000.change_s1000inclusaodadosIsencao'))
        ...
        if tab or 's1000_inclusao_dadosisencao' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1000_inclusao_dadosisencao_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1000_inclusao_dadosisencao').all()
        ...
        context = {
            ...
            'modulos': ['s1000', ],
            'paginas': ['s1000_inclusao_dadosisencao', ],
