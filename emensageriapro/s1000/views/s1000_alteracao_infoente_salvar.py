...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1000_alteracao_infoente = get_object_or_404(s1000alteracaoinfoEnte, id=pk)
        evento_dados = s1000_alteracao_infoente.evento()

    if request.user.has_perm('s1000.can_see_s1000alteracaoinfoEnte'):
        ...
        if request.method == 'POST':
            ...
            if s1000_alteracao_infoente_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1000_alteracao_infoente), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1000_alteracao_infoente_apagar', 
                    's1000_alteracao_infoente_salvar', 
                    's1000_alteracao_infoente'):
        ...
        s1000_alteracao_infoente_form = disabled_form_fields(
            s1000_alteracao_infoente_form, 
            request.user.has_perm('s1000.change_s1000alteracaoinfoEnte'))
        ...
        if tab or 's1000_alteracao_infoente' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1000_alteracao_infoente_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1000_alteracao_infoente').all()
