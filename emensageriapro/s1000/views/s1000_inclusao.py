...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1000_inclusao_id:
            s1000_inclusao_form = form_s1000_inclusao(request.POST or None, instance = s1000_inclusao, slug = db_slug)
        else:
            s1000_inclusao_form = form_s1000_inclusao(request.POST or None, slug = db_slug, initial={})
            ...
            if s1000_inclusao_form.is_valid():
                ...
                if s1000_inclusao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1000_inclusao_campos_multiple_passo1
                        s1000inclusao.objects.using(db_slug).filter(id=s1000_inclusao_id).update(**dados)
                        obj = s1000inclusao.objects.using(db_slug).get(id=s1000_inclusao_id)
                        #s1000_inclusao_editar_custom
                        #s1000_inclusao_campos_multiple_passo2
                    ...
                    #s1000_inclusao_cadastrar_campos_multiple_passo1
                    obj = s1000inclusao(**dados)
                    obj.save(using = db_slug)
                    #s1000_inclusao_cadastrar_custom
                    #s1000_inclusao_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1000inclusao.objects.using( db_slug ).filter(id = s1000_inclusao_id).delete()
