...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1000_inclusao_infoop')
    ...
    if s1000_inclusao_infoop_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['s1000_inclusao_infoop_apagar'] = 0
            dict_permissoes['s1000_inclusao_infoop_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1000_inclusao_infoop), indent=4, sort_keys=True, default=str)
            s1000inclusaoinfoOP.objects.using( db_slug ).filter(id = s1000_inclusao_infoop_id).delete()
            #s1000_inclusao_infoop_apagar_custom
            #s1000_inclusao_infoop_apagar_custom
    ...
    return render(request, 's1000_inclusao_infoop_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1000inclusaoinfoOPList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1000inclusaoinfoOP.objects.using(db_slug).all()
    serializer_class = s1000inclusaoinfoOPSerializer
    permission_classes = (IsAdminUser,)


class s1000inclusaoinfoOPDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1000inclusaoinfoOP.objects.using(db_slug).all()
    serializer_class = s1000inclusaoinfoOPSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1000_inclusao_infoop_lista = s1000inclusaoinfoOP.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1000_inclusao_infoop_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1000_inclusao_infoop_id:
            s1000_inclusao_infoop_form = form_s1000_inclusao_infoop(request.POST or None, instance = s1000_inclusao_infoop, slug = db_slug)
        else:
            s1000_inclusao_infoop_form = form_s1000_inclusao_infoop(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s1000_inclusao_infoop_form.is_valid():
                dados = s1000_inclusao_infoop_form.cleaned_data
                import json
                from django.forms.models import model_to_dict
                if s1000_inclusao_infoop_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1000_inclusao_infoop_campos_multiple_passo1
                        s1000inclusaoinfoOP.objects.using(db_slug).filter(id=s1000_inclusao_infoop_id).update(**dados)
                        obj = s1000inclusaoinfoOP.objects.using(db_slug).get(id=s1000_inclusao_infoop_id)
                        #s1000_inclusao_infoop_editar_custom
                        #s1000_inclusao_infoop_campos_multiple_passo2
                        messages.success(request, 'Alterado com sucesso!')
                        gravar_auditoria(json.dumps(model_to_dict(s1000_inclusao_infoop), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1000_inclusao_infoop', s1000_inclusao_infoop_id, usuario_id, 2)
                    else:
                    ...
                    #s1000_inclusao_infoop_cadastrar_campos_multiple_passo1
                    obj = s1000inclusaoinfoOP(**dados)
                    obj.save(using = db_slug)
                    #s1000_inclusao_infoop_cadastrar_custom
                    #s1000_inclusao_infoop_cadastrar_campos_multiple_passo2
                    messages.success(request, 'Cadastrado com sucesso!')
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1000_inclusao_infoop', obj.id, usuario_id, 1)
                    if request.session['retorno_pagina'] not in ('s1000_inclusao_infoop_apagar', 's1000_inclusao_infoop_salvar', 's1000_inclusao_infoop'):
                        return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                    if s1000_inclusao_infoop_id != obj.id:
                        url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % (obj.id) )
                        return redirect('s1000_inclusao_infoop_salvar', hash=url_hash)
            else:
                messages.error(request, 'Erro ao salvar!')
        s1000_inclusao_infoop_form = disabled_form_fields(s1000_inclusao_infoop_form, permissao.permite_editar)
        if s1000_inclusao_infoop_id:
            if dados_evento['status'] != 0:
                s1000_inclusao_infoop_form = disabled_form_fields(s1000_inclusao_infoop_form, 0)
        #s1000_inclusao_infoop_campos_multiple_passo3

        for field in s1000_inclusao_infoop_form.fields.keys():
        ...
        if s1000_inclusao_infoop_id:
            ...
            s1000_inclusao_infoente_form = form_s1000_inclusao_infoente(initial={ 's1000_inclusao_infoop': s1000_inclusao_infoop }, slug=db_slug)
            s1000_inclusao_infoente_form.fields['s1000_inclusao_infoop'].widget.attrs['readonly'] = True
            s1000_inclusao_infoente_lista = s1000inclusaoinfoEnte.objects.using( db_slug ).filter(excluido = False, s1000_inclusao_infoop_id=s1000_inclusao_infoop.id).all()
        else:
            s1000_inclusao_infoop = None
        #s1000_inclusao_infoop_salvar_custom_variaveis#
        tabelas_secundarias = []
        #[FUNCOES_ESPECIAIS_SALVAR]
        if dict_hash['tab'] or 's1000_inclusao_infoop' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 's1000_inclusao_infoop_salvar'
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1000_inclusao_infoop_id, tabela='s1000_inclusao_infoop').all()
        context = {
