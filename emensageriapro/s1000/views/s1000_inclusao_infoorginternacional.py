...
def apagar(request, hash):
    ...
    if s1000_inclusao_infoorginternacional_id:
        dados_evento = s1000_inclusao_infoorginternacional.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s1000_inclusao_infoorginternacional_apagar'] = 0
            dict_permissoes['s1000_inclusao_infoorginternacional_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1000_inclusao_infoorginternacional), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1000_inclusao_infoorginternacional', s1000_inclusao_infoorginternacional_id, usuario_id, 3)
        else:
...
class s1000inclusaoinfoOrgInternacionalList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1000inclusaoinfoOrgInternacional.objects.using(db_slug).all()
    serializer_class = s1000inclusaoinfoOrgInternacionalSerializer
    # permission_classes = (IsAdminUser,)


class s1000inclusaoinfoOrgInternacionalDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1000inclusaoinfoOrgInternacional.objects.using(db_slug).all()
    serializer_class = s1000inclusaoinfoOrgInternacionalSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1000_inclusao_infoorginternacional_listar.html',
                filename="s1000_inclusao_infoorginternacional.pdf",
                context=context,
            ...
            response = render_to_response('s1000_inclusao_infoorginternacional_listar.html', context)
            filename = "s1000_inclusao_infoorginternacional.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1000_inclusao_infoorginternacional_csv.html', context)
            filename = "s1000_inclusao_infoorginternacional.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if s1000_inclusao_infoorginternacional_form.is_valid():
                ...
                obj = s1000_inclusao_infoorginternacional_form.save(request=request)
                messages.success(request, 'Salvo com sucesso!')

                if not s1000_inclusao_infoorginternacional_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1000_inclusao_infoorginternacional', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1000_inclusao_infoorginternacional), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1000_inclusao_infoorginternacional', s1000_inclusao_infoorginternacional_id, usuario_id, 2)
        ...
        if s1000_inclusao_infoorginternacional_id:
            if dados_evento['status'] != 0:
                s1000_inclusao_infoorginternacional_form = disabled_form_fields(s1000_inclusao_infoorginternacional_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1000_inclusao_infoorginternacional_id, tabela='s1000_inclusao_infoorginternacional').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1000_inclusao_infoorginternacional_salvar.html',
                filename="s1000_inclusao_infoorginternacional.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1000_inclusao_infoorginternacional_salvar.html', context)
            filename = "s1000_inclusao_infoorginternacional.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
