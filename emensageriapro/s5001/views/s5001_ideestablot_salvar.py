...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5001_ideestablot = get_object_or_404(s5001ideEstabLot, id=pk)
        evento_dados = s5001_ideestablot.evento()

    if request.user.has_perm('s5001.can_see_s5001ideEstabLot'):
        
        if pk:
        
            s5001_ideestablot_form = form_s5001_ideestablot(
                request.POST or None, 
                instance=s5001_ideestablot)
                                         
        else:
        
            s5001_ideestablot_form = form_s5001_ideestablot(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s5001_ideestablot_form.is_valid():
            
                obj = s5001_ideestablot_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5001_ideestablot), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5001_ideestablot_apagar', 
                    's5001_ideestablot_salvar', 
                    's5001_ideestablot'):
        ...
        s5001_ideestablot_form = disabled_form_fields(
            s5001_ideestablot_form, 
            request.user.has_perm('s5001.change_s5001ideEstabLot'))
        ...
        if pk:
            ...
            s5001_infocategincid_form = form_s5001_infocategincid(
                initial={ 's5001_ideestablot': s5001_ideestablot })
            ...
            s5001_infocategincid_lista = s5001infoCategIncid.objects.\
                filter(s5001_ideestablot_id=s5001_ideestablot.id).all()
        ...
        if tab or 's5001_ideestablot' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5001_ideestablot_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5001_ideestablot').all()
