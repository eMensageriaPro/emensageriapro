...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s5001_infocp')
    ...
    if s5001_infocp_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['s5001_infocp_apagar'] = 0
            dict_permissoes['s5001_infocp_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5001_infocp), indent=4, sort_keys=True, default=str)
            s5001infoCp.objects.using( db_slug ).filter(id = s5001_infocp_id).delete()
            #s5001_infocp_apagar_custom
            #s5001_infocp_apagar_custom
    ...
    return render(request, 's5001_infocp_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s5001_infocp_form.is_valid():
                ...
                if s5001_infocp_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s5001_infocp_campos_multiple_passo1
                        s5001infoCp.objects.using(db_slug).filter(id=s5001_infocp_id).update(**dados)
                        obj = s5001infoCp.objects.using(db_slug).get(id=s5001_infocp_id)
                        #s5001_infocp_editar_custom
                        #s5001_infocp_campos_multiple_passo2
                    ...
                    #s5001_infocp_cadastrar_campos_multiple_passo1
                    obj = s5001infoCp(**dados)
                    obj.save(using = db_slug)
                    #s5001_infocp_cadastrar_custom
                    #s5001_infocp_cadastrar_campos_multiple_passo2
        ...
        s5001_infocp_form = disabled_form_fields(s5001_infocp_form, permissao.permite_editar)
        if s5001_infocp_id:
            if dados_evento['status'] != 0:
                s5001_infocp_form = disabled_form_fields(s5001_infocp_form, 0)
        #s5001_infocp_campos_multiple_passo3

        for field in s5001_infocp_form.fields.keys():
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5001_infocp_id, tabela='s5001_infocp').all()
        context = {
...
class s5001infoCpList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s5001infoCp.objects.using(db_slug).all()
    serializer_class = s5001infoCpSerializer
    permission_classes = (IsAdminUser,)


class s5001infoCpDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s5001infoCp.objects.using(db_slug).all()
    serializer_class = s5001infoCpSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s5001_infocp_lista = s5001infoCp.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s5001_infocp_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5001_infocp_listar.html',
                filename="s5001_infocp.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s5001_infocp_listar.html', context)
            filename = "s5001_infocp.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5001_infocp_csv.html', context)
            filename = "s5001_infocp.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
