...
class s2250cancAvPrevioList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2250cancAvPrevio.objects.using(db_slug).all()
    serializer_class = s2250cancAvPrevioSerializer
    # permission_classes = (IsAdminUser,)


class s2250cancAvPrevioDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2250cancAvPrevio.objects.using(db_slug).all()
    serializer_class = s2250cancAvPrevioSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2250_cancavprevio_id:
            s2250_cancavprevio_form = form_s2250_cancavprevio(request.POST or None, instance = s2250_cancavprevio,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2250_cancavprevio_form = form_s2250_cancavprevio(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
