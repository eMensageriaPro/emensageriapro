...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        RetornosEventosOcorrencias.objects.using( db_slug ).filter(id = retornos_eventos_ocorrencias_id).update(excluido = True)
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if retornos_eventos_ocorrencias_form.is_valid():
                dados = retornos_eventos_ocorrencias_form.cleaned_data
                if retornos_eventos_ocorrencias_id:
                    dados['modificado_por_id'] = usuario_id
                    dados['modificado_em'] = datetime.datetime.now()
                    #retornos_eventos_ocorrencias_campos_multiple_passo1
                    RetornosEventosOcorrencias.objects.using(db_slug).filter(id=retornos_eventos_ocorrencias_id).update(**dados)
                    obj = RetornosEventosOcorrencias.objects.using(db_slug).get(id=retornos_eventos_ocorrencias_id)
                    #retornos_eventos_ocorrencias_editar_custom
                    ...
                    #retornos_eventos_ocorrencias_cadastrar_campos_multiple_passo1
                    obj = RetornosEventosOcorrencias(**dados)
                    obj.save(using = db_slug)
                    #retornos_eventos_ocorrencias_cadastrar_custom
                    #retornos_eventos_ocorrencias_cadastrar_campos_multiple_passo2
