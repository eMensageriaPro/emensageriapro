...
class ImportacaoArquivosList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = ImportacaoArquivos.objects.using(db_slug).all()
    serializer_class = ImportacaoArquivosSerializer
    # permission_classes = (IsAdminUser,)


class ImportacaoArquivosDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = ImportacaoArquivos.objects.using(db_slug).all()
    serializer_class = ImportacaoArquivosSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if importacao_arquivos_id:
            ...
            importacao_arquivos_form = form_importacao_arquivos(request.POST or None, slug = db_slug,
                                initial = {'excluido': False})
        if request.method == 'POST':
