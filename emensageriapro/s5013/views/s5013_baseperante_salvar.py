...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5013_baseperante = get_object_or_404(s5013basePerAntE, id=pk)
        evento_dados = s5013_baseperante.evento()

    if request.user.has_perm('s5013.can_see_s5013basePerAntE'):
        ...
        if request.method == 'POST':
            ...
            if s5013_baseperante_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5013_baseperante), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5013_baseperante_apagar', 
                    's5013_baseperante_salvar', 
                    's5013_baseperante'):
        ...
        s5013_baseperante_form = disabled_form_fields(
            s5013_baseperante_form, 
            request.user.has_perm('s5013.change_s5013basePerAntE'))
        ...
        if tab or 's5013_baseperante' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5013_baseperante_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5013_baseperante').all()
