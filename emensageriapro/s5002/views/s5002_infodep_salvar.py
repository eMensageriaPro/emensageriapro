...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5002_infodep = get_object_or_404(s5002infoDep, id=pk)
        evento_dados = s5002_infodep.evento()

    if request.user.has_perm('s5002.can_see_s5002infoDep'):
        
        if pk:
        
            s5002_infodep_form = form_s5002_infodep(
                request.POST or None, 
                instance=s5002_infodep)
                                         
        else:
        
            s5002_infodep_form = form_s5002_infodep(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s5002_infodep_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's5002_infodep_apagar', 
                    's5002_infodep_salvar', 
                    's5002_infodep'):
        ...
        s5002_infodep_form = disabled_form_fields(
            s5002_infodep_form, 
            request.user.has_perm('s5002.change_s5002infoDep'))
        ...
        if tab or 's5002_infodep' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5002_infodep_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5002_infodep').all()
