...
def apagar(request, hash):
    ...
    if s5002_irrf_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s5002_irrf_apagar'] = 0
            dict_permissoes['s5002_irrf_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5002_irrf), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's5002_irrf', s5002_irrf_id, usuario_id, 3)
        else:
...
class s5002irrfList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s5002irrf.objects.using(db_slug).all()
    serializer_class = s5002irrfSerializer
    # permission_classes = (IsAdminUser,)


class s5002irrfDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s5002irrf.objects.using(db_slug).all()
    serializer_class = s5002irrfSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5002_irrf_listar.html',
                filename="s5002_irrf.pdf",
                context=context,
            ...
            response = render_to_response('s5002_irrf_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5002_irrf_csv.html', context)
            filename = "s5002_irrf.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5002_irrf_id:
            s5002_irrf_form = form_s5002_irrf(request.POST or None, instance = s5002_irrf,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s5002_irrf_form = form_s5002_irrf(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
        ...
        if s5002_irrf_id:
            if dados_evento['status'] != 0:
                s5002_irrf_form = disabled_form_fields(s5002_irrf_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5002_irrf_id, tabela='s5002_irrf').all()
        context = {
