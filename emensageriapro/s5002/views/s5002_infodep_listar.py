...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            post = True
            dict_fields = { 
                's5002_evtirrfbenef__icontains': 's5002_evtirrfbenef__icontains',
                'vrdeddep__icontains': 'vrdeddep__icontains', }
        ...
        s5002_infodep_lista = s5002infoDep.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/s5002_infodep_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/s5002_infodep.csv', context)
