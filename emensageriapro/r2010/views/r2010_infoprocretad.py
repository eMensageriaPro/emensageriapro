...
class r2010infoProcRetAdList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2010infoProcRetAd.objects.using(db_slug).all()
    serializer_class = r2010infoProcRetAdSerializer
    # permission_classes = (IsAdminUser,)


class r2010infoProcRetAdDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2010infoProcRetAd.objects.using(db_slug).all()
    serializer_class = r2010infoProcRetAdSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2010_infoprocretad_id:
            r2010_infoprocretad_form = form_r2010_infoprocretad(request.POST or None, instance = r2010_infoprocretad,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            r2010_infoprocretad_form = form_r2010_infoprocretad(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
