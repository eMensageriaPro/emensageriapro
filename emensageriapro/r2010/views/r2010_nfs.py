...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2010_nfs')
    ...
    if r2010_nfs_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['r2010_nfs_apagar'] = 0
            dict_permissoes['r2010_nfs_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(r2010_nfs), indent=4, sort_keys=True, default=str)
            r2010nfs.objects.using( db_slug ).filter(id = r2010_nfs_id).delete()
            #r2010_nfs_apagar_custom
            #r2010_nfs_apagar_custom
...
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
    ...
    return render(request, 'r2010_nfs_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class r2010nfsList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2010nfs.objects.using(db_slug).all()
    serializer_class = r2010nfsSerializer
    permission_classes = (IsAdminUser,)


class r2010nfsDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2010nfs.objects.using(db_slug).all()
    serializer_class = r2010nfsSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2010_nfs_lista = r2010nfs.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(r2010_nfs_lista) > 100:
            filtrar = True
...
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='r2010_nfs_listar.html',
            ...
            response = render_to_response('r2010_nfs_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2010_nfs_csv.html', context)
            filename = "r2010_nfs.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2010_nfs_id:
            r2010_nfs_form = form_r2010_nfs(request.POST or None, instance = r2010_nfs, slug = db_slug)
        else:
            r2010_nfs_form = form_r2010_nfs(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r2010_nfs_form.is_valid():
                ...
                if r2010_nfs_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r2010_nfs_campos_multiple_passo1
                        r2010nfs.objects.using(db_slug).filter(id=r2010_nfs_id).update(**dados)
                        obj = r2010nfs.objects.using(db_slug).get(id=r2010_nfs_id)
                        #r2010_nfs_editar_custom
                        #r2010_nfs_campos_multiple_passo2
                    ...
                    #r2010_nfs_cadastrar_campos_multiple_passo1
                    obj = r2010nfs(**dados)
                    obj.save(using = db_slug)
                    #r2010_nfs_cadastrar_custom
                    #r2010_nfs_cadastrar_campos_multiple_passo2
        ...
        r2010_nfs_form = disabled_form_fields(r2010_nfs_form, permissao.permite_editar)
        if r2010_nfs_id:
            if dados_evento['status'] != 0:
                r2010_nfs_form = disabled_form_fields(r2010_nfs_form, 0)
        #r2010_nfs_campos_multiple_passo3

        for field in r2010_nfs_form.fields.keys():
...
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r2010_nfs_id, tabela='r2010_nfs').all()
        context = {
