...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2010_infotpserv')
    ...
    r2010_infotpserv = get_object_or_404(r2010infoTpServ.objects.using( db_slug ), excluido = False, id = r2010_infotpserv_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(r2010_infotpserv), indent=4, sort_keys=True, default=str)
            r2010infoTpServ.objects.using( db_slug ).filter(id = r2010_infotpserv_id).delete()
            #r2010_infotpserv_apagar_custom
            #r2010_infotpserv_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             'r2010_infotpserv', r2010_infotpserv_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 'r2010_infotpserv_salvar':
            return redirect('r2010_infotpserv', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'r2010_infotpserv_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class r2010infoTpServList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2010infoTpServ.objects.using(db_slug).all()
    serializer_class = r2010infoTpServSerializer
    permission_classes = (IsAdminUser,)


class r2010infoTpServDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2010infoTpServ.objects.using(db_slug).all()
    serializer_class = r2010infoTpServSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2010_infotpserv_lista = r2010infoTpServ.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2010_infotpserv_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2010_infotpserv_id:
            r2010_infotpserv_form = form_r2010_infotpserv(request.POST or None, instance = r2010_infotpserv, slug = db_slug)
        else:
            r2010_infotpserv_form = form_r2010_infotpserv(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r2010_infotpserv_form.is_valid():
                ...
                if r2010_infotpserv_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r2010_infotpserv_campos_multiple_passo1
                        r2010infoTpServ.objects.using(db_slug).filter(id=r2010_infotpserv_id).update(**dados)
                        obj = r2010infoTpServ.objects.using(db_slug).get(id=r2010_infotpserv_id)
                        #r2010_infotpserv_editar_custom
                        #r2010_infotpserv_campos_multiple_passo2
                    ...
                    #r2010_infotpserv_cadastrar_campos_multiple_passo1
                    obj = r2010infoTpServ(**dados)
                    obj.save(using = db_slug)
                    #r2010_infotpserv_cadastrar_custom
                    #r2010_infotpserv_cadastrar_campos_multiple_passo2
        ...
        for field in r2010_infotpserv_form.fields.keys():
            r2010_infotpserv_form.fields[field].widget.attrs['ng-model'] = 'r2010_infotpserv_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r2010_infotpserv_id, tabela='r2010_infotpserv').all()
        context = {
