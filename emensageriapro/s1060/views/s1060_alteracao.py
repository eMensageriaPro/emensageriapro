...
class s1060alteracaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1060alteracao.objects.using(db_slug).all()
    serializer_class = s1060alteracaoSerializer
    # permission_classes = (IsAdminUser,)


class s1060alteracaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1060alteracao.objects.using(db_slug).all()
    serializer_class = s1060alteracaoSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1060_alteracao_id:
            ...
            s1060_alteracao_form = form_s1060_alteracao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
