...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2230_infoatestado')
    permissao = ConfigPermissoes.objects.using( db_slug ).get(excluido = False, config_paginas=pagina, config_perfis=usuario.config_perfis)
    if s2230_infoatestado_id:
        s2230_infoatestado = get_object_or_404(s2230infoAtestado.objects.using( db_slug ), excluido = False, id = s2230_infoatestado_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2230_infoatestado_id:
            s2230_infoatestado_form = form_s2230_infoatestado(request.POST or None, instance = s2230_infoatestado, slug = db_slug)
        else:
            s2230_infoatestado_form = form_s2230_infoatestado(request.POST or None, slug = db_slug, initial={})
            ...
            if s2230_infoatestado_form.is_valid():
                ...
                if s2230_infoatestado_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2230_infoatestado_campos_multiple_passo1
                        s2230infoAtestado.objects.using(db_slug).filter(id=s2230_infoatestado_id).update(**dados)
                        obj = s2230infoAtestado.objects.using(db_slug).get(id=s2230_infoatestado_id)
                        #s2230_infoatestado_editar_custom
                        #s2230_infoatestado_campos_multiple_passo2
                    ...
                    #s2230_infoatestado_cadastrar_campos_multiple_passo1
                    obj = s2230infoAtestado(**dados)
                    obj.save(using = db_slug)
                    #s2230_infoatestado_cadastrar_custom
                    #s2230_infoatestado_cadastrar_campos_multiple_passo2
        ...
        for field in s2230_infoatestado_form.fields.keys():
            s2230_infoatestado_form.fields[field].widget.attrs['ng-model'] = 's2230_infoatestado_'+field
        ...
        if s2230_infoatestado_id:
            ...
            s2230_emitente_form = form_s2230_emitente(initial={ 's2230_infoatestado': s2230_infoatestado }, slug=db_slug)
            s2230_emitente_form.fields['s2230_infoatestado'].widget.attrs['readonly'] = True
            s2230_emitente_lista = s2230emitente.objects.using( db_slug ).filter(excluido = False, s2230_infoatestado_id=s2230_infoatestado.id).all()
        else:
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2230_infoatestado_id, tabela='s2230_infoatestado').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2230_infoatestado), indent=4, sort_keys=True, default=str)
            s2230infoAtestado.objects.using( db_slug ).filter(id = s2230_infoatestado_id).delete()
            #s2230_infoatestado_apagar_custom
            #s2230_infoatestado_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2230_infoatestado', s2230_infoatestado_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's2230_infoatestado_salvar':
            return redirect('s2230_infoatestado', hash=request.session['retorno_hash'])
        else:
            return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
    context = {
        'usuario': usuario,
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2230_infoatestado_lista = s2230infoAtestado.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2230_infoatestado_lista) > 100:
            filtrar = True
            s2230_infoatestado_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        s2230_iniafastamento_lista = s2230iniAfastamento.objects.using( db_slug ).filter(excluido = False).all()
        #s2230_infoatestado_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 's2230_infoatestado'
        context = {
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2230_infoatestado_csv.html', context)
