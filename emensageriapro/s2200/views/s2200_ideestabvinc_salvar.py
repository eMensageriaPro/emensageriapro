...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2200_ideestabvinc = get_object_or_404(s2200ideEstabVinc, id=pk)
        evento_dados = s2200_ideestabvinc.evento()
    ...
    if request.user.has_perm('s2200.can_see_s2200ideEstabVinc'):
        ...
        if request.method == 'POST':
            ...
            if s2200_ideestabvinc_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2200_ideestabvinc),
                #            indent=4,
                #            sort_keys=True,
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2200_ideestabvinc_apagar',
                    's2200_ideestabvinc_salvar',
                    's2200_ideestabvinc'):
        ...
        s2200_ideestabvinc_form = disabled_form_fields(
            s2200_ideestabvinc_form,
            request.user.has_perm('s2200.change_s2200ideEstabVinc'))
        ...
        if tab or 's2200_ideestabvinc' in request.session['return_page']:

            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2200_ideestabvinc_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_ideestabvinc').all()
