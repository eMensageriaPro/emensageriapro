...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2200_rne = get_object_or_404(s2200RNE, id=pk)
        evento_dados = s2200_rne.evento()

    if request.user.has_perm('s2200.can_see_s2200RNE'):
        ...
        if pk:
            ...
            s2200_rne_form = form_s2200_rne(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2200_rne_form.is_valid():
                ...
                if 's2200-rne' not in request.session['return']:
        ...
        s2200_rne_form = disabled_form_fields(
            s2200_rne_form,
            request.user.has_perm('s2200.change_s2200RNE'))
        ...
        #if tab or 's2200_rne' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2200_rne_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_rne').all()
