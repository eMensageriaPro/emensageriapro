...
class s2200dependenteList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2200dependente.objects.using(db_slug).all()
    serializer_class = s2200dependenteSerializer
    # permission_classes = (IsAdminUser,)


class s2200dependenteDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2200dependente.objects.using(db_slug).all()
    serializer_class = s2200dependenteSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2200_dependente_id:
            ...
            s2200_dependente_form = form_s2200_dependente(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
