...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2200_trabtemporario), indent=4, sort_keys=True, default=str)
            obj = s2200trabTemporario.objects.using( db_slug ).get(id = s2200_trabtemporario_id)
            obj.delete(request=request)
            #s2200_trabtemporario_apagar_custom
            #s2200_trabtemporario_apagar_custom
    ...
    return render(request, 's2200_trabtemporario_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2200trabTemporarioList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2200trabTemporario.objects.using(db_slug).all()
    serializer_class = s2200trabTemporarioSerializer
    # permission_classes = (IsAdminUser,)


class s2200trabTemporarioDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2200trabTemporario.objects.using(db_slug).all()
    serializer_class = s2200trabTemporarioSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            ...
            dict_fields = {
                'hipleg': 'hipleg',
                'idetomadorserv': 'idetomadorserv',
                'justcontr__icontains': 'justcontr__icontains',
        ...
        s2200_trabtemporario_lista = s2200trabTemporario.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2200_trabtemporario_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2200_trabtemporario_listar.html',
            ...
            response = render_to_response('s2200_trabtemporario_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2200_trabtemporario_csv.html', context)
            filename = "s2200_trabtemporario.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2200_trabtemporario_id:
            s2200_trabtemporario_form = form_s2200_trabtemporario(request.POST or None, instance = s2200_trabtemporario,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2200_trabtemporario_form = form_s2200_trabtemporario(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2200_trabtemporario_form.is_valid():
                ...
                if not s2200_trabtemporario_id:
                    gravar_auditoria('{}',
