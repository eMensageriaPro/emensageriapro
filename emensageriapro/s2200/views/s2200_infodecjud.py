...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2200_infodecjud_id:
            s2200_infodecjud_form = form_s2200_infodecjud(request.POST or None, instance = s2200_infodecjud, slug = db_slug)
        else:
            s2200_infodecjud_form = form_s2200_infodecjud(request.POST or None, slug = db_slug, initial={})
            ...
            if s2200_infodecjud_form.is_valid():
                ...
                if s2200_infodecjud_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2200_infodecjud_campos_multiple_passo1
                        s2200infoDecJud.objects.using(db_slug).filter(id=s2200_infodecjud_id).update(**dados)
                        obj = s2200infoDecJud.objects.using(db_slug).get(id=s2200_infodecjud_id)
                        #s2200_infodecjud_editar_custom
                        #s2200_infodecjud_campos_multiple_passo2
                    ...
                    #s2200_infodecjud_cadastrar_campos_multiple_passo1
                    obj = s2200infoDecJud(**dados)
                    obj.save(using = db_slug)
                    #s2200_infodecjud_cadastrar_custom
                    #s2200_infodecjud_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2200infoDecJud.objects.using( db_slug ).filter(id = s2200_infodecjud_id).delete()
