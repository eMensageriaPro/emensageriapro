...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('s2200.can_see_s2200RIC'):
        ...
        s2200_ric_form = disabled_form_fields(
            s2200_ric_form, 
            request.user.has_perm('s2200.change_s2200RIC'))
        ...
        if tab or 's2200_ric' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2200_ric_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_ric').all()
