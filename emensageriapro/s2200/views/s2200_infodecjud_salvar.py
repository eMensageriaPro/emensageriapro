...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s2200_infodecjud.evento()

    if request.user.has_perm('s2200.can_see_s2200infoDecJud'):
        ...
        if request.method == 'POST':
            ...
            if s2200_infodecjud_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2200_infodecjud), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2200_infodecjud_apagar', 
                    's2200_infodecjud_salvar', 
                    's2200_infodecjud'):
        ...
        s2200_infodecjud_form = disabled_form_fields(
            s2200_infodecjud_form, 
            request.user.has_perm('s2200.change_s2200infoDecJud'))
        ...
        if tab or 's2200_infodecjud' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2200_infodecjud_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_infodecjud').all()
