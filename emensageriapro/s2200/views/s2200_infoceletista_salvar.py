...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2200_infoceletista = get_object_or_404(s2200infoCeletista, id=pk)
        evento_dados = s2200_infoceletista.evento()

    if request.user.has_perm('s2200.can_see_s2200infoCeletista'):
        ...
        if request.method == 'POST':
            ...
            if s2200_infoceletista_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2200_infoceletista), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2200_infoceletista_apagar', 
                    's2200_infoceletista_salvar', 
                    's2200_infoceletista'):
        ...
        s2200_infoceletista_form = disabled_form_fields(
            s2200_infoceletista_form, 
            request.user.has_perm('s2200.change_s2200infoCeletista'))
        ...
        if tab or 's2200_infoceletista' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2200_infoceletista_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_infoceletista').all()
