...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2200_horcontratual = get_object_or_404(s2200horContratual, id=pk)
        evento_dados = s2200_horcontratual.evento()
    ...
    if request.user.has_perm('s2200.can_see_s2200horContratual'):
        ...
        if request.method == 'POST':
            ...
            if s2200_horcontratual_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s2200_horcontratual), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2200_horcontratual_apagar', 
                    's2200_horcontratual_salvar', 
                    's2200_horcontratual'):
        ...
        s2200_horcontratual_form = disabled_form_fields(
            s2200_horcontratual_form, 
            request.user.has_perm('s2200.change_s2200horContratual'))
        ...
        if tab or 's2200_horcontratual' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2200_horcontratual_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2200_horcontratual').all()
