...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2200RNE.objects.using( db_slug ).filter(id = s2200_rne_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2200_rne_id:
            s2200_rne_form = form_s2200_rne(request.POST or None, instance = s2200_rne, slug = db_slug)
        else:
            s2200_rne_form = form_s2200_rne(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s2200_rne_form.is_valid():
                ...
                if s2200_rne_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2200_rne_campos_multiple_passo1
                        s2200RNE.objects.using(db_slug).filter(id=s2200_rne_id).update(**dados)
                        obj = s2200RNE.objects.using(db_slug).get(id=s2200_rne_id)
                        #s2200_rne_editar_custom
                        #s2200_rne_campos_multiple_passo2
                    ...
                    #s2200_rne_cadastrar_campos_multiple_passo1
                    obj = s2200RNE(**dados)
                    obj.save(using = db_slug)
                    #s2200_rne_cadastrar_custom
                    #s2200_rne_cadastrar_campos_multiple_passo2
