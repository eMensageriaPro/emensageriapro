...
def apagar(request, hash):
    ...
    if s2200_infoestatutario_id:
        dados_evento = s2200_infoestatutario.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2200_infoestatutario_apagar'] = 0
            dict_permissoes['s2200_infoestatutario_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2200_infoestatutario), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2200_infoestatutario', s2200_infoestatutario_id, usuario_id, 3)
        else:
...
class s2200infoEstatutarioList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2200infoEstatutario.objects.using(db_slug).all()
    serializer_class = s2200infoEstatutarioSerializer
    # permission_classes = (IsAdminUser,)


class s2200infoEstatutarioDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2200infoEstatutario.objects.using(db_slug).all()
    serializer_class = s2200infoEstatutarioSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2200_infoestatutario_listar.html',
                filename="s2200_infoestatutario.pdf",
                context=context,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2200_infoestatutario_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2200_infoestatutario_id:
            s2200_infoestatutario_form = form_s2200_infoestatutario(request.POST or None, instance = s2200_infoestatutario,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2200_infoestatutario_form = form_s2200_infoestatutario(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2200_infoestatutario_form.is_valid():

                dados = s2200_infoestatutario_form.cleaned_data
                obj = s2200_infoestatutario_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2200_infoestatutario_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2200_infoestatutario', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2200_infoestatutario), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2200_infoestatutario', s2200_infoestatutario_id, usuario_id, 2)
        ...
        if s2200_infoestatutario_id:
            if dados_evento['status'] != 0:
                s2200_infoestatutario_form = disabled_form_fields(s2200_infoestatutario_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2200_infoestatutario_id, tabela='s2200_infoestatutario').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2200_infoestatutario_salvar.html',
                filename="s2200_infoestatutario.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2200_infoestatutario_salvar.html', context)
            filename = "s2200_infoestatutario.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
