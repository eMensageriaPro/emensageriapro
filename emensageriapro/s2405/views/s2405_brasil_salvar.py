...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2405_brasil = get_object_or_404(s2405brasil, id=pk)
        evento_dados = s2405_brasil.evento()

    if request.user.has_perm('s2405.can_see_s2405brasil'):
        ...
        if request.method == 'POST':
            ...
            if s2405_brasil_form.is_valid():
                ...
                if 's2405-brasil' not in request.session['return']:
        ...
        s2405_brasil_form = disabled_form_fields(
            s2405_brasil_form,
            request.user.has_perm('s2405.change_s2405brasil'))
        ...
        #if tab or 's2405_brasil' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2405_brasil_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2405_brasil').all()
