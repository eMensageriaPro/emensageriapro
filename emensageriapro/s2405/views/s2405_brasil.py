...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2405brasil.objects.using( db_slug ).filter(id = s2405_brasil_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2405_brasil_id:
            s2405_brasil_form = form_s2405_brasil(request.POST or None, instance = s2405_brasil, slug = db_slug)
        else:
            s2405_brasil_form = form_s2405_brasil(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s2405_brasil_form.is_valid():
                ...
                if s2405_brasil_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2405_brasil_campos_multiple_passo1
                        s2405brasil.objects.using(db_slug).filter(id=s2405_brasil_id).update(**dados)
                        obj = s2405brasil.objects.using(db_slug).get(id=s2405_brasil_id)
                        #s2405_brasil_editar_custom
                        #s2405_brasil_campos_multiple_passo2
                    ...
                    #s2405_brasil_cadastrar_campos_multiple_passo1
                    obj = s2405brasil(**dados)
                    obj.save(using = db_slug)
                    #s2405_brasil_cadastrar_custom
                    #s2405_brasil_cadastrar_campos_multiple_passo2
