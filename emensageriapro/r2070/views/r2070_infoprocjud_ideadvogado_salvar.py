...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('r2070.can_see_r2070infoProcJudideAdvogado'):
        ...
        if request.method == 'POST':
            ...
            if r2070_infoprocjud_ideadvogado_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(r2070_infoprocjud_ideadvogado), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    'r2070_infoprocjud_ideadvogado_apagar', 
        ...
        if tab or 'r2070_infoprocjud_ideadvogado' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 'r2070_infoprocjud_ideadvogado_salvar'
