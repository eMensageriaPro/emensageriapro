...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(r2070_pgtopj_despprocjud), indent=4, sort_keys=True, default=str)
            obj = r2070pgtoPJdespProcJud.objects.using( db_slug ).get(id = r2070_pgtopj_despprocjud_id)
            obj.delete(request=request)
            #r2070_pgtopj_despprocjud_apagar_custom
            #r2070_pgtopj_despprocjud_apagar_custom
    ...
    return render(request, 'r2070_pgtopj_despprocjud_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class r2070pgtoPJdespProcJudList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2070pgtoPJdespProcJud.objects.using(db_slug).all()
    serializer_class = r2070pgtoPJdespProcJudSerializer
    # permission_classes = (IsAdminUser,)


class r2070pgtoPJdespProcJudDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2070pgtoPJdespProcJud.objects.using(db_slug).all()
    serializer_class = r2070pgtoPJdespProcJudSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2070_pgtopj_despprocjud_lista = r2070pgtoPJdespProcJud.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(r2070_pgtopj_despprocjud_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='r2070_pgtopj_despprocjud_listar.html',
            ...
            response = render_to_response('r2070_pgtopj_despprocjud_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2070_pgtopj_despprocjud_csv.html', context)
            filename = "r2070_pgtopj_despprocjud.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2070_pgtopj_despprocjud_id:
            r2070_pgtopj_despprocjud_form = form_r2070_pgtopj_despprocjud(request.POST or None, instance = r2070_pgtopj_despprocjud,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            r2070_pgtopj_despprocjud_form = form_r2070_pgtopj_despprocjud(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if r2070_pgtopj_despprocjud_form.is_valid():

                dados = r2070_pgtopj_despprocjud_form.cleaned_data
                obj = r2070_pgtopj_despprocjud_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not r2070_pgtopj_despprocjud_id:
                    gravar_auditoria('{}',
