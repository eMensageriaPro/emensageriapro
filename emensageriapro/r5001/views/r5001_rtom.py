...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r5001_rtom_id:
            r5001_rtom_form = form_r5001_rtom(request.POST or None, instance = r5001_rtom, slug = db_slug)
        else:
            r5001_rtom_form = form_r5001_rtom(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r5001_rtom_form.is_valid():
                ...
                if r5001_rtom_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r5001_rtom_campos_multiple_passo1
                        r5001RTom.objects.using(db_slug).filter(id=r5001_rtom_id).update(**dados)
                        obj = r5001RTom.objects.using(db_slug).get(id=r5001_rtom_id)
                        #r5001_rtom_editar_custom
                        #r5001_rtom_campos_multiple_passo2
                    ...
                    #r5001_rtom_cadastrar_campos_multiple_passo1
                    obj = r5001RTom(**dados)
                    obj.save(using = db_slug)
                    #r5001_rtom_cadastrar_custom
                    #r5001_rtom_cadastrar_campos_multiple_passo2
