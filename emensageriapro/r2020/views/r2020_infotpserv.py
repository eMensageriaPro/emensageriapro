...
class r2020infoTpServList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2020infoTpServ.objects.using(db_slug).all()
    serializer_class = r2020infoTpServSerializer
    # permission_classes = (IsAdminUser,)


class r2020infoTpServDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2020infoTpServ.objects.using(db_slug).all()
    serializer_class = r2020infoTpServSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2020_infotpserv_id:
            r2020_infotpserv_form = form_r2020_infotpserv(request.POST or None, instance = r2020_infotpserv,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            r2020_infotpserv_form = form_r2020_infotpserv(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
