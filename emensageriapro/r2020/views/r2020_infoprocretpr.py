...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2020_infoprocretpr')
    ...
    r2020_infoprocretpr = get_object_or_404(r2020infoProcRetPr.objects.using( db_slug ), excluido = False, id = r2020_infoprocretpr_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(r2020_infoprocretpr), indent=4, sort_keys=True, default=str)
            r2020infoProcRetPr.objects.using( db_slug ).filter(id = r2020_infoprocretpr_id).delete()
            #r2020_infoprocretpr_apagar_custom
            #r2020_infoprocretpr_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             'r2020_infoprocretpr', r2020_infoprocretpr_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
        
        if request.session['retorno_pagina']== 'r2020_infoprocretpr_salvar':
            return redirect('r2020_infoprocretpr', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'r2020_infoprocretpr_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2020_infoprocretpr_id:
            r2020_infoprocretpr_form = form_r2020_infoprocretpr(request.POST or None, instance = r2020_infoprocretpr, slug = db_slug)
        else:
            r2020_infoprocretpr_form = form_r2020_infoprocretpr(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r2020_infoprocretpr_form.is_valid():
                ...
                if r2020_infoprocretpr_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r2020_infoprocretpr_campos_multiple_passo1
                        r2020infoProcRetPr.objects.using(db_slug).filter(id=r2020_infoprocretpr_id).update(**dados)
                        obj = r2020infoProcRetPr.objects.using(db_slug).get(id=r2020_infoprocretpr_id)
                        #r2020_infoprocretpr_editar_custom
                        #r2020_infoprocretpr_campos_multiple_passo2
                    ...
                    #r2020_infoprocretpr_cadastrar_campos_multiple_passo1
                    obj = r2020infoProcRetPr(**dados)
                    obj.save(using = db_slug)
                    #r2020_infoprocretpr_cadastrar_custom
                    #r2020_infoprocretpr_cadastrar_campos_multiple_passo2
        ...
        for field in r2020_infoprocretpr_form.fields.keys():
            r2020_infoprocretpr_form.fields[field].widget.attrs['ng-model'] = 'r2020_infoprocretpr_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r2020_infoprocretpr_id, tabela='r2020_infoprocretpr').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='r2020_infoprocretpr_salvar.html',
...
class r2020infoProcRetPrList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2020infoProcRetPr.objects.using(db_slug).all()
    serializer_class = r2020infoProcRetPrSerializer
    permission_classes = (IsAdminUser,)


class r2020infoProcRetPrDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2020infoProcRetPr.objects.using(db_slug).all()
    serializer_class = r2020infoProcRetPrSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2020_infoprocretpr_lista = r2020infoProcRetPr.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'r2020_infoprocretpr'
        context = {
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2020_infoprocretpr_csv.html', context)
