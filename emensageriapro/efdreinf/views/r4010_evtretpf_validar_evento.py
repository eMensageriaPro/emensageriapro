...
def validar_evento_funcao(request, r4010_evtretpf_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r4010_evtretpf_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r4010_evtretpf_validar import validacoes_r4010_evtretpf
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r4010_evtretpf(arquivo)
    ...
    if lista_validacoes:
        ...
        r4010evtRetPF.objects.\
            filter(id=r4010_evtretpf_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r4010_evtretpf', r4010_evtretpf_id)
...
def validar_evento(request, hash):
    ...
    if r4010_evtretpf_id:
        ...
        if r4010_evtretpf.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r4010_evtretpf_id)
            
            if r4010_evtretpf.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4010evtRetPF.objects.\
                    filter(id=r4010_evtretpf_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r4010_evtretpf.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4010evtRetPF.objects.\
                    filter(id=r4010_evtretpf_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
