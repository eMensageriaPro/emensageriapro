    ...
    if request.user.has_perm('efdreinf.can_view_r9012evtRetCons'):
        ...
        if request.method == 'POST':
            ...
            if r9012_evtretcons_form.is_valid():
                ...
                dados = r9012_evtretcons_form.cleaned_data
                obj = r9012_evtretcons_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not r9012_evtretcons_id:
                    ...
                    gravar_auditoria('{}',
                                 ...
                                 'r9012_evtretcons', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(r9012_evtretcons), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'r9012_evtretcons', r9012_evtretcons_id, usuario_id, 2)
                                 
                if request.session['retorno_pagina'] not in ('r9012_evtretcons_apagar', 'r9012_evtretcons_salvar', 'r9012_evtretcons'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                    
                if r9012_evtretcons_id != obj.id:
        ...
        r9012_evtretcons_form = disabled_form_fields(r9012_evtretcons_form, request.user.has_perm('efdreinf.change_r9012evtRetCons'))
        ...
        if dict_hash['tab'] or 'r9012_evtretcons' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 'r9012_evtretcons_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=r9012_evtretcons_id, tabela='r9012_evtretcons').all()
