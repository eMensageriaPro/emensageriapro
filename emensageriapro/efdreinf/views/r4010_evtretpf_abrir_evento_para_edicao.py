...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r4010evtRetPF'):

        if r4010_evtretpf_id:
            r4010_evtretpf = get_object_or_404(r4010evtRetPF, excluido=False, id=r4010_evtretpf_id)
            ...
            if r4010_evtretpf.status in status_list:
                r4010evtRetPF.objects.filter(id=r4010_evtretpf_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r4010_evtretpf.identidade, BASE_DIR, r4010_evtretpf.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r4010_evtretpf/%s.xml %s/arquivos/Eventos/r4010_evtretpf/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r4010_evtretpf/%s_backup_%s.xml' % (r4010_evtretpf.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r4010_evtretpf', r4010_evtretpf_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r4010_evtretpf_id )
                return redirect('r4010_evtretpf_salvar', hash=url_hash)
