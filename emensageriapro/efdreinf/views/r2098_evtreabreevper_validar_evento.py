...
def validar_evento_funcao(request, pk):
    ...
    schema_filename = 'efdreinf/%s/evtReabreEvPer.xsd' % (r2098_evtreabreevper.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 'r2098_evtreabreevper' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('r2098_evtreabreevper', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if r2098_evtreabreevper.status in STATUS_VALIDACAO:
