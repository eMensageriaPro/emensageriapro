...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r5001_evttotal_verificar.html' % r5001_evttotal.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r5001_evttotal_verificar.html' % r5001_evttotal.versao,
            ...
            response =  render_to_response('%s/r5001_evttotal_verificar.html' % r5001_evttotal.versao, context)
            ...
            return render(request, '%s/r5001_evttotal_verificar.html' % r5001_evttotal.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r5001(r5001_evttotal_id, db_slug):
    from django.template.loader import get_template
    if r5001_evttotal_id:
        ...
        #return render(request, 'xml/%s/r5001_evttotal.html' % r5001_evttotal.versao, context, content_type='text/xml')
        t = get_template('%s/r5001_evttotal_xml.html' % r5001_evttotal.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if r5001_evttotal.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r5001_evttotal.retornos_evttotal_id, excluido=False)
        ...
        if r5001_evttotal.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r5001_evttotal.retornos_evttotalcontrib_id, excluido=False)
...
def gerar_xml_assinado(r5001_evttotal_id, db_slug):
    ...
    if r5001_evttotal.status in (0,1,2,11):
        r5001evtTotal.objects.using(db_slug).filter(id=r5001_evttotal_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    r5001_evttotal_id = int(dict_hash['id'])
    if r5001_evttotal_id:
        r5001_evttotal = get_object_or_404(r5001evtTotal.objects.using(db_slug), excluido=False,
                                                    id=r5001_evttotal_id)

        arquivo = 'arquivos/Eventos/r5001_evttotal/%s.xml' % r5001_evttotal.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r5001_evttotal_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r5001_evttotal/%s.xml' % r5001_evttotal.identidade)
        salvar_arquivo('arquivos/Eventos/r5001_evttotal/%s_duplicado_temp.xml' % r5001_evttotal.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r5001_evttotal/%s_duplicado_temp.xml' % r5001_evttotal.identidade, request)
        ...
        r5001evtTotal.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r5001_evttotal.identidade), 
            'r5001_evttotal', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r5001_evttotal_id:
        ...
        salvar_arquivo('arquivos/Eventos/r5001_evttotal/%s_alteracao_temp.xml' % r5001_evttotal.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r5001_evttotal/%s_alteracao_temp.xml' % r5001_evttotal.identidade, request)
        from emensageriapro.efdreinf.views.r5001_evttotal import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r5001evtTotal.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r5001_evttotal.identidade), 
            'r5001_evttotal', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r5001_evttotal_id:
        ...
        salvar_arquivo('arquivos/Eventos/r5001_evttotal/%s_exclusao_temp.xml' % r5001_evttotal.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r5001_evttotal/%s_exclusao_temp.xml' % r5001_evttotal.identidade, request)
        from emensageriapro.efdreinf.views.r5001_evttotal import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r5001evtTotal.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r5001_evttotal.identidade), 
            'r5001_evttotal', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r5001_evttotal_id:
        ...
        if r5001_evttotal.status == 0:
            from emensageriapro.efdreinf.views.r5001_evttotal import identidade_evento
            dent = identidade_evento(r5001_evttotal_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r5001_evttotal_id:
        r5001_evttotal = get_object_or_404(r5001evtTotal.objects.using(db_slug), excluido=False, id=r5001_evttotal_id)
        if r5001_evttotal.status in (0, 1, 2, 3, 4, 5, 10, 11):
...
def validar_evento_funcao(r5001_evttotal_id, db_slug):
    ...
    lista_validacoes = []
    r5001_evttotal = get_object_or_404(r5001evtTotal.objects.using(db_slug), excluido=False, id=r5001_evttotal_id)
    if r5001_evttotal.transmissor_lote_efdreinf:
        if r5001_evttotal.transmissor_lote_efdreinf.transmissor:
            if r5001_evttotal.transmissor_lote_efdreinf.transmissor.verificar_predecessao:
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r5001_evttotal import validacoes_r5001_evttotal
                lista = validacoes_r5001_evttotal(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r5001_evttotal import validacoes_r5001_evttotal
                lista = validacoes_r5001_evttotal(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if r5001_evttotal_id:
        lista_validacoes = []
        r5001_evttotal = get_object_or_404(r5001evtTotal.objects.using(db_slug), excluido=False, id=r5001_evttotal_id)
        if r5001_evttotal.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r5001_evttotal_id, db_slug)
