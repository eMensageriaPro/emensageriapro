...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            return render(request, 'r2030_evtassocdesprec_verificar.html', context)
...
def gerar_xml_r2030(r2030_evtassocdesprec_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r2030_evtassocdesprec.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r2030_evtassocdesprec_id, db_slug):
    ...
    if r2030_evtassocdesprec.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        r2030evtAssocDespRec.objects.using(db_slug).\
            filter(id=r2030_evtassocdesprec_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if r2030_evtassocdesprec_id:
        ...
        texto = gerar_xml_r2030(r2030_evtassocdesprec_id, db_slug, versao="|")
        dados = read_r2030_evtassocdesprec_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(r2030_evtassocdesprec)

        r2030evtAssocDespRec.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if r2030_evtassocdesprec_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2030_evtassocdesprec.identidade),
            'r2030_evtassocdesprec', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2030_evtassocdesprec_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2030_evtassocdesprec.identidade),
            'r2030_evtassocdesprec', dados['id'], request.user.id, 1)
    ...
    if r2030_evtassocdesprec_id:
        ...
        if r2030_evtassocdesprec.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                'r2030_evtassocdesprec', r2030_evtassocdesprec_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2030_evtassocdesprec_id:
        ...
        if r2030_evtassocdesprec.status in status_list:
...
def validar_evento_funcao(r2030_evtassocdesprec_id, db_slug):
    ...
    if lista_validacoes:
        ...
        r2030evtAssocDespRec.objects.using( db_slug ).\
            filter(id=r2030_evtassocdesprec_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('esocial', 'r2030_evtassocdesprec', r2030_evtassocdesprec_id)
...
def validar_evento(request, hash):
    ...
    if r2030_evtassocdesprec_id:
        ...
        if r2030_evtassocdesprec.versao in VERSOES_EFDREINF:

            validar_evento_funcao(r2030_evtassocdesprec_id, db_slug)

            if r2030_evtassocdesprec.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2030evtAssocDespRec.objects.using(db_slug).\
                    filter(excluido=False, id=r2030_evtassocdesprec_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2030_evtassocdesprec.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2030evtAssocDespRec.objects.using(db_slug).\
                    filter(excluido=False, id=r2030_evtassocdesprec_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
