...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if r5011_evttotalcontrib.status == STATUS_EVENTO_CADASTRADO:
            ...
            obj = r5011evtTotalContrib.objects.get(id=pk)
            ...
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r5011_evttotalcontrib', pk, request.user.id, 3)
        else:
        
            messages.error(request, u'''Não foi possivel apagar o evento, somente é 
                                        possível apagar os eventos com status "Cadastrado"!''')
            
        if 'r5011_evttotalcontrib' in request.session['return_page']:
        
            return redirect('r5011_evttotalcontrib')
