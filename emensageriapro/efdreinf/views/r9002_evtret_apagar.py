...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if r9002_evtret.status == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(r9002_evtret), indent=4, sort_keys=True, default=str)
            obj = r9002evtRet.objects.get(id = r9002_evtret_id)
            obj.delete(request=request)
            #r9002_evtret_apagar_custom
            #r9002_evtret_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r9002_evtret', r9002_evtret_id, usuario_id, 3)
        else:
            messages.error(request, u'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
            
        if request.session['retorno_pagina']== 'r9002_evtret_salvar':
            return redirect('r9002_evtret', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'r9002_evtret_apagar.html', context)