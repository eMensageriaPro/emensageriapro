...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r9012evtRetCons'):

        if r9012_evtretcons_id:
            r9012_evtretcons = get_object_or_404(r9012evtRetCons, excluido=False, id=r9012_evtretcons_id)
            ...
            if r9012_evtretcons.status in status_list:
                r9012evtRetCons.objects.filter(id=r9012_evtretcons_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r9012_evtretcons.identidade, BASE_DIR, r9012_evtretcons.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r9012_evtretcons/%s.xml %s/arquivos/Eventos/r9012_evtretcons/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r9012_evtretcons/%s_backup_%s.xml' % (r9012_evtretcons.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r9012_evtretcons', r9012_evtretcons_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r9012_evtretcons_id )
