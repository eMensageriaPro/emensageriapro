...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if r9000_evtexclusao.status == STATUS_EVENTO_CADASTRADO:
            ...
            situacao_anterior = json.dumps(model_to_dict(r9000_evtexclusao), sort_keys=True, default=str)
            obj = r9000evtExclusao.objects.get(id=pk)
            obj.delete(request=request)

            sql_softdelete = ler_arquivo('/database/sql/r9000_softdelete_cascade.sql')
        ...
        if 'return_page' in request.session and request.session['return_page'] and 'r9000-evtexclusao' in request.session['return_page']:
