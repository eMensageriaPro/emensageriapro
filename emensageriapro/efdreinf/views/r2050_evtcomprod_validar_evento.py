...
def validar_evento_funcao(request, r2050_evtcomprod_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2050_evtcomprod_id)
...
def validar_evento(request, hash):
    ...
    if r2050_evtcomprod_id:
        ...
        if r2050_evtcomprod.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r2050_evtcomprod_id)
            
            if r2050_evtcomprod.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2050evtComProd.objects.\
                    filter(id=r2050_evtcomprod_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2050_evtcomprod.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2050evtComProd.objects.\
                    filter(id=r2050_evtcomprod_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
