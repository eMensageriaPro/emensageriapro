...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if r4040_evtbenefnid.status == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(r4040_evtbenefnid), indent=4, sort_keys=True, default=str)
            obj = r4040evtBenefNId.objects.get(id = r4040_evtbenefnid_id)
            obj.delete(request=request)
            #r4040_evtbenefnid_apagar_custom
            #r4040_evtbenefnid_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r4040_evtbenefnid', r4040_evtbenefnid_id, usuario_id, 3)
        else:
            messages.error(request, u'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
            
        if request.session['retorno_pagina']== 'r4040_evtbenefnid_salvar':
            return redirect('r4040_evtbenefnid', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 'r4040_evtbenefnid_apagar.html', context)