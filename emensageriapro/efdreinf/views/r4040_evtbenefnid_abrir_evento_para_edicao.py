...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r4040evtBenefNId'):

        if r4040_evtbenefnid_id:
            r4040_evtbenefnid = get_object_or_404(r4040evtBenefNId, excluido=False, id=r4040_evtbenefnid_id)
            ...
            if r4040_evtbenefnid.status in status_list:
                r4040evtBenefNId.objects.filter(id=r4040_evtbenefnid_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r4040_evtbenefnid.identidade, BASE_DIR, r4040_evtbenefnid.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r4040_evtbenefnid/%s.xml %s/arquivos/Eventos/r4040_evtbenefnid/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r4040_evtbenefnid/%s_backup_%s.xml' % (r4040_evtbenefnid.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r4040_evtbenefnid', r4040_evtbenefnid_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r4040_evtbenefnid_id )
                return redirect('r4040_evtbenefnid_salvar', hash=url_hash)
