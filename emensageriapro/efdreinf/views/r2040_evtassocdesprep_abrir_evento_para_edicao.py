...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r2040evtAssocDespRep'):

        if r2040_evtassocdesprep_id:
            r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep, excluido=False, id=r2040_evtassocdesprep_id)
            ...
            if r2040_evtassocdesprep.status in status_list:
                r2040evtAssocDespRep.objects.filter(id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
