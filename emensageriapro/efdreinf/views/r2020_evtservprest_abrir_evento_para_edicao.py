...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r2020evtServPrest'):

        if r2020_evtservprest_id:
            r2020_evtservprest = get_object_or_404(r2020evtServPrest, excluido=False, id=r2020_evtservprest_id)
            ...
            if r2020_evtservprest.status in status_list:
                r2020evtServPrest.objects.filter(id=r2020_evtservprest_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
