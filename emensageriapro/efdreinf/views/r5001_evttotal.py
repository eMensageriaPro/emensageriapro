...
def identidade_evento(r5001_evttotal_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEfdreinf
    dados_evento = r5001evtTotal.objects.using( db_slug ).get(id=r5001_evttotal_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            r5001evtTotal.objects.using(db_slug).filter(id=r5001_evttotal_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if r5001_evttotal_id:
            ...
            r5001_evttotal_form = form_r5001_evttotal(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_EFDREINF, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
            ...
            if r5001_evttotal_form.is_valid():
                ...
                if r5001_evttotal_id:
                    if r5001_evttotal.status == 0:
                        ...
                        #r5001_evttotal_campos_multiple_passo1
                        r5001evtTotal.objects.using(db_slug).filter(id=r5001_evttotal_id).update(**dados)
                        obj = r5001evtTotal.objects.using(db_slug).get(id=r5001_evttotal_id)
                        #r5001_evttotal_editar_custom
                        #r5001_evttotal_campos_multiple_passo2
                    ...
                    #r5001_evttotal_cadastrar_campos_multiple_passo1
                    obj = r5001evtTotal(**dados)
                    obj.save(using = db_slug)
                    #r5001_evttotal_cadastrar_custom
                    #r5001_evttotal_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if r5001_evttotal.status == 0:
            ...
            r5001evtTotal.objects.using( db_slug ).filter(id = r5001_evttotal_id).delete()
