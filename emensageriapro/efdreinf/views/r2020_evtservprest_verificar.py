...
def gerar_xml_assinado(r2020_evtservprest_id, db_slug):
    ...
    if r2020_evtservprest.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        r2020evtServPrest.objects.using(db_slug).\
            filter(id=r2020_evtservprest_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        if r2020_evtservprest.status in status_list:
            r2020evtServPrest.objects.using(db_slug).filter(id=r2020_evtservprest_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2020_evtservprest/%s.xml' % (r2020_evtservprest.identidade)
...
def validar_evento(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        if r2020_evtservprest.versao in VERSOES_EFDREINF:
            ...
            if r2020_evtservprest.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2020evtServPrest.objects.using(db_slug).\
                    filter(excluido=False, id=r2020_evtservprest_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2020_evtservprest.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2020evtServPrest.objects.using(db_slug).\
                    filter(excluido=False, id=r2020_evtservprest_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
