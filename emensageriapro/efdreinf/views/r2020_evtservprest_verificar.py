    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='r2020_evtservprest_verificar.html',
            ...
            response =  render_to_response('r2020_evtservprest_verificar.html', context)
            ...
            return render(request, 'r2020_evtservprest_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_r2020(r2020_evtservprest_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r2020_evtservprest.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r2020_evtservprest_id, db_slug):
    ...
    if r2020_evtservprest.status in (0,1,2,11):
        ...
        r2020evtServPrest.objects.using(db_slug).\
            filter(id=r2020_evtservprest_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.efdreinf.views.r2020_evtservprest_importar import read_r2020_evtservprest_string
    ...
    if r2020_evtservprest_id:
        ...
        texto = gerar_xml_r2020(r2020_evtservprest_id, db_slug, versao="|")
        dados = read_r2020_evtservprest_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        r2020evtServPrest.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2020_evtservprest.identidade),
            'r2020_evtservprest', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2020_evtservprest.identidade),
            'r2020_evtservprest', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2020_evtservprest.identidade),
            'r2020_evtservprest', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        if r2020_evtservprest.status == 0:
            ...
            nova_identidade = identidade_evento(r2020_evtservprest_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        if r2020_evtservprest.status in (0, 1, 2, 3, 4, 10, 11) or r2020_evtservprest.processamento_codigo_resposta in (401,402):
...
=======
    if r2020_evtservprest.transmissor_lote_esocial:
        if r2020_evtservprest.transmissor_lote_esocial.transmissor:
            if r2020_evtservprest.transmissor_lote_esocial.transmissor.verificar_predecessao:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
    ...
    r2020evtServPrest.objects.using( db_slug ).\
        filter(id=r2020_evtservprest_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.efdreinf.views.r2020_evtservprest_validar import validacoes_r2020_evtservprest
...
<<<<<<< HEAD
        if r2020_evtservprest.versao in VERSOES_EFDREINF:
=======
        if r2020_evtservprest.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(r2020_evtservprest_id, db_slug)
