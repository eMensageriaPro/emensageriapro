...
def validar_evento_funcao(request, r9011_evttotalcontrib_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r9011_evttotalcontrib_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r9011_evttotalcontrib_validar import validacoes_r9011_evttotalcontrib
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r9011_evttotalcontrib(arquivo)
    ...
    if lista_validacoes:
        ...
        r9011evtTotalContrib.objects.\
            filter(id=r9011_evttotalcontrib_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r9011_evttotalcontrib', r9011_evttotalcontrib_id)
...
def validar_evento(request, hash):
    ...
    if r9011_evttotalcontrib_id:
        ...
        r9011_evttotalcontrib = get_object_or_404(
            ...
            excluido=False,
            id=r9011_evttotalcontrib_id)

        if r9011_evttotalcontrib.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r9011_evttotalcontrib_id)
            
            if r9011_evttotalcontrib.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9011evtTotalContrib.objects.\
                    filter(id=r9011_evttotalcontrib_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r9011_evttotalcontrib.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9011evtTotalContrib.objects.\
                    filter(id=r9011_evttotalcontrib_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
