...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r3010_evtespdesportivo_verificar.html' % r3010_evtespdesportivo.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r3010_evtespdesportivo_verificar.html' % r3010_evtespdesportivo.versao,
            ...
            response =  render_to_response('%s/r3010_evtespdesportivo_verificar.html' % r3010_evtespdesportivo.versao, context)
            ...
            return render(request, '%s/r3010_evtespdesportivo_verificar.html' % r3010_evtespdesportivo.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r3010(r3010_evtespdesportivo_id, db_slug):
    from django.template.loader import get_template
    if r3010_evtespdesportivo_id:
        ...
        #return render(request, 'xml/%s/r3010_evtespdesportivo.html' % r3010_evtespdesportivo.versao, context, content_type='text/xml')
        t = get_template('%s/r3010_evtespdesportivo_xml.html' % r3010_evtespdesportivo.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r3010_evtespdesportivo = get_object_or_404(
            ...
            excluido = False, id = r3010_evtespdesportivo_id)
    

        if r3010_evtespdesportivo.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r3010_evtespdesportivo.retornos_evttotal_id, excluido=False)
        ...
        if r3010_evtespdesportivo.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r3010_evtespdesportivo.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r3010_evtespdesportivo_recibo_pdf.html', context)
            filename = "%s.xls" % r3010_evtespdesportivo.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r3010_evtespdesportivo_recibo_csv.html', context)
            filename = "%s.csv" % r3010_evtespdesportivo.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r3010_evtespdesportivo_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r3010_evtespdesportivo_id, db_slug):
    import os
    ...
    if r3010_evtespdesportivo.arquivo_original:
        ...
        xml = gerar_xml_r3010(r3010_evtespdesportivo_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r3010_evtespdesportivo.status in (0,1,2,11):
        r3010evtEspDesportivo.objects.using(db_slug).filter(id=r3010_evtespdesportivo_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r3010_evtespdesportivo_id:
        r3010_evtespdesportivo = get_object_or_404(r3010evtEspDesportivo.objects.using(db_slug), excluido=False, id=r3010_evtespdesportivo_id)
        xml_assinado = gerar_xml_assinado(r3010_evtespdesportivo_id, db_slug)
...
def duplicar(request, hash):
    ...
    if r3010_evtespdesportivo_id:
        ...
        salvar_arquivo('arquivos/Eventos/r3010_evtespdesportivo/%s_duplicado_temp.xml' % r3010_evtespdesportivo.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r3010_evtespdesportivo/%s_duplicado_temp.xml' % r3010_evtespdesportivo.identidade, request)
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r3010_evtespdesportivo.identidade), 
...
def criar_alteracao(request, hash):
    ...
    if r3010_evtespdesportivo_id:
        ...
        salvar_arquivo('arquivos/Eventos/r3010_evtespdesportivo/%s_alteracao_temp.xml' % r3010_evtespdesportivo.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r3010_evtespdesportivo/%s_alteracao_temp.xml' % r3010_evtespdesportivo.identidade, request)
        from emensageriapro.efdreinf.views.r3010_evtespdesportivo import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r3010evtEspDesportivo.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r3010_evtespdesportivo.identidade), 
            'r3010_evtespdesportivo', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r3010_evtespdesportivo_id:
        ...
        if r3010_evtespdesportivo.status == 0:
            from emensageriapro.efdreinf.views.r3010_evtespdesportivo import identidade_evento
            dent = identidade_evento(r3010_evtespdesportivo_id, db_slug)
            ...
            'r3010_evtespdesportivo', r3010_evtespdesportivo_id, usuario_id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r3010_evtespdesportivo_id:
        r3010_evtespdesportivo = get_object_or_404(r3010evtEspDesportivo.objects.using(db_slug), excluido=False, id=r3010_evtespdesportivo_id)
        if r3010_evtespdesportivo.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r3010evtEspDesportivo.objects.using(db_slug).filter(id=r3010_evtespdesportivo_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r3010_evtespdesportivo/%s.xml' % (r3010_evtespdesportivo.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, r3010_evtespdesportivo.identidade, BASE_DIR, r3010_evtespdesportivo.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/r3010_evtespdesportivo/%s.xml %s/arquivos/Eventos/r3010_evtespdesportivo/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/r3010_evtespdesportivo/%s_backup_%s.xml' % (r3010_evtespdesportivo.identidade, data_hora_atual), 
                    1)
...
def validar_evento_funcao(r3010_evtespdesportivo_id, db_slug):
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r3010_evtespdesportivo_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r3010_evtespdesportivo import validacoes_r3010_evtespdesportivo
                lista = validacoes_r3010_evtespdesportivo(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r3010_evtespdesportivo import validacoes_r3010_evtespdesportivo
                lista = validacoes_r3010_evtespdesportivo(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r3010_evtespdesportivo SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r3010_evtespdesportivo_id), False)
    else:
        executar_sql("UPDATE public.r3010_evtespdesportivo SET validacoes='', status=4 WHERE id=%s;" % (r3010_evtespdesportivo_id), False)
...
def validar_evento(request, hash):
    ...
    if r3010_evtespdesportivo_id:
        ...
        if r3010_evtespdesportivo.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r3010_evtespdesportivo_id, db_slug)
