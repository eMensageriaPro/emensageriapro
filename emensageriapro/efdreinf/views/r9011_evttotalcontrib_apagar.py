...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if r9011_evttotalcontrib.status == STATUS_EVENTO_CADASTRADO:
            ...
            obj = r9011evtTotalContrib.objects.get(id=pk)
            obj.delete(request=request)
            #r9011_evttotalcontrib_apagar_custom
            #r9011_evttotalcontrib_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r9011_evttotalcontrib', pk, request.user.id, 3)
        else:
        
            messages.error(request, u'''Não foi possivel apagar o evento, somente é 
                                        possível apagar os eventos com status "Cadastrado"!''')
            
        if 'r9011_evttotalcontrib' in request.session['return_page']:
