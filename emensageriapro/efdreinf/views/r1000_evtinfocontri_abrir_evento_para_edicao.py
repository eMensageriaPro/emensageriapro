...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r1000evtInfoContri'):

        if r1000_evtinfocontri_id:
            r1000_evtinfocontri = get_object_or_404(r1000evtInfoContri, excluido=False, id=r1000_evtinfocontri_id)
            ...
            if r1000_evtinfocontri.status in status_list:
                r1000evtInfoContri.objects.filter(id=r1000_evtinfocontri_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
