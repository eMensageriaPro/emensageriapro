...
def validar_evento_funcao(request, r2060_evtcprb_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2060_evtcprb_id)
...
def validar_evento(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        if r2060_evtcprb.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r2060_evtcprb_id)
            
            if r2060_evtcprb.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2060evtCPRB.objects.\
                    filter(id=r2060_evtcprb_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2060_evtcprb.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2060evtCPRB.objects.\
                    filter(id=r2060_evtcprb_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
