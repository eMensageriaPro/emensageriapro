...
def validar_evento_funcao(request, pk):
    ...
    if not r2060_evtcprb.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(r2060_evtcprb, 'efdreinf')
        r2060_evtcprb = get_object_or_404(r2060evtCPRB, id=pk)
    ...
    schema_filename = 'efdreinf/%s/evtCPRB.xsd' % (r2060_evtcprb.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 'r2060_evtcprb' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('r2060_evtcprb', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if r2060_evtcprb.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('r2060_evtcprb_salvar', pk=pk)