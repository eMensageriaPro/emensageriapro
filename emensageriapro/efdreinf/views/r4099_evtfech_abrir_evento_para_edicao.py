...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r4099evtFech'):

        if r4099_evtfech_id:
            r4099_evtfech = get_object_or_404(r4099evtFech, excluido=False, id=r4099_evtfech_id)
            ...
            if r4099_evtfech.status in status_list:
                r4099evtFech.objects.filter(id=r4099_evtfech_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r4099_evtfech.identidade, BASE_DIR, r4099_evtfech.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r4099_evtfech/%s.xml %s/arquivos/Eventos/r4099_evtfech/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r4099_evtfech/%s_backup_%s.xml' % (r4099_evtfech.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r4099_evtfech', r4099_evtfech_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r4099_evtfech_id )
