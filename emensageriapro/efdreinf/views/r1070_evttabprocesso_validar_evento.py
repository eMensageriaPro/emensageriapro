...
def validar_evento_funcao(request, r1070_evttabprocesso_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r1070_evttabprocesso_id)
...
def validar_evento(request, hash):
    ...
    if r1070_evttabprocesso_id:
        ...
        if r1070_evttabprocesso.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r1070_evttabprocesso_id)
            
            if r1070_evttabprocesso.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r1070evtTabProcesso.objects.\
                    filter(id=r1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r1070_evttabprocesso.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r1070evtTabProcesso.objects.\
                    filter(id=r1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
