...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if r2098_evtreabreevper.status == STATUS_EVENTO_CADASTRADO:
            ...
            situacao_anterior = json.dumps(model_to_dict(r2098_evtreabreevper), sort_keys=True, default=str)
            obj = r2098evtReabreEvPer.objects.get(id=pk)
            obj.delete(request=request)

            sql_softdelete = ler_arquivo('/database/sql/r2098_softdelete_cascade.sql')
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             'r2098_evtreabreevper', pk, request.user.id, 3)
        else:

            messages.error(request, u'''Não foi possivel apagar o evento, somente é
                                        possível apagar os eventos com status "Cadastrado"!''')

        if 'return_page' in request.session and request.session['return_page'] and 'r2098-evtreabreevper' in request.session['return_page']:
