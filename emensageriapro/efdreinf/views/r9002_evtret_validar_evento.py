...
def validar_evento_funcao(request, r9002_evtret_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r9002_evtret_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r9002_evtret_validar import validacoes_r9002_evtret
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r9002_evtret(arquivo)
    ...
    if lista_validacoes:
        ...
        r9002evtRet.objects.\
            filter(id=r9002_evtret_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r9002_evtret', r9002_evtret_id)
...
def validar_evento(request, hash):
    ...
    if r9002_evtret_id:
        ...
        if r9002_evtret.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r9002_evtret_id)
            
            if r9002_evtret.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9002evtRet.objects.\
                    filter(id=r9002_evtret_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r9002_evtret.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9002evtRet.objects.\
                    filter(id=r9002_evtret_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
