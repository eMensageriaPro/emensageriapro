...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2060_evtcprb_verificar.html' % r2060_evtcprb.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2060_evtcprb_verificar.html' % r2060_evtcprb.versao,
            ...
            response =  render_to_response('%s/r2060_evtcprb_verificar.html' % r2060_evtcprb.versao, context)
            ...
            return render(request, '%s/r2060_evtcprb_verificar.html' % r2060_evtcprb.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2060(r2060_evtcprb_id, db_slug):
    from django.template.loader import get_template
    if r2060_evtcprb_id:
        r2060_evtcprb = get_object_or_404(r2060evtCPRB.objects.using( db_slug ), excluido = False, id = r2060_evtcprb_id)
        r2060_evtcprb_lista = r2060evtCPRB.objects.using( db_slug ).filter(id=r2060_evtcprb_id, excluido = False).all()
        ...
        #return render(request, 'xml/%s/r2060_evtcprb.html' % r2060_evtcprb.versao, context, content_type='text/xml')
        t = get_template('%s/r2060_evtcprb_xml.html' % r2060_evtcprb.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if r2060_evtcprb.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2060_evtcprb.retornos_evttotal_id, excluido=False)
        ...
        if r2060_evtcprb.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2060_evtcprb.retornos_evttotalcontrib_id, excluido=False)
...
def gerar_xml_assinado(r2060_evtcprb_id, db_slug):
    ...
    if r2060_evtcprb.status in (0,1,2,11):
        r2060evtCPRB.objects.using(db_slug).filter(id=r2060_evtcprb_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    r2060_evtcprb_id = int(dict_hash['id'])
    if r2060_evtcprb_id:
        r2060_evtcprb = get_object_or_404(r2060evtCPRB.objects.using(db_slug), excluido=False,
                                                    id=r2060_evtcprb_id)

        arquivo = 'arquivos/Eventos/r2060_evtcprb/%s.xml' % r2060_evtcprb.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r2060_evtcprb_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r2060_evtcprb/%s.xml' % r2060_evtcprb.identidade)
        salvar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_duplicado_temp.xml' % r2060_evtcprb.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_duplicado_temp.xml' % r2060_evtcprb.identidade, request)
        from emensageriapro.efdreinf.views.r2060_evtcprb import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2060evtCPRB.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2060_evtcprb.identidade), 
            'r2060_evtcprb', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_alteracao_temp.xml' % r2060_evtcprb.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_alteracao_temp.xml' % r2060_evtcprb.identidade, request)
        from emensageriapro.efdreinf.views.r2060_evtcprb import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2060evtCPRB.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2060_evtcprb.identidade), 
            'r2060_evtcprb', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_exclusao_temp.xml' % r2060_evtcprb.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2060_evtcprb/%s_exclusao_temp.xml' % r2060_evtcprb.identidade, request)
        from emensageriapro.efdreinf.views.r2060_evtcprb import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2060evtCPRB.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2060_evtcprb.identidade), 
            'r2060_evtcprb', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        if r2060_evtcprb.status == 0:
            from emensageriapro.efdreinf.views.r2060_evtcprb import identidade_evento
            dent = identidade_evento(r2060_evtcprb_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2060_evtcprb_id:
        r2060_evtcprb = get_object_or_404(r2060evtCPRB.objects.using(db_slug), excluido=False, id=r2060_evtcprb_id)
        if r2060_evtcprb.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r2060evtCPRB.objects.using(db_slug).filter(id=r2060_evtcprb_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2060_evtcprb/%s.xml' % (r2060_evtcprb.identidade)
...
def validar_evento_funcao(r2060_evtcprb_id, db_slug):
    ...
    if quant <= 0:
        ...
        precedencia = 1
    executar_sql("UPDATE public.r2060_evtcprb SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, r2060_evtcprb_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/r2060_evtcprb/%s.xml' % (r2060_evtcprb.identidade)
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2060_evtcprb import validacoes_r2060_evtcprb
                lista = validacoes_r2060_evtcprb(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2060_evtcprb import validacoes_r2060_evtcprb
                lista = validacoes_r2060_evtcprb(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r2060_evtcprb SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r2060_evtcprb_id), False)
    else:
        executar_sql("UPDATE public.r2060_evtcprb SET validacoes='', status=4 WHERE id=%s;" % (r2060_evtcprb_id), False)
...
def validar_evento(request, hash):
    ...
    if r2060_evtcprb_id:
        lista_validacoes = []
        r2060_evtcprb = get_object_or_404(r2060evtCPRB.objects.using(db_slug), excluido=False, id=r2060_evtcprb_id)
        if r2060_evtcprb.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2060_evtcprb_id, db_slug)
