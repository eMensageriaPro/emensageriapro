...
def gerar_xml_assinado(r2060_evtcprb_id, db_slug):
    ...
    if r2060_evtcprb.status in (0,1,2,11):
        ...
        r2060evtCPRB.objects.using(db_slug).\
            filter(id=r2060_evtcprb_id,excluido=False).update(status=10)
...
def alterar_identidade(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        if r2060_evtcprb.status == 0:
            ...
            nova_identidade = identidade_evento(r2060_evtcprb_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        if r2060_evtcprb.status in (0, 1, 2, 3, 4, 10, 11) or r2060_evtcprb.processamento_codigo_resposta in (401,402):
...
def validar_evento_funcao(r2060_evtcprb_id, db_slug):
    ...
    lista_validacoes = []
    r2060_evtcprb = get_object_or_404(r2060evtCPRB.objects.using(db_slug), excluido=False, id=r2060_evtcprb_id)
    if r2060_evtcprb.transmissor_lote_esocial:
        if r2060_evtcprb.transmissor_lote_esocial.transmissor:
            if r2060_evtcprb.transmissor_lote_esocial.transmissor.verificar_predecessao:
