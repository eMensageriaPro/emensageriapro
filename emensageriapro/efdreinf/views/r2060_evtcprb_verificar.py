...
def validar_evento_funcao(r2060_evtcprb_id, db_slug):
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('esocial', 'r2060_evtcprb', r2060_evtcprb_id)
...
def validar_evento(request, hash):
    ...
    if r2060_evtcprb_id:
        ...
        if r2060_evtcprb.versao in VERSOES_EFDREINF:
            ...
            if r2060_evtcprb.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2060evtCPRB.objects.using(db_slug).\
                    filter(excluido=False, id=r2060_evtcprb_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2060_evtcprb.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2060evtCPRB.objects.using(db_slug).\
                    filter(excluido=False, id=r2060_evtcprb_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
