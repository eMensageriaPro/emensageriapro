...
def validar_evento_funcao(request, r5001_evttotal_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r5001_evttotal_id)
...
def validar_evento(request, hash):
    ...
    if r5001_evttotal_id:
        ...
        if r5001_evttotal.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r5001_evttotal_id)
            
            if r5001_evttotal.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r5001evtTotal.objects.\
                    filter(id=r5001_evttotal_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r5001_evttotal.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r5001evtTotal.objects.\
                    filter(id=r5001_evttotal_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
