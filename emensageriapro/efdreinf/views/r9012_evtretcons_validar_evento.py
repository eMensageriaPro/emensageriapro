...
def validar_evento_funcao(request, r9012_evtretcons_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r9012_evtretcons_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r9012_evtretcons_validar import validacoes_r9012_evtretcons
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r9012_evtretcons(arquivo)
    ...
    if lista_validacoes:
        ...
        r9012evtRetCons.objects.\
            filter(id=r9012_evtretcons_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r9012_evtretcons', r9012_evtretcons_id)
...
def validar_evento(request, hash):
    ...
    if r9012_evtretcons_id:
        ...
        if r9012_evtretcons.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r9012_evtretcons_id)
            
            if r9012_evtretcons.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9012evtRetCons.objects.\
                    filter(id=r9012_evtretcons_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r9012_evtretcons.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9012evtRetCons.objects.\
                    filter(id=r9012_evtretcons_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
