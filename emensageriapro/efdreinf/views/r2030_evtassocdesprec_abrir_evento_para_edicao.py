...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r2030evtAssocDespRec'):

        if r2030_evtassocdesprec_id:
            r2030_evtassocdesprec = get_object_or_404(r2030evtAssocDespRec, excluido=False, id=r2030_evtassocdesprec_id)
            ...
            if r2030_evtassocdesprec.status in status_list:
                r2030evtAssocDespRec.objects.filter(id=r2030_evtassocdesprec_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
