...
def validar_evento_funcao(request, r9001_evttotal_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r9001_evttotal_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r9001_evttotal_validar import validacoes_r9001_evttotal
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r9001_evttotal(arquivo)
    ...
    if lista_validacoes:
        ...
        r9001evtTotal.objects.\
            filter(id=r9001_evttotal_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r9001_evttotal', r9001_evttotal_id)
...
def validar_evento(request, hash):
    ...
    if r9001_evttotal_id:
        ...
        if r9001_evttotal.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r9001_evttotal_id)
            
            if r9001_evttotal.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9001evtTotal.objects.\
                    filter(id=r9001_evttotal_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r9001_evttotal.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r9001evtTotal.objects.\
                    filter(id=r9001_evttotal_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
