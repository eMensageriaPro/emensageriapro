...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r9002evtRet'):

        if r9002_evtret_id:
            r9002_evtret = get_object_or_404(r9002evtRet, excluido=False, id=r9002_evtret_id)
            ...
            if r9002_evtret.status in status_list:
                r9002evtRet.objects.filter(id=r9002_evtret_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r9002_evtret', r9002_evtret_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r9002_evtret_id )
