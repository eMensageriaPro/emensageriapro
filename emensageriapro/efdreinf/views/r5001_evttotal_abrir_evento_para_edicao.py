...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r5001evtTotal'):

        if r5001_evttotal_id:
            r5001_evttotal = get_object_or_404(r5001evtTotal, excluido=False, id=r5001_evttotal_id)
            ...
            if r5001_evttotal.status in status_list:
                r5001evtTotal.objects.filter(id=r5001_evttotal_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
