...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r1070_evttabprocesso_verificar.html' % r1070_evttabprocesso.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r1070_evttabprocesso_verificar.html' % r1070_evttabprocesso.versao,
            ...
            response =  render_to_response('%s/r1070_evttabprocesso_verificar.html' % r1070_evttabprocesso.versao, context)
            ...
            return render(request, '%s/r1070_evttabprocesso_verificar.html' % r1070_evttabprocesso.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r1070(r1070_evttabprocesso_id, db_slug):
    from django.template.loader import get_template
    if r1070_evttabprocesso_id:
        ...
        #return render(request, 'xml/%s/r1070_evttabprocesso.html' % r1070_evttabprocesso.versao, context, content_type='text/xml')
        t = get_template('%s/r1070_evttabprocesso_xml.html' % r1070_evttabprocesso.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if r1070_evttabprocesso.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r1070_evttabprocesso.retornos_evttotal_id, excluido=False)
        ...
        if r1070_evttabprocesso.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r1070_evttabprocesso.retornos_evttotalcontrib_id, excluido=False)
...
def gerar_xml_assinado(r1070_evttabprocesso_id, db_slug):
    ...
    if r1070_evttabprocesso.status in (0,1,2,11):
        r1070evtTabProcesso.objects.using(db_slug).filter(id=r1070_evttabprocesso_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    r1070_evttabprocesso_id = int(dict_hash['id'])
    if r1070_evttabprocesso_id:
        r1070_evttabprocesso = get_object_or_404(r1070evtTabProcesso.objects.using(db_slug), excluido=False,
                                                    id=r1070_evttabprocesso_id)

        arquivo = 'arquivos/Eventos/r1070_evttabprocesso/%s.xml' % r1070_evttabprocesso.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r1070_evttabprocesso_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s.xml' % r1070_evttabprocesso.identidade)
        salvar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_duplicado_temp.xml' % r1070_evttabprocesso.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_duplicado_temp.xml' % r1070_evttabprocesso.identidade, request)
        ...
        r1070evtTabProcesso.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r1070_evttabprocesso.identidade), 
            'r1070_evttabprocesso', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r1070_evttabprocesso_id:
        ...
        salvar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_alteracao_temp.xml' % r1070_evttabprocesso.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_alteracao_temp.xml' % r1070_evttabprocesso.identidade, request)
        from emensageriapro.efdreinf.views.r1070_evttabprocesso import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r1070evtTabProcesso.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r1070_evttabprocesso.identidade), 
            'r1070_evttabprocesso', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r1070_evttabprocesso_id:
        ...
        salvar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_exclusao_temp.xml' % r1070_evttabprocesso.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r1070_evttabprocesso/%s_exclusao_temp.xml' % r1070_evttabprocesso.identidade, request)
        from emensageriapro.efdreinf.views.r1070_evttabprocesso import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r1070evtTabProcesso.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r1070_evttabprocesso.identidade), 
            'r1070_evttabprocesso', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r1070_evttabprocesso_id:
        ...
        if r1070_evttabprocesso.status == 0:
            from emensageriapro.efdreinf.views.r1070_evttabprocesso import identidade_evento
            dent = identidade_evento(r1070_evttabprocesso_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r1070_evttabprocesso_id:
        r1070_evttabprocesso = get_object_or_404(r1070evtTabProcesso.objects.using(db_slug), excluido=False, id=r1070_evttabprocesso_id)
        if r1070_evttabprocesso.status in (0, 1, 2, 3, 4, 5, 10, 11):
...
def validar_evento_funcao(r1070_evttabprocesso_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r1070_evttabprocesso import validacoes_r1070_evttabprocesso
                lista = validacoes_r1070_evttabprocesso(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r1070_evttabprocesso import validacoes_r1070_evttabprocesso
                lista = validacoes_r1070_evttabprocesso(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if r1070_evttabprocesso_id:
        lista_validacoes = []
        r1070_evttabprocesso = get_object_or_404(r1070evtTabProcesso.objects.using(db_slug), excluido=False, id=r1070_evttabprocesso_id)
        if r1070_evttabprocesso.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r1070_evttabprocesso_id, db_slug)
