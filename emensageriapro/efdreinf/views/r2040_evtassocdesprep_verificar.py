...
def verificar(request, hash, slug=0):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2040_evtassocdesprep')
    ...
    if permissao.permite_listar:
        ...
        r2040_evtassocdesprep_lista = r2040evtAssocDespRep.objects.using( db_slug ).filter(id=r2040_evtassocdesprep_id, excluido = False).all()
        
    
        r2040_recursosrep_lista = r2040recursosRep.objects.using(db_slug).filter(r2040_evtassocdesprep_id__in = listar_ids(r2040_evtassocdesprep_lista) ).filter(excluido=False).all()    
        r2040_inforecurso_lista = r2040infoRecurso.objects.using(db_slug).filter(r2040_recursosrep_id__in = listar_ids(r2040_recursosrep_lista) ).filter(excluido=False).all()    
        r2040_infoproc_lista = r2040infoProc.objects.using(db_slug).filter(r2040_recursosrep_id__in = listar_ids(r2040_recursosrep_lista) ).filter(excluido=False).all()
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2040_evtassocdesprep_verificar.html' % r2040_evtassocdesprep.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2040_evtassocdesprep_verificar.html' % r2040_evtassocdesprep.versao,
            ...
            response =  render_to_response('%s/r2040_evtassocdesprep_verificar.html' % r2040_evtassocdesprep.versao, context)
            ...
            return render(request, '%s/r2040_evtassocdesprep_verificar.html' % r2040_evtassocdesprep.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2040(r2040_evtassocdesprep_id, db_slug):
    from django.template.loader import get_template
    if r2040_evtassocdesprep_id:
        ...
        #return render(request, 'xml/%s/r2040_evtassocdesprep.html' % r2040_evtassocdesprep.versao, context, content_type='text/xml')
        t = get_template('%s/r2040_evtassocdesprep_xml.html' % r2040_evtassocdesprep.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2040_evtassocdesprep = get_object_or_404(
            r2040evtAssocDespRep.objects.using( db_slug ),
            excluido = False, id = r2040_evtassocdesprep_id)
    

        if r2040_evtassocdesprep.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2040_evtassocdesprep.retornos_evttotal_id, excluido=False)
        ...
        if r2040_evtassocdesprep.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2040_evtassocdesprep.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r2040_evtassocdesprep_recibo_pdf.html', context)
            filename = "%s.xls" % r2040_evtassocdesprep.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r2040_evtassocdesprep_recibo_csv.html', context)
            filename = "%s.csv" % r2040_evtassocdesprep.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r2040_evtassocdesprep_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r2040_evtassocdesprep_id, db_slug):
    import os
    ...
    if r2040_evtassocdesprep.arquivo_original:
        ...
        xml = gerar_xml_r2040(r2040_evtassocdesprep_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r2040_evtassocdesprep.status in (0,1,2,11):
        r2040evtAssocDespRep.objects.using(db_slug).filter(id=r2040_evtassocdesprep_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r2040_evtassocdesprep_id:
        r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep.objects.using(db_slug), excluido=False, id=r2040_evtassocdesprep_id)
        xml_assinado = gerar_xml_assinado(r2040_evtassocdesprep_id, db_slug)
...
def duplicar(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2040_evtassocdesprep/%s_duplicado_temp.xml' % r2040_evtassocdesprep.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2040_evtassocdesprep/%s_duplicado_temp.xml' % r2040_evtassocdesprep.identidade, request)
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2040_evtassocdesprep.identidade), 
...
def criar_exclusao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2040_evtassocdesprep/%s_exclusao_temp.xml' % r2040_evtassocdesprep.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2040_evtassocdesprep/%s_exclusao_temp.xml' % r2040_evtassocdesprep.identidade, request)
        from emensageriapro.efdreinf.views.r2040_evtassocdesprep import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2040evtAssocDespRep.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2040_evtassocdesprep.identidade), 
            'r2040_evtassocdesprep', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.status == 0:
            from emensageriapro.efdreinf.views.r2040_evtassocdesprep import identidade_evento
            dent = identidade_evento(r2040_evtassocdesprep_id, db_slug)
            ...
            'r2040_evtassocdesprep', r2040_evtassocdesprep_id, usuario_id, 1)
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r2040_evtassocdesprep_id )
            return redirect('r2040_evtassocdesprep_salvar', hash=url_hash)
        else:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep.objects.using(db_slug), excluido=False, id=r2040_evtassocdesprep_id)
        if r2040_evtassocdesprep.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r2040evtAssocDespRep.objects.using(db_slug).filter(id=r2040_evtassocdesprep_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2040_evtassocdesprep/%s.xml' % (r2040_evtassocdesprep.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, r2040_evtassocdesprep.identidade, BASE_DIR, r2040_evtassocdesprep.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/r2040_evtassocdesprep/%s.xml %s/arquivos/Eventos/r2040_evtassocdesprep/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/r2040_evtassocdesprep/%s_backup_%s.xml' % (r2040_evtassocdesprep.identidade, data_hora_atual), 
                    1)
...
@login_required
def validar_evento_funcao(r2040_evtassocdesprep_id, db_slug):
    ...
    lista_validacoes = []
    r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep.objects.using(db_slug), excluido=False, id=r2040_evtassocdesprep_id)
    if r2040_evtassocdesprep.transmissor_lote_efdreinf:
        if r2040_evtassocdesprep.transmissor_lote_efdreinf.transmissor:
            if r2040_evtassocdesprep.transmissor_lote_efdreinf.transmissor.verificar_predecessao:
                quant = validar_precedencia('efdreinf', 'r2040_evtassocdesprep', r2040_evtassocdesprep_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.r2040_evtassocdesprep SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, r2040_evtassocdesprep_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/r2040_evtassocdesprep/%s.xml' % (r2040_evtassocdesprep.identidade)
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r2040_evtassocdesprep_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2040_evtassocdesprep import validacoes_r2040_evtassocdesprep
                lista = validacoes_r2040_evtassocdesprep(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2040_evtassocdesprep import validacoes_r2040_evtassocdesprep
                lista = validacoes_r2040_evtassocdesprep(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r2040_evtassocdesprep SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r2040_evtassocdesprep_id), False)
    else:
        executar_sql("UPDATE public.r2040_evtassocdesprep SET validacoes='', status=4 WHERE id=%s;" % (r2040_evtassocdesprep_id), False)
...
def validar_evento(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        lista_validacoes = []
        r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep.objects.using(db_slug), excluido=False, id=r2040_evtassocdesprep_id)
        if r2040_evtassocdesprep.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2040_evtassocdesprep_id, db_slug)
