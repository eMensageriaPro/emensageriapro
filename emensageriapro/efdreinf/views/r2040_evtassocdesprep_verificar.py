...
def gerar_xml_r2040(r2040_evtassocdesprep_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r2040_evtassocdesprep.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r2040_evtassocdesprep_id, db_slug):
    ...
    if r2040_evtassocdesprep.status in (0,1,2,11):
        ...
        r2040evtAssocDespRep.objects.using(db_slug).\
            filter(id=r2040_evtassocdesprep_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.efdreinf.views.r2040_evtassocdesprep_importar import read_r2040_evtassocdesprep_string
    ...
    if r2040_evtassocdesprep_id:
        ...
        texto = gerar_xml_r2040(r2040_evtassocdesprep_id, db_slug, versao="|")
        dados = read_r2040_evtassocdesprep_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        r2040evtAssocDespRep.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2040_evtassocdesprep.identidade),
            'r2040_evtassocdesprep', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2040_evtassocdesprep.identidade),
            'r2040_evtassocdesprep', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2040_evtassocdesprep.identidade),
            'r2040_evtassocdesprep', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.status == 0:
            ...
            nova_identidade = identidade_evento(r2040_evtassocdesprep_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                'r2040_evtassocdesprep', r2040_evtassocdesprep_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.status in (0, 1, 2, 3, 4, 10, 11) or r2040_evtassocdesprep.processamento_codigo_resposta in (401,402):
...
=======
    if r2040_evtassocdesprep.transmissor_lote_esocial:
        if r2040_evtassocdesprep.transmissor_lote_esocial.transmissor:
            if r2040_evtassocdesprep.transmissor_lote_esocial.transmissor.verificar_predecessao:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
    ...
    r2040evtAssocDespRep.objects.using( db_slug ).\
        filter(id=r2040_evtassocdesprep_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.efdreinf.views.r2040_evtassocdesprep_validar import validacoes_r2040_evtassocdesprep
...
<<<<<<< HEAD
        if r2040_evtassocdesprep.versao in VERSOES_EFDREINF:
=======
        if r2040_evtassocdesprep.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(r2040_evtassocdesprep_id, db_slug)
