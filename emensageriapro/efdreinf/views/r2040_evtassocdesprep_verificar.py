...
def gerar_xml_assinado(r2040_evtassocdesprep_id, db_slug):
    ...
    if r2040_evtassocdesprep.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        r2040evtAssocDespRep.objects.using(db_slug).\
            filter(id=r2040_evtassocdesprep_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def alterar_identidade(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.status == STATUS_EVENTO_CADASTRADO:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.status in status_list:
            r2040evtAssocDespRep.objects.using(db_slug).filter(id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2040_evtassocdesprep/%s.xml' % (r2040_evtassocdesprep.identidade)
...
def validar_evento(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.versao in VERSOES_EFDREINF:
            ...
            if r2040_evtassocdesprep.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2040evtAssocDespRep.objects.using(db_slug).\
                    filter(excluido=False, id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2040_evtassocdesprep.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2040evtAssocDespRep.objects.using(db_slug).\
                    filter(excluido=False, id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
