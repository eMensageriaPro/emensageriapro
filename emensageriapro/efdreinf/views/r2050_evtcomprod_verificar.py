...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2050_evtcomprod_lista = r2050evtComProd.objects.using( db_slug ).filter(id=r2050_evtcomprod_id, excluido = False).all()
        
    
        r2050_tipocom_lista = r2050tipoCom.objects.using(db_slug).filter(r2050_evtcomprod_id__in = listar_ids(r2050_evtcomprod_lista) ).filter(excluido=False).all()    
        r2050_infoproc_lista = r2050infoProc.objects.using(db_slug).filter(r2050_tipocom_id__in = listar_ids(r2050_tipocom_lista) ).filter(excluido=False).all()
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2050_evtcomprod_verificar.html' % r2050_evtcomprod.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2050_evtcomprod_verificar.html' % r2050_evtcomprod.versao,
            ...
            response =  render_to_response('%s/r2050_evtcomprod_verificar.html' % r2050_evtcomprod.versao, context)
            ...
            return render(request, '%s/r2050_evtcomprod_verificar.html' % r2050_evtcomprod.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2050(r2050_evtcomprod_id, db_slug):
    from django.template.loader import get_template
    if r2050_evtcomprod_id:
        ...
        #return render(request, 'xml/%s/r2050_evtcomprod.html' % r2050_evtcomprod.versao, context, content_type='text/xml')
        t = get_template('%s/r2050_evtcomprod_xml.html' % r2050_evtcomprod.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2050_evtcomprod = get_object_or_404(
            r2050evtComProd.objects.using( db_slug ),
            excluido = False, id = r2050_evtcomprod_id)
    

        if r2050_evtcomprod.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2050_evtcomprod.retornos_evttotal_id, excluido=False)
        ...
        if r2050_evtcomprod.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2050_evtcomprod.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r2050_evtcomprod_recibo_pdf.html', context)
            filename = "%s.xls" % r2050_evtcomprod.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r2050_evtcomprod_recibo_csv.html', context)
            filename = "%s.csv" % r2050_evtcomprod.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r2050_evtcomprod_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r2050_evtcomprod_id, db_slug):
    import os
    ...
    if r2050_evtcomprod.arquivo_original:
        ...
        xml = gerar_xml_r2050(r2050_evtcomprod_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r2050_evtcomprod.status in (0,1,2,11):
        r2050evtComProd.objects.using(db_slug).filter(id=r2050_evtcomprod_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r2050_evtcomprod_id:
        r2050_evtcomprod = get_object_or_404(r2050evtComProd.objects.using(db_slug), excluido=False, id=r2050_evtcomprod_id)
        xml_assinado = gerar_xml_assinado(r2050_evtcomprod_id, db_slug)
...
def duplicar(request, hash):
    ...
    r2050_evtcomprod_id = int(dict_hash['id'])
    if r2050_evtcomprod_id:
        r2050_evtcomprod = get_object_or_404(r2050evtComProd.objects.using(db_slug), excluido=False,
                                                    id=r2050_evtcomprod_id)

        arquivo = 'arquivos/Eventos/r2050_evtcomprod/%s.xml' % r2050_evtcomprod.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r2050_evtcomprod_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r2050_evtcomprod/%s.xml' % r2050_evtcomprod.identidade)
        salvar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_duplicado_temp.xml' % r2050_evtcomprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_duplicado_temp.xml' % r2050_evtcomprod.identidade, request)
        from emensageriapro.efdreinf.views.r2050_evtcomprod import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2050evtComProd.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2050_evtcomprod.identidade), 
            'r2050_evtcomprod', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2050_evtcomprod_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_alteracao_temp.xml' % r2050_evtcomprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_alteracao_temp.xml' % r2050_evtcomprod.identidade, request)
        from emensageriapro.efdreinf.views.r2050_evtcomprod import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2050evtComProd.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2050_evtcomprod.identidade), 
            'r2050_evtcomprod', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2050_evtcomprod_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_exclusao_temp.xml' % r2050_evtcomprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2050_evtcomprod/%s_exclusao_temp.xml' % r2050_evtcomprod.identidade, request)
        from emensageriapro.efdreinf.views.r2050_evtcomprod import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2050evtComProd.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2050_evtcomprod.identidade), 
            'r2050_evtcomprod', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2050_evtcomprod_id:
        ...
        if r2050_evtcomprod.status == 0:
            from emensageriapro.efdreinf.views.r2050_evtcomprod import identidade_evento
            dent = identidade_evento(r2050_evtcomprod_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2050_evtcomprod_id:
        r2050_evtcomprod = get_object_or_404(r2050evtComProd.objects.using(db_slug), excluido=False, id=r2050_evtcomprod_id)
        if r2050_evtcomprod.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r2050evtComProd.objects.using(db_slug).filter(id=r2050_evtcomprod_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2050_evtcomprod/%s.xml' % (r2050_evtcomprod.identidade)
...
@login_required
def validar_evento_funcao(r2050_evtcomprod_id, db_slug):
    ...
    lista_validacoes = []
    r2050_evtcomprod = get_object_or_404(r2050evtComProd.objects.using(db_slug), excluido=False, id=r2050_evtcomprod_id)
    if r2050_evtcomprod.transmissor_lote_efdreinf:
        if r2050_evtcomprod.transmissor_lote_efdreinf.transmissor:
            if r2050_evtcomprod.transmissor_lote_efdreinf.transmissor.verificar_predecessao:
                quant = validar_precedencia('efdreinf', 'r2050_evtcomprod', r2050_evtcomprod_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.r2050_evtcomprod SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, r2050_evtcomprod_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/r2050_evtcomprod/%s.xml' % (r2050_evtcomprod.identidade)
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r2050_evtcomprod_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2050_evtcomprod import validacoes_r2050_evtcomprod
                lista = validacoes_r2050_evtcomprod(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2050_evtcomprod import validacoes_r2050_evtcomprod
                lista = validacoes_r2050_evtcomprod(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r2050_evtcomprod SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r2050_evtcomprod_id), False)
    else:
        executar_sql("UPDATE public.r2050_evtcomprod SET validacoes='', status=4 WHERE id=%s;" % (r2050_evtcomprod_id), False)
...
def validar_evento(request, hash):
    ...
    if r2050_evtcomprod_id:
        lista_validacoes = []
        r2050_evtcomprod = get_object_or_404(r2050evtComProd.objects.using(db_slug), excluido=False, id=r2050_evtcomprod_id)
        if r2050_evtcomprod.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2050_evtcomprod_id, db_slug)
