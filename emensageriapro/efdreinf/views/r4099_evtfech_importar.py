...
def read_r4099_evtfech(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_r4099_evtfech_obj(request, doc, status, validar, arquivo)

    r4099evtFech.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_r4099_evtfech_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        r4099_evtfech_dados['arquivo'] = arquivo
