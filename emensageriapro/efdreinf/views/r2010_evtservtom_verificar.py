...
def gerar_xml_assinado(r2010_evtservtom_id, db_slug):
    ...
    if r2010_evtservtom.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        r2010evtServTom.objects.using(db_slug).\
            filter(id=r2010_evtservtom_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def validar_evento(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        if r2010_evtservtom.versao in VERSOES_EFDREINF:
            ...
            if r2010_evtservtom.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2010evtServTom.objects.using(db_slug).\
                    filter(excluido=False, id=r2010_evtservtom_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2010_evtservtom.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2010evtServTom.objects.using(db_slug).\
