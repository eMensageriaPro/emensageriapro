...
def gerar_xml_r2010(r2010_evtservtom_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r2010_evtservtom.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r2010_evtservtom_id, db_slug):
    ...
    if r2010_evtservtom.status in (0,1,2,11):
        ...
        r2010evtServTom.objects.using(db_slug).\
            filter(id=r2010_evtservtom_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        texto = gerar_xml_r2010(r2010_evtservtom_id, db_slug, versao="|")
        dados = read_r2010_evtservtom_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        r2010evtServTom.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2010_evtservtom.identidade),
...
def criar_alteracao(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2010_evtservtom.identidade),
            'r2010_evtservtom', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        if r2010_evtservtom.status == 0:
            ...
            nova_identidade = identidade_evento(r2010_evtservtom_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                'r2010_evtservtom', r2010_evtservtom_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        if r2010_evtservtom.status in (0, 1, 2, 3, 4, 10, 11) or r2010_evtservtom.processamento_codigo_resposta in (401,402):
...
def validar_evento_funcao(r2010_evtservtom_id, db_slug):
    ...
    lista_validacoes = []
    r2010_evtservtom = get_object_or_404(r2010evtServTom.objects.using(db_slug), excluido=False, id=r2010_evtservtom_id)
    if r2010_evtservtom.transmissor_lote_esocial:
        if r2010_evtservtom.transmissor_lote_esocial.transmissor:
            if r2010_evtservtom.transmissor_lote_esocial.transmissor.verificar_predecessao:
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.efdreinf.views.r2010_evtservtom_validar import validacoes_r2010_evtservtom
