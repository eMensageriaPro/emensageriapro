...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2010_evtservtom_lista = r2010evtServTom.objects.using( db_slug ).filter(id=r2010_evtservtom_id, excluido = False).all()
        
    
        r2010_nfs_lista = r2010nfs.objects.using(db_slug).filter(r2010_evtservtom_id__in = listar_ids(r2010_evtservtom_lista) ).filter(excluido=False).all()    
        r2010_infotpserv_lista = r2010infoTpServ.objects.using(db_slug).filter(r2010_nfs_id__in = listar_ids(r2010_nfs_lista) ).filter(excluido=False).all()    
        r2010_infoprocretpr_lista = r2010infoProcRetPr.objects.using(db_slug).filter(r2010_evtservtom_id__in = listar_ids(r2010_evtservtom_lista) ).filter(excluido=False).all()    
        r2010_infoprocretad_lista = r2010infoProcRetAd.objects.using(db_slug).filter(r2010_evtservtom_id__in = listar_ids(r2010_evtservtom_lista) ).filter(excluido=False).all()
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2010_evtservtom_verificar.html' % r2010_evtservtom.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2010_evtservtom_verificar.html' % r2010_evtservtom.versao,
            ...
            response =  render_to_response('%s/r2010_evtservtom_verificar.html' % r2010_evtservtom.versao, context)
            ...
            return render(request, '%s/r2010_evtservtom_verificar.html' % r2010_evtservtom.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2010(r2010_evtservtom_id, db_slug):
    from django.template.loader import get_template
    if r2010_evtservtom_id:
        ...
        #return render(request, 'xml/%s/r2010_evtservtom.html' % r2010_evtservtom.versao, context, content_type='text/xml')
        t = get_template('%s/r2010_evtservtom_xml.html' % r2010_evtservtom.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2010_evtservtom = get_object_or_404(
            ...
            excluido = False, id = r2010_evtservtom_id)
    

        if r2010_evtservtom.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2010_evtservtom.retornos_evttotal_id, excluido=False)
        ...
        if r2010_evtservtom.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2010_evtservtom.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r2010_evtservtom_recibo_pdf.html', context)
            filename = "%s.xls" % r2010_evtservtom.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r2010_evtservtom_recibo_csv.html', context)
            filename = "%s.csv" % r2010_evtservtom.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r2010_evtservtom_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r2010_evtservtom_id, db_slug):
    import os
    ...
    if r2010_evtservtom.arquivo_original:
        ...
        xml = gerar_xml_r2010(r2010_evtservtom_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r2010_evtservtom.status in (0,1,2,11):
        r2010evtServTom.objects.using(db_slug).filter(id=r2010_evtservtom_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r2010_evtservtom_id:
        r2010_evtservtom = get_object_or_404(r2010evtServTom.objects.using(db_slug), excluido=False, id=r2010_evtservtom_id)
        xml_assinado = gerar_xml_assinado(r2010_evtservtom_id, db_slug)
...
def duplicar(request, hash):
    ...
    r2010_evtservtom_id = int(dict_hash['id'])
    if r2010_evtservtom_id:
        r2010_evtservtom = get_object_or_404(r2010evtServTom.objects.using(db_slug), excluido=False,
                                                    id=r2010_evtservtom_id)

        arquivo = 'arquivos/Eventos/r2010_evtservtom/%s.xml' % r2010_evtservtom.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r2010_evtservtom_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r2010_evtservtom/%s.xml' % r2010_evtservtom.identidade)
        salvar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_duplicado_temp.xml' % r2010_evtservtom.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_duplicado_temp.xml' % r2010_evtservtom.identidade, request)
        from emensageriapro.efdreinf.views.r2010_evtservtom import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2010evtServTom.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2010_evtservtom.identidade), 
            'r2010_evtservtom', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_alteracao_temp.xml' % r2010_evtservtom.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_alteracao_temp.xml' % r2010_evtservtom.identidade, request)
        from emensageriapro.efdreinf.views.r2010_evtservtom import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2010evtServTom.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2010_evtservtom.identidade), 
            'r2010_evtservtom', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_exclusao_temp.xml' % r2010_evtservtom.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2010_evtservtom/%s_exclusao_temp.xml' % r2010_evtservtom.identidade, request)
        from emensageriapro.efdreinf.views.r2010_evtservtom import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2010evtServTom.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2010_evtservtom.identidade), 
            'r2010_evtservtom', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        if r2010_evtservtom.status == 0:
            from emensageriapro.efdreinf.views.r2010_evtservtom import identidade_evento
            dent = identidade_evento(r2010_evtservtom_id, db_slug)
            ...
            'r2010_evtservtom', r2010_evtservtom_id, usuario_id, 1)
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r2010_evtservtom_id )
            return redirect('r2010_evtservtom_salvar', hash=url_hash)
        else:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2010_evtservtom_id:
        r2010_evtservtom = get_object_or_404(r2010evtServTom.objects.using(db_slug), excluido=False, id=r2010_evtservtom_id)
        if r2010_evtservtom.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r2010evtServTom.objects.using(db_slug).filter(id=r2010_evtservtom_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2010_evtservtom/%s.xml' % (r2010_evtservtom.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, r2010_evtservtom.identidade, BASE_DIR, r2010_evtservtom.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/r2010_evtservtom/%s.xml %s/arquivos/Eventos/r2010_evtservtom/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/r2010_evtservtom/%s_backup_%s.xml' % (r2010_evtservtom.identidade, data_hora_atual), 
                    1)
...
@login_required
def validar_evento_funcao(r2010_evtservtom_id, db_slug):
    ...
    if quant <= 0:
        ...
        precedencia = 1
    executar_sql("UPDATE public.r2010_evtservtom SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, r2010_evtservtom_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/r2010_evtservtom/%s.xml' % (r2010_evtservtom.identidade)
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r2010_evtservtom_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2010_evtservtom import validacoes_r2010_evtservtom
                lista = validacoes_r2010_evtservtom(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2010_evtservtom import validacoes_r2010_evtservtom
                lista = validacoes_r2010_evtservtom(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r2010_evtservtom SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r2010_evtservtom_id), False)
    else:
        executar_sql("UPDATE public.r2010_evtservtom SET validacoes='', status=4 WHERE id=%s;" % (r2010_evtservtom_id), False)
...
def validar_evento(request, hash):
    ...
    if r2010_evtservtom_id:
        lista_validacoes = []
        r2010_evtservtom = get_object_or_404(r2010evtServTom.objects.using(db_slug), excluido=False, id=r2010_evtservtom_id)
        if r2010_evtservtom.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2010_evtservtom_id, db_slug)
