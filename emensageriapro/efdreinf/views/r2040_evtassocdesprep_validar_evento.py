...
def validar_evento_funcao(request, r2040_evtassocdesprep_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2040_evtassocdesprep_id)
...
def validar_evento(request, hash):
    ...
    if r2040_evtassocdesprep_id:
        ...
        if r2040_evtassocdesprep.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r2040_evtassocdesprep_id)
            
            if r2040_evtassocdesprep.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2040evtAssocDespRep.objects.\
                    filter(id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2040_evtassocdesprep.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2040evtAssocDespRep.objects.\
                    filter(id=r2040_evtassocdesprep_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
