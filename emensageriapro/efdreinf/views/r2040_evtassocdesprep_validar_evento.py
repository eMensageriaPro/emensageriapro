...
def validar_evento_funcao(request, pk):
    ...
    if not r2040_evtassocdesprep.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(r2040_evtassocdesprep, 'efdreinf')
        r2040_evtassocdesprep = get_object_or_404(r2040evtAssocDespRep, id=pk)
    ...
    schema_filename = 'efdreinf/%s/evtAssocDespRep.xsd' % (r2040_evtassocdesprep.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 'r2040_evtassocdesprep' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('r2040_evtassocdesprep', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if r2040_evtassocdesprep.status in STATUS_VALIDACAO:
