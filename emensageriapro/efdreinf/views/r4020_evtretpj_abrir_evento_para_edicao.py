...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r4020evtRetPJ'):

        if r4020_evtretpj_id:
            r4020_evtretpj = get_object_or_404(r4020evtRetPJ, excluido=False, id=r4020_evtretpj_id)
            ...
            if r4020_evtretpj.status in status_list:
                r4020evtRetPJ.objects.filter(id=r4020_evtretpj_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r4020_evtretpj.identidade, BASE_DIR, r4020_evtretpj.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r4020_evtretpj/%s.xml %s/arquivos/Eventos/r4020_evtretpj/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r4020_evtretpj/%s_backup_%s.xml' % (r4020_evtretpj.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r4020_evtretpj', r4020_evtretpj_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r4020_evtretpj_id )
