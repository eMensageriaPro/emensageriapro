...
def read_r2010_evtservtom(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_r2010_evtservtom_obj(request, doc, status, validar, arquivo)

    r2010evtServTom.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_r2010_evtservtom_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        r2010_evtservtom_dados['arquivo'] = arquivo
