...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if r2098_evtreabreevper.status == 0:
            ...
            r2098evtReabreEvPer.objects.using( db_slug ).filter(id = r2098_evtreabreevper_id).delete()
...
def identidade_evento(r2098_evtreabreevper_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEfdreinf
    dados_evento = r2098evtReabreEvPer.objects.using( db_slug ).get(id=r2098_evtreabreevper_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            r2098evtReabreEvPer.objects.using(db_slug).filter(id=r2098_evtreabreevper_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if r2098_evtreabreevper_form.is_valid():
                ...
                if r2098_evtreabreevper_id:
                    if r2098_evtreabreevper.status == 0:
                        ...
                        #r2098_evtreabreevper_campos_multiple_passo1
                        r2098evtReabreEvPer.objects.using(db_slug).filter(id=r2098_evtreabreevper_id).update(**dados)
                        obj = r2098evtReabreEvPer.objects.using(db_slug).get(id=r2098_evtreabreevper_id)
                        #r2098_evtreabreevper_editar_custom
                        #r2098_evtreabreevper_campos_multiple_passo2
                    ...
                    #r2098_evtreabreevper_cadastrar_campos_multiple_passo1
                    obj = r2098evtReabreEvPer(**dados)
                    obj.save(using = db_slug)
                    #r2098_evtreabreevper_cadastrar_custom
                    #r2098_evtreabreevper_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    r2098_evtreabreevper_form = form_r2098_evtreabreevper(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            r2098_evtreabreevper_form.fields['tpamb'].widget.attrs['disabled'] = True
            r2098_evtreabreevper_form.fields['tpamb'].widget.attrs['readonly'] = True
            r2098_evtreabreevper_form.fields['tpamb'].value = TP_AMB
            r2098_evtreabreevper_form.fields['procemi'].widget.attrs['disabled'] = True
            r2098_evtreabreevper_form.fields['procemi'].widget.attrs['readonly'] = True
            r2098_evtreabreevper_form.fields['procemi'].value = 1
            r2098_evtreabreevper_form.fields['verproc'].widget.attrs['readonly'] = True
            r2098_evtreabreevper_form.fields['verproc'].value = VERSAO_EMENSAGERIA
            r2098_evtreabreevper_form.fields['status'].widget.attrs['disabled'] = True
            r2098_evtreabreevper_form.fields['status'].widget.attrs['readonly'] = True
            r2098_evtreabreevper_form.fields['transmissor_lote_efdreinf'].widget.attrs['disabled'] = True
