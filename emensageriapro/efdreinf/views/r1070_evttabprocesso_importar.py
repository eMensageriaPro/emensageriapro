...
def read_r1070_evttabprocesso(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_r1070_evttabprocesso_obj(request, doc, status, validar, arquivo)

    r1070evtTabProcesso.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_r1070_evttabprocesso_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        r1070_evttabprocesso_dados['arquivo'] = arquivo
