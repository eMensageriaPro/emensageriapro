...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2099_evtfechaevper_verificar.html' % r2099_evtfechaevper.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2099_evtfechaevper_verificar.html' % r2099_evtfechaevper.versao,
            ...
            response =  render_to_response('%s/r2099_evtfechaevper_verificar.html' % r2099_evtfechaevper.versao, context)
            ...
            return render(request, '%s/r2099_evtfechaevper_verificar.html' % r2099_evtfechaevper.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2099(r2099_evtfechaevper_id, db_slug):
    ...
    if r2099_evtfechaevper_id:
        ...
        #return render(request, 'xml/%s/r2099_evtfechaevper.html' % r2099_evtfechaevper.versao, context, content_type='text/xml')
        t = get_template('%s/r2099_evtfechaevper_xml.html' % r2099_evtfechaevper.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        if r2099_evtfechaevper.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2099_evtfechaevper.retornos_evttotal_id, excluido=False)
        ...
        if r2099_evtfechaevper.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2099_evtfechaevper.retornos_evttotalcontrib_id, excluido=False)
...
def gerar_xml_assinado(r2099_evtfechaevper_id, db_slug):
    ...
    if r2099_evtfechaevper.status in (0,1,2,11):
        r2099evtFechaEvPer.objects.using(db_slug).filter(id=r2099_evtfechaevper_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    r2099_evtfechaevper_id = int(dict_hash['id'])
    if r2099_evtfechaevper_id:
        r2099_evtfechaevper = get_object_or_404(r2099evtFechaEvPer.objects.using(db_slug), excluido=False,
                                                    id=r2099_evtfechaevper_id)

        arquivo = 'arquivos/Eventos/r2099_evtfechaevper/%s.xml' % r2099_evtfechaevper.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r2099_evtfechaevper_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s.xml' % r2099_evtfechaevper.identidade)
        salvar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_duplicado_temp.xml' % r2099_evtfechaevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_duplicado_temp.xml' % r2099_evtfechaevper.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_alteracao_temp.xml' % r2099_evtfechaevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_alteracao_temp.xml' % r2099_evtfechaevper.identidade, request)
        from emensageriapro.efdreinf.views.r2099_evtfechaevper import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2099evtFechaEvPer.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def criar_exclusao(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_exclusao_temp.xml' % r2099_evtfechaevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2099_evtfechaevper/%s_exclusao_temp.xml' % r2099_evtfechaevper.identidade, request)
        from emensageriapro.efdreinf.views.r2099_evtfechaevper import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2099evtFechaEvPer.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2099_evtfechaevper_id:
        r2099_evtfechaevper = get_object_or_404(r2099evtFechaEvPer.objects.using(db_slug), excluido=False, id=r2099_evtfechaevper_id)
        if r2099_evtfechaevper.status in (0, 1, 2, 3, 4, 5, 10, 11):
...
def validar_evento_funcao(r2099_evtfechaevper_id, db_slug):
    ...
    lista_validacoes = []
    r2099_evtfechaevper = get_object_or_404(r2099evtFechaEvPer.objects.using(db_slug), excluido=False, id=r2099_evtfechaevper_id)
    if r2099_evtfechaevper.transmissor_lote_efdreinf:
        if r2099_evtfechaevper.transmissor_lote_efdreinf.transmissor:
            if r2099_evtfechaevper.transmissor_lote_efdreinf.transmissor.verificar_predecessao:
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2099_evtfechaevper import validacoes_r2099_evtfechaevper
                lista = validacoes_r2099_evtfechaevper(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2099_evtfechaevper import validacoes_r2099_evtfechaevper
                lista = validacoes_r2099_evtfechaevper(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        if r2099_evtfechaevper.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2099_evtfechaevper_id, db_slug)
