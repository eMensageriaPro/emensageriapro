...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        if r2099_evtfechaevper.status in status_list:
...
def validar_evento_funcao(r2099_evtfechaevper_id, db_slug):
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('esocial', 'r2099_evtfechaevper', r2099_evtfechaevper_id)
...
def validar_evento(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        if r2099_evtfechaevper.versao in VERSOES_EFDREINF:
            ...
            if r2099_evtfechaevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2099evtFechaEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=r2099_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2099_evtfechaevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2099evtFechaEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=r2099_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
