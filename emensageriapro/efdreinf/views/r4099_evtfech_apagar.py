...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if r4099_evtfech.status == STATUS_EVENTO_CADASTRADO:
            ...
            obj = r4099evtFech.objects.get(id = r4099_evtfech_id)
            ...
            gravar_auditoria(situacao_anterior,
                             '', 
                             'r4099_evtfech', r4099_evtfech_id, usuario_id, 3)
        else:
            messages.error(request, u'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
            
        if request.session['retorno_pagina']== 'r4099_evtfech_salvar':
            return redirect('r4099_evtfech', hash=request.session['retorno_hash'])
        else:
