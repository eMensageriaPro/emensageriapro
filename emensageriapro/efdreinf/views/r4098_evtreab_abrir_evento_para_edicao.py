...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r4098evtReab'):

        if r4098_evtreab_id:
            r4098_evtreab = get_object_or_404(r4098evtReab, excluido=False, id=r4098_evtreab_id)
            ...
            if r4098_evtreab.status in status_list:
                r4098evtReab.objects.filter(id=r4098_evtreab_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r4098_evtreab.identidade, BASE_DIR, r4098_evtreab.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r4098_evtreab/%s.xml %s/arquivos/Eventos/r4098_evtreab/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r4098_evtreab/%s_backup_%s.xml' % (r4098_evtreab.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r4098_evtreab', r4098_evtreab_id, usuario_id, 1)
