...
def validar_evento_funcao(request, r2020_evtservprest_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2020_evtservprest_id)
...
def validar_evento(request, hash):
    ...
    if r2020_evtservprest_id:
        ...
        if r2020_evtservprest.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r2020_evtservprest_id)
            
            if r2020_evtservprest.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2020evtServPrest.objects.\
                    filter(id=r2020_evtservprest_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2020_evtservprest.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2020evtServPrest.objects.\
                    filter(id=r2020_evtservprest_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
