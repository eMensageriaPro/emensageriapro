...
def validar_evento_funcao(request, pk):
    ...
    schema_filename = 'efdreinf/%s/evtServPrest.xsd' % (r2020_evtservprest.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 'r2020_evtservprest' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('r2020_evtservprest', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if r2020_evtservprest.status in STATUS_VALIDACAO:
