...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r5011evtTotalContrib'):

        if r5011_evttotalcontrib_id:
            r5011_evttotalcontrib = get_object_or_404(r5011evtTotalContrib, excluido=False, id=r5011_evttotalcontrib_id)
            ...
            if r5011_evttotalcontrib.status in status_list:
                r5011evtTotalContrib.objects.filter(id=r5011_evttotalcontrib_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
