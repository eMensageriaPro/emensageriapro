...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r9000evtExclusao'):

        if r9000_evtexclusao_id:
            r9000_evtexclusao = get_object_or_404(r9000evtExclusao, excluido=False, id=r9000_evtexclusao_id)
            ...
            if r9000_evtexclusao.status in status_list:
                r9000evtExclusao.objects.filter(id=r9000_evtexclusao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
