...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='r4040_evtbenefnid')
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if r4040_evtbenefnid_form.is_valid():
                ...
                dados = r4040_evtbenefnid_form.cleaned_data
                obj = r4040_evtbenefnid_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not r4040_evtbenefnid_id:
                    from emensageriapro.functions import identidade_evento
                    identidade_evento(obj)
                  
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'r4040_evtbenefnid', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(r4040_evtbenefnid), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'r4040_evtbenefnid', r4040_evtbenefnid_id, usuario_id, 2)
                                 
                if request.session['retorno_pagina'] not in ('r4040_evtbenefnid_apagar', 'r4040_evtbenefnid_salvar', 'r4040_evtbenefnid'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                if r4040_evtbenefnid_id != obj.id:
                    url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % (obj.id) )
                    return redirect('r4040_evtbenefnid_salvar', hash=url_hash)

            else:
                messages.error(request, u'Erro ao salvar!')
        r4040_evtbenefnid_form = disabled_form_fields(r4040_evtbenefnid_form, permissao.permite_editar)
        ...
        if dict_hash['tab'] or 'r4040_evtbenefnid' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 'r4040_evtbenefnid_salvar'
        controle_alteracoes = Auditoria.objects.filter(identidade=r4040_evtbenefnid_id, tabela='r4040_evtbenefnid').all()
        context = {
