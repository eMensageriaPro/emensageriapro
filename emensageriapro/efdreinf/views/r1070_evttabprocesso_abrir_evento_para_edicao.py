...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r1070evtTabProcesso'):

        if r1070_evttabprocesso_id:
            r1070_evttabprocesso = get_object_or_404(r1070evtTabProcesso, excluido=False, id=r1070_evttabprocesso_id)
            ...
            if r1070_evttabprocesso.status in status_list:
                r1070evtTabProcesso.objects.filter(id=r1070_evttabprocesso_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
