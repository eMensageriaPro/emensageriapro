...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            return render(request, 'r9000_evtexclusao_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_r9000(r9000_evtexclusao_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r9000_evtexclusao.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r9000_evtexclusao_id, db_slug):
    ...
    if r9000_evtexclusao.status in (0,1,2,11):
        ...
        r9000evtExclusao.objects.using(db_slug).\
            filter(id=r9000_evtexclusao_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        texto = gerar_xml_r9000(r9000_evtexclusao_id, db_slug, versao="|")
        dados = read_r9000_evtexclusao_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        r9000evtExclusao.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r9000_evtexclusao.identidade),
            'r9000_evtexclusao', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r9000_evtexclusao.identidade),
            'r9000_evtexclusao', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        if r9000_evtexclusao.status == 0:
            ...
            nova_identidade = identidade_evento(r9000_evtexclusao_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        if r9000_evtexclusao.status in (0, 1, 2, 3, 4, 10, 11) or r9000_evtexclusao.processamento_codigo_resposta in (401,402):
