...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r9000_evtexclusao_lista = r9000evtExclusao.objects.using( db_slug ).filter(id=r9000_evtexclusao_id, excluido = False).all()
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r9000_evtexclusao_verificar.html' % r9000_evtexclusao.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r9000_evtexclusao_verificar.html' % r9000_evtexclusao.versao,
            ...
            response =  render_to_response('%s/r9000_evtexclusao_verificar.html' % r9000_evtexclusao.versao, context)
            ...
            return render(request, '%s/r9000_evtexclusao_verificar.html' % r9000_evtexclusao.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r9000(r9000_evtexclusao_id, db_slug):
    from django.template.loader import get_template
    if r9000_evtexclusao_id:
        ...
        #return render(request, 'xml/%s/r9000_evtexclusao.html' % r9000_evtexclusao.versao, context, content_type='text/xml')
        t = get_template('%s/r9000_evtexclusao_xml.html' % r9000_evtexclusao.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r9000_evtexclusao = get_object_or_404(
            r9000evtExclusao.objects.using( db_slug ),
            excluido = False, id = r9000_evtexclusao_id)
    

        if r9000_evtexclusao.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r9000_evtexclusao.retornos_evttotal_id, excluido=False)
        ...
        if r9000_evtexclusao.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r9000_evtexclusao.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r9000_evtexclusao_recibo_pdf.html', context)
            filename = "%s.xls" % r9000_evtexclusao.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r9000_evtexclusao_recibo_csv.html', context)
            filename = "%s.csv" % r9000_evtexclusao.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r9000_evtexclusao_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r9000_evtexclusao_id, db_slug):
    import os
    ...
    if r9000_evtexclusao.arquivo_original:
        ...
        xml = gerar_xml_r9000(r9000_evtexclusao_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r9000_evtexclusao.status in (0,1,2,11):
        r9000evtExclusao.objects.using(db_slug).filter(id=r9000_evtexclusao_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r9000_evtexclusao_id:
        r9000_evtexclusao = get_object_or_404(r9000evtExclusao.objects.using(db_slug), excluido=False, id=r9000_evtexclusao_id)
        xml_assinado = gerar_xml_assinado(r9000_evtexclusao_id, db_slug)
...
def duplicar(request, hash):
    ...
    r9000_evtexclusao_id = int(dict_hash['id'])
    if r9000_evtexclusao_id:
        r9000_evtexclusao = get_object_or_404(r9000evtExclusao.objects.using(db_slug), excluido=False,
                                                    id=r9000_evtexclusao_id)

        arquivo = 'arquivos/Eventos/r9000_evtexclusao/%s.xml' % r9000_evtexclusao.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r9000_evtexclusao_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r9000_evtexclusao/%s.xml' % r9000_evtexclusao.identidade)
        salvar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_duplicado_temp.xml' % r9000_evtexclusao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_duplicado_temp.xml' % r9000_evtexclusao.identidade, request)
        from emensageriapro.efdreinf.views.r9000_evtexclusao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r9000evtExclusao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r9000_evtexclusao.identidade), 
            'r9000_evtexclusao', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        salvar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_alteracao_temp.xml' % r9000_evtexclusao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_alteracao_temp.xml' % r9000_evtexclusao.identidade, request)
        from emensageriapro.efdreinf.views.r9000_evtexclusao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r9000evtExclusao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r9000_evtexclusao.identidade), 
            'r9000_evtexclusao', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        salvar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_exclusao_temp.xml' % r9000_evtexclusao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r9000_evtexclusao/%s_exclusao_temp.xml' % r9000_evtexclusao.identidade, request)
        from emensageriapro.efdreinf.views.r9000_evtexclusao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r9000evtExclusao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r9000_evtexclusao.identidade), 
            'r9000_evtexclusao', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        if r9000_evtexclusao.status == 0:
            from emensageriapro.efdreinf.views.r9000_evtexclusao import identidade_evento
            dent = identidade_evento(r9000_evtexclusao_id, db_slug)
            ...
            'r9000_evtexclusao', r9000_evtexclusao_id, usuario_id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r9000_evtexclusao_id:
        r9000_evtexclusao = get_object_or_404(r9000evtExclusao.objects.using(db_slug), excluido=False, id=r9000_evtexclusao_id)
        if r9000_evtexclusao.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r9000evtExclusao.objects.using(db_slug).filter(id=r9000_evtexclusao_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r9000_evtexclusao/%s.xml' % (r9000_evtexclusao.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, r9000_evtexclusao.identidade, BASE_DIR, r9000_evtexclusao.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/r9000_evtexclusao/%s.xml %s/arquivos/Eventos/r9000_evtexclusao/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/r9000_evtexclusao/%s_backup_%s.xml' % (r9000_evtexclusao.identidade, data_hora_atual), 
                    1)
...
def validar_evento_funcao(r9000_evtexclusao_id, db_slug):
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r9000_evtexclusao_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r9000_evtexclusao import validacoes_r9000_evtexclusao
                lista = validacoes_r9000_evtexclusao(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r9000_evtexclusao import validacoes_r9000_evtexclusao
                lista = validacoes_r9000_evtexclusao(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r9000_evtexclusao SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r9000_evtexclusao_id), False)
    else:
        executar_sql("UPDATE public.r9000_evtexclusao SET validacoes='', status=4 WHERE id=%s;" % (r9000_evtexclusao_id), False)
...
def validar_evento(request, hash):
    ...
    if r9000_evtexclusao_id:
        ...
        if r9000_evtexclusao.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r9000_evtexclusao_id, db_slug)
