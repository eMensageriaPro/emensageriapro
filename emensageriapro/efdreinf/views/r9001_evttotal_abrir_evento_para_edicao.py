...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r9001evtTotal'):

        if r9001_evttotal_id:
            r9001_evttotal = get_object_or_404(r9001evtTotal, excluido=False, id=r9001_evttotal_id)
            ...
            if r9001_evttotal.status in status_list:
                r9001evtTotal.objects.filter(id=r9001_evttotal_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r9001_evttotal.identidade, BASE_DIR, r9001_evttotal.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r9001_evttotal/%s.xml %s/arquivos/Eventos/r9001_evttotal/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r9001_evttotal/%s_backup_%s.xml' % (r9001_evttotal.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r9001_evttotal', r9001_evttotal_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r9001_evttotal_id )
