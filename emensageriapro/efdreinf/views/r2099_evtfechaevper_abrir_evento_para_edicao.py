...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r2099evtFechaEvPer'):

        if r2099_evtfechaevper_id:
            r2099_evtfechaevper = get_object_or_404(r2099evtFechaEvPer, excluido=False, id=r2099_evtfechaevper_id)
            ...
            if r2099_evtfechaevper.status in status_list:
                r2099evtFechaEvPer.objects.filter(id=r2099_evtfechaevper_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
