...
def criar_alteracao(request, pk):
    ...
    from emensageriapro.efdreinf.views.r5011_evttotalcontrib_gerar_xml import gerar_xml_r5011
    from emensageriapro.functions import identidade_evento
    
    if request.user.has_perm('efdreinf.can_create_change_r5011evtTotalContrib'):

        r5011_evttotalcontrib = get_object_or_404(
            r5011evtTotalContrib,
            id=pk)

        texto = gerar_xml_r5011(request, pk, versao="|")
        texto = texto.replace('<inclusao>','<alteracao>').replace('</inclusao>','</alteracao>')
        dados = read_r5011_evttotalcontrib_string(request, {}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(r5011_evttotalcontrib)

        r5011evtTotalContrib.objects.filter(id=dados['id']).\
        ...
        return redirect('r5011_evttotalcontrib_salvar', pk=return_pk, tab='master')
        ...
        return redirect('r5011_evttotalcontrib_salvar', pk=pk, tab='master')