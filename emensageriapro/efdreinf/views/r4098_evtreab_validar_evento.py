...
def validar_evento_funcao(request, r4098_evtreab_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r4098_evtreab_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r4098_evtreab_validar import validacoes_r4098_evtreab
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r4098_evtreab(arquivo)
    ...
    if lista_validacoes:
        ...
        r4098evtReab.objects.\
            filter(id=r4098_evtreab_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r4098_evtreab', r4098_evtreab_id)
...
def validar_evento(request, hash):
    ...
    if r4098_evtreab_id:
        ...
        if r4098_evtreab.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r4098_evtreab_id)
            
            if r4098_evtreab.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4098evtReab.objects.\
                    filter(id=r4098_evtreab_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r4098_evtreab.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4098evtReab.objects.\
                    filter(id=r4098_evtreab_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
