...
def validar_evento_funcao(request, r4040_evtbenefnid_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r4040_evtbenefnid_id)
        
    if os.path.exists(BASE_DIR + '/' + arquivo):
    
        from emensageriapro.efdreinf.views.r4040_evtbenefnid_validar import validacoes_r4040_evtbenefnid
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        lista = validacoes_r4040_evtbenefnid(arquivo)
    ...
    if lista_validacoes:
        ...
        r4040evtBenefNId.objects.\
            filter(id=r4040_evtbenefnid_id, excluido = False).\
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('efdreinf', 'r4040_evtbenefnid', r4040_evtbenefnid_id)
...
def validar_evento(request, hash):
    ...
    if r4040_evtbenefnid_id:
        ...
        if r4040_evtbenefnid.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r4040_evtbenefnid_id)
            
            if r4040_evtbenefnid.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4040evtBenefNId.objects.\
                    filter(id=r4040_evtbenefnid_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r4040_evtbenefnid.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r4040evtBenefNId.objects.\
                    filter(id=r4040_evtbenefnid_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
