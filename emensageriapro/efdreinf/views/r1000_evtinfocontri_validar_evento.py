...
def validar_evento_funcao(request, r1000_evtinfocontri_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r1000_evtinfocontri_id)
...
def validar_evento(request, hash):
    ...
    if r1000_evtinfocontri_id:
        ...
        if r1000_evtinfocontri.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r1000_evtinfocontri_id)
            
            if r1000_evtinfocontri.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r1000evtInfoContri.objects.\
                    filter(id=r1000_evtinfocontri_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r1000_evtinfocontri.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r1000evtInfoContri.objects.\
                    filter(id=r1000_evtinfocontri_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
