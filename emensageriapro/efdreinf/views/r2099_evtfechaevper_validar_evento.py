...
def validar_evento_funcao(request, r2099_evtfechaevper_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2099_evtfechaevper_id)
...
def validar_evento(request, hash):
    ...
    if r2099_evtfechaevper_id:
        ...
        if r2099_evtfechaevper.versao in VERSOES_EFDREINF:
        
            validar_evento_funcao(request, r2099_evtfechaevper_id)
            
            if r2099_evtfechaevper.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2099evtFechaEvPer.objects.\
                    filter(id=r2099_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2099_evtfechaevper.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2099evtFechaEvPer.objects.\
                    filter(id=r2099_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
