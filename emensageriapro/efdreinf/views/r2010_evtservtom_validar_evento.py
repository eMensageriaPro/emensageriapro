...
def validar_evento_funcao(request, r2010_evtservtom_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, r2010_evtservtom_id)
...
def validar_evento(request, hash):
    ...
    if r2010_evtservtom_id:
        ...
        if r2010_evtservtom.versao in VERSOES_EFDREINF:
            ...
            validar_evento_funcao(request, r2010_evtservtom_id)
            
            if r2010_evtservtom.transmissor_lote_efdreinf and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2010evtServTom.objects.\
                    filter(id=r2010_evtservtom_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2010_evtservtom.transmissor_lote_efdreinf and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2010evtServTom.objects.\
                    filter(id=r2010_evtservtom_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
