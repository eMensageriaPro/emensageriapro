...
def validar_evento_funcao(request, pk):
    ...
    if not r2010_evtservtom.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(r2010_evtservtom, 'efdreinf')
        r2010_evtservtom = get_object_or_404(r2010evtServTom, id=pk)
    ...
    schema_filename = 'efdreinf/%s/evtServTom.xsd' % (r2010_evtservtom.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 'r2010_evtservtom' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('r2010_evtservtom', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if r2010_evtservtom.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('r2010_evtservtom_salvar', pk=pk)