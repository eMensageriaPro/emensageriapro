...
def gerar_xml_assinado(r2098_evtreabreevper_id, db_slug):
    ...
    if r2098_evtreabreevper.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        r2098evtReabreEvPer.objects.using(db_slug).\
            filter(id=r2098_evtreabreevper_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        if r2098_evtreabreevper.status in status_list:
            r2098evtReabreEvPer.objects.using(db_slug).filter(id=r2098_evtreabreevper_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2098_evtreabreevper/%s.xml' % (r2098_evtreabreevper.identidade)
...
def validar_evento_funcao(r2098_evtreabreevper_id, db_slug):
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO:
            ...
            quant = validar_precedencia('esocial', 'r2098_evtreabreevper', r2098_evtreabreevper_id)
...
def validar_evento(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        if r2098_evtreabreevper.versao in VERSOES_EFDREINF:
            ...
            if r2098_evtreabreevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2098evtReabreEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=r2098_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif r2098_evtreabreevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                r2098evtReabreEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=r2098_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
