...
def gerar_xml_r2098(r2098_evtreabreevper_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=r2098_evtreabreevper.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(r2098_evtreabreevper_id, db_slug):
    ...
    if r2098_evtreabreevper.status in (0,1,2,11):
        ...
        r2098evtReabreEvPer.objects.using(db_slug).\
            filter(id=r2098_evtreabreevper_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        texto = gerar_xml_r2098(r2098_evtreabreevper_id, db_slug, versao="|")
        dados = read_r2098_evtreabreevper_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        r2098evtReabreEvPer.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2098_evtreabreevper.identidade),
...
def criar_alteracao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2098_evtreabreevper.identidade),
            'r2098_evtreabreevper', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, r2098_evtreabreevper.identidade),
            'r2098_evtreabreevper', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        if r2098_evtreabreevper.status == 0:
            ...
            nova_identidade = identidade_evento(r2098_evtreabreevper_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        if r2098_evtreabreevper.status in (0, 1, 2, 3, 4, 10, 11) or r2098_evtreabreevper.processamento_codigo_resposta in (401,402):
...
=======
    if r2098_evtreabreevper.transmissor_lote_esocial:
        if r2098_evtreabreevper.transmissor_lote_esocial.transmissor:
            if r2098_evtreabreevper.transmissor_lote_esocial.transmissor.verificar_predecessao:
...
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
            ...
            validar_evento_funcao(r2098_evtreabreevper_id, db_slug)
