...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2098_evtreabreevper_lista = r2098evtReabreEvPer.objects.using( db_slug ).filter(id=r2098_evtreabreevper_id, excluido = False).all()
        ...
        context = {
            ...
            'r2098_evtreabreevper_id': r2098_evtreabreevper_id, 
        ...
        if for_print == 2:
            #return render_to_pdf('%s/r2098_evtreabreevper_verificar.html' % r2098_evtreabreevper.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/r2098_evtreabreevper_verificar.html' % r2098_evtreabreevper.versao,
            ...
            response =  render_to_response('%s/r2098_evtreabreevper_verificar.html' % r2098_evtreabreevper.versao, context)
            ...
            return render(request, '%s/r2098_evtreabreevper_verificar.html' % r2098_evtreabreevper.versao, context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_r2098(r2098_evtreabreevper_id, db_slug):
    from django.template.loader import get_template
    if r2098_evtreabreevper_id:
        ...
        #return render(request, 'xml/%s/r2098_evtreabreevper.html' % r2098_evtreabreevper.versao, context, content_type='text/xml')
        t = get_template('%s/r2098_evtreabreevper_xml.html' % r2098_evtreabreevper.versao)
...
def recibo(request, hash, tipo, slug=0):
    ...
    if permissao.permite_listar:
        ...
        r2098_evtreabreevper = get_object_or_404(
            r2098evtReabreEvPer.objects.using( db_slug ),
            excluido = False, id = r2098_evtreabreevper_id)
    

        if r2098_evtreabreevper.retornos_evttotal_id:

            r5001evtTotal_dados = get_object_or_404( r5001evtTotal.objects.using(db_slug),
                id=r2098_evtreabreevper.retornos_evttotal_id, excluido=False)
        ...
        if r2098_evtreabreevper.retornos_evttotalcontrib_id:

            r5011evtTotalContrib_dados = get_object_or_404( r5011evtTotalContrib.objects.using(db_slug),
                id=r2098_evtreabreevper.retornos_evttotalcontrib_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('r2098_evtreabreevper_recibo_pdf.html', context)
            filename = "%s.xls" % r2098_evtreabreevper.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('r2098_evtreabreevper_recibo_csv.html', context)
            filename = "%s.csv" % r2098_evtreabreevper.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('r2098_evtreabreevper_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
...
def gerar_xml_assinado(r2098_evtreabreevper_id, db_slug):
    import os
    ...
    if r2098_evtreabreevper.arquivo_original:
        ...
        xml = gerar_xml_r2098(r2098_evtreabreevper_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_efdreinf(xml)
    if r2098_evtreabreevper.status in (0,1,2,11):
        r2098evtReabreEvPer.objects.using(db_slug).filter(id=r2098_evtreabreevper_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash, slug=0):
    ...
    if r2098_evtreabreevper_id:
        r2098_evtreabreevper = get_object_or_404(r2098evtReabreEvPer.objects.using(db_slug), excluido=False, id=r2098_evtreabreevper_id)
        xml_assinado = gerar_xml_assinado(r2098_evtreabreevper_id, db_slug)
...
def duplicar(request, hash):
    ...
    r2098_evtreabreevper_id = int(dict_hash['id'])
    if r2098_evtreabreevper_id:
        r2098_evtreabreevper = get_object_or_404(r2098evtReabreEvPer.objects.using(db_slug), excluido=False,
                                                    id=r2098_evtreabreevper_id)

        arquivo = 'arquivos/Eventos/r2098_evtreabreevper/%s.xml' % r2098_evtreabreevper.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(r2098_evtreabreevper_id, db_slug)
        
        texto = ler_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s.xml' % r2098_evtreabreevper.identidade)
        salvar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_duplicado_temp.xml' % r2098_evtreabreevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_duplicado_temp.xml' % r2098_evtreabreevper.identidade, request)
        from emensageriapro.efdreinf.views.r2098_evtreabreevper import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2098evtReabreEvPer.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2098_evtreabreevper.identidade), 
            'r2098_evtreabreevper', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_alteracao_temp.xml' % r2098_evtreabreevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_alteracao_temp.xml' % r2098_evtreabreevper.identidade, request)
        from emensageriapro.efdreinf.views.r2098_evtreabreevper import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2098evtReabreEvPer.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2098_evtreabreevper.identidade), 
            'r2098_evtreabreevper', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        salvar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_exclusao_temp.xml' % r2098_evtreabreevper.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/r2098_evtreabreevper/%s_exclusao_temp.xml' % r2098_evtreabreevper.identidade, request)
        from emensageriapro.efdreinf.views.r2098_evtreabreevper import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        r2098evtReabreEvPer.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, r2098_evtreabreevper.identidade), 
            'r2098_evtreabreevper', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if r2098_evtreabreevper_id:
        ...
        if r2098_evtreabreevper.status == 0:
            from emensageriapro.efdreinf.views.r2098_evtreabreevper import identidade_evento
            dent = identidade_evento(r2098_evtreabreevper_id, db_slug)
            ...
            'r2098_evtreabreevper', r2098_evtreabreevper_id, usuario_id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if r2098_evtreabreevper_id:
        r2098_evtreabreevper = get_object_or_404(r2098evtReabreEvPer.objects.using(db_slug), excluido=False, id=r2098_evtreabreevper_id)
        if r2098_evtreabreevper.status in (0, 1, 2, 3, 4, 5, 10, 11):
            r2098evtReabreEvPer.objects.using(db_slug).filter(id=r2098_evtreabreevper_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/r2098_evtreabreevper/%s.xml' % (r2098_evtreabreevper.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, r2098_evtreabreevper.identidade, BASE_DIR, r2098_evtreabreevper.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/r2098_evtreabreevper/%s.xml %s/arquivos/Eventos/r2098_evtreabreevper/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/r2098_evtreabreevper/%s_backup_%s.xml' % (r2098_evtreabreevper.identidade, data_hora_atual), 
                    1)
...
@login_required
def validar_evento_funcao(r2098_evtreabreevper_id, db_slug):
    ...
    lista_validacoes = []
    r2098_evtreabreevper = get_object_or_404(r2098evtReabreEvPer.objects.using(db_slug), excluido=False, id=r2098_evtreabreevper_id)
    if r2098_evtreabreevper.transmissor_lote_efdreinf:
        if r2098_evtreabreevper.transmissor_lote_efdreinf.transmissor:
            if r2098_evtreabreevper.transmissor_lote_efdreinf.transmissor.verificar_predecessao:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.r2098_evtreabreevper SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, r2098_evtreabreevper_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/r2098_evtreabreevper/%s.xml' % (r2098_evtreabreevper.identidade)
    ...
    tipo = 'efdreinf'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(r2098_evtreabreevper_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.r2098_evtreabreevper import validacoes_r2098_evtreabreevper
                lista = validacoes_r2098_evtreabreevper(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.r2098_evtreabreevper import validacoes_r2098_evtreabreevper
                lista = validacoes_r2098_evtreabreevper(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.r2098_evtreabreevper SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), r2098_evtreabreevper_id), False)
    else:
        executar_sql("UPDATE public.r2098_evtreabreevper SET validacoes='', status=4 WHERE id=%s;" % (r2098_evtreabreevper_id), False)
...
def validar_evento(request, hash):
    ...
    if r2098_evtreabreevper_id:
        lista_validacoes = []
        r2098_evtreabreevper = get_object_or_404(r2098evtReabreEvPer.objects.using(db_slug), excluido=False, id=r2098_evtreabreevper_id)
        if r2098_evtreabreevper.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(r2098_evtreabreevper_id, db_slug)
