...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_open_event_r9011evtTotalContrib'):

        if r9011_evttotalcontrib_id:
            r9011_evttotalcontrib = get_object_or_404(r9011evtTotalContrib, excluido=False, id=r9011_evttotalcontrib_id)
            ...
            if r9011_evttotalcontrib.status in status_list:
                r9011evtTotalContrib.objects.filter(id=r9011_evttotalcontrib_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
                ...
                if os.path.exists(BASE_DIR + '/' + arquivo):
                    ...
                    dad = (BASE_DIR, r9011_evttotalcontrib.identidade, BASE_DIR, r9011_evttotalcontrib.identidade, data_hora_atual)
                    os.system('mv %s/arquivos/Eventos/r9011_evttotalcontrib/%s.xml %s/arquivos/Eventos/r9011_evttotalcontrib/%s_backup_%s.xml' % dad)
                    gravar_nome_arquivo('/arquivos/Eventos/r9011_evttotalcontrib/%s_backup_%s.xml' % (r9011_evttotalcontrib.identidade, data_hora_atual),
                        1)
    
                messages.success(request, 'Evento aberto para edição!')
                usuario_id = request.user.id
                gravar_auditoria(u'{}', u'{"funcao": "Evento aberto para edição"}',
                    'r9011_evttotalcontrib', r9011_evttotalcontrib_id, usuario_id, 1)
    
                url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % r9011_evttotalcontrib_id )
