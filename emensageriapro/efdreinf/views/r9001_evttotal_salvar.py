...
def salvar(request, hash):
    ...
    if request.user.has_perm('efdreinf.can_view_r9001evtTotal'):
        ...
        if request.method == 'POST':
            ...
            if r9001_evttotal_form.is_valid():
                ...
                dados = r9001_evttotal_form.cleaned_data
                obj = r9001_evttotal_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not r9001_evttotal_id:
                    ...
                    gravar_auditoria('{}',
                                 ...
                                 'r9001_evttotal', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(r9001_evttotal), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'r9001_evttotal', r9001_evttotal_id, usuario_id, 2)
                                 
                if request.session['retorno_pagina'] not in ('r9001_evttotal_apagar', 'r9001_evttotal_salvar', 'r9001_evttotal'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                    
                if r9001_evttotal_id != obj.id:
        ...
        r9001_evttotal_form = disabled_form_fields(r9001_evttotal_form, request.user.has_perm('efdreinf.change_r9001evtTotal'))
        ...
        if dict_hash['tab'] or 'r9001_evttotal' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 'r9001_evttotal_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=r9001_evttotal_id, tabela='r9001_evttotal').all()
