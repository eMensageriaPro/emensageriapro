...
def read_r5011_evttotalcontrib(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_r5011_evttotalcontrib_obj(request, doc, status, validar, arquivo)

    r5011evtTotalContrib.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_r5011_evttotalcontrib_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        r5011_evttotalcontrib_dados['arquivo'] = arquivo
