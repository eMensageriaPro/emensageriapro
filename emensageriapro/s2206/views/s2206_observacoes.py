...
def apagar(request, hash):
    ...
    if s2206_observacoes_id:
        dados_evento = s2206_observacoes.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2206_observacoes_apagar'] = 0
            dict_permissoes['s2206_observacoes_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2206_observacoes), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2206_observacoes', s2206_observacoes_id, usuario_id, 3)
        else:
...
class s2206observacoesList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2206observacoes.objects.using(db_slug).all()
    serializer_class = s2206observacoesSerializer
    # permission_classes = (IsAdminUser,)


class s2206observacoesDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2206observacoes.objects.using(db_slug).all()
    serializer_class = s2206observacoesSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = render_to_response('s2206_observacoes_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2206_observacoes_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2206_observacoes_id:
            s2206_observacoes_form = form_s2206_observacoes(request.POST or None, instance = s2206_observacoes,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2206_observacoes_form = form_s2206_observacoes(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2206_observacoes_form.is_valid():

                dados = s2206_observacoes_form.cleaned_data
                obj = s2206_observacoes_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2206_observacoes_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2206_observacoes', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2206_observacoes), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2206_observacoes', s2206_observacoes_id, usuario_id, 2)
        ...
        if s2206_observacoes_id:
            if dados_evento['status'] != 0:
                s2206_observacoes_form = disabled_form_fields(s2206_observacoes_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2206_observacoes_id, tabela='s2206_observacoes').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2206_observacoes_salvar.html',
                filename="s2206_observacoes.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2206_observacoes_salvar.html', context)
            filename = "s2206_observacoes.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
