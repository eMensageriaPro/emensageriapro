...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2206_aprend = get_object_or_404(s2206aprend, id=pk)
        evento_dados = s2206_aprend.evento()
    ...
    if request.user.has_perm('s2206.can_see_s2206aprend'):
        ...
        if request.method == 'POST':
            ...
            if s2206_aprend_form.is_valid():
                ...
                if 's2206-aprend' not in request.session['return']:
        ...
        s2206_aprend_form = disabled_form_fields(
            s2206_aprend_form,
            request.user.has_perm('s2206.change_s2206aprend'))
        ...
        #if tab or 's2206_aprend' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2206_aprend_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2206_aprend').all()
