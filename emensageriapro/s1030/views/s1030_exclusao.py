...
def apagar(request, hash):
    ...
    if s1030_exclusao_id:
        dados_evento = s1030_exclusao.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1030_exclusao_apagar'] = 0
            dict_permissoes['s1030_exclusao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1030_exclusao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1030_exclusao', s1030_exclusao_id, usuario_id, 3)
        else:
...
class s1030exclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1030exclusao.objects.using(db_slug).all()
    serializer_class = s1030exclusaoSerializer
    # permission_classes = (IsAdminUser,)


class s1030exclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1030exclusao.objects.using(db_slug).all()
    serializer_class = s1030exclusaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1030_exclusao_listar.html',
                filename="s1030_exclusao.pdf",
                context=context,
            ...
            response = render_to_response('s1030_exclusao_listar.html', context)
            filename = "s1030_exclusao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1030_exclusao_csv.html', context)
            filename = "s1030_exclusao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1030_exclusao_id:
            s1030_exclusao_form = form_s1030_exclusao(request.POST or None, instance = s1030_exclusao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1030_exclusao_form = form_s1030_exclusao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1030_exclusao_form.is_valid():

                dados = s1030_exclusao_form.cleaned_data
                obj = s1030_exclusao_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s1030_exclusao_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1030_exclusao', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1030_exclusao), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1030_exclusao', s1030_exclusao_id, usuario_id, 2)
        ...
        if s1030_exclusao_id:
            if dados_evento['status'] != 0:
                s1030_exclusao_form = disabled_form_fields(s1030_exclusao_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1030_exclusao_id, tabela='s1030_exclusao').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1030_exclusao_salvar.html',
                filename="s1030_exclusao.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1030_exclusao_salvar.html', context)
            filename = "s1030_exclusao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
