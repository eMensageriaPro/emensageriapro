...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1030_alteracao_novavalidade = get_object_or_404(s1030alteracaonovaValidade, id=pk)
        evento_dados = s1030_alteracao_novavalidade.evento()

    if request.user.has_perm('s1030.can_see_s1030alteracaonovaValidade'):
        ...
        if request.method == 'POST':
            ...
            if s1030_alteracao_novavalidade_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s1030_alteracao_novavalidade), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1030_alteracao_novavalidade_apagar', 
                    's1030_alteracao_novavalidade_salvar', 
                    's1030_alteracao_novavalidade'):
        ...
        s1030_alteracao_novavalidade_form = disabled_form_fields(
            s1030_alteracao_novavalidade_form, 
            request.user.has_perm('s1030.change_s1030alteracaonovaValidade'))
        ...
        if tab or 's1030_alteracao_novavalidade' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1030_alteracao_novavalidade_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1030_alteracao_novavalidade').all()
