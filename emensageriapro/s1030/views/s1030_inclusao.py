...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1030_inclusao')
    permissao = ConfigPermissoes.objects.using( db_slug ).get(excluido = False, config_paginas=pagina, config_perfis=usuario.config_perfis)
    if s1030_inclusao_id:
        s1030_inclusao = get_object_or_404(s1030inclusao.objects.using( db_slug ), excluido = False, id = s1030_inclusao_id)
    ...
    if s1030_inclusao_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['s1030_inclusao_apagar'] = 0
            dict_permissoes['s1030_inclusao_editar'] = 0
    ...
    if permissao.permite_visualizar:
            ...
            if s1030_inclusao_form.is_valid():
                ...
                if s1030_inclusao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1030_inclusao_campos_multiple_passo1
                        s1030inclusao.objects.using(db_slug).filter(id=s1030_inclusao_id).update(**dados)
                        obj = s1030inclusao.objects.using(db_slug).get(id=s1030_inclusao_id)
                        #s1030_inclusao_editar_custom
                        #s1030_inclusao_campos_multiple_passo2
                    ...
                    #s1030_inclusao_cadastrar_campos_multiple_passo1
                    obj = s1030inclusao(**dados)
                    obj.save(using = db_slug)
                    #s1030_inclusao_cadastrar_custom
                    #s1030_inclusao_cadastrar_campos_multiple_passo2
        ...
        s1030_inclusao_form = disabled_form_fields(s1030_inclusao_form, permissao.permite_editar)
        if s1030_inclusao_id:
            if dados_evento['status'] != 0:
                s1030_inclusao_form = disabled_form_fields(s1030_inclusao_form, 0)
        #s1030_inclusao_campos_multiple_passo3

        for field in s1030_inclusao_form.fields.keys():
            s1030_inclusao_form.fields[field].widget.attrs['ng-model'] = 's1030_inclusao_'+field
        ...
        if s1030_inclusao_id:
            ...
            s1030_inclusao_cargopublico_form = form_s1030_inclusao_cargopublico(initial={ 's1030_inclusao': s1030_inclusao }, slug=db_slug)
            s1030_inclusao_cargopublico_form.fields['s1030_inclusao'].widget.attrs['readonly'] = True
            s1030_inclusao_cargopublico_lista = s1030inclusaocargoPublico.objects.using( db_slug ).filter(excluido = False, s1030_inclusao_id=s1030_inclusao.id).all()
        else:
            s1030_inclusao = None
        #s1030_inclusao_salvar_custom_variaveis#
        tabelas_secundarias = []
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1030_inclusao_id, tabela='s1030_inclusao').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1030_inclusao), indent=4, sort_keys=True, default=str)
            s1030inclusao.objects.using( db_slug ).filter(id = s1030_inclusao_id).delete()
            #s1030_inclusao_apagar_custom
            #s1030_inclusao_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1030_inclusao', s1030_inclusao_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's1030_inclusao_salvar':
            return redirect('s1030_inclusao', hash=request.session['retorno_hash'])
        else:
            return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
    context = {
        'usuario': usuario,
    ...
    return render(request, 's1030_inclusao_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1030inclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1030inclusao.objects.using(db_slug).all()
    serializer_class = s1030inclusaoSerializer
    permission_classes = (IsAdminUser,)


class s1030inclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1030inclusao.objects.using(db_slug).all()
    serializer_class = s1030inclusaoSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1030_inclusao_lista = s1030inclusao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1030_inclusao_listar.html', context)
            filename = "s1030_inclusao.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1030_inclusao_csv.html', context)
