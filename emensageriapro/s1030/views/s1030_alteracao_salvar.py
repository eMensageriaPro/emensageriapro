...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1030_alteracao = get_object_or_404(s1030alteracao, id=pk)
        evento_dados = s1030_alteracao.evento()
    ...
    if request.user.has_perm('s1030.can_see_s1030alteracao'):
        ...
        if request.method == 'POST':
            ...
            if s1030_alteracao_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's1030_alteracao_apagar', 
                    's1030_alteracao_salvar', 
                    's1030_alteracao'):
        ...
        s1030_alteracao_form = disabled_form_fields(
            s1030_alteracao_form, 
            request.user.has_perm('s1030.change_s1030alteracao'))
        ...
        if tab or 's1030_alteracao' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1030_alteracao_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1030_alteracao').all()
