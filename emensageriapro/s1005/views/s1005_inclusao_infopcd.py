...
def apagar(request, hash):
    ...
    if s1005_inclusao_infopcd_id:
        dados_evento = s1005_inclusao_infopcd.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1005_inclusao_infopcd_apagar'] = 0
            dict_permissoes['s1005_inclusao_infopcd_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1005_inclusao_infopcd), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1005_inclusao_infopcd', s1005_inclusao_infopcd_id, usuario_id, 3)
        else:
...
class s1005inclusaoinfoPCDList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1005inclusaoinfoPCD.objects.using(db_slug).all()
    serializer_class = s1005inclusaoinfoPCDSerializer
    # permission_classes = (IsAdminUser,)


class s1005inclusaoinfoPCDDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1005inclusaoinfoPCD.objects.using(db_slug).all()
    serializer_class = s1005inclusaoinfoPCDSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1005_inclusao_infopcd_listar.html',
                filename="s1005_inclusao_infopcd.pdf",
                context=context,
            ...
            response = render_to_response('s1005_inclusao_infopcd_listar.html', context)
            filename = "s1005_inclusao_infopcd.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1005_inclusao_infopcd_csv.html', context)
            filename = "s1005_inclusao_infopcd.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1005_inclusao_infopcd_id:
            s1005_inclusao_infopcd_form = form_s1005_inclusao_infopcd(request.POST or None, instance = s1005_inclusao_infopcd,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1005_inclusao_infopcd_form = form_s1005_inclusao_infopcd(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1005_inclusao_infopcd_form.is_valid():

                dados = s1005_inclusao_infopcd_form.cleaned_data
                obj = s1005_inclusao_infopcd_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s1005_inclusao_infopcd_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1005_inclusao_infopcd', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1005_inclusao_infopcd), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1005_inclusao_infopcd', s1005_inclusao_infopcd_id, usuario_id, 2)
        ...
        if s1005_inclusao_infopcd_id:
            if dados_evento['status'] != 0:
                s1005_inclusao_infopcd_form = disabled_form_fields(s1005_inclusao_infopcd_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1005_inclusao_infopcd_id, tabela='s1005_inclusao_infopcd').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1005_inclusao_infopcd_salvar.html',
                filename="s1005_inclusao_infopcd.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1005_inclusao_infopcd_salvar.html', context)
            filename = "s1005_inclusao_infopcd.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
