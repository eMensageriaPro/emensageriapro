...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1005_alteracao_infoobra_id:
            s1005_alteracao_infoobra_form = form_s1005_alteracao_infoobra(request.POST or None, instance = s1005_alteracao_infoobra, slug = db_slug)
        else:
            s1005_alteracao_infoobra_form = form_s1005_alteracao_infoobra(request.POST or None, slug = db_slug, initial={})
            ...
            if s1005_alteracao_infoobra_form.is_valid():
                ...
                if s1005_alteracao_infoobra_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1005_alteracao_infoobra_campos_multiple_passo1
                        s1005alteracaoinfoObra.objects.using(db_slug).filter(id=s1005_alteracao_infoobra_id).update(**dados)
                        obj = s1005alteracaoinfoObra.objects.using(db_slug).get(id=s1005_alteracao_infoobra_id)
                        #s1005_alteracao_infoobra_editar_custom
                        #s1005_alteracao_infoobra_campos_multiple_passo2
                    ...
                    #s1005_alteracao_infoobra_cadastrar_campos_multiple_passo1
                    obj = s1005alteracaoinfoObra(**dados)
                    obj.save(using = db_slug)
                    #s1005_alteracao_infoobra_cadastrar_custom
                    #s1005_alteracao_infoobra_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1005alteracaoinfoObra.objects.using( db_slug ).filter(id = s1005_alteracao_infoobra_id).delete()
