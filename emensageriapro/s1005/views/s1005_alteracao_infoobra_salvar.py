...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s1005_alteracao_infoobra.evento()

    if request.user.has_perm('s1005.can_see_s1005alteracaoinfoObra'):
        
        if pk:
        
            s1005_alteracao_infoobra_form = form_s1005_alteracao_infoobra(
                request.POST or None, 
                instance=s1005_alteracao_infoobra)
                                         
        else:
        
            s1005_alteracao_infoobra_form = form_s1005_alteracao_infoobra(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s1005_alteracao_infoobra_form.is_valid():
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s1005_alteracao_infoobra), 
                #            indent=4, 
                #            sort_keys=True, 
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1005_alteracao_infoobra_apagar', 
                    's1005_alteracao_infoobra_salvar', 
                    's1005_alteracao_infoobra'):
        ...
        s1005_alteracao_infoobra_form = disabled_form_fields(
            s1005_alteracao_infoobra_form, 
            request.user.has_perm('s1005.change_s1005alteracaoinfoObra'))
        ...
        if tab or 's1005_alteracao_infoobra' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1005_alteracao_infoobra_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1005_alteracao_infoobra').all()
