...
def apagar(request, hash):
    ...
    if s1005_inclusao_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1005_inclusao_apagar'] = 0
            dict_permissoes['s1005_inclusao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1005_inclusao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1005_inclusao', s1005_inclusao_id, usuario_id, 3)
        else:
...
class s1005inclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1005inclusao.objects.using(db_slug).all()
    serializer_class = s1005inclusaoSerializer
    # permission_classes = (IsAdminUser,)


class s1005inclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1005inclusao.objects.using(db_slug).all()
    serializer_class = s1005inclusaoSerializer
    # permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = render_to_response('s1005_inclusao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1005_inclusao_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1005_inclusao_id:
            s1005_inclusao_form = form_s1005_inclusao(request.POST or None, instance = s1005_inclusao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1005_inclusao_form = form_s1005_inclusao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1005_inclusao_id, tabela='s1005_inclusao').all()
        context = {
