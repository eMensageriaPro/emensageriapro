...
class s1005exclusaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1005exclusao.objects.using(db_slug).all()
    serializer_class = s1005exclusaoSerializer
    # permission_classes = (IsAdminUser,)


class s1005exclusaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1005exclusao.objects.using(db_slug).all()
    serializer_class = s1005exclusaoSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1005_exclusao_id:
            ...
            s1005_exclusao_form = form_s1005_exclusao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
