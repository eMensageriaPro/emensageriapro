...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1202_infoperant = get_object_or_404(s1202infoPerAnt, id=pk)
        evento_dados = s1202_infoperant.evento()

    if request.user.has_perm('s1202.can_see_s1202infoPerAnt'):
        
        if pk:
        
            s1202_infoperant_form = form_s1202_infoperant(
                request.POST or None, 
                instance=s1202_infoperant)
                                         
        else:
        
            s1202_infoperant_form = form_s1202_infoperant(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s1202_infoperant_form.is_valid():
            
                obj = s1202_infoperant_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1202_infoperant), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1202_infoperant_apagar', 
                    's1202_infoperant_salvar', 
                    's1202_infoperant'):
        ...
        s1202_infoperant_form = disabled_form_fields(
            s1202_infoperant_form, 
            request.user.has_perm('s1202.change_s1202infoPerAnt'))
        ...
        if pk:
            ...
            s1202_infoperant_ideadc_form = form_s1202_infoperant_ideadc(
                initial={ 's1202_infoperant': s1202_infoperant })
            ...
            s1202_infoperant_ideadc_lista = s1202infoPerAntideADC.objects.\
                filter(s1202_infoperant_id=s1202_infoperant.id).all()
        ...
        if tab or 's1202_infoperant' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1202_infoperant_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1202_infoperant').all()
