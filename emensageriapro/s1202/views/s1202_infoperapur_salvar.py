...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('s1202.can_see_s1202infoPerApur'):
        ...
        if request.method == 'POST':
            ...
            if s1202_infoperapur_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1202_infoperapur), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
        ...
        if tab or 's1202_infoperapur' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1202_infoperapur_salvar'
