...
def apagar(request, hash):
    ...
    if s1202_infoperapur_id:
        dados_evento = s1202_infoperapur.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s1202_infoperapur_apagar'] = 0
            dict_permissoes['s1202_infoperapur_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1202_infoperapur), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1202_infoperapur', s1202_infoperapur_id, usuario_id, 3)
        else:
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1202_infoperapur_id:
            ...
            s1202_infoperapur_form = form_s1202_infoperapur(request.POST or None, slug = db_slug, initial={'s1202_dmdev': '1'})
        if request.method == 'POST':
            if s1202_infoperapur_form.is_valid():
                ...
                if s1202_infoperapur_id:
                    if dados_evento['status'] == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s1202_infoperapur), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1202_infoperapur', s1202_infoperapur_id, usuario_id, 2)
                    else:
                    ...
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1202_infoperapur', obj.id, usuario_id, 1)
        ...
        if s1202_infoperapur_id:
            ...
            s1202_infoperapur_ideestab_form = form_s1202_infoperapur_ideestab(initial={ 's1202_infoperapur': s1202_infoperapur , 's1202_infoperapur': 1}, slug=db_slug)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1202_infoperapur_id, tabela='s1202_infoperapur').all()
        context = {
...
class s1202infoPerApurList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1202infoPerApur.objects.using(db_slug).all()
    serializer_class = s1202infoPerApurSerializer
    permission_classes = (IsAdminUser,)


class s1202infoPerApurDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1202infoPerApur.objects.using(db_slug).all()
    serializer_class = s1202infoPerApurSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1202_infoperapur_listar.html',
                filename="s1202_infoperapur.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1202_infoperapur_listar.html', context)
            filename = "s1202_infoperapur.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1202_infoperapur_csv.html', context)
            filename = "s1202_infoperapur.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
