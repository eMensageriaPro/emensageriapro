...
# Raises django's ImproperlyConfigured if SECRET_KEY isnt in os.environ
...
DATABASES = {
    'default': {
        ...
        'NAME': os.environ.get('SQL_DATABASE', 'emensageriapro_db'),
        'USER': os.environ.get('SQL_USER', 'emensageriapro'),
        'PASSWORD': os.environ.get('SQL_PASSWORD', 'emensageria'),
...
LINK_WEBSITE = os.environ.get('LINK_WEBSITE')
...
CONSTANCE_CONFIG = {'SYSTEM_MANUAL_SHOW_IN_MENU': (
    False, u'Visualiza manual do sistema no menu.',
    bool),


    'SYSTEM_MANUAL_LINK': ('http://', u'Link do manual do sistema.', str),


    'LOGO_IMAGE_IN_LOGIN': (False, u'''Visualiza imagem do logotipo
        na tela de Login.''', bool),


    'LOGO_IMAGE': ('', 'Logotipo da empresa', 'image_field'),


    'SYSTEM_TOKEN_SCHEDULE': ('9944b09199c62bcf9418ad846dd0123e4bbdfc6ee4b', u'''
        Token de autenticação do sistema para acesso aos webservices''', str),


    'ESOCIAL_VALIDATE_RUN_EVERY_MINS': (10, u'''Tempo entre validações (em
        minutos) dos eventos do eSocial.''', int),


    'ESOCIAL_SEND_RUN_EVERY_MINS': (10, u'''Tempo entre envios (em minutos)
        dos eventos do eSocial.''', int),


    'ESOCIAL_CONSULT_RUN_EVERY_MINS': (10, u'''Tempo entre consultas
        (em minutos) dos eventos do eSocial.''', int),


    'ESOCIAL_LOTE_MIN': (1, u'Quantidade do mínima do lote do eSocial.', int),


    'ESOCIAL_LOTE_MAX': (60, u'Quantidade do máxima do lote do eSocial.', int),


    'ESOCIAL_TIMEOUT': (3600, u'Timeout do eSocial.', int),


    'ESOCIAL_AUTOMATIC_FUNCTIONS_ENABLED': (False, u'''Envio automático do
        eSocial.''', bool),


    'ESOCIAL_CA_CERT_PEM_FILE': (
        'certificado/webservicesproducaorestritaesocialgovbr.crt',
        u'Caminho completo do Certificado do SERPRO para o eSocial', str),


    'ESOCIAL_TP_AMB': (
        '2', u'Tipo de ambiente padrão do sistema do eSocial.',
        'choices_tp_amb'),


    'ESOCIAL_FORCE_PRODUCAO_RESTRITA': (True, u'''Força o sistema para envio
        pelo ambiente produção restrita do eSocial.''', bool),


    'ESOCIAL_VERIFICAR_PREDECESSAO_ANTES_ENVIO': (False, u'''Ativa a função de
        verificar predecessão antes dos envios dos eventos do eSocial.''',
                                                  bool),


    'IMPORT_FILES_RUN_EVERY_MINS': (10, u'''Tempo de leitura de arquivos
        importados (em minutos).''', int),

    'IMPORT_LEN_EVENTS': (10, u'''Quantidade do lote de arquivos de eventos
        para importação.''', int),

    'IMPORT_AUTOMATIC_FUNCTIONS_ENABLED': (False, u'''Funções de importação
        automáticas ativadas.''', bool),

    'EFDREINF_VALIDADE_RUN_EVERY_MINS': (10, u'''Tempo entre validações
        (em minutos) dos eventos do EFD-Reinf.''', int),

    'EFDREINF_SEND_RUN_EVERY_MINS': (10, u'''Tempo entre envios (em minutos)
        dos eventos do EFD-Reinf.''', int),

    'EFDREINF_CONSULT_RUN_EVERY_MINS': (10, u'''Tempo entre consultas
        (em minutos) dos eventos do EFD-Reinf.''', int),

    'EFDREINF_CA_CERT_PEM_FILE': (
        'certificados/acserproacfv5.crt',
        u'''Caminho completo do Certificado do
        SERPRO para o EFD-Reinf''', str),

    'EFDREINF_LOTE_MIN': (1, u'Quantidade do mínima do lote do EFD-Reinf.',
                          int),

    'EFDREINF_LOTE_MAX': (60, u'Quantidade do máxima do lote do EFD-Reinf.',
                          int),

    'EFDREINF_TIMEOUT': (3600, u'Timeout do EFD-Reinf.', int),

    'EFDREINF_AUTOMATIC_FUNCTIONS_ENABLED': (False, u'''Envio automático
        do EFD-Reinf.''', bool),

    'EFDREINF_TP_AMB': ('2', u'''Tipo de ambiente padrão
        do sistema do EFD-Reinf.''', 'choices_tp_amb'),

    'EFDREINF_FORCE_PRODUCAO_RESTRITA': (True, u'''Força o sistema para envio
        pelo ambiente produção restrita do EFD-Reinf.''', bool),

    'EFDREINF_VERIFICAR_PREDECESSAO_ANTES_ENVIO': (False, u'''Ativa a função
        de verificar predecessão antes dos envios dos eventos do EFD-Reinf.''',
                                                   bool),

    'EMAIL_RECUPERACAO_SENHA': ('emensageria@emensageria.com.br',
                                u'E-mail de recuperação de senha.', str),

    'EMAIL_RECUPERACAO_SENHA_ASSUNTO': (
        u'Criação/Recuperação de senha | eMensageria',
        u'Assunto padrão do e-mail de recuperação de senha.', unicode),

    'EMAIL_RECUPERACAO_SENHA_MENSAGEM': (u'''<p>Prezado %(nome)s,<br>Acesse
        o sistema pelo link <a href="%(endereco)s">eMensageriaPro</a>
        <br>Utilizando o usuário: <strong>%(usuario)s</strong><br>
        Senha: <strong>%(senha)s</strong><br>E-mail gerado automaticamente
        pelo sistema eMensageria</p>''',
                                         u'''Mensagem padrão do e-mail de
                                         recuperação de senha.''', unicode),
...
if not DEBUG:
    ...
    DJANGO_LOG_DIR = os.environ.get('DJANGO_LOG_DIR',
                                    default='/var/log/django')
    LOG_FILENAME = os.environ.get('LOG_FILENAME', default='emensageria.log')
