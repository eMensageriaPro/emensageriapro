...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('s5012.can_see_s5012infoCRContrib'):
        ...
        if request.method == 'POST':
            ...
            if s5012_infocrcontrib_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5012_infocrcontrib), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5012_infocrcontrib_apagar', 
                    's5012_infocrcontrib_salvar', 
                    's5012_infocrcontrib'):
        ...
        s5012_infocrcontrib_form = disabled_form_fields(
            s5012_infocrcontrib_form, 
            request.user.has_perm('s5012.change_s5012infoCRContrib'))
        ...
        if tab or 's5012_infocrcontrib' in request.session['return_page']:
            ...
            request.session['return_page'] = 's5012_infocrcontrib_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5012_infocrcontrib').all()
