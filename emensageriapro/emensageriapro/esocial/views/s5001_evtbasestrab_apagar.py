...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if s5001_evtbasestrab.status == STATUS_EVENTO_CADASTRADO:
            ...
            obj = s5001evtBasesTrab.objects.get(id=pk)
            ...
            gravar_auditoria(situacao_anterior,
                             '', 
                             's5001_evtbasestrab', pk, request.user.id, 3)
        else:
        
            messages.error(request, u'''Não foi possivel apagar o evento, somente é 
                                        possível apagar os eventos com status "Cadastrado"!''')
            
        if 's5001_evtbasestrab' in request.session['return_page']:
        
            return redirect('s5001_evtbasestrab')
