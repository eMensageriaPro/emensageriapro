...
def criar_exclusao(request, pk):
    ...
    from emensageriapro.esocial.views.s5003_evtbasesfgts_gerar_xml import gerar_xml_s5003
    from emensageriapro.functions import identidade_evento
    
    if request.user.has_perm('esocial.can_create_delete_s5003evtBasesFGTS'):
    
        s5003_evtbasesfgts = get_object_or_404(
            s5003evtBasesFGTS,
            id=pk)

        texto = gerar_xml_s5003(request, pk, versao="|")
        texto = texto.replace('<inclusao>','<exclusao>').replace('</inclusao>','</exclusao>')
        texto = texto.replace('<alteracao>','<exclusao>').replace('</alteracao>','</exclusao>')
        dados = read_s5003_evtbasesfgts_string(request, {}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s5003_evtbasesfgts)

        s5003evtBasesFGTS.objects.filter(id=dados['id']).\
        ...
        return redirect('s5003_evtbasesfgts_salvar', pk=return_pk)
        ...
        return redirect('s5003_evtbasesfgts_salvar', pk=pk)