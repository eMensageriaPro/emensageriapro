    ...
    if request.method == 'POST':
        ...
        if s2299_evtdeslig.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2299_evtdeslig', pk, request.user.id, 3)
        else:

            messages.error(request, u'''Não foi possivel apagar o evento, somente é
                                        possível apagar os eventos com status "Cadastrado"!''')

        if 'return_page' in request.session and request.session['return_page'] and 's2299-evtdeslig' in request.session['return_page']:
            return redirect('s2299_evtdeslig')
