...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2240_fimexprisco_respreg = get_object_or_404(s2240fimExpRiscorespReg, id=pk)
        evento_dados = s2240_fimexprisco_respreg.evento()
    ...
    if request.user.has_perm('s2240.can_see_s2240fimExpRiscorespReg'):
        ...
        if pk:
            ...
            s2240_fimexprisco_respreg_form = form_s2240_fimexprisco_respreg(
                ...
                instance=s2240_fimexprisco_respreg)
                                         
        else:
        
            s2240_fimexprisco_respreg_form = form_s2240_fimexprisco_respreg(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s2240_fimexprisco_respreg_form.is_valid():
            
                obj = s2240_fimexprisco_respreg_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2240_fimexprisco_respreg), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2240_fimexprisco_respreg_apagar', 
                    's2240_fimexprisco_respreg_salvar', 
                    's2240_fimexprisco_respreg'):
        ...
        s2240_fimexprisco_respreg_form = disabled_form_fields(
            s2240_fimexprisco_respreg_form, 
            request.user.has_perm('s2240.change_s2240fimExpRiscorespReg'))
        ...
        if tab or 's2240_fimexprisco_respreg' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2240_fimexprisco_respreg_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2240_fimexprisco_respreg').all()
