...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2240_altexprisco_infoamb = get_object_or_404(s2240altExpRiscoinfoAmb, id=pk)
        evento_dados = s2240_altexprisco_infoamb.evento()

    if request.user.has_perm('s2240.can_see_s2240altExpRiscoinfoAmb'):
        ...
        if request.method == 'POST':
            ...
            if s2240_altexprisco_infoamb_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2240_altexprisco_infoamb), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2240_altexprisco_infoamb_apagar', 
                    's2240_altexprisco_infoamb_salvar', 
                    's2240_altexprisco_infoamb'):
        ...
        s2240_altexprisco_infoamb_form = disabled_form_fields(
            s2240_altexprisco_infoamb_form, 
            request.user.has_perm('s2240.change_s2240altExpRiscoinfoAmb'))
        ...
        if tab or 's2240_altexprisco_infoamb' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2240_altexprisco_infoamb_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2240_altexprisco_infoamb').all()
        ...
        context = {
            ...
            'modulos': ['s2240', ],
            'paginas': ['s2240_altexprisco_infoamb', ],
