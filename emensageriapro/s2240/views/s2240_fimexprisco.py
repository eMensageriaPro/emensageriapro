...
def apagar(request, hash):
    ...
    if s2240_fimexprisco_id:
        dados_evento = s2240_fimexprisco.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2240_fimexprisco_apagar'] = 0
            dict_permissoes['s2240_fimexprisco_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2240_fimexprisco), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2240_fimexprisco', s2240_fimexprisco_id, usuario_id, 3)
        else:
...
class s2240fimExpRiscoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2240fimExpRisco.objects.using(db_slug).all()
    serializer_class = s2240fimExpRiscoSerializer
    # permission_classes = (IsAdminUser,)


class s2240fimExpRiscoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2240fimExpRisco.objects.using(db_slug).all()
    serializer_class = s2240fimExpRiscoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2240_fimexprisco_listar.html',
                filename="s2240_fimexprisco.pdf",
                context=context,
            ...
            response = render_to_response('s2240_fimexprisco_listar.html', context)
            filename = "s2240_fimexprisco.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2240_fimexprisco_csv.html', context)
            filename = "s2240_fimexprisco.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2240_fimexprisco_id:
            s2240_fimexprisco_form = form_s2240_fimexprisco(request.POST or None, instance = s2240_fimexprisco,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2240_fimexprisco_form = form_s2240_fimexprisco(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2240_fimexprisco_form.is_valid():

                dados = s2240_fimexprisco_form.cleaned_data
                obj = s2240_fimexprisco_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2240_fimexprisco_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2240_fimexprisco', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2240_fimexprisco), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2240_fimexprisco', s2240_fimexprisco_id, usuario_id, 2)
        ...
        if s2240_fimexprisco_id:
            if dados_evento['status'] != 0:
                s2240_fimexprisco_form = disabled_form_fields(s2240_fimexprisco_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2240_fimexprisco_id, tabela='s2240_fimexprisco').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2240_fimexprisco_salvar.html',
                filename="s2240_fimexprisco.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2240_fimexprisco_salvar.html', context)
            filename = "s2240_fimexprisco.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
