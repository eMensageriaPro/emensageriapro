...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        evento_dados = s1270_remunavnp.evento()
    ...
    if request.user.has_perm('s1270.can_see_s1270remunAvNP'):
        ...
        if request.method == 'POST':
            ...
            if s1270_remunavnp_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's1270_remunavnp_apagar', 
                    's1270_remunavnp_salvar', 
                    's1270_remunavnp'):
        ...
        s1270_remunavnp_form = disabled_form_fields(
            s1270_remunavnp_form, 
            request.user.has_perm('s1270.change_s1270remunAvNP'))
        ...
        if tab or 's1270_remunavnp' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1270_remunavnp_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1270_remunavnp').all()
