...
def salvar(request, hash):
    ...
    if s1270_remunavnp_id:
        ...
        s1270_remunavnp = get_object_or_404(s1270remunAvNP, id=s1270_remunavnp_id)
        dados_evento = s1270_remunavnp.evento()

    if request.user.has_perm('s1270.can_view_s1270remunAvNP'):
            ...
            if s1270_remunavnp_form.is_valid():
                ...
                if not s1270_remunavnp_id:
                    ...
                    gravar_auditoria('{}',
                                 ...
                                 's1270_remunavnp', obj.id, usuario_id, 1)
                                 
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(s1270_remunavnp), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     's1270_remunavnp', s1270_remunavnp_id, usuario_id, 2)
        ...
        s1270_remunavnp_form = disabled_form_fields(s1270_remunavnp_form, request.user.has_perm('s1270.change_s1270remunAvNP'))
        
        if s1270_remunavnp_id:
        
            if dados_evento['status'] != 0:
            
                s1270_remunavnp_form = disabled_form_fields(s1270_remunavnp_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.filter(identidade=s1270_remunavnp_id, tabela='s1270_remunavnp').all()
        ...
        if for_print in (0, 1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1270_remunavnp_salvar.html',
            ...
            response = render_to_response('s1270_remunavnp_salvar.html', context)
