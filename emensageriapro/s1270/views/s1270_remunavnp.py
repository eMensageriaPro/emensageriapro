...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1270_remunavnp_id, tabela='s1270_remunavnp').all()
        context = {
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1270_remunavnp_csv.html', context)
