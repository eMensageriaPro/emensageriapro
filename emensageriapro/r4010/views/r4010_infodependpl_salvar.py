...
def salvar(request, hash):
    ...
    try: 
        ...
        r4010_infodependpl_id = int(dict_hash['id'])
    ...
    pagina = ConfigPaginas.objects.get( endereco='r4010_infodependpl')
    permissao = ConfigPermissoes.objects.get( config_paginas=pagina, config_perfis=usuario.config_perfis)
    if r4010_infodependpl_id:
        r4010_infodependpl = get_object_or_404(r4010infoDependPl, id = r4010_infodependpl_id)
    ...
    if r4010_infodependpl_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['r4010_infodependpl_apagar'] = 0
            dict_permissoes['r4010_infodependpl_editar'] = 0

    if permissao.permite_visualizar:
        mensagem = None
        if r4010_infodependpl_id:
            r4010_infodependpl_form = form_r4010_infodependpl(request.POST or None, instance = r4010_infodependpl,  
        ...
        if request.method == 'POST':
            if r4010_infodependpl_form.is_valid():
                ...
                dados = r4010_infodependpl_form.cleaned_data
                obj = r4010_infodependpl_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                
                if not r4010_infodependpl_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                 'r4010_infodependpl', obj.id, usuario_id, 1)
                else:
                
                    gravar_auditoria(json.dumps(model_to_dict(r4010_infodependpl), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str), 
                                     'r4010_infodependpl', r4010_infodependpl_id, usuario_id, 2)
                                     
                if request.session['retorno_pagina'] not in ('r4010_infodependpl_apagar', 'r4010_infodependpl_salvar', 'r4010_infodependpl'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                if r4010_infodependpl_id != obj.id:
                    url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % (obj.id) )
                    return redirect('r4010_infodependpl_salvar', hash=url_hash)
            else:
                messages.error(request, u'Erro ao salvar!')
        r4010_infodependpl_form = disabled_form_fields(r4010_infodependpl_form, permissao.permite_editar)
        ...
        if dict_hash['tab'] or 'r4010_infodependpl' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 'r4010_infodependpl_salvar'
        controle_alteracoes = Auditoria.objects.filter(identidade=r4010_infodependpl_id, tabela='r4010_infodependpl').all()
        context = {
            ...
            'mensagem': mensagem, 
            'r4010_infodependpl_id': int(r4010_infodependpl_id),
            'usuario': usuario, 
