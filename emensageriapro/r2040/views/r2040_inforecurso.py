...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='r2040_inforecurso')
    ...
    r2040_inforecurso = get_object_or_404(r2040infoRecurso.objects.using( db_slug ), excluido = False, id = r2040_inforecurso_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(r2040_inforecurso), indent=4, sort_keys=True, default=str)
            r2040infoRecurso.objects.using( db_slug ).filter(id = r2040_inforecurso_id).delete()
    ...
    return render(request, 'r2040_inforecurso_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if r2040_inforecurso_id:
            r2040_inforecurso_form = form_r2040_inforecurso(request.POST or None, instance = r2040_inforecurso, slug = db_slug)
        else:
            r2040_inforecurso_form = form_r2040_inforecurso(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if r2040_inforecurso_form.is_valid():
                ...
                if r2040_inforecurso_id:
                    if dados_evento['status'] == 0:
                        ...
                        #r2040_inforecurso_campos_multiple_passo1
                        r2040infoRecurso.objects.using(db_slug).filter(id=r2040_inforecurso_id).update(**dados)
                        obj = r2040infoRecurso.objects.using(db_slug).get(id=r2040_inforecurso_id)
                        #r2040_inforecurso_editar_custom
                        #r2040_inforecurso_campos_multiple_passo2
                    ...
                    #r2040_inforecurso_cadastrar_campos_multiple_passo1
                    obj = r2040infoRecurso(**dados)
                    obj.save(using = db_slug)
                    #r2040_inforecurso_cadastrar_custom
                    #r2040_inforecurso_cadastrar_campos_multiple_passo2
        ...
        for field in r2040_inforecurso_form.fields.keys():
            r2040_inforecurso_form.fields[field].widget.attrs['ng-model'] = 'r2040_inforecurso_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=r2040_inforecurso_id, tabela='r2040_inforecurso').all()
        context = {
...
class r2040infoRecursoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = r2040infoRecurso.objects.using(db_slug).all()
    serializer_class = r2040infoRecursoSerializer
    permission_classes = (IsAdminUser,)


class r2040infoRecursoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = r2040infoRecurso.objects.using(db_slug).all()
    serializer_class = r2040infoRecursoSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        r2040_inforecurso_lista = r2040infoRecurso.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'r2040_inforecurso'
        context = {
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/r2040_inforecurso_csv.html', context)
