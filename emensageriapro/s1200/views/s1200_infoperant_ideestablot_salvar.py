...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1200_infoperant_ideestablot = get_object_or_404(s1200infoPerAntideEstabLot, id=pk)
        evento_dados = s1200_infoperant_ideestablot.evento()
    ...
    if request.user.has_perm('s1200.can_see_s1200infoPerAntideEstabLot'):
        ...
        if request.method == 'POST':
            ...
            if s1200_infoperant_ideestablot_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1200_infoperant_ideestablot), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1200_infoperant_ideestablot_apagar', 
                    's1200_infoperant_ideestablot_salvar', 
                    's1200_infoperant_ideestablot'):
        ...
        s1200_infoperant_ideestablot_form = disabled_form_fields(
            s1200_infoperant_ideestablot_form, 
            request.user.has_perm('s1200.change_s1200infoPerAntideEstabLot'))
        ...
        if tab or 's1200_infoperant_ideestablot' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1200_infoperant_ideestablot_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1200_infoperant_ideestablot').all()
