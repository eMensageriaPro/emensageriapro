            ...
            if s1200_infoperapur_ideestablot_form.is_valid():
                ...
                if s1200_infoperapur_ideestablot_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1200_infoperapur_ideestablot_campos_multiple_passo1
                        s1200infoPerApurideEstabLot.objects.using(db_slug).filter(id=s1200_infoperapur_ideestablot_id).update(**dados)
                        obj = s1200infoPerApurideEstabLot.objects.using(db_slug).get(id=s1200_infoperapur_ideestablot_id)
                        #s1200_infoperapur_ideestablot_editar_custom
                        #s1200_infoperapur_ideestablot_campos_multiple_passo2
                    ...
                    #s1200_infoperapur_ideestablot_cadastrar_campos_multiple_passo1
                    obj = s1200infoPerApurideEstabLot(**dados)
                    obj.save(using = db_slug)
                    #s1200_infoperapur_ideestablot_cadastrar_custom
                    #s1200_infoperapur_ideestablot_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1200infoPerApurideEstabLot.objects.using( db_slug ).filter(id = s1200_infoperapur_ideestablot_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1200_infoperapur_lista = s1200infoPerApur.objects.using( db_slug ).filter(excluido = False).all()
