...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1200_infoperapur_infoagnocivo = get_object_or_404(s1200infoPerApurinfoAgNocivo, id=pk)
        evento_dados = s1200_infoperapur_infoagnocivo.evento()
    ...
    if request.user.has_perm('s1200.can_see_s1200infoPerApurinfoAgNocivo'):
        ...
        if request.method == 'POST':
            ...
            if s1200_infoperapur_infoagnocivo_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        ...
                        json.dumps(
                            ...
                            sort_keys=True, 
                            default=str), 
                        's1200_infoperapur_infoagnocivo', 
                        obj.id, 
                        request.user.id, 1)
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1200_infoperapur_infoagnocivo), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1200_infoperapur_infoagnocivo_apagar', 
                    's1200_infoperapur_infoagnocivo_salvar', 
                    's1200_infoperapur_infoagnocivo'):
        ...
        s1200_infoperapur_infoagnocivo_form = disabled_form_fields(
            s1200_infoperapur_infoagnocivo_form, 
            request.user.has_perm('s1200.change_s1200infoPerApurinfoAgNocivo'))
        ...
        if tab or 's1200_infoperapur_infoagnocivo' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1200_infoperapur_infoagnocivo_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1200_infoperapur_infoagnocivo').all()
