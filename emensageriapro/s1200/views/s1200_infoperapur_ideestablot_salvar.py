...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1200_infoperapur_ideestablot = get_object_or_404(s1200infoPerApurideEstabLot, id=pk)
        evento_dados = s1200_infoperapur_ideestablot.evento()

    if request.user.has_perm('s1200.can_see_s1200infoPerApurideEstabLot'):
        
        if pk:
        
            s1200_infoperapur_ideestablot_form = form_s1200_infoperapur_ideestablot(
                request.POST or None, 
                instance=s1200_infoperapur_ideestablot)
                                         
        else:
        
            s1200_infoperapur_ideestablot_form = form_s1200_infoperapur_ideestablot(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s1200_infoperapur_ideestablot_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1200_infoperapur_ideestablot), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1200_infoperapur_ideestablot_apagar', 
                    's1200_infoperapur_ideestablot_salvar', 
                    's1200_infoperapur_ideestablot'):
        ...
        s1200_infoperapur_ideestablot_form = disabled_form_fields(
            s1200_infoperapur_ideestablot_form, 
            request.user.has_perm('s1200.change_s1200infoPerApurideEstabLot'))
        ...
        if pk:
            ...
            s1200_infoperapur_remunperapur_form = form_s1200_infoperapur_remunperapur(
                initial={ 's1200_infoperapur_ideestablot': s1200_infoperapur_ideestablot })
            ...
            s1200_infoperapur_remunperapur_lista = s1200infoPerApurremunPerApur.objects.\
                filter(s1200_infoperapur_ideestablot_id=s1200_infoperapur_ideestablot.id).all()
        ...
        context = {
            ...
            'modulos': ['s1200', ],
            'paginas': ['s1200_infoperapur_ideestablot', ],
