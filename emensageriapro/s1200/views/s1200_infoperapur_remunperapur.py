...
def salvar(request, hash):
    ...
    try:
        ...
        s1200_infoperapur_remunperapur_id = int(dict_hash['id'])
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1200_infoperapur_remunperapur')
    ...
    if s1200_infoperapur_remunperapur_id:
        dados_evento = s1200_infoperapur_remunperapur.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s1200_infoperapur_remunperapur_apagar'] = 0
            dict_permissoes['s1200_infoperapur_remunperapur_editar'] = 0

    if permissao.permite_visualizar:
        mensagem = None
        if s1200_infoperapur_remunperapur_id:
            s1200_infoperapur_remunperapur_form = form_s1200_infoperapur_remunperapur(request.POST or None, instance = s1200_infoperapur_remunperapur, slug = db_slug)
        else:
        ...
        #s1200_infoperapur_remunperapur_campos_multiple_passo3

        for field in s1200_infoperapur_remunperapur_form.fields.keys():
        ...
        if s1200_infoperapur_remunperapur_id:
            ...
            s1200_infoperapur_infotrabinterm_form = form_s1200_infoperapur_infotrabinterm(initial={ 's1200_infoperapur_remunperapur': s1200_infoperapur_remunperapur }, slug=db_slug)
            s1200_infoperapur_infotrabinterm_form.fields['s1200_infoperapur_remunperapur'].widget.attrs['readonly'] = True
            s1200_infoperapur_infotrabinterm_lista = s1200infoPerApurinfoTrabInterm.objects.using( db_slug ).filter(excluido = False, s1200_infoperapur_remunperapur_id=s1200_infoperapur_remunperapur.id).all()
        else:
            s1200_infoperapur_remunperapur = None
        #s1200_infoperapur_remunperapur_salvar_custom_variaveis#
        tabelas_secundarias = []
        #[FUNCOES_ESPECIAIS_SALVAR]
        if dict_hash['tab'] or 's1200_infoperapur_remunperapur' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 's1200_infoperapur_remunperapur_salvar'
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1200_infoperapur_remunperapur_id, tabela='s1200_infoperapur_remunperapur').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                ...
                filename="s1200_infoperapur_remunperapur.pdf",
                context=context,
...
def apagar(request, hash):
    ...
    return render(request, 's1200_infoperapur_remunperapur_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1200infoPerApurremunPerApurList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1200infoPerApurremunPerApur.objects.using(db_slug).all()
    serializer_class = s1200infoPerApurremunPerApurSerializer
    permission_classes = (IsAdminUser,)


class s1200infoPerApurremunPerApurDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1200infoPerApurremunPerApur.objects.using(db_slug).all()
    serializer_class = s1200infoPerApurremunPerApurSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1200_infoperapur_remunperapur_lista = s1200infoPerApurremunPerApur.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1200_infoperapur_remunperapur_lista) > 100:
            filtrar = True
            s1200_infoperapur_remunperapur_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        s1200_infoperapur_ideestablot_lista = s1200infoPerApurideEstabLot.objects.using( db_slug ).filter(excluido = False).all()
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1200_infoperapur_remunperapur_listar.html',
                filename="s1200_infoperapur_remunperapur.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1200_infoperapur_remunperapur_listar.html', context)
            filename = "s1200_infoperapur_remunperapur.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1200_infoperapur_remunperapur_csv.html', context)
            filename = "s1200_infoperapur_remunperapur.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
