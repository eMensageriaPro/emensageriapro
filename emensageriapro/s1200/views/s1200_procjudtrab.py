...
def apagar(request, hash):
    ...
    if s1200_procjudtrab_id:
        dados_evento = s1200_procjudtrab.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1200_procjudtrab_apagar'] = 0
            dict_permissoes['s1200_procjudtrab_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1200_procjudtrab), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1200_procjudtrab', s1200_procjudtrab_id, usuario_id, 3)
        else:
...
class s1200procJudTrabList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1200procJudTrab.objects.using(db_slug).all()
    serializer_class = s1200procJudTrabSerializer
    # permission_classes = (IsAdminUser,)


class s1200procJudTrabDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1200procJudTrab.objects.using(db_slug).all()
    serializer_class = s1200procJudTrabSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1200_procjudtrab_listar.html',
                filename="s1200_procjudtrab.pdf",
                context=context,
            ...
            response = render_to_response('s1200_procjudtrab_listar.html', context)
            filename = "s1200_procjudtrab.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1200_procjudtrab_csv.html', context)
            filename = "s1200_procjudtrab.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1200_procjudtrab_id:
            s1200_procjudtrab_form = form_s1200_procjudtrab(request.POST or None, instance = s1200_procjudtrab,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1200_procjudtrab_form = form_s1200_procjudtrab(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1200_procjudtrab_form.is_valid():

                dados = s1200_procjudtrab_form.cleaned_data
                obj = s1200_procjudtrab_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s1200_procjudtrab_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1200_procjudtrab', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1200_procjudtrab), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1200_procjudtrab', s1200_procjudtrab_id, usuario_id, 2)
        ...
        if s1200_procjudtrab_id:
            if dados_evento['status'] != 0:
                s1200_procjudtrab_form = disabled_form_fields(s1200_procjudtrab_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1200_procjudtrab_id, tabela='s1200_procjudtrab').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1200_procjudtrab_salvar.html',
                filename="s1200_procjudtrab.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1200_procjudtrab_salvar.html', context)
            filename = "s1200_procjudtrab.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
