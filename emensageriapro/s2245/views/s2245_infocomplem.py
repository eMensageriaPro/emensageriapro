...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2245infoComplem.objects.using( db_slug ).filter(id = s2245_infocomplem_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2245_infocomplem_id:
            s2245_infocomplem_form = form_s2245_infocomplem(request.POST or None, instance = s2245_infocomplem, slug = db_slug)
        else:
            s2245_infocomplem_form = form_s2245_infocomplem(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s2245_infocomplem_form.is_valid():
                ...
                if s2245_infocomplem_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2245_infocomplem_campos_multiple_passo1
                        s2245infoComplem.objects.using(db_slug).filter(id=s2245_infocomplem_id).update(**dados)
                        obj = s2245infoComplem.objects.using(db_slug).get(id=s2245_infocomplem_id)
                        #s2245_infocomplem_editar_custom
                        #s2245_infocomplem_campos_multiple_passo2
                    ...
                    #s2245_infocomplem_cadastrar_campos_multiple_passo1
                    obj = s2245infoComplem(**dados)
                    obj.save(using = db_slug)
                    #s2245_infocomplem_cadastrar_custom
                    #s2245_infocomplem_cadastrar_campos_multiple_passo2
