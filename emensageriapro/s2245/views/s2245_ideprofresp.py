...
class s2245ideProfRespList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2245ideProfResp.objects.using(db_slug).all()
    serializer_class = s2245ideProfRespSerializer
    permission_classes = (IsAdminUser,)


class s2245ideProfRespDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2245ideProfResp.objects.using(db_slug).all()
    serializer_class = s2245ideProfRespSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        s2245_infocomplem_lista = s2245infoComplem.objects.using( db_slug ).filter(excluido = False).all()
