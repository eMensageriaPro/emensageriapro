...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1207_infoperapur_remunperapur = get_object_or_404(s1207infoPerApurremunPerApur, id=pk)
        evento_dados = s1207_infoperapur_remunperapur.evento()

    if request.user.has_perm('s1207.can_see_s1207infoPerApurremunPerApur'):
        
        if pk:
        
            s1207_infoperapur_remunperapur_form = form_s1207_infoperapur_remunperapur(
                request.POST or None, 
                instance=s1207_infoperapur_remunperapur)
                                         
        else:
        
            s1207_infoperapur_remunperapur_form = form_s1207_infoperapur_remunperapur(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s1207_infoperapur_remunperapur_form.is_valid():
            
                obj = s1207_infoperapur_remunperapur_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1207_infoperapur_remunperapur), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1207_infoperapur_remunperapur_apagar', 
                    's1207_infoperapur_remunperapur_salvar', 
                    's1207_infoperapur_remunperapur'):
        ...
        s1207_infoperapur_remunperapur_form = disabled_form_fields(
            s1207_infoperapur_remunperapur_form, 
            request.user.has_perm('s1207.change_s1207infoPerApurremunPerApur'))
        ...
        if pk:
            ...
            s1207_infoperapur_itensremun_form = form_s1207_infoperapur_itensremun(
                initial={ 's1207_infoperapur_remunperapur': s1207_infoperapur_remunperapur })
            ...
            s1207_infoperapur_itensremun_lista = s1207infoPerApuritensRemun.objects.\
                filter(s1207_infoperapur_remunperapur_id=s1207_infoperapur_remunperapur.id).all()
        ...
        if tab or 's1207_infoperapur_remunperapur' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1207_infoperapur_remunperapur_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1207_infoperapur_remunperapur').all()
        ...
        context = {
            ...
            'modulos': ['s1207', ],
            'paginas': ['s1207_infoperapur_remunperapur', ],
