...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1207_dmdev = get_object_or_404(s1207dmDev, id=pk)
        evento_dados = s1207_dmdev.evento()

    if request.user.has_perm('s1207.can_see_s1207dmDev'):

        if pk:

            s1207_dmdev_form = form_s1207_dmdev(
                request.POST or None,
                instance=s1207_dmdev)
                     
        else:

            s1207_dmdev_form = form_s1207_dmdev(request.POST or None)
                     
        if request.method == 'POST':

            if s1207_dmdev_form.is_valid():

                obj = s1207_dmdev_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')
                 
                if 's1207-dmdev' not in request.session['return']:
        ...
        s1207_dmdev_form = disabled_form_fields(
            s1207_dmdev_form,
            request.user.has_perm('s1207.change_s1207dmDev'))
        ...
        #if tab or 's1207_dmdev' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's1207_dmdev_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1207_dmdev').all()
