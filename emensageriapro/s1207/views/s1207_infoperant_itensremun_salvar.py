...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('s1207.can_see_s1207infoPerAntitensRemun'):
        ...
        if request.method == 'POST':
            ...
            if s1207_infoperant_itensremun_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1207_infoperant_itensremun), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1207_infoperant_itensremun_apagar', 
        ...
        if tab or 's1207_infoperant_itensremun' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1207_infoperant_itensremun_salvar'
