...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1260_ideadquir')
    ...
    if s1260_ideadquir_id:
        ...
        if dados_evento['status'] != 0:
            dict_permissoes['s1260_ideadquir_apagar'] = 0
            dict_permissoes['s1260_ideadquir_editar'] = 0

    if permissao.permite_visualizar:
        mensagem = None
        if s1260_ideadquir_id:
            s1260_ideadquir_form = form_s1260_ideadquir(request.POST or None, instance = s1260_ideadquir, slug = db_slug)
        else:
            s1260_ideadquir_form = form_s1260_ideadquir(request.POST or None, slug = db_slug, initial={})
            ...
            if s1260_ideadquir_form.is_valid():
                ...
                if s1260_ideadquir_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1260_ideadquir_campos_multiple_passo1
                        s1260ideAdquir.objects.using(db_slug).filter(id=s1260_ideadquir_id).update(**dados)
                        obj = s1260ideAdquir.objects.using(db_slug).get(id=s1260_ideadquir_id)
                        #s1260_ideadquir_editar_custom
                        #s1260_ideadquir_campos_multiple_passo2
                    ...
                    #s1260_ideadquir_cadastrar_campos_multiple_passo1
                    obj = s1260ideAdquir(**dados)
                    obj.save(using = db_slug)
                    #s1260_ideadquir_cadastrar_custom
                    #s1260_ideadquir_cadastrar_campos_multiple_passo2
        ...
        s1260_ideadquir_form = disabled_form_fields(s1260_ideadquir_form, permissao.permite_editar)
        if s1260_ideadquir_id:
            if dados_evento['status'] != 0:
                s1260_ideadquir_form = disabled_form_fields(s1260_ideadquir_form, 0)
        #s1260_ideadquir_campos_multiple_passo3

        for field in s1260_ideadquir_form.fields.keys():
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1260_ideadquir_id, tabela='s1260_ideadquir').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1260ideAdquir.objects.using( db_slug ).filter(id = s1260_ideadquir_id).delete()
    ...
    return render(request, 's1260_ideadquir_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1260ideAdquirList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1260ideAdquir.objects.using(db_slug).all()
    serializer_class = s1260ideAdquirSerializer
    permission_classes = (IsAdminUser,)


class s1260ideAdquirDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1260ideAdquir.objects.using(db_slug).all()
    serializer_class = s1260ideAdquirSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1260_ideadquir_lista = s1260ideAdquir.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1260_ideadquir_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1260_ideadquir_listar.html',
                filename="s1260_ideadquir.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1260_ideadquir_listar.html', context)
            filename = "s1260_ideadquir.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1260_ideadquir_csv.html', context)
            filename = "s1260_ideadquir.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
