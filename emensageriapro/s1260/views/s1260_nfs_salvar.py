...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1260_nfs = get_object_or_404(s1260nfs, id=pk)
        evento_dados = s1260_nfs.evento()

    if request.user.has_perm('s1260.can_see_s1260nfs'):
        ...
        if pk:
            ...
            s1260_nfs_form = form_s1260_nfs(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s1260_nfs_form.is_valid():
                ...
                if 's1260-nfs' not in request.session['return']:
        ...
        s1260_nfs_form = disabled_form_fields(
            s1260_nfs_form,
            request.user.has_perm('s1260.change_s1260nfs'))
        ...
        #if tab or 's1260_nfs' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's1260_nfs_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1260_nfs').all()
