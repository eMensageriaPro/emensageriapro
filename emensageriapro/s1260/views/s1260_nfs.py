            ...
            if s1260_nfs_form.is_valid():
                ...
                if s1260_nfs_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1260_nfs_campos_multiple_passo1
                        s1260nfs.objects.using(db_slug).filter(id=s1260_nfs_id).update(**dados)
                        obj = s1260nfs.objects.using(db_slug).get(id=s1260_nfs_id)
                        #s1260_nfs_editar_custom
                        #s1260_nfs_campos_multiple_passo2
                    ...
                    #s1260_nfs_cadastrar_campos_multiple_passo1
                    obj = s1260nfs(**dados)
                    obj.save(using = db_slug)
                    #s1260_nfs_cadastrar_custom
                    #s1260_nfs_cadastrar_campos_multiple_passo2
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1260nfs.objects.using( db_slug ).filter(id = s1260_nfs_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1260_ideadquir_lista = s1260ideAdquir.objects.using( db_slug ).filter(excluido = False).all()
