...
def apagar(request, hash):
    ...
    if s1070_inclusao_dadosprocjud_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1070_inclusao_dadosprocjud_apagar'] = 0
            dict_permissoes['s1070_inclusao_dadosprocjud_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1070_inclusao_dadosprocjud', s1070_inclusao_dadosprocjud_id, usuario_id, 3)
        else:
...
class s1070inclusaodadosProcJudList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1070inclusaodadosProcJud.objects.using(db_slug).all()
    serializer_class = s1070inclusaodadosProcJudSerializer
    # permission_classes = (IsAdminUser,)


class s1070inclusaodadosProcJudDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1070inclusaodadosProcJud.objects.using(db_slug).all()
    serializer_class = s1070inclusaodadosProcJudSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1070_inclusao_dadosprocjud_listar.html',
            ...
            response = render_to_response('s1070_inclusao_dadosprocjud_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1070_inclusao_dadosprocjud_csv.html', context)
            filename = "s1070_inclusao_dadosprocjud.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1070_inclusao_dadosprocjud_id:
            s1070_inclusao_dadosprocjud_form = form_s1070_inclusao_dadosprocjud(request.POST or None, instance = s1070_inclusao_dadosprocjud,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1070_inclusao_dadosprocjud_form = form_s1070_inclusao_dadosprocjud(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1070_inclusao_dadosprocjud_form.is_valid():
                ...
                if not s1070_inclusao_dadosprocjud_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1070_inclusao_dadosprocjud', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1070_inclusao_dadosprocjud), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1070_inclusao_dadosprocjud', s1070_inclusao_dadosprocjud_id, usuario_id, 2)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1070_inclusao_dadosprocjud_id, tabela='s1070_inclusao_dadosprocjud').all()
        context = {
