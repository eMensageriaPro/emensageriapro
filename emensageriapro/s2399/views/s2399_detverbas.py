...
def apagar(request, hash):
    ...
    if s2399_detverbas_id:
        dados_evento = s2399_detverbas.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2399_detverbas_apagar'] = 0
            dict_permissoes['s2399_detverbas_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2399_detverbas), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2399_detverbas', s2399_detverbas_id, usuario_id, 3)
        else:
...
class s2399detVerbasList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2399detVerbas.objects.using(db_slug).all()
    serializer_class = s2399detVerbasSerializer
    # permission_classes = (IsAdminUser,)


class s2399detVerbasDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2399detVerbas.objects.using(db_slug).all()
    serializer_class = s2399detVerbasSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2399_detverbas_listar.html',
                filename="s2399_detverbas.pdf",
                context=context,
            ...
            response = render_to_response('s2399_detverbas_listar.html', context)
            filename = "s2399_detverbas.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2399_detverbas_csv.html', context)
            filename = "s2399_detverbas.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2399_detverbas_id:
            s2399_detverbas_form = form_s2399_detverbas(request.POST or None, instance = s2399_detverbas,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2399_detverbas_form = form_s2399_detverbas(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2399_detverbas_form.is_valid():

                dados = s2399_detverbas_form.cleaned_data
                obj = s2399_detverbas_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2399_detverbas_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2399_detverbas', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2399_detverbas), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2399_detverbas', s2399_detverbas_id, usuario_id, 2)
        ...
        if s2399_detverbas_id:
            if dados_evento['status'] != 0:
                s2399_detverbas_form = disabled_form_fields(s2399_detverbas_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2399_detverbas_id, tabela='s2399_detverbas').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2399_detverbas_salvar.html',
                filename="s2399_detverbas.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2399_detverbas_salvar.html', context)
            filename = "s2399_detverbas.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
