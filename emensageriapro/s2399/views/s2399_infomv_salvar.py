...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2399_infomv = get_object_or_404(s2399infoMV, id=pk)
        evento_dados = s2399_infomv.evento()

    if request.user.has_perm('s2399.can_see_s2399infoMV'):

        if pk:

            s2399_infomv_form = form_s2399_infomv(
                request.POST or None,
                instance=s2399_infomv)
                     
        else:

            s2399_infomv_form = form_s2399_infomv(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2399_infomv_form.is_valid():
                ...
                if 's2399-infomv' not in request.session['return']:
        ...
        s2399_infomv_form = disabled_form_fields(
            s2399_infomv_form,
            request.user.has_perm('s2399.change_s2399infoMV'))
        ...
        if pk:
            ...
            s2399_remunoutrempr_form = form_s2399_remunoutrempr(
                initial={ 's2399_infomv': s2399_infomv })
            ...
            s2399_remunoutrempr_lista = s2399remunOutrEmpr.objects.\
                filter(s2399_infomv_id=s2399_infomv.id).all()
        ...
        #if tab or 's2399_infomv' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2399_infomv_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2399_infomv').all()
