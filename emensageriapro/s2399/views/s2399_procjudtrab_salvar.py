...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2399_procjudtrab = get_object_or_404(s2399procJudTrab, id=pk)
        evento_dados = s2399_procjudtrab.evento()

    if request.user.has_perm('s2399.can_see_s2399procJudTrab'):

        if pk:

            s2399_procjudtrab_form = form_s2399_procjudtrab(
                request.POST or None,
                instance=s2399_procjudtrab)
                     
        else:

            s2399_procjudtrab_form = form_s2399_procjudtrab(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2399_procjudtrab_form.is_valid():
                ...
                if 's2399-procjudtrab' not in request.session['return']:
        ...
        s2399_procjudtrab_form = disabled_form_fields(
            ...
            request.user.has_perm('s2399.change_s2399procJudTrab'))
        ...
        #if tab or 's2399_procjudtrab' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2399_procjudtrab_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2399_procjudtrab').all()
