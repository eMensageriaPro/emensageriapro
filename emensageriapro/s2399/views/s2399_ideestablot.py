...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2399_ideestablot')
    ...
    s2399_ideestablot = get_object_or_404(s2399ideEstabLot.objects.using( db_slug ), excluido = False, id = s2399_ideestablot_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s2399_ideestablot), indent=4, sort_keys=True, default=str)
            s2399ideEstabLot.objects.using( db_slug ).filter(id = s2399_ideestablot_id).delete()
            #s2399_ideestablot_apagar_custom
            #s2399_ideestablot_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2399_ideestablot', s2399_ideestablot_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's2399_ideestablot_salvar':
            return redirect('s2399_ideestablot', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's2399_ideestablot_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2399ideEstabLotList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2399ideEstabLot.objects.using(db_slug).all()
    serializer_class = s2399ideEstabLotSerializer
    permission_classes = (IsAdminUser,)


class s2399ideEstabLotDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2399ideEstabLot.objects.using(db_slug).all()
    serializer_class = s2399ideEstabLotSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2399_ideestablot_lista = s2399ideEstabLot.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2399_ideestablot_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2399_ideestablot_listar.html',
            ...
            response = render_to_response('s2399_ideestablot_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2399_ideestablot_csv.html', context)
            filename = "s2399_ideestablot.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2399_ideestablot_id:
            s2399_ideestablot_form = form_s2399_ideestablot(request.POST or None, instance = s2399_ideestablot, slug = db_slug)
        else:
            s2399_ideestablot_form = form_s2399_ideestablot(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s2399_ideestablot_form.is_valid():
                ...
                if s2399_ideestablot_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2399_ideestablot_campos_multiple_passo1
                        s2399ideEstabLot.objects.using(db_slug).filter(id=s2399_ideestablot_id).update(**dados)
                        obj = s2399ideEstabLot.objects.using(db_slug).get(id=s2399_ideestablot_id)
                        #s2399_ideestablot_editar_custom
                        #s2399_ideestablot_campos_multiple_passo2
                    ...
                    #s2399_ideestablot_cadastrar_campos_multiple_passo1
                    obj = s2399ideEstabLot(**dados)
                    obj.save(using = db_slug)
                    #s2399_ideestablot_cadastrar_custom
                    #s2399_ideestablot_cadastrar_campos_multiple_passo2
        ...
        for field in s2399_ideestablot_form.fields.keys():
            s2399_ideestablot_form.fields[field].widget.attrs['ng-model'] = 's2399_ideestablot_'+field
        ...
        if s2399_ideestablot_id:
            ...
            s2399_infosimples_form = form_s2399_infosimples(initial={ 's2399_ideestablot': s2399_ideestablot }, slug=db_slug)
            s2399_infosimples_form.fields['s2399_ideestablot'].widget.attrs['readonly'] = True
            s2399_infosimples_lista = s2399infoSimples.objects.using( db_slug ).filter(excluido = False, s2399_ideestablot_id=s2399_ideestablot.id).all()
        else:
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2399_ideestablot_id, tabela='s2399_ideestablot').all()
        context = {
