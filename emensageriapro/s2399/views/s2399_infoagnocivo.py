...
class s2399infoAgNocivoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2399infoAgNocivo.objects.using(db_slug).all()
    serializer_class = s2399infoAgNocivoSerializer
    # permission_classes = (IsAdminUser,)


class s2399infoAgNocivoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2399infoAgNocivo.objects.using(db_slug).all()
    serializer_class = s2399infoAgNocivoSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2399_infoagnocivo_id:
            ...
            s2399_infoagnocivo_form = form_s2399_infoagnocivo(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
