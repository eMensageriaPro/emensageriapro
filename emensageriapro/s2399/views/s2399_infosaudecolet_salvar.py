...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2399_infosaudecolet = get_object_or_404(s2399infoSaudeColet, id=pk)
        evento_dados = s2399_infosaudecolet.evento()

    if request.user.has_perm('s2399.can_see_s2399infoSaudeColet'):
        
        if pk:
        
            s2399_infosaudecolet_form = form_s2399_infosaudecolet(
                request.POST or None, 
                instance=s2399_infosaudecolet)
                                         
        else:
        
            s2399_infosaudecolet_form = form_s2399_infosaudecolet(request.POST or None)
                                         
        if request.method == 'POST':
        
            if s2399_infosaudecolet_form.is_valid():
            
                obj = s2399_infosaudecolet_form.save(request=request)
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s2399_infosaudecolet), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's2399_infosaudecolet_apagar', 
                    's2399_infosaudecolet_salvar', 
                    's2399_infosaudecolet'):
        ...
        s2399_infosaudecolet_form = disabled_form_fields(
            s2399_infosaudecolet_form, 
            request.user.has_perm('s2399.change_s2399infoSaudeColet'))
        ...
        if tab or 's2399_infosaudecolet' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2399_infosaudecolet_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2399_infosaudecolet').all()
