...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2399infoMV.objects.using( db_slug ).filter(id = s2399_infomv_id).delete()
            ...
            if s2399_infomv_form.is_valid():
                ...
                if s2399_infomv_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2399_infomv_campos_multiple_passo1
                        s2399infoMV.objects.using(db_slug).filter(id=s2399_infomv_id).update(**dados)
                        obj = s2399infoMV.objects.using(db_slug).get(id=s2399_infomv_id)
                        #s2399_infomv_editar_custom
                        #s2399_infomv_campos_multiple_passo2
                    ...
                    #s2399_infomv_cadastrar_campos_multiple_passo1
                    obj = s2399infoMV(**dados)
                    obj.save(using = db_slug)
                    #s2399_infomv_cadastrar_custom
                    #s2399_infomv_cadastrar_campos_multiple_passo2
