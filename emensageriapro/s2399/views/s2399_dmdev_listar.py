...
def listar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='s2399_dmdev')
    ...
    if permissao.permite_listar:
        ...
        s2399_dmdev_lista = s2399dmDev.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            #return render_to_pdf('tables/s2399_dmdev_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/s2399_dmdev.csv', context)
