            ...
            if s2399_procjudtrab_form.is_valid():
                ...
                if s2399_procjudtrab_id:
                    if dados_evento['status'] == 0:
                        ...
                        s2399procJudTrab.objects.using(db_slug).filter(id=s2399_procjudtrab_id).update(**dados)
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s2399procJudTrab.objects.using( db_slug ).filter(id = s2399_procjudtrab_id).delete()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2399_verbasresc_lista = s2399verbasResc.objects.using( db_slug ).filter(excluido = False).all()
