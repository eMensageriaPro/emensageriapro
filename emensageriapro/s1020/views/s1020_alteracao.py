    ...
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1020_alteracao', s1020_alteracao_id, usuario_id, 3)
        else:
...
class s1020alteracaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1020alteracao.objects.using(db_slug).all()
    serializer_class = s1020alteracaoSerializer
    # permission_classes = (IsAdminUser,)


class s1020alteracaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1020alteracao.objects.using(db_slug).all()
    serializer_class = s1020alteracaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1020_alteracao_listar.html',
            ...
            response = render_to_response('s1020_alteracao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1020_alteracao_csv.html', context)
            filename = "s1020_alteracao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1020_alteracao_id:
            s1020_alteracao_form = form_s1020_alteracao(request.POST or None, instance = s1020_alteracao,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1020_alteracao_form = form_s1020_alteracao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1020_alteracao_form.is_valid():
                ...
                if not s1020_alteracao_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1020_alteracao', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1020_alteracao), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1020_alteracao', s1020_alteracao_id, usuario_id, 2)
        ...
        if s1020_alteracao_id:
            ...
            s1020_alteracao_procjudterceiro_form = form_s1020_alteracao_procjudterceiro(initial={ 's1020_alteracao': s1020_alteracao }, slug=db_slug)
            s1020_alteracao_procjudterceiro_form.fields['s1020_alteracao'].widget.attrs['readonly'] = True
            s1020_alteracao_procjudterceiro_lista = s1020alteracaoprocJudTerceiro.objects.using( db_slug ).filter(excluido = False, s1020_alteracao_id=s1020_alteracao.id).all()
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1020_alteracao_id, tabela='s1020_alteracao').all()
        context = {
