...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5003evtBasesFGTS'):

        if s5003_evtbasesfgts_id:
            s5003_evtbasesfgts = get_object_or_404(s5003evtBasesFGTS, excluido=False, id=s5003_evtbasesfgts_id)
            ...
            if s5003_evtbasesfgts.status in status_list:
                s5003evtBasesFGTS.objects.filter(id=s5003_evtbasesfgts_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
