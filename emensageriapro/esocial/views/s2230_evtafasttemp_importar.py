...
def read_s2230_evtafasttemp(request, dados, arquivo, validar=False):
    ...
    s2230evtAfastTemp.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2230_evtafasttemp_obj(request, doc, status, validar=False):
    ...
    s2230_evtafasttemp_dados['status'] = STATUS_EVENTO_IMPORTADO
