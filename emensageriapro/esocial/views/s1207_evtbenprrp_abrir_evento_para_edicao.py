...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1207evtBenPrRP'):

        if s1207_evtbenprrp_id:
            s1207_evtbenprrp = get_object_or_404(s1207evtBenPrRP, excluido=False, id=s1207_evtbenprrp_id)
            ...
            if s1207_evtbenprrp.status in status_list:
                s1207evtBenPrRP.objects.filter(id=s1207_evtbenprrp_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
