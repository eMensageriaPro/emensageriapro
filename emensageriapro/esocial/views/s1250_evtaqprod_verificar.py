...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1250_evtaqprod_verificar.html' % s1250_evtaqprod.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1250_evtaqprod_verificar.html' % s1250_evtaqprod.versao,
            ...
            response =  render_to_response('%s/s1250_evtaqprod_verificar.html' % s1250_evtaqprod.versao, context)
            ...
            return render(request, '%s/s1250_evtaqprod_verificar.html' % s1250_evtaqprod.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1250(s1250_evtaqprod_id, db_slug):
    from django.template.loader import get_template
    if s1250_evtaqprod_id:
        ...
        #return render(request, 'xml/%s/s1250_evtaqprod.html' % s1250_evtaqprod.versao, context, content_type='text/xml')
        t = get_template('%s/s1250_evtaqprod_xml.html' % s1250_evtaqprod.versao)
...
def gerar_xml_assinado(s1250_evtaqprod_id, db_slug):
    ...
    if s1250_evtaqprod.status in (0,1,2,11):
        s1250evtAqProd.objects.using(db_slug).filter(id=s1250_evtaqprod_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s1250_evtaqprod_id = int(dict_hash['id'])
    if s1250_evtaqprod_id:
        s1250_evtaqprod = get_object_or_404(s1250evtAqProd.objects.using(db_slug), excluido=False,
                                                    id=s1250_evtaqprod_id)

        arquivo = 'arquivos/Eventos/s1250_evtaqprod/%s.xml' % s1250_evtaqprod.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1250_evtaqprod_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1250_evtaqprod/%s.xml' % s1250_evtaqprod.identidade)
        salvar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_duplicado_temp.xml' % s1250_evtaqprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_duplicado_temp.xml' % s1250_evtaqprod.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if s1250_evtaqprod_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_alteracao_temp.xml' % s1250_evtaqprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_alteracao_temp.xml' % s1250_evtaqprod.identidade, request)
        from emensageriapro.esocial.views.s1250_evtaqprod import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1250evtAqProd.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def criar_exclusao(request, hash):
    ...
    if s1250_evtaqprod_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_exclusao_temp.xml' % s1250_evtaqprod.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1250_evtaqprod/%s_exclusao_temp.xml' % s1250_evtaqprod.identidade, request)
        from emensageriapro.esocial.views.s1250_evtaqprod import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1250evtAqProd.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def validar_evento_funcao(s1250_evtaqprod_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1250_evtaqprod import validacoes_s1250_evtaqprod
                lista = validacoes_s1250_evtaqprod(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1250_evtaqprod import validacoes_s1250_evtaqprod
                lista = validacoes_s1250_evtaqprod(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s1250_evtaqprod_id:
        lista_validacoes = []
        s1250_evtaqprod = get_object_or_404(s1250evtAqProd.objects.using(db_slug), excluido=False, id=s1250_evtaqprod_id)
        if s1250_evtaqprod.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1250_evtaqprod_id, db_slug)
