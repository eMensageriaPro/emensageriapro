...
def gerar_xml_assinado(s1250_evtaqprod_id, db_slug):
    ...
    if s1250_evtaqprod.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1250evtAqProd.objects.using(db_slug).\
            filter(id=s1250_evtaqprod_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1250_evtaqprod_id:
        ...
        if s1250_evtaqprod.status in status_list:
            s1250evtAqProd.objects.using(db_slug).filter(id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1250_evtaqprod/%s.xml' % (s1250_evtaqprod.identidade)
...
def validar_evento(request, hash):
    ...
    if s1250_evtaqprod_id:
        ...
        if s1250_evtaqprod.versao in VERSOES_ESOCIAL:
            ...
            if s1250_evtaqprod.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1250evtAqProd.objects.using(db_slug).\
                    filter(excluido=False, id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1250_evtaqprod.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1250evtAqProd.objects.using(db_slug).\
                    filter(excluido=False, id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
