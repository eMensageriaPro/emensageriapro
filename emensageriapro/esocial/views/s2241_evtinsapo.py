...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2241_evtinsapo.status == 0:
            ...
            s2241evtInsApo.objects.using( db_slug ).filter(id = s2241_evtinsapo_id).delete()
            #s2241_evtinsapo_apagar_custom
            #s2241_evtinsapo_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2241_evtinsapo', s2241_evtinsapo_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 's2241_evtinsapo_salvar':
            return redirect('s2241_evtinsapo', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's2241_evtinsapo_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2241evtInsApoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2241evtInsApo.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2241_evtinsapo_lista = s2241evtInsApo.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2241_evtinsapo_lista) > 100:
            filtrar = True
            s2241_evtinsapo_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        transmissor_lote_esocial_lista = TransmissorLoteEsocial.objects.using( db_slug ).filter(excluido = False).all()
        #s2241_evtinsapo_listar_custom
        ...
        #return render(request, 's2241_evtinsapo_listar.html', context)
        if for_print in (0,1):
            return render(request, 's2241_evtinsapo_listar.html', context)
        elif for_print == 2:
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2241_evtinsapo_listar.html',
            ...
            response = render_to_response('s2241_evtinsapo_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2241_evtinsapo_csv.html', context)
            filename = "s2241_evtinsapo.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s2241_evtinsapo_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2241evtInsApo.objects.using( db_slug ).get(id=s2241_evtinsapo_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2241evtInsApo.objects.using(db_slug).filter(id=s2241_evtinsapo_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s2241_evtinsapo_form.is_valid():
                ...
                if s2241_evtinsapo_id:
                    if s2241_evtinsapo.status == 0:
                        ...
                        #s2241_evtinsapo_campos_multiple_passo1
                        s2241evtInsApo.objects.using(db_slug).filter(id=s2241_evtinsapo_id).update(**dados)
                        obj = s2241evtInsApo.objects.using(db_slug).get(id=s2241_evtinsapo_id)
                        #s2241_evtinsapo_editar_custom
                        #s2241_evtinsapo_campos_multiple_passo2
                    ...
                    #s2241_evtinsapo_cadastrar_campos_multiple_passo1
                    obj = s2241evtInsApo(**dados)
                    obj.save(using = db_slug)
                    #s2241_evtinsapo_cadastrar_custom
                    #s2241_evtinsapo_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s2241_evtinsapo_form = form_s2241_evtinsapo(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if s2241_evtinsapo_id:
            ...
            s2241_iniinsalperic_form = form_s2241_iniinsalperic(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_iniinsalperic_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_iniinsalperic_lista = s2241iniInsalPeric.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
            s2241_altinsalperic_form = form_s2241_altinsalperic(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_altinsalperic_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_altinsalperic_lista = s2241altInsalPeric.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
            s2241_fiminsalperic_form = form_s2241_fiminsalperic(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_fiminsalperic_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_fiminsalperic_lista = s2241fimInsalPeric.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
            s2241_iniaposentesp_form = form_s2241_iniaposentesp(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_iniaposentesp_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_iniaposentesp_lista = s2241iniAposentEsp.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
            s2241_altaposentesp_form = form_s2241_altaposentesp(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_altaposentesp_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_altaposentesp_lista = s2241altAposentEsp.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
            s2241_fimaposentesp_form = form_s2241_fimaposentesp(initial={ 's2241_evtinsapo': s2241_evtinsapo }, slug=db_slug)
            s2241_fimaposentesp_form.fields['s2241_evtinsapo'].widget.attrs['readonly'] = True
            s2241_fimaposentesp_lista = s2241fimAposentEsp.objects.using( db_slug ).filter(excluido = False, s2241_evtinsapo_id=s2241_evtinsapo.id).all()
        else:
        ...
        #             s2241_evtinsapo_form.fields['tpamb'].widget.attrs['disabled'] = True
        #             s2241_evtinsapo_form.fields['tpamb'].widget.attrs['readonly'] = True
        #             s2241_evtinsapo_form.fields['tpamb'].value = TP_AMB
        #             s2241_evtinsapo_form.fields['procemi'].widget.attrs['disabled'] = True
        #             s2241_evtinsapo_form.fields['procemi'].widget.attrs['readonly'] = True
        #             s2241_evtinsapo_form.fields['procemi'].value = 1
        #             s2241_evtinsapo_form.fields['verproc'].widget.attrs['readonly'] = True
        #             s2241_evtinsapo_form.fields['verproc'].value = VERSAO_EMENSAGERIA
        #             s2241_evtinsapo_form.fields['status'].widget.attrs['disabled'] = True
        #             s2241_evtinsapo_form.fields['status'].widget.attrs['readonly'] = True
        #             s2241_evtinsapo_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2241_evtinsapo_id, tabela='s2241_evtinsapo').all()
        context = {
