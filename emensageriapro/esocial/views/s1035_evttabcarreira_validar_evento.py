...
def validar_evento_funcao(request, s1035_evttabcarreira_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1035_evttabcarreira_id)
...
def validar_evento(request, hash):
    ...
    if s1035_evttabcarreira_id:
        ...
        if s1035_evttabcarreira.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1035_evttabcarreira_id)
            
            if s1035_evttabcarreira.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1035evtTabCarreira.objects.\
                    filter(id=s1035_evttabcarreira_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1035_evttabcarreira.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1035evtTabCarreira.objects.\
                    filter(id=s1035_evttabcarreira_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
