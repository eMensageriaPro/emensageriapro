...
def validar_evento_funcao(request, s2245_evttreicap_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2245_evttreicap_id)
...
def validar_evento(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        if s2245_evttreicap.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2245_evttreicap_id)
            
            if s2245_evttreicap.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2245evtTreiCap.objects.\
                    filter(id=s2245_evttreicap_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2245_evttreicap.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2245evtTreiCap.objects.\
                    filter(id=s2245_evttreicap_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
