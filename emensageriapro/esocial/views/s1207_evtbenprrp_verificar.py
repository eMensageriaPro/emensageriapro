...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1207_infoperapur_ideestab_lista = s1207infoPerApurideEstab.objects.using(db_slug).filter(s1207_dmdev_id__in = listar_ids(s1207_dmdev_lista) ).filter(excluido=False).all()
        s1207_infoperapur_itensremun_lista = s1207infoPerApuritensRemun.objects.using(db_slug).filter(s1207_infoperapur_ideestab_id__in = listar_ids(s1207_infoperapur_ideestab_lista) ).filter(excluido=False).all()
        s1207_infoperant_ideadc_lista = s1207infoPerAntideADC.objects.using(db_slug).filter(s1207_dmdev_id__in = listar_ids(s1207_dmdev_lista) ).filter(excluido=False).all()
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1207_evtbenprrp.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            response =  render_to_response('s1207_evtbenprrp_recibo_pdf.html', context)
            filename = "%s.xls" % s1207_evtbenprrp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            response =  render_to_response('s1207_evtbenprrp_recibo_csv.html', context)
            filename = "%s.csv" % s1207_evtbenprrp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
            ...
            return render_to_pdf('s1207_evtbenprrp_recibo_pdf.html', context)
...
def gerar_xml_assinado(s1207_evtbenprrp_id, db_slug):
    ...
    if s1207_evtbenprrp.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1207evtBenPrRP.objects.using(db_slug).\
            filter(id=s1207_evtbenprrp_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1207_evtbenprrp_id:
        ...
        if s1207_evtbenprrp.status in  or s1207_evtbenprrp.processamento_codigo_resposta in (401,402):
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s1207_evtbenprrp.identidade, BASE_DIR, s1207_evtbenprrp.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1207_evtbenprrp/%s.xml %s/arquivos/Eventos/s1207_evtbenprrp/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s1207_evtbenprrp/%s_backup_%s.xml' % (s1207_evtbenprrp.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s1207_evtbenprrp_id, db_slug):
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s1207_evtbenprrp_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
