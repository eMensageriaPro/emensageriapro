...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5012evtIrrf'):

        if s5012_evtirrf_id:
            s5012_evtirrf = get_object_or_404(s5012evtIrrf, excluido=False, id=s5012_evtirrf_id)
            ...
            if s5012_evtirrf.status in status_list:
                s5012evtIrrf.objects.filter(id=s5012_evtirrf_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
