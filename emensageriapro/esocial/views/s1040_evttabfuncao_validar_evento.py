...
def validar_evento_funcao(request, s1040_evttabfuncao_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1040_evttabfuncao_id)
...
def validar_evento(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        if s1040_evttabfuncao.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1040_evttabfuncao_id)
            
            if s1040_evttabfuncao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1040evtTabFuncao.objects.\
                    filter(id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1040_evttabfuncao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1040evtTabFuncao.objects.\
                    filter(id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
