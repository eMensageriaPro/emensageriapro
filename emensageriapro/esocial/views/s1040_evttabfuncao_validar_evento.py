...
def validar_evento_funcao(request, pk):
    ...
    if not s1040_evttabfuncao.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1040_evttabfuncao, 'esocial')
        s1040_evttabfuncao = get_object_or_404(s1040evtTabFuncao, id=pk)
    ...
    schema_filename = 'esocial/%s/evtTabFuncao.xsd' % (s1040_evttabfuncao.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1040_evttabfuncao' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1040_evttabfuncao', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1040_evttabfuncao.status in STATUS_VALIDACAO:
