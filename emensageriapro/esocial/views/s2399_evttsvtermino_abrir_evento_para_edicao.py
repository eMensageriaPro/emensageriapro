...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2399evtTSVTermino'):

        if s2399_evttsvtermino_id:
            s2399_evttsvtermino = get_object_or_404(s2399evtTSVTermino, excluido=False, id=s2399_evttsvtermino_id)
            ...
            if s2399_evttsvtermino.status in status_list:
                s2399evtTSVTermino.objects.filter(id=s2399_evttsvtermino_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
