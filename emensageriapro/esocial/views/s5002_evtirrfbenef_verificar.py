...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s5002_evtirrfbenef_verificar.html' % s5002_evtirrfbenef.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s5002_evtirrfbenef_verificar.html' % s5002_evtirrfbenef.versao,
            ...
            response =  render_to_response('%s/s5002_evtirrfbenef_verificar.html' % s5002_evtirrfbenef.versao, context)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s5002_evtirrfbenef = get_object_or_404(
            s5002evtIrrfBenef.objects.using( db_slug ),
            excluido = False, id = s5002_evtirrfbenef_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s5002_evtirrfbenef.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s5002_evtirrfbenef_recibo_pdf.html', context)
            filename = "%s.xls" % s5002_evtirrfbenef.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s5002_evtirrfbenef_recibo_csv.html', context)
            filename = "%s.csv" % s5002_evtirrfbenef.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s5002_evtirrfbenef_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug):
    import os
    ...
    s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef.objects.using(db_slug), excluido=False, id=s5002_evtirrfbenef_id)
    if s5002_evtirrfbenef.arquivo_original:
        xml = ler_arquivo(s5002_evtirrfbenef.arquivo)
    else:
    ...
    if s5002_evtirrfbenef.status in (0,1,2,11):
        s5002evtIrrfBenef.objects.using(db_slug).filter(id=s5002_evtirrfbenef_id,excluido=False).update(status=10)
    arquivo = 'arquivos/Eventos/s5002_evtirrfbenef/%s.xml' % (s5002_evtirrfbenef.identidade)
    os.system('mkdir -p %s/arquivos/Eventos/s5002_evtirrfbenef/' % BASE_DIR)
...
def gerar_xml(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef.objects.using(db_slug), excluido=False, id=s5002_evtirrfbenef_id)
        xml_assinado = gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug)
...
def duplicar(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        arquivo = 'arquivos/Eventos/s5002_evtirrfbenef/%s.xml' % s5002_evtirrfbenef.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s.xml' % s5002_evtirrfbenef.identidade)
        salvar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_duplicado_temp.xml' % s5002_evtirrfbenef.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_duplicado_temp.xml' % s5002_evtirrfbenef.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        salvar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_alteracao_temp.xml' % s5002_evtirrfbenef.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_alteracao_temp.xml' % s5002_evtirrfbenef.identidade, request)
        from emensageriapro.esocial.views.s5002_evtirrfbenef import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s5002evtIrrfBenef.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def criar_exclusao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        salvar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_exclusao_temp.xml' % s5002_evtirrfbenef.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5002_evtirrfbenef/%s_exclusao_temp.xml' % s5002_evtirrfbenef.identidade, request)
        from emensageriapro.esocial.views.s5002_evtirrfbenef import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s5002evtIrrfBenef.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef.objects.using(db_slug), excluido=False, id=s5002_evtirrfbenef_id)
        if s5002_evtirrfbenef.status in (0, 1, 2, 3, 4, 10, 11) or s5002_evtirrfbenef.processamento_codigo_resposta in (401,402):
            s5002evtIrrfBenef.objects.using(db_slug).filter(id=s5002_evtirrfbenef_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s5002_evtirrfbenef/%s.xml' % (s5002_evtirrfbenef.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s5002_evtirrfbenef.identidade, BASE_DIR, s5002_evtirrfbenef.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s5002_evtirrfbenef/%s.xml %s/arquivos/Eventos/s5002_evtirrfbenef/%s_backup_%s.xml' % dad)
...
def validar_evento_funcao(s5002_evtirrfbenef_id, db_slug):
    ...
    lista_validacoes = []
    s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef.objects.using(db_slug), excluido=False, id=s5002_evtirrfbenef_id)
    if s5002_evtirrfbenef.transmissor_lote_esocial:
        if s5002_evtirrfbenef.transmissor_lote_esocial.transmissor:
            if s5002_evtirrfbenef.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's5002_evtirrfbenef', s5002_evtirrfbenef_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s5002_evtirrfbenef SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s5002_evtirrfbenef_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s5002_evtirrfbenef/%s.xml' % (s5002_evtirrfbenef.identidade)
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s5002_evtirrfbenef import validacoes_s5002_evtirrfbenef
                lista = validacoes_s5002_evtirrfbenef(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s5002_evtirrfbenef import validacoes_s5002_evtirrfbenef
                lista = validacoes_s5002_evtirrfbenef(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s5002_evtirrfbenef SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s5002_evtirrfbenef_id), False)
    else:
        executar_sql("UPDATE public.s5002_evtirrfbenef SET validacoes='', status=4 WHERE id=%s;" % (s5002_evtirrfbenef_id), False)
...
def validar_evento(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        lista_validacoes = []
        s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef.objects.using(db_slug), excluido=False, id=s5002_evtirrfbenef_id)
        if s5002_evtirrfbenef.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s5002_evtirrfbenef_id, db_slug)
