...
def gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug):
    ...
    if s5002_evtirrfbenef.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s5002evtIrrfBenef.objects.using(db_slug).\
            filter(id=s5002_evtirrfbenef_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        if s5002_evtirrfbenef.status in status_list:
...
def validar_evento(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        if s5002_evtirrfbenef.versao in VERSOES_ESOCIAL:
            ...
            if s5002_evtirrfbenef.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5002evtIrrfBenef.objects.using(db_slug).\
                    filter(excluido=False, id=s5002_evtirrfbenef_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5002_evtirrfbenef.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5002evtIrrfBenef.objects.using(db_slug).\
                    filter(excluido=False, id=s5002_evtirrfbenef_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
