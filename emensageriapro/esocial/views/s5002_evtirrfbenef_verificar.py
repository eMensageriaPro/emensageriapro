...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            return render(request, 's5002_evtirrfbenef_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s5002(s5002_evtirrfbenef_id, db_slug, versao=None):
    ...
    if s5002_evtirrfbenef_id:
        ...
        if not versao:
            ...
            versao = s5002_evtirrfbenef.versao
        ...
        t = get_template('s5002_evtirrfbenef.xml')
...
def gerar_xml_assinado(s5002_evtirrfbenef_id, db_slug):
    ...
    if s5002_evtirrfbenef.status in (0,1,2,11):
        ...
        s5002evtIrrfBenef.objects.using(db_slug).\
            filter(id=s5002_evtirrfbenef_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        texto = gerar_xml_s5002(s5002_evtirrfbenef_id, db_slug, versao="|")
        dados = read_s5002_evtirrfbenef_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s5002evtIrrfBenef.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s5002_evtirrfbenef.identidade),
...
def criar_alteracao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s5002_evtirrfbenef.identidade),
            's5002_evtirrfbenef', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s5002_evtirrfbenef.identidade),
            's5002_evtirrfbenef', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        if s5002_evtirrfbenef.status == 0:
            ...
            nova_identidade = identidade_evento(s5002_evtirrfbenef_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's5002_evtirrfbenef', s5002_evtirrfbenef_id, request.user.id, 1)
...
def validar_evento_funcao(s5002_evtirrfbenef_id, db_slug):
    ...
    s5002evtIrrfBenef.objects.using( db_slug ).\
        filter(id=s5002_evtirrfbenef_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s5002_evtirrfbenef_validar import validacoes_s5002_evtirrfbenef
...
<<<<<<< HEAD
        if s5002_evtirrfbenef.versao in VERSOES_ESOCIAL:
=======
        if s5002_evtirrfbenef.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s5002_evtirrfbenef_id, db_slug)
