...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2230evtAfastTemp'):

        if s2230_evtafasttemp_id:
            s2230_evtafasttemp = get_object_or_404(s2230evtAfastTemp, excluido=False, id=s2230_evtafasttemp_id)
            ...
            if s2230_evtafasttemp.status in status_list:
                s2230evtAfastTemp.objects.filter(id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
