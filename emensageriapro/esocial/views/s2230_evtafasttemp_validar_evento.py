...
def validar_evento_funcao(request, s2230_evtafasttemp_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2230_evtafasttemp_id)
...
def validar_evento(request, hash):
    ...
    if s2230_evtafasttemp_id:
        ...
        if s2230_evtafasttemp.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2230_evtafasttemp_id)
            
            if s2230_evtafasttemp.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2230evtAfastTemp.objects.\
                    filter(id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2230_evtafasttemp.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2230evtAfastTemp.objects.\
                    filter(id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
