...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1210evtPgtos'):

        if s1210_evtpgtos_id:
            s1210_evtpgtos = get_object_or_404(s1210evtPgtos, excluido=False, id=s1210_evtpgtos_id)
            ...
            if s1210_evtpgtos.status in status_list:
                s1210evtPgtos.objects.filter(id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
