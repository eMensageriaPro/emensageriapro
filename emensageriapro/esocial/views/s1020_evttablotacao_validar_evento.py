...
def validar_evento_funcao(request, s1020_evttablotacao_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1020_evttablotacao_id)
...
def validar_evento(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        if s1020_evttablotacao.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1020_evttablotacao_id)
            
            if s1020_evttablotacao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1020evtTabLotacao.objects.\
                    filter(id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1020_evttablotacao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1020evtTabLotacao.objects.\
                    filter(id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
