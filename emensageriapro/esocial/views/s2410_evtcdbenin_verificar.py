...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2410_evtcdbenin_verificar.html' % s2410_evtcdbenin.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2410_evtcdbenin_verificar.html' % s2410_evtcdbenin.versao,
            ...
            response =  render_to_response('%s/s2410_evtcdbenin_verificar.html' % s2410_evtcdbenin.versao, context)
            ...
            return render(request, '%s/s2410_evtcdbenin_verificar.html' % s2410_evtcdbenin.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2410(s2410_evtcdbenin_id, db_slug):
    from django.template.loader import get_template
    if s2410_evtcdbenin_id:
        ...
        #return render(request, 'xml/%s/s2410_evtcdbenin.html' % s2410_evtcdbenin.versao, context, content_type='text/xml')
...
def gerar_xml_assinado(s2410_evtcdbenin_id, db_slug):
    ...
    if s2410_evtcdbenin.status in (0,1,2,11):
        s2410evtCdBenIn.objects.using(db_slug).filter(id=s2410_evtcdbenin_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        dados = importar_arquivo('arquivos/Eventos/s2410_evtcdbenin/%s_duplicado_temp.xml' % s2410_evtcdbenin.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2410_evtcdbenin/%s_alteracao_temp.xml' % s2410_evtcdbenin.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2410_evtcdbenin/%s_alteracao_temp.xml' % s2410_evtcdbenin.identidade, request)
        from emensageriapro.esocial.views.s2410_evtcdbenin import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2410evtCdBenIn.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
...
def validar_evento(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2410_evtcdbenin_id, db_slug)
