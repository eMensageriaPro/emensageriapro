...
def gerar_xml_assinado(s2410_evtcdbenin_id, db_slug):
    ...
    if s2410_evtcdbenin.status in (0,1,2,11):

        s2410evtCdBenIn.objects.using(db_slug).\
            filter(id=s2410_evtcdbenin_id,excluido=False).update(status=10)
...
def alterar_identidade(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.status == 0:
            ...
            nova_identidade = identidade_evento(s2410_evtcdbenin_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.status in (0, 1, 2, 3, 4, 10, 11) or s2410_evtcdbenin.processamento_codigo_resposta in (401,402):
            s2410evtCdBenIn.objects.using(db_slug).filter(id=s2410_evtcdbenin_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2410_evtcdbenin/%s.xml' % (s2410_evtcdbenin.identidade)
...
=======
        if s2410_evtcdbenin.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
