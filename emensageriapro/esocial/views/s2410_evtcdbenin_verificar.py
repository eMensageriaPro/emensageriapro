...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2410_homologtc_lista = s2410homologTC.objects.using(db_slug).filter(s2410_evtcdbenin_id__in = listar_ids(s2410_evtcdbenin_lista) ).filter(excluido=False).all()
...
def gerar_xml_s2410(s2410_evtcdbenin_id, db_slug, versao=None):
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s2410_evtcdbenin.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            response =  render_to_response('s2410_evtcdbenin_recibo_pdf.html', context)
            filename = "%s.xls" % s2410_evtcdbenin.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            response =  render_to_response('s2410_evtcdbenin_recibo_csv.html', context)
            filename = "%s.csv" % s2410_evtcdbenin.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
            ...
            return render_to_pdf('s2410_evtcdbenin_recibo_pdf.html', context)
...
def gerar_xml_assinado(s2410_evtcdbenin_id, db_slug):
    ...
    if s2410_evtcdbenin.arquivo_original:
        ...
        xml = gerar_xml_s2410(s2410_evtcdbenin_id, db_slug)
    ...
    if s2410_evtcdbenin.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2410evtCdBenIn.objects.using(db_slug).\
            filter(id=s2410_evtcdbenin_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        xml_assinado = gerar_xml_assinado(s2410_evtcdbenin_id, db_slug)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2410_evtcdbenin_importar import read_s2410_evtcdbenin_string
    ...
    if s2410_evtcdbenin_id:
        ...
        texto = gerar_xml_s2410(s2410_evtcdbenin_id, db_slug, versao="|")
        dados = read_s2410_evtcdbenin_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2410_evtcdbenin)

        s2410evtCdBenIn.objects.using(db_slug).filter(id=dados['id']).\
            update(status=STATUS_EVENTO_CADASTRADO,
                   arquivo_original=0,
                   arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2410_evtcdbenin.identidade),
            's2410_evtcdbenin', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2410_evtcdbenin.identidade),
            's2410_evtcdbenin', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2410_evtcdbenin.identidade),
            's2410_evtcdbenin', dados['id'], request.user.id, 1)
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2410_evtcdbenin', s2410_evtcdbenin_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.status in status_list:
            s2410evtCdBenIn.objects.using(db_slug).filter(id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2410_evtcdbenin/%s.xml' % (s2410_evtcdbenin.identidade)
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s2410_evtcdbenin.identidade, BASE_DIR, s2410_evtcdbenin.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s2410_evtcdbenin/%s.xml %s/arquivos/Eventos/s2410_evtcdbenin/%s_backup_%s.xml' % dad)
...
def validar_evento_funcao(s2410_evtcdbenin_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2410_evtcdbenin_validar import validacoes_s2410_evtcdbenin
    ...
    if lista_validacoes:
        ...
        s2410evtCdBenIn.objects.using( db_slug ).\
            filter(id=s2410_evtcdbenin_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2410_evtcdbenin_id, db_slug)

            if s2410_evtcdbenin.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2410evtCdBenIn.objects.using(db_slug).\
                    filter(excluido=False, id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2410_evtcdbenin.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2410evtCdBenIn.objects.using(db_slug).\
                    filter(excluido=False, id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
