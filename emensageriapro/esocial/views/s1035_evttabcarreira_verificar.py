...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1035_evttabcarreira_verificar.html' % s1035_evttabcarreira.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1035_evttabcarreira_verificar.html' % s1035_evttabcarreira.versao,
            ...
            response =  render_to_response('%s/s1035_evttabcarreira_verificar.html' % s1035_evttabcarreira.versao, context)
            ...
            return render(request, '%s/s1035_evttabcarreira_verificar.html' % s1035_evttabcarreira.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1035(s1035_evttabcarreira_id, db_slug):
    from django.template.loader import get_template
    if s1035_evttabcarreira_id:
        ...
        #return render(request, 'xml/%s/s1035_evttabcarreira.html' % s1035_evttabcarreira.versao, context, content_type='text/xml')
        t = get_template('%s/s1035_evttabcarreira_xml.html' % s1035_evttabcarreira.versao)
...
def gerar_xml_assinado(s1035_evttabcarreira_id, db_slug):
    ...
    if s1035_evttabcarreira.status in (0,1,2,11):
        s1035evtTabCarreira.objects.using(db_slug).filter(id=s1035_evttabcarreira_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s1035_evttabcarreira_id = int(dict_hash['id'])
    if s1035_evttabcarreira_id:
        s1035_evttabcarreira = get_object_or_404(s1035evtTabCarreira.objects.using(db_slug), excluido=False,
                                                    id=s1035_evttabcarreira_id)

        arquivo = 'arquivos/Eventos/s1035_evttabcarreira/%s.xml' % s1035_evttabcarreira.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1035_evttabcarreira_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s.xml' % s1035_evttabcarreira.identidade)
        salvar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_duplicado_temp.xml' % s1035_evttabcarreira.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_duplicado_temp.xml' % s1035_evttabcarreira.identidade, request)
        from emensageriapro.esocial.views.s1035_evttabcarreira import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1035evtTabCarreira.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1035_evttabcarreira.identidade),
            's1035_evttabcarreira', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1035_evttabcarreira_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_alteracao_temp.xml' % s1035_evttabcarreira.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_alteracao_temp.xml' % s1035_evttabcarreira.identidade, request)
        from emensageriapro.esocial.views.s1035_evttabcarreira import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1035evtTabCarreira.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1035_evttabcarreira.identidade),
            's1035_evttabcarreira', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1035_evttabcarreira_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_exclusao_temp.xml' % s1035_evttabcarreira.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1035_evttabcarreira/%s_exclusao_temp.xml' % s1035_evttabcarreira.identidade, request)
        from emensageriapro.esocial.views.s1035_evttabcarreira import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1035evtTabCarreira.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1035_evttabcarreira.identidade),
            's1035_evttabcarreira', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1035_evttabcarreira_id:
        ...
        if s1035_evttabcarreira.status == 0:
            from emensageriapro.esocial.views.s1035_evttabcarreira import identidade_evento
            dent = identidade_evento(s1035_evttabcarreira_id, db_slug)
...
def validar_evento_funcao(s1035_evttabcarreira_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1035_evttabcarreira import validacoes_s1035_evttabcarreira
                lista = validacoes_s1035_evttabcarreira(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1035_evttabcarreira import validacoes_s1035_evttabcarreira
                lista = validacoes_s1035_evttabcarreira(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s1035_evttabcarreira_id:
        lista_validacoes = []
        s1035_evttabcarreira = get_object_or_404(s1035evtTabCarreira.objects.using(db_slug), excluido=False, id=s1035_evttabcarreira_id)
        if s1035_evttabcarreira.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1035_evttabcarreira_id, db_slug)
