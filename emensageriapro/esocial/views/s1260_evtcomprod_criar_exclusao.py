...
def criar_exclusao(request, pk):
    ...
    if request.user.has_perm('esocial.can_create_delete_s1260evtComProd'):
    
        s1260_evtcomprod = get_object_or_404(
            s1260evtComProd,
            id=pk)

        texto = gerar_xml_s1260(request, pk, versao="|")
        texto = texto.replace('<inclusao>','<exclusao>').replace('</inclusao>','</exclusao>')
        texto = texto.replace('<alteracao>','<exclusao>').replace('</alteracao>','</exclusao>')
        dados = read_s1260_evtcomprod_string(request, {}, texto.encode('utf-8'), 0)
        ...
        return redirect('s1260_evtcomprod_salvar', pk=return_pk, tab='master')
        ...
        return redirect('s1260_evtcomprod_salvar', pk=pk, tab='master')