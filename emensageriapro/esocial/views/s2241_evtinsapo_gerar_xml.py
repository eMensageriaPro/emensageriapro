...
def gerar_xml_s2241(request, s2241_evtinsapo_id, versao=None):
    ...
    if s2241_evtinsapo_id:
        ...
        s2241_evtinsapo_lista = s2241evtInsApo.objects. \
            filter(id=s2241_evtinsapo_id, excluido = False).all()
...
def gerar_xml_assinado(request, s2241_evtinsapo_id):
    ...
    if s2241_evtinsapo.arquivo_original:
        ...
        xml = gerar_xml_s2241(request, s2241_evtinsapo_id)
    ...
    if 'Signature' in xml:
        ...
        xml_assinado = assinar_esocial(request, xml, s2241_evtinsapo.transmissor_lote_esocial_id)
    ...
    if s2241_evtinsapo.status in (
        ...
        s2241evtInsApo.objects.\
            filter(id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2241_evtinsapo_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s2241_evtinsapo_id)
