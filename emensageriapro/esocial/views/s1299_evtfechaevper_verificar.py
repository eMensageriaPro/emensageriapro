...
def gerar_xml_assinado(s1299_evtfechaevper_id, db_slug):
    ...
    if s1299_evtfechaevper.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1299evtFechaEvPer.objects.using(db_slug).\
            filter(id=s1299_evtfechaevper_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1299_evtfechaevper_id:
        ...
        if s1299_evtfechaevper.status in status_list:
...
def validar_evento(request, hash):
    ...
    if s1299_evtfechaevper_id:
        ...
        if s1299_evtfechaevper.versao in VERSOES_ESOCIAL:
            ...
            if s1299_evtfechaevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1299evtFechaEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1299_evtfechaevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1299evtFechaEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
