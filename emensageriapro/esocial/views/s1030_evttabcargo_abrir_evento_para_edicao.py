...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1030evtTabCargo'):

        if s1030_evttabcargo_id:
            s1030_evttabcargo = get_object_or_404(s1030evtTabCargo, excluido=False, id=s1030_evttabcargo_id)
            ...
            if s1030_evttabcargo.status in status_list:
                s1030evtTabCargo.objects.filter(id=s1030_evttabcargo_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
