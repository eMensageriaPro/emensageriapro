...
def validar_evento_funcao(request, pk):
    ...
    if not s2205_evtaltcadastral.identidade:
        ...
        ident = identidade_evento(s2205_evtaltcadastral, 'esocial')
        s2205_evtaltcadastral = get_object_or_404(s2205evtAltCadastral, id=pk)
    ...
    schema_filename = 'esocial/%s/evtAltCadastral.xsd' % (s2205_evtaltcadastral.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2205_evtaltcadastral' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2205_evtaltcadastral', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2205_evtaltcadastral.status in STATUS_VALIDACAO:
