...
def validar_evento_funcao(request, s2205_evtaltcadastral_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2205_evtaltcadastral_id)
...
def validar_evento(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2205_evtaltcadastral_id)
            
            if s2205_evtaltcadastral.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2205evtAltCadastral.objects.\
                    filter(id=s2205_evtaltcadastral_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2205_evtaltcadastral.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2205evtAltCadastral.objects.\
                    filter(id=s2205_evtaltcadastral_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
