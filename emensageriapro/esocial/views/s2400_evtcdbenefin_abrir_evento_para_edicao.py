...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2400evtCdBenefIn'):

        if s2400_evtcdbenefin_id:
            s2400_evtcdbenefin = get_object_or_404(s2400evtCdBenefIn, excluido=False, id=s2400_evtcdbenefin_id)
            ...
            if s2400_evtcdbenefin.status in status_list:
                s2400evtCdBenefIn.objects.filter(id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
