    ...
    if request.user.has_perm('esocial.can_see_s1210evtPgtos'):
        ...
        if request.method == 'POST':
            ...
            if s1210_evtpgtos_form.is_valid():
                ...
                if 'return_page' in request.session and request.session['return_page'] and 's1210-evtpgtos' not in request.session['return_page']:
                ...
                if pk != obj.id:
                    ...
                    return redirect(
                        's1210_evtpgtos_salvar',
                        pk=obj.id)

            else:
                messages.error(request, u'Erro ao salvar!')

        s1210_evtpgtos_form = disabled_form_fields(
             s1210_evtpgtos_form,
             request.user.has_perm('esocial.change_s1210evtPgtos'))
