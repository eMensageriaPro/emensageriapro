...
def validar_evento_funcao(request, s1202_evtrmnrpps_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1202_evtrmnrpps_id)
...
def validar_evento(request, hash):
    ...
    if s1202_evtrmnrpps_id:
        ...
        if s1202_evtrmnrpps.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1202_evtrmnrpps_id)
            
            if s1202_evtrmnrpps.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1202evtRmnRPPS.objects.\
                    filter(id=s1202_evtrmnrpps_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1202_evtrmnrpps.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1202evtRmnRPPS.objects.\
                    filter(id=s1202_evtrmnrpps_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
