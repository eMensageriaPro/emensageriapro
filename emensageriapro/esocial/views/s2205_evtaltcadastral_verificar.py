...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2205_evtaltcadastral_verificar.html' % s2205_evtaltcadastral.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2205_evtaltcadastral_verificar.html' % s2205_evtaltcadastral.versao,
            ...
            response =  render_to_response('%s/s2205_evtaltcadastral_verificar.html' % s2205_evtaltcadastral.versao, context)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s2205_evtaltcadastral.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s2205_evtaltcadastral_recibo_pdf.html', context)
            filename = "%s.xls" % s2205_evtaltcadastral.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s2205_evtaltcadastral_recibo_csv.html', context)
            filename = "%s.csv" % s2205_evtaltcadastral.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s2205_evtaltcadastral_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s2205_evtaltcadastral_id, db_slug):
    import os
    ...
    if s2205_evtaltcadastral.status in (0,1,2,11):
        s2205evtAltCadastral.objects.using(db_slug).filter(id=s2205_evtaltcadastral_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2205_evtaltcadastral_id = int(dict_hash['id'])
    if s2205_evtaltcadastral_id:
        s2205_evtaltcadastral = get_object_or_404(s2205evtAltCadastral.objects.using(db_slug), excluido=False,
                                                    id=s2205_evtaltcadastral_id)

        arquivo = 'arquivos/Eventos/s2205_evtaltcadastral/%s.xml' % s2205_evtaltcadastral.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2205_evtaltcadastral_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s.xml' % s2205_evtaltcadastral.identidade)
        salvar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_duplicado_temp.xml' % s2205_evtaltcadastral.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_duplicado_temp.xml' % s2205_evtaltcadastral.identidade, request)
        from emensageriapro.esocial.views.s2205_evtaltcadastral import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2205evtAltCadastral.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2205_evtaltcadastral.identidade),
            's2205_evtaltcadastral', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_alteracao_temp.xml' % s2205_evtaltcadastral.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_alteracao_temp.xml' % s2205_evtaltcadastral.identidade, request)
        from emensageriapro.esocial.views.s2205_evtaltcadastral import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2205evtAltCadastral.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2205_evtaltcadastral.identidade),
            's2205_evtaltcadastral', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_exclusao_temp.xml' % s2205_evtaltcadastral.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2205_evtaltcadastral/%s_exclusao_temp.xml' % s2205_evtaltcadastral.identidade, request)
        from emensageriapro.esocial.views.s2205_evtaltcadastral import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2205evtAltCadastral.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2205_evtaltcadastral.identidade),
            's2205_evtaltcadastral', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.status == 0:
            from emensageriapro.esocial.views.s2205_evtaltcadastral import identidade_evento
            dent = identidade_evento(s2205_evtaltcadastral_id, db_slug)
            messages.success(request, 'Identidade do evento alterada com sucesso!')
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % s2205_evtaltcadastral_id )
            usuario_id = request.user.id
            gravar_auditoria(u'{}', u'{"funcao": "Identidade do evento foi alterada"}',
            's2205_evtaltcadastral', s2205_evtaltcadastral_id, usuario_id, 1)
            return redirect('s2205_evtaltcadastral_salvar', hash=url_hash)
        else:
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.status in (0, 1, 2, 3, 4, 10, 11) or s2205_evtaltcadastral.processamento_codigo_resposta in (401,402):
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s2205_evtaltcadastral.identidade, BASE_DIR, s2205_evtaltcadastral.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s2205_evtaltcadastral/%s.xml %s/arquivos/Eventos/s2205_evtaltcadastral/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s2205_evtaltcadastral/%s_backup_%s.xml' % (s2205_evtaltcadastral.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s2205_evtaltcadastral_id, db_slug):
    ...
    lista_validacoes = []
    s2205_evtaltcadastral = get_object_or_404(s2205evtAltCadastral.objects.using(db_slug), excluido=False, id=s2205_evtaltcadastral_id)
    if s2205_evtaltcadastral.transmissor_lote_esocial:
        if s2205_evtaltcadastral.transmissor_lote_esocial.transmissor:
            if s2205_evtaltcadastral.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's2205_evtaltcadastral', s2205_evtaltcadastral_id)
                if quant <= 0:
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s2205_evtaltcadastral_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2205_evtaltcadastral import validacoes_s2205_evtaltcadastral
                lista = validacoes_s2205_evtaltcadastral(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2205_evtaltcadastral import validacoes_s2205_evtaltcadastral
                lista = validacoes_s2205_evtaltcadastral(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        lista_validacoes = []
        s2205_evtaltcadastral = get_object_or_404(s2205evtAltCadastral.objects.using(db_slug), excluido=False, id=s2205_evtaltcadastral_id)
        if s2205_evtaltcadastral.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2205_evtaltcadastral_id, db_slug)
