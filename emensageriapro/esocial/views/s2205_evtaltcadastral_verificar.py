...
def gerar_xml_s2205(s2205_evtaltcadastral_id, db_slug, versao=None):
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2205_evtaltcadastral_importar import read_s2205_evtaltcadastral_string
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2205_evtaltcadastral', s2205_evtaltcadastral_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.status in status_list:
...
def validar_evento_funcao(s2205_evtaltcadastral_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2205_evtaltcadastral_validar import validacoes_s2205_evtaltcadastral
...
def validar_evento(request, hash):
    ...
    if s2205_evtaltcadastral_id:
        ...
        if s2205_evtaltcadastral.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2205_evtaltcadastral_id, db_slug)

            if s2205_evtaltcadastral.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2205evtAltCadastral.objects.using(db_slug).\
                    filter(excluido=False, id=s2205_evtaltcadastral_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2205_evtaltcadastral.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2205evtAltCadastral.objects.using(db_slug).\
                    filter(excluido=False, id=s2205_evtaltcadastral_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
