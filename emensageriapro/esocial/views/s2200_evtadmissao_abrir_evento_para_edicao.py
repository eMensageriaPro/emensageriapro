...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2200evtAdmissao'):

        if s2200_evtadmissao_id:
            s2200_evtadmissao = get_object_or_404(s2200evtAdmissao, excluido=False, id=s2200_evtadmissao_id)
            ...
            if s2200_evtadmissao.status in status_list:
                s2200evtAdmissao.objects.filter(id=s2200_evtadmissao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
