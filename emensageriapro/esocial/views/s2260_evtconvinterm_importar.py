...
def read_s2260_evtconvinterm(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s2260_evtconvinterm_obj(request, doc, status, validar, arquivo)

    s2260evtConvInterm.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2260_evtconvinterm_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s2260_evtconvinterm_dados['arquivo'] = arquivo
