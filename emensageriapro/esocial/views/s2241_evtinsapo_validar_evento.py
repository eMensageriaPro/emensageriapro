...
def validar_evento_funcao(request, pk):
    ...
    if not s2241_evtinsapo.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2241_evtinsapo, 'esocial')
        s2241_evtinsapo = get_object_or_404(s2241evtInsApo, id=pk)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2241_evtinsapo' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2241_evtinsapo', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if tab == 'mapa':
        ...
        return redirect('s2241_evtinsapo_salvar', pk=pk)