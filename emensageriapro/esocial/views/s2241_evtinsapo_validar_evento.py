...
def validar_evento_funcao(request, s2241_evtinsapo_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2241_evtinsapo_id)
...
def validar_evento(request, hash):
    ...
    if s2241_evtinsapo_id:
        ...
        if s2241_evtinsapo.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2241_evtinsapo_id)
            
            if s2241_evtinsapo.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2241evtInsApo.objects.\
                    filter(id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2241_evtinsapo.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2241evtInsApo.objects.\
                    filter(id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
