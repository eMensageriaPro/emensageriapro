...
def validar_evento_funcao(request, s1060_evttabambiente_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1060_evttabambiente_id)
...
def validar_evento(request, hash):
    ...
    if s1060_evttabambiente_id:
        ...
        if s1060_evttabambiente.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1060_evttabambiente_id)
            
            if s1060_evttabambiente.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1060evtTabAmbiente.objects.\
                    filter(id=s1060_evttabambiente_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1060_evttabambiente.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1060evtTabAmbiente.objects.\
                    filter(id=s1060_evttabambiente_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
