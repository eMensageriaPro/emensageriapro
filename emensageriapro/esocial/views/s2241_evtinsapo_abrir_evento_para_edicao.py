...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2241evtInsApo'):

        if s2241_evtinsapo_id:
            s2241_evtinsapo = get_object_or_404(s2241evtInsApo, excluido=False, id=s2241_evtinsapo_id)
            ...
            if s2241_evtinsapo.status in status_list:
                s2241evtInsApo.objects.filter(id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
