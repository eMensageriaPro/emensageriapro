...
def validar_evento_funcao(request, s2220_evtmonit_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2220_evtmonit_id)
...
def validar_evento(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        if s2220_evtmonit.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2220_evtmonit_id)
            
            if s2220_evtmonit.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2220evtMonit.objects.\
                    filter(id=s2220_evtmonit_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2220_evtmonit.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2220evtMonit.objects.\
                    filter(id=s2220_evtmonit_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
