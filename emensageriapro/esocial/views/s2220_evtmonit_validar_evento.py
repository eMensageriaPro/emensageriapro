...
def validar_evento_funcao(request, pk):
    ...
    if not s2220_evtmonit.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2220_evtmonit, 'esocial')
        s2220_evtmonit = get_object_or_404(s2220evtMonit, id=pk)
    ...
    schema_filename = 'esocial/%s/evtMonit.xsd' % (s2220_evtmonit.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2220_evtmonit' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2220_evtmonit', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2220_evtmonit.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s2220_evtmonit_salvar', pk=pk)