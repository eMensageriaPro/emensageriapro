...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2206evtAltContratual'):

        if s2206_evtaltcontratual_id:
            s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual, excluido=False, id=s2206_evtaltcontratual_id)
            ...
            if s2206_evtaltcontratual.status in status_list:
                s2206evtAltContratual.objects.filter(id=s2206_evtaltcontratual_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
