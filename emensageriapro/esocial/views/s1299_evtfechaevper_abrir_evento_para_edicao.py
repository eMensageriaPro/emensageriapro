...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1299evtFechaEvPer'):

        if s1299_evtfechaevper_id:
            s1299_evtfechaevper = get_object_or_404(s1299evtFechaEvPer, excluido=False, id=s1299_evtfechaevper_id)
            ...
            if s1299_evtfechaevper.status in status_list:
                s1299evtFechaEvPer.objects.filter(id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
