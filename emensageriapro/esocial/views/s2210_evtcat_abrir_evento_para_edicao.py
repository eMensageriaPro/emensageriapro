...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2210evtCAT'):

        if s2210_evtcat_id:
            s2210_evtcat = get_object_or_404(s2210evtCAT, excluido=False, id=s2210_evtcat_id)
            ...
            if s2210_evtcat.status in status_list:
                s2210evtCAT.objects.filter(id=s2210_evtcat_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
