    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s1280_evtinfocomplper_verificar.html',
            ...
            response =  render_to_response('s1280_evtinfocomplper_verificar.html', context)
            ...
            return render(request, 's1280_evtinfocomplper_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1280(s1280_evtinfocomplper_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1280_evtinfocomplper_id, db_slug):
    ...
    if s1280_evtinfocomplper.status in (0,1,2,11):
        ...
        s1280evtInfoComplPer.objects.using(db_slug).\
            filter(id=s1280_evtinfocomplper_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s1280_evtinfocomplper_importar import read_s1280_evtinfocomplper_string
    ...
    if s1280_evtinfocomplper_id:
        ...
        texto = gerar_xml_s1280(s1280_evtinfocomplper_id, db_slug, versao="|")
        dados = read_s1280_evtinfocomplper_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1280evtInfoComplPer.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1280_evtinfocomplper.identidade),
            's1280_evtinfocomplper', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1280_evtinfocomplper.identidade),
            's1280_evtinfocomplper', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1280_evtinfocomplper.identidade),
            's1280_evtinfocomplper', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        if s1280_evtinfocomplper.status == 0:
            ...
            nova_identidade = identidade_evento(s1280_evtinfocomplper_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1280_evtinfocomplper', s1280_evtinfocomplper_id, request.user.id, 1)
...
def validar_evento_funcao(s1280_evtinfocomplper_id, db_slug):
    ...
    s1280evtInfoComplPer.objects.using( db_slug ).\
        filter(id=s1280_evtinfocomplper_id, excluido = False).\
...
<<<<<<< HEAD
        if s1280_evtinfocomplper.versao in VERSOES_ESOCIAL:
=======
        if s1280_evtinfocomplper.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1280_evtinfocomplper_id, db_slug)
