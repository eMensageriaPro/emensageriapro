...
def gerar_xml_assinado(s1280_evtinfocomplper_id, db_slug):
    ...
    if s1280_evtinfocomplper.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1280evtInfoComplPer.objects.using(db_slug).\
            filter(id=s1280_evtinfocomplper_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        if s1280_evtinfocomplper.status in status_list:
            s1280evtInfoComplPer.objects.using(db_slug).filter(id=s1280_evtinfocomplper_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1280_evtinfocomplper/%s.xml' % (s1280_evtinfocomplper.identidade)
...
def validar_evento(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        if s1280_evtinfocomplper.versao in VERSOES_ESOCIAL:
            ...
            if s1280_evtinfocomplper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1280evtInfoComplPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1280_evtinfocomplper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1280_evtinfocomplper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1280evtInfoComplPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1280_evtinfocomplper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
