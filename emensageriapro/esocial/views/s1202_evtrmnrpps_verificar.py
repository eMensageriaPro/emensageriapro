...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1202_evtrmnrpps_id:
        ...
        if s1202_evtrmnrpps.status in status_list:
    ...
    if s1202_evtrmnrpps_id:
        ...
        if s1202_evtrmnrpps.versao in VERSOES_ESOCIAL:
            ...
            if s1202_evtrmnrpps.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1202evtRmnRPPS.objects.using(db_slug).\
                    filter(excluido=False, id=s1202_evtrmnrpps_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1202_evtrmnrpps.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1202evtRmnRPPS.objects.using(db_slug).\
                    filter(excluido=False, id=s1202_evtrmnrpps_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
