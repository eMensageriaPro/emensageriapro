...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2250evtAvPrevio'):

        if s2250_evtavprevio_id:
            s2250_evtavprevio = get_object_or_404(s2250evtAvPrevio, excluido=False, id=s2250_evtavprevio_id)
            ...
            if s2250_evtavprevio.status in status_list:
                s2250evtAvPrevio.objects.filter(id=s2250_evtavprevio_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
