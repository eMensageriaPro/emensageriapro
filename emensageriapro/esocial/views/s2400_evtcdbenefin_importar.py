...
def read_s2400_evtcdbenefin(request, dados, arquivo, validar=False):
    ...
    s2400evtCdBenefIn.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2400_evtcdbenefin_obj(request, doc, status, validar=False):
    ...
    s2400_evtcdbenefin_dados['status'] = STATUS_EVENTO_IMPORTADO
