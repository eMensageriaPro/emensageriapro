...
def gerar_xml_s2206(request, s2206_evtaltcontratual_id, versao=None):
    ...
    if s2206_evtaltcontratual_id:
        ...
        s2206_evtaltcontratual_lista = s2206evtAltContratual.objects. \
            filter(id=s2206_evtaltcontratual_id, excluido = False).all()
...
def gerar_xml_assinado(request, s2206_evtaltcontratual_id):
    ...
    if s2206_evtaltcontratual.arquivo_original:
        ...
        xml = gerar_xml_s2206(request, s2206_evtaltcontratual_id)
    ...
    if 'Signature' in xml:
        ...
        if not s2206_evtaltcontratual.transmissor_lote_esocial:
            ...
            grupo = get_grupo(s2206evtAltContratual)
        ...
        xml_assinado = assinar_esocial(request, xml, s2206_evtaltcontratual.transmissor_lote_esocial_id)
    ...
    if s2206_evtaltcontratual.status in (
        ...
        s2206evtAltContratual.objects.\
            filter(id=s2206_evtaltcontratual_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s2206_evtaltcontratual_id)
