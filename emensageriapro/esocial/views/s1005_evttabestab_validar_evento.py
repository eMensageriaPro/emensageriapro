...
def validar_evento_funcao(request, s1005_evttabestab_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1005_evttabestab_id)
...
def validar_evento(request, hash):
    ...
    if s1005_evttabestab_id:
        ...
        if s1005_evttabestab.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1005_evttabestab_id)
            
            if s1005_evttabestab.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1005evtTabEstab.objects.\
                    filter(id=s1005_evttabestab_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1005_evttabestab.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1005evtTabEstab.objects.\
                    filter(id=s1005_evttabestab_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
