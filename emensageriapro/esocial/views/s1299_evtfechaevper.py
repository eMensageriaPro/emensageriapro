...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1299_evtfechaevper.status == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s1299_evtfechaevper), indent=4, sort_keys=True, default=str)
...
class s1299evtFechaEvPerList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1299evtFechaEvPer.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1299_evtfechaevper_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1299_evtfechaevper_id:
            ...
            s1299_evtfechaevper_form = form_s1299_evtfechaevper(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1299_evtfechaevper_form.is_valid():
                ...
                if s1299_evtfechaevper_id:
                    ...
                    s1299_evtfechaevper_form = form_s1299_evtfechaevper(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        #             s1299_evtfechaevper_form.fields['status'].widget.attrs['disabled'] = True
        #             s1299_evtfechaevper_form.fields['status'].widget.attrs['readonly'] = True
        #             s1299_evtfechaevper_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1299_evtfechaevper_id, tabela='s1299_evtfechaevper').all()
        context = {
