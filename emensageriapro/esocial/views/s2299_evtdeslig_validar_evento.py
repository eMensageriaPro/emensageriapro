...
def validar_evento_funcao(request, pk):
    ...
    if not s2299_evtdeslig.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2299_evtdeslig, 'esocial')
        s2299_evtdeslig = get_object_or_404(s2299evtDeslig, id=pk)
    ...
    schema_filename = 'esocial/%s/evtDeslig.xsd' % (s2299_evtdeslig.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2299_evtdeslig' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2299_evtdeslig', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2299_evtdeslig.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s2299_evtdeslig_salvar', pk=pk)