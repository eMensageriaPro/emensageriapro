...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if s2420_evtcdbenterm.status == STATUS_EVENTO_CADASTRADO:
            ...
            situacao_anterior = json.dumps(model_to_dict(s2420_evtcdbenterm), sort_keys=True, default=str)
            obj = s2420evtCdBenTerm.objects.get(id=pk)
            obj.delete(request=request)

            sql_softdelete = ler_arquivo('/database/sql/s2420_softdelete_cascade.sql')
        ...
        if 'return_page' in request.session and request.session['return_page'] and 's2420-evtcdbenterm' in request.session['return_page']:
