...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s5013_evtfgts.status == 0:
            ...
            s5013evtFGTS.objects.using( db_slug ).filter(id = s5013_evtfgts_id).delete()
...
def identidade_evento(s5013_evtfgts_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s5013evtFGTS.objects.using( db_slug ).get(id=s5013_evtfgts_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s5013evtFGTS.objects.using(db_slug).filter(id=s5013_evtfgts_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s5013_evtfgts_form.is_valid():
                ...
                if s5013_evtfgts_id:
                    if s5013_evtfgts.status == 0:
                        ...
                        #s5013_evtfgts_campos_multiple_passo1
                        s5013evtFGTS.objects.using(db_slug).filter(id=s5013_evtfgts_id).update(**dados)
                        obj = s5013evtFGTS.objects.using(db_slug).get(id=s5013_evtfgts_id)
                        #s5013_evtfgts_editar_custom
                        #s5013_evtfgts_campos_multiple_passo2
                    ...
                    #s5013_evtfgts_cadastrar_campos_multiple_passo1
                    obj = s5013evtFGTS(**dados)
                    obj.save(using = db_slug)
                    #s5013_evtfgts_cadastrar_custom
                    #s5013_evtfgts_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s5013_evtfgts_form = form_s5013_evtfgts(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s5013_evtfgts_form.fields['tpamb'].widget.attrs['disabled'] = True
            s5013_evtfgts_form.fields['tpamb'].widget.attrs['readonly'] = True
            s5013_evtfgts_form.fields['tpamb'].value = TP_AMB
            s5013_evtfgts_form.fields['procemi'].widget.attrs['disabled'] = True
            s5013_evtfgts_form.fields['procemi'].widget.attrs['readonly'] = True
            ...
            s5013_evtfgts_form.fields['status'].widget.attrs['disabled'] = True
            s5013_evtfgts_form.fields['status'].widget.attrs['readonly'] = True
            s5013_evtfgts_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
