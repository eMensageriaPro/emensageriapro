...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s5013_evtfgts import identidade_evento
...
def alterar_identidade(request, hash):
    ...
    if s5013_evtfgts_id:
        ...
        if s5013_evtfgts.status == 0:
            ...
            nova_identidade = identidade_evento(s5013_evtfgts_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5013_evtfgts_id:
        ...
        if s5013_evtfgts.status in (0, 1, 2, 3, 4, 10, 11) or s5013_evtfgts.processamento_codigo_resposta in (401,402):
            s5013evtFGTS.objects.using(db_slug).filter(id=s5013_evtfgts_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s5013_evtfgts/%s.xml' % (s5013_evtfgts.identidade)
...
def validar_evento_funcao(s5013_evtfgts_id, db_slug):
    ...
    #executar_sql("UPDATE public.s5013_evtfgts SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s5013_evtfgts_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s5013_evtfgts/%s.xml' % (s5013_evtfgts.identidade)
    ...
    if lista_validacoes:
        ...
        #executar_sql("UPDATE public.s5013_evtfgts SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s5013_evtfgts_id), False)
        ...
        #executar_sql("UPDATE public.s5013_evtfgts SET validacoes='', status=4 WHERE id=%s;" % (s5013_evtfgts_id), False)
...
=======
        if s5013_evtfgts.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3
