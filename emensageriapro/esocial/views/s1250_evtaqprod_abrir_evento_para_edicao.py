...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1250evtAqProd'):

        if s1250_evtaqprod_id:
            s1250_evtaqprod = get_object_or_404(s1250evtAqProd, excluido=False, id=s1250_evtaqprod_id)
            ...
            if s1250_evtaqprod.status in status_list:
                s1250evtAqProd.objects.filter(id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
