...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2245evtTreiCap'):

        if s2245_evttreicap_id:
            s2245_evttreicap = get_object_or_404(s2245evtTreiCap, excluido=False, id=s2245_evttreicap_id)
            ...
            if s2245_evttreicap.status in status_list:
                s2245evtTreiCap.objects.filter(id=s2245_evttreicap_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
