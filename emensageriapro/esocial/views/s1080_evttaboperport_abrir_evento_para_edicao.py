...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1080evtTabOperPort'):

        if s1080_evttaboperport_id:
            s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort, excluido=False, id=s1080_evttaboperport_id)
            ...
            if s1080_evttaboperport.status in status_list:
                s1080evtTabOperPort.objects.filter(id=s1080_evttaboperport_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
