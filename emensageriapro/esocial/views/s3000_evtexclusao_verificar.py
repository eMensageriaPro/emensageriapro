...
def gerar_xml_s3000(s3000_evtexclusao_id, db_slug, versao=None):
...
def gerar_xml_assinado(s3000_evtexclusao_id, db_slug):
    ...
    if s3000_evtexclusao.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s3000evtExclusao.objects.using(db_slug).\
            filter(id=s3000_evtexclusao_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if s3000_evtexclusao_id:
        ...
        texto = gerar_xml_s3000(s3000_evtexclusao_id, db_slug, versao="|")
        dados = read_s3000_evtexclusao_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s3000_evtexclusao)

        s3000evtExclusao.objects.using(db_slug).filter(id=dados['id']).\
            update(status=STATUS_EVENTO_CADASTRADO,
                   arquivo_original=0,
                   arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s3000_evtexclusao.identidade),
...
def criar_alteracao(request, hash):
    ...
    if s3000_evtexclusao_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s3000_evtexclusao.identidade),
            's3000_evtexclusao', dados['id'], request.user.id, 1)
    ...
    if s3000_evtexclusao_id:
        ...
        if s3000_evtexclusao.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's3000_evtexclusao', s3000_evtexclusao_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s3000_evtexclusao_id:
        ...
        if s3000_evtexclusao.status in status_list:
...
def validar_evento_funcao(s3000_evtexclusao_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s3000_evtexclusao_validar import validacoes_s3000_evtexclusao
    ...
    if lista_validacoes:
        ...
        s3000evtExclusao.objects.using( db_slug ).\
            filter(id=s3000_evtexclusao_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s3000_evtexclusao_id:
        ...
        if s3000_evtexclusao.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s3000_evtexclusao_id, db_slug)

            if s3000_evtexclusao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s3000evtExclusao.objects.using(db_slug).\
                    filter(excluido=False, id=s3000_evtexclusao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s3000_evtexclusao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s3000evtExclusao.objects.using(db_slug).\
                    filter(excluido=False, id=s3000_evtexclusao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
