...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1020_evttablotacao_verificar.html' % s1020_evttablotacao.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1020_evttablotacao_verificar.html' % s1020_evttablotacao.versao,
            ...
            response =  render_to_response('%s/s1020_evttablotacao_verificar.html' % s1020_evttablotacao.versao, context)
            ...
            return render(request, '%s/s1020_evttablotacao_verificar.html' % s1020_evttablotacao.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1020(s1020_evttablotacao_id, db_slug):
    ...
    if s1020_evttablotacao_id:
        ...
        t = get_template('%s/s1020_evttablotacao_xml.html' % s1020_evttablotacao.versao)
...
def gerar_xml_assinado(s1020_evttablotacao_id, db_slug):
    ...
    if s1020_evttablotacao.status in (0,1,2,11):
        s1020evtTabLotacao.objects.using(db_slug).filter(id=s1020_evttablotacao_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s1020_evttablotacao_id = int(dict_hash['id'])
    if s1020_evttablotacao_id:
        s1020_evttablotacao = get_object_or_404(s1020evtTabLotacao.objects.using(db_slug), excluido=False,
                                                    id=s1020_evttablotacao_id)

        arquivo = 'arquivos/Eventos/s1020_evttablotacao/%s.xml' % s1020_evttablotacao.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1020_evttablotacao_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1020_evttablotacao/%s.xml' % s1020_evttablotacao.identidade)
        salvar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_duplicado_temp.xml' % s1020_evttablotacao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_duplicado_temp.xml' % s1020_evttablotacao.identidade, request)
        ...
        s1020evtTabLotacao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1020_evttablotacao.identidade),
            's1020_evttablotacao', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_alteracao_temp.xml' % s1020_evttablotacao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_alteracao_temp.xml' % s1020_evttablotacao.identidade, request)
        from emensageriapro.esocial.views.s1020_evttablotacao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1020evtTabLotacao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1020_evttablotacao.identidade),
            's1020_evttablotacao', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_exclusao_temp.xml' % s1020_evttablotacao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1020_evttablotacao/%s_exclusao_temp.xml' % s1020_evttablotacao.identidade, request)
        from emensageriapro.esocial.views.s1020_evttablotacao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1020evtTabLotacao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1020_evttablotacao.identidade),
            's1020_evttablotacao', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        if s1020_evttablotacao.status == 0:
            from emensageriapro.esocial.views.s1020_evttablotacao import identidade_evento
            dent = identidade_evento(s1020_evttablotacao_id, db_slug)
...
def validar_evento_funcao(s1020_evttablotacao_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1020_evttablotacao import validacoes_s1020_evttablotacao
                lista = validacoes_s1020_evttablotacao(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1020_evttablotacao import validacoes_s1020_evttablotacao
                lista = validacoes_s1020_evttablotacao(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        if s1020_evttablotacao.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1020_evttablotacao_id, db_slug)
