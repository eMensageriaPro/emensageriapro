...
def gerar_xml_assinado(s1020_evttablotacao_id, db_slug):
    ...
    if s1020_evttablotacao.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1020evtTabLotacao.objects.using(db_slug).\
            filter(id=s1020_evttablotacao_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        if s1020_evttablotacao.status in status_list:
            s1020evtTabLotacao.objects.using(db_slug).filter(id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1020_evttablotacao/%s.xml' % (s1020_evttablotacao.identidade)
...
def validar_evento(request, hash):
    ...
    if s1020_evttablotacao_id:
        ...
        if s1020_evttablotacao.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s1020_evttablotacao_id, db_slug)

            if s1020_evttablotacao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1020evtTabLotacao.objects.using(db_slug).\
                    filter(excluido=False, id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1020_evttablotacao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1020evtTabLotacao.objects.using(db_slug).\
                    filter(excluido=False, id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
