...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2299_evtdeslig_verificar.html' % s2299_evtdeslig.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2299_evtdeslig_verificar.html' % s2299_evtdeslig.versao,
            ...
            response =  render_to_response('%s/s2299_evtdeslig_verificar.html' % s2299_evtdeslig.versao, context)
...
def gerar_xml_assinado(s2299_evtdeslig_id, db_slug):
    ...
    if s2299_evtdeslig.status in (0,1,2,11):
        s2299evtDeslig.objects.using(db_slug).filter(id=s2299_evtdeslig_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2299_evtdeslig_id = int(dict_hash['id'])
    if s2299_evtdeslig_id:
        s2299_evtdeslig = get_object_or_404(s2299evtDeslig.objects.using(db_slug), excluido=False,
                                                    id=s2299_evtdeslig_id)

        arquivo = 'arquivos/Eventos/s2299_evtdeslig/%s.xml' % s2299_evtdeslig.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2299_evtdeslig_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2299_evtdeslig/%s.xml' % s2299_evtdeslig.identidade)
        salvar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_duplicado_temp.xml' % s2299_evtdeslig.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_duplicado_temp.xml' % s2299_evtdeslig.identidade, request)
        ...
        s2299evtDeslig.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2299_evtdeslig.identidade),
            's2299_evtdeslig', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2299_evtdeslig_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_alteracao_temp.xml' % s2299_evtdeslig.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_alteracao_temp.xml' % s2299_evtdeslig.identidade, request)
        from emensageriapro.esocial.views.s2299_evtdeslig import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2299evtDeslig.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2299_evtdeslig.identidade),
            's2299_evtdeslig', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2299_evtdeslig_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_exclusao_temp.xml' % s2299_evtdeslig.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2299_evtdeslig/%s_exclusao_temp.xml' % s2299_evtdeslig.identidade, request)
        from emensageriapro.esocial.views.s2299_evtdeslig import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2299evtDeslig.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2299_evtdeslig.identidade),
            's2299_evtdeslig', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2299_evtdeslig_id:
        ...
        if s2299_evtdeslig.status == 0:
            from emensageriapro.esocial.views.s2299_evtdeslig import identidade_evento
            dent = identidade_evento(s2299_evtdeslig_id, db_slug)
...
def validar_evento_funcao(s2299_evtdeslig_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2299_evtdeslig import validacoes_s2299_evtdeslig
                lista = validacoes_s2299_evtdeslig(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2299_evtdeslig import validacoes_s2299_evtdeslig
                lista = validacoes_s2299_evtdeslig(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s2299_evtdeslig_id:
        ...
        if s2299_evtdeslig.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2299_evtdeslig_id, db_slug)
