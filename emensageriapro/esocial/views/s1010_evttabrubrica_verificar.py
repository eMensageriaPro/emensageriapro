...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1010_evttabrubrica_verificar.html' % s1010_evttabrubrica.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1010_evttabrubrica_verificar.html' % s1010_evttabrubrica.versao,
            ...
            response =  render_to_response('%s/s1010_evttabrubrica_verificar.html' % s1010_evttabrubrica.versao, context)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1010_evttabrubrica.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s1010_evttabrubrica_recibo_pdf.html', context)
            filename = "%s.xls" % s1010_evttabrubrica.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s1010_evttabrubrica_recibo_csv.html', context)
            filename = "%s.csv" % s1010_evttabrubrica.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s1010_evttabrubrica_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s1010_evttabrubrica_id, db_slug):
    import os
    ...
    if s1010_evttabrubrica.status in (0,1,2,11):
        s1010evtTabRubrica.objects.using(db_slug).filter(id=s1010_evttabrubrica_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s1010_evttabrubrica_id = int(dict_hash['id'])
    if s1010_evttabrubrica_id:
        s1010_evttabrubrica = get_object_or_404(s1010evtTabRubrica.objects.using(db_slug), excluido=False,
                                                    id=s1010_evttabrubrica_id)

        arquivo = 'arquivos/Eventos/s1010_evttabrubrica/%s.xml' % s1010_evttabrubrica.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1010_evttabrubrica_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s.xml' % s1010_evttabrubrica.identidade)
        salvar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_duplicado_temp.xml' % s1010_evttabrubrica.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_duplicado_temp.xml' % s1010_evttabrubrica.identidade, request)
        from emensageriapro.esocial.views.s1010_evttabrubrica import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1010evtTabRubrica.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1010_evttabrubrica.identidade),
            's1010_evttabrubrica', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_alteracao_temp.xml' % s1010_evttabrubrica.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_alteracao_temp.xml' % s1010_evttabrubrica.identidade, request)
        from emensageriapro.esocial.views.s1010_evttabrubrica import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1010evtTabRubrica.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1010_evttabrubrica.identidade),
            's1010_evttabrubrica', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_exclusao_temp.xml' % s1010_evttabrubrica.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1010_evttabrubrica/%s_exclusao_temp.xml' % s1010_evttabrubrica.identidade, request)
        from emensageriapro.esocial.views.s1010_evttabrubrica import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1010evtTabRubrica.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1010_evttabrubrica.identidade),
            's1010_evttabrubrica', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        if s1010_evttabrubrica.status == 0:
            from emensageriapro.esocial.views.s1010_evttabrubrica import identidade_evento
            dent = identidade_evento(s1010_evttabrubrica_id, db_slug)
            messages.success(request, 'Identidade do evento alterada com sucesso!')
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % s1010_evttabrubrica_id )
            usuario_id = request.user.id
            gravar_auditoria(u'{}', u'{"funcao": "Identidade do evento foi alterada"}',
            's1010_evttabrubrica', s1010_evttabrubrica_id, usuario_id, 1)
            return redirect('s1010_evttabrubrica_salvar', hash=url_hash)
        else:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1010_evttabrubrica_id:
        s1010_evttabrubrica = get_object_or_404(s1010evtTabRubrica.objects.using(db_slug), excluido=False, id=s1010_evttabrubrica_id)
        if s1010_evttabrubrica.status in (0, 1, 2, 3, 4, 10, 11) or s1010_evttabrubrica.processamento_codigo_resposta in (401,402):
            s1010evtTabRubrica.objects.using(db_slug).filter(id=s1010_evttabrubrica_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1010_evttabrubrica/%s.xml' % (s1010_evttabrubrica.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s1010_evttabrubrica.identidade, BASE_DIR, s1010_evttabrubrica.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1010_evttabrubrica/%s.xml %s/arquivos/Eventos/s1010_evttabrubrica/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s1010_evttabrubrica/%s_backup_%s.xml' % (s1010_evttabrubrica.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s1010_evttabrubrica_id, db_slug):
    ...
    lista_validacoes = []
    s1010_evttabrubrica = get_object_or_404(s1010evtTabRubrica.objects.using(db_slug), excluido=False, id=s1010_evttabrubrica_id)
    if s1010_evttabrubrica.transmissor_lote_esocial:
        if s1010_evttabrubrica.transmissor_lote_esocial.transmissor:
            if s1010_evttabrubrica.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's1010_evttabrubrica', s1010_evttabrubrica_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s1010_evttabrubrica SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s1010_evttabrubrica_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s1010_evttabrubrica/%s.xml' % (s1010_evttabrubrica.identidade)
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s1010_evttabrubrica_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1010_evttabrubrica import validacoes_s1010_evttabrubrica
                lista = validacoes_s1010_evttabrubrica(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1010_evttabrubrica import validacoes_s1010_evttabrubrica
                lista = validacoes_s1010_evttabrubrica(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s1010_evttabrubrica SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s1010_evttabrubrica_id), False)
    else:
        executar_sql("UPDATE public.s1010_evttabrubrica SET validacoes='', status=4 WHERE id=%s;" % (s1010_evttabrubrica_id), False)
...
def validar_evento(request, hash):
    ...
    if s1010_evttabrubrica_id:
        lista_validacoes = []
        s1010_evttabrubrica = get_object_or_404(s1010evtTabRubrica.objects.using(db_slug), excluido=False, id=s1010_evttabrubrica_id)
        if s1010_evttabrubrica.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1010_evttabrubrica_id, db_slug)
