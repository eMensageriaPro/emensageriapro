...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1010_inclusao_ideprocessocprp_lista = s1010inclusaoideProcessoCPRP.objects.using(db_slug).filter(s1010_inclusao_id__in = listar_ids(s1010_inclusao_lista) ).filter(excluido=False).all()
...
def gerar_xml_s1010(s1010_evttabrubrica_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1010_evttabrubrica_id, db_slug):
    ...
    if s1010_evttabrubrica.status in (0,1,2,11):
        ...
        s1010evtTabRubrica.objects.using(db_slug).\
            filter(id=s1010_evttabrubrica_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1010_evttabrubrica.identidade),
...
def alterar_identidade(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        if s1010_evttabrubrica.status == 0:
            ...
            nova_identidade = identidade_evento(s1010_evttabrubrica_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1010_evttabrubrica', s1010_evttabrubrica_id, request.user.id, 1)
...
def validar_evento_funcao(s1010_evttabrubrica_id, db_slug):
    ...
    s1010evtTabRubrica.objects.using( db_slug ).\
        filter(id=s1010_evttabrubrica_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1010_evttabrubrica_validar import validacoes_s1010_evttabrubrica
...
<<<<<<< HEAD
        if s1010_evttabrubrica.versao in VERSOES_ESOCIAL:
=======
        if s1010_evttabrubrica.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1010_evttabrubrica_id, db_slug)
