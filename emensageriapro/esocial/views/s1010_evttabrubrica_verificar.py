...
def gerar_xml_assinado(s1010_evttabrubrica_id, db_slug):
    ...
    if s1010_evttabrubrica.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1010evtTabRubrica.objects.using(db_slug).\
            filter(id=s1010_evttabrubrica_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        if s1010_evttabrubrica.status in status_list:
            s1010evtTabRubrica.objects.using(db_slug).filter(id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1010_evttabrubrica/%s.xml' % (s1010_evttabrubrica.identidade)
...
def validar_evento(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        if s1010_evttabrubrica.versao in VERSOES_ESOCIAL:
            ...
            if s1010_evttabrubrica.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1010evtTabRubrica.objects.using(db_slug).\
                    filter(excluido=False, id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1010_evttabrubrica.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1010evtTabRubrica.objects.using(db_slug).\
                    filter(excluido=False, id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
