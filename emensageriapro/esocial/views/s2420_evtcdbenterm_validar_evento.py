...
def validar_evento_funcao(request, s2420_evtcdbenterm_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2420_evtcdbenterm_id)
...
def validar_evento(request, hash):
    ...
    if s2420_evtcdbenterm_id:
        ...
        if s2420_evtcdbenterm.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2420_evtcdbenterm_id)
            
            if s2420_evtcdbenterm.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2420evtCdBenTerm.objects.\
                    filter(id=s2420_evtcdbenterm_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2420_evtcdbenterm.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2420evtCdBenTerm.objects.\
                    filter(id=s2420_evtcdbenterm_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
