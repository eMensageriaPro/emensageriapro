...
def validar_evento_funcao(request, s1280_evtinfocomplper_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1280_evtinfocomplper_id)
...
def validar_evento(request, hash):
    ...
    if s1280_evtinfocomplper_id:
        ...
        if s1280_evtinfocomplper.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1280_evtinfocomplper_id)
            
            if s1280_evtinfocomplper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1280evtInfoComplPer.objects.\
                    filter(id=s1280_evtinfocomplper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1280_evtinfocomplper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1280evtInfoComplPer.objects.\
                    filter(id=s1280_evtinfocomplper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
