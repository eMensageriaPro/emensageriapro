...
def gerar_xml_s1207_func(pk, versao=None):
...
def gerar_xml_s1207(request, pk, versao=None):

    s1207_evtbenprrp = get_object_or_404(
        s1207evtBenPrRP,
        id=pk)

    return gerar_xml_s1207_func(pk, versao)
...
def gerar_xml_assinado(request, pk):
    ...
    s1207_evtbenprrp = get_object_or_404(
        s1207evtBenPrRP, id=pk)

    if not s1207_evtbenprrp.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1207_evtbenprrp, 'esocial')
    ...
    if 'Signature' in xml and s1207_evtbenprrp.status in STATUS_ANT:
        ...
        if 'Signature' in xml_assinado and s1207_evtbenprrp.status in STATUS_ANT:
