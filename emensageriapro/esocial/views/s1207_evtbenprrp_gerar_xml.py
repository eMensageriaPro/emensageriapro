...
def gerar_xml_s1207(request, s1207_evtbenprrp_id, versao=None):
    ...
    if s1207_evtbenprrp_id:
        ...
        s1207_evtbenprrp_lista = s1207evtBenPrRP.objects. \
            filter(id=s1207_evtbenprrp_id, excluido = False).all()
...
def gerar_xml_assinado(request, s1207_evtbenprrp_id):
    ...
    if s1207_evtbenprrp.arquivo_original:
        ...
        xml = gerar_xml_s1207(request, s1207_evtbenprrp_id)
    ...
    if 'Signature' in xml:
        ...
        if not s1207_evtbenprrp.transmissor_lote_esocial:
            ...
            grupo = get_grupo(s1207evtBenPrRP)
        ...
        xml_assinado = assinar_esocial(request, xml, s1207_evtbenprrp.transmissor_lote_esocial_id)
    ...
    if s1207_evtbenprrp.status in (
        ...
        s1207evtBenPrRP.objects.\
            filter(id=s1207_evtbenprrp_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s1207_evtbenprrp_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s1207_evtbenprrp_id)
