...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1000evtInfoEmpregador'):

        if s1000_evtinfoempregador_id:
            s1000_evtinfoempregador = get_object_or_404(s1000evtInfoEmpregador, excluido=False, id=s1000_evtinfoempregador_id)
            ...
            if s1000_evtinfoempregador.status in status_list:
                s1000evtInfoEmpregador.objects.filter(id=s1000_evtinfoempregador_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
