...
def validar_evento_funcao(request, s2416_evtcdbenalt_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2416_evtcdbenalt_id)
...
def validar_evento(request, hash):
    ...
    if s2416_evtcdbenalt_id:
        ...
        if s2416_evtcdbenalt.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2416_evtcdbenalt_id)
            
            if s2416_evtcdbenalt.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2416evtCdBenAlt.objects.\
                    filter(id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2416_evtcdbenalt.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2416evtCdBenAlt.objects.\
                    filter(id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
