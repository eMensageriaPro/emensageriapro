...
def salvar(request, hash):
    ...
    if s5001_evtbasestrab_ocorrencias_id:
        dados_evento = s5001_evtbasestrab_ocorrencias.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s5001_evtbasestrab_ocorrencias_apagar'] = 0
            dict_permissoes['s5001_evtbasestrab_ocorrencias_editar'] = 0
    ...
    if permissao.permite_visualizar:
            ...
            if s5001_evtbasestrab_ocorrencias_form.is_valid():
                ...
                if s5001_evtbasestrab_ocorrencias_id:
                    if dados_evento['status'] == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s5001_evtbasestrab_ocorrencias), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's5001_evtbasestrab_ocorrencias', s5001_evtbasestrab_ocorrencias_id, usuario_id, 2)
                    else:
                    ...
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's5001_evtbasestrab_ocorrencias', obj.id, usuario_id, 1)
        ...
        if s5001_evtbasestrab_ocorrencias_id:
            if dados_evento['status'] != 0:
                s5001_evtbasestrab_ocorrencias_form = disabled_form_fields(s5001_evtbasestrab_ocorrencias_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5001_evtbasestrab_ocorrencias_id, tabela='s5001_evtbasestrab_ocorrencias').all()
        context = {
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            return render_to_pdf('tables/s5001_evtbasestrab_ocorrencias_pdf_xls.html', context)
        elif for_print == 3:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5001_evtbasestrab_ocorrencias_pdf_xls.html', context)
            filename = "s5001_evtbasestrab_ocorrencias.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5001_evtbasestrab_ocorrencias_csv.html', context)
            filename = "s5001_evtbasestrab_ocorrencias.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5001_evtbasestrab_ocorrencias), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's5001_evtbasestrab_ocorrencias', s5001_evtbasestrab_ocorrencias_id, usuario_id, 3)
        else:
