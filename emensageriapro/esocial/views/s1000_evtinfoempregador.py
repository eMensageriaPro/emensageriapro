...
class s1000evtInfoEmpregadorList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1000evtInfoEmpregador.objects.using(db_slug).all()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1000_evtinfoempregador_id:
            ...
            s1000_evtinfoempregador_form = form_s1000_evtinfoempregador(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1000_evtinfoempregador_form.is_valid():
                ...
                if s1000_evtinfoempregador_id:
                    ...
                    s1000_evtinfoempregador_form = form_s1000_evtinfoempregador(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        #             s1000_evtinfoempregador_form.fields['status'].widget.attrs['disabled'] = True
        #             s1000_evtinfoempregador_form.fields['status'].widget.attrs['readonly'] = True
        #             s1000_evtinfoempregador_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
