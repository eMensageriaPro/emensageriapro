...
def identidade_evento(s2221_evttoxic_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2221evtToxic.objects.using( db_slug ).get(id=s2221_evttoxic_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2221evtToxic.objects.using(db_slug).filter(id=s2221_evttoxic_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2221_evttoxic_id:
            ...
            s2221_evttoxic_form = form_s2221_evttoxic(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
            ...
            if s2221_evttoxic_form.is_valid():
                ...
                if s2221_evttoxic_id:
                    if s2221_evttoxic.status == 0:
                        ...
                        #s2221_evttoxic_campos_multiple_passo1
                        s2221evtToxic.objects.using(db_slug).filter(id=s2221_evttoxic_id).update(**dados)
                        obj = s2221evtToxic.objects.using(db_slug).get(id=s2221_evttoxic_id)
                        #s2221_evttoxic_editar_custom
                        #s2221_evttoxic_campos_multiple_passo2
                    ...
                    #s2221_evttoxic_cadastrar_campos_multiple_passo1
                    obj = s2221evtToxic(**dados)
                    obj.save(using = db_slug)
                    #s2221_evttoxic_cadastrar_custom
                    #s2221_evttoxic_cadastrar_campos_multiple_passo2
        ...
        if not evento_totalizador:
            s2221_evttoxic_form.fields['tpamb'].widget.attrs['disabled'] = True
            s2221_evttoxic_form.fields['tpamb'].widget.attrs['readonly'] = True
            s2221_evttoxic_form.fields['tpamb'].value = TP_AMB
            s2221_evttoxic_form.fields['procemi'].widget.attrs['disabled'] = True
            s2221_evttoxic_form.fields['procemi'].widget.attrs['readonly'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2221_evttoxic_id, tabela='s2221_evttoxic').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2221_evttoxic.status == 0:
            ...
            s2221evtToxic.objects.using( db_slug ).filter(id = s2221_evttoxic_id).delete()
            #s2221_evttoxic_apagar_custom
            #s2221_evttoxic_apagar_custom
    ...
    return render(request, 's2221_evttoxic_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2221evtToxicList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2221evtToxic.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2221_evttoxic_lista = s2221evtToxic.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2221_evttoxic_csv.html', context)
