...
def validar_evento_funcao(request, s1000_evtinfoempregador_id):
    ...
    lista_validacoes = []
    s1000_evtinfoempregador = get_object_or_404(s1000evtInfoEmpregador, id=s1000_evtinfoempregador_id)
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1000_evtinfoempregador_id)
...
def validar_evento(request, hash):
    ...
    if s1000_evtinfoempregador_id:
        ...
        if s1000_evtinfoempregador.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1000_evtinfoempregador_id)
            
            if s1000_evtinfoempregador.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1000evtInfoEmpregador.objects.\
                    filter(id=s1000_evtinfoempregador_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1000_evtinfoempregador.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1000evtInfoEmpregador.objects.\
                    filter(id=s1000_evtinfoempregador_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
