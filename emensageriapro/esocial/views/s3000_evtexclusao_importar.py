...
def read_s3000_evtexclusao(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s3000_evtexclusao_obj(request, doc, status, validar, arquivo)

    s3000evtExclusao.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s3000_evtexclusao_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s3000_evtexclusao_dados['arquivo'] = arquivo
