...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2260evtConvInterm'):

        if s2260_evtconvinterm_id:
            s2260_evtconvinterm = get_object_or_404(s2260evtConvInterm, excluido=False, id=s2260_evtconvinterm_id)
            ...
            if s2260_evtconvinterm.status in status_list:
                s2260evtConvInterm.objects.filter(id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
