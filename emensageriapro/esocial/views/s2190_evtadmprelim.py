...
def apagar(request, hash):
    ...
    if s2190_evtadmprelim_id:
        if s2190_evtadmprelim.status != 0:
            dict_permissoes['s2190_evtadmprelim_apagar'] = 0
            dict_permissoes['s2190_evtadmprelim_editar'] = 0
    ...
    if request.method == 'POST':
        if s2190_evtadmprelim.status == 0:
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2190_evtadmprelim', s2190_evtadmprelim_id, usuario_id, 3)
        else:
...
class s2190evtAdmPrelimList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2190evtAdmPrelim.objects.using(db_slug).all()
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2190_evtadmprelim_listar.html',
            ...
            response = render_to_response('s2190_evtadmprelim_listar.html', context)
            filename = "s2190_evtadmprelim.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2190_evtadmprelim_csv.html', context)
            filename = "s2190_evtadmprelim.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2190_evtadmprelim_id:
            ...
            s2190_evtadmprelim_form = form_s2190_evtadmprelim(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s2190_evtadmprelim_form.is_valid():
                ...
                if s2190_evtadmprelim_id:
                    if s2190_evtadmprelim.status == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s2190_evtadmprelim), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's2190_evtadmprelim', s2190_evtadmprelim_id, usuario_id, 2)
                    else:
                        obj = s2190evtAdmPrelim.objects.using(db_slug).get(id=s2190_evtadmprelim_id)
                    ...
                    s2190_evtadmprelim_form = form_s2190_evtadmprelim(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2190_evtadmprelim', obj.id, usuario_id, 1)
        ...
        if s2190_evtadmprelim_id:
            if s2190_evtadmprelim.status != 0:
                s2190_evtadmprelim_form = disabled_form_fields(s2190_evtadmprelim_form, False)
        ...
        if not evento_totalizador:
            ...
            s2190_evtadmprelim_form.fields['status'].widget.attrs['disabled'] = True
            s2190_evtadmprelim_form.fields['status'].widget.attrs['readonly'] = True
            s2190_evtadmprelim_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2190_evtadmprelim_id, tabela='s2190_evtadmprelim').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2190_evtadmprelim_salvar.html',
                filename="s2190_evtadmprelim.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2190_evtadmprelim_salvar.html', context)
            filename = "s2190_evtadmprelim.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
