...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1300evtContrSindPatr'):

        if s1300_evtcontrsindpatr_id:
            s1300_evtcontrsindpatr = get_object_or_404(s1300evtContrSindPatr, excluido=False, id=s1300_evtcontrsindpatr_id)
            ...
            if s1300_evtcontrsindpatr.status in status_list:
                s1300evtContrSindPatr.objects.filter(id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
