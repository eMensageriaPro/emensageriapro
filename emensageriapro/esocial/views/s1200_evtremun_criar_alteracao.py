...
def criar_alteracao(request, pk):
    ...
    from emensageriapro.esocial.views.s1200_evtremun_gerar_xml import gerar_xml_s1200
    from emensageriapro.functions import identidade_evento

    if request.user.has_perm('esocial.can_create_change_s1200evtRemun'):

        s1200_evtremun = get_object_or_404(
            s1200evtRemun,
            id=pk)

        texto = gerar_xml_s1200(request, pk, versao="|")
        texto = texto.replace('<inclusao>','<alteracao>').replace('</inclusao>','</alteracao>')
        dados = read_s1200_evtremun_string(request, {}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s1200_evtremun, 'esocial')

        s1200evtRemun.objects.filter(id=dados['id']).\
        ...
        return redirect('s1200_evtremun_salvar', pk=return_pk)
        ...
        return redirect('s1200_evtremun_salvar', pk=pk)