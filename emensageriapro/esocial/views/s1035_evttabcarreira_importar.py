...
def read_s1035_evttabcarreira(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s1035_evttabcarreira_obj(request, doc, status, validar, arquivo)

    s1035evtTabCarreira.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s1035_evttabcarreira_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s1035_evttabcarreira_dados['arquivo'] = arquivo
