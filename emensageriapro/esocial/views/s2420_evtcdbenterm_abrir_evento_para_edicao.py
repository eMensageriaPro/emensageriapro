...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2420evtCdBenTerm'):

        if s2420_evtcdbenterm_id:
            s2420_evtcdbenterm = get_object_or_404(s2420evtCdBenTerm, excluido=False, id=s2420_evtcdbenterm_id)
            ...
            if s2420_evtcdbenterm.status in status_list:
                s2420evtCdBenTerm.objects.filter(id=s2420_evtcdbenterm_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
