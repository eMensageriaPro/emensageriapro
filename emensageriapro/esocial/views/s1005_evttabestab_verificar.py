...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1005_evttabestab_verificar.html' % s1005_evttabestab.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1005_evttabestab_verificar.html' % s1005_evttabestab.versao,
            ...
            response =  render_to_response('%s/s1005_evttabestab_verificar.html' % s1005_evttabestab.versao, context)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s1005_evttabestab = get_object_or_404(
            ...
            excluido = False, id = s1005_evttabestab_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1005_evttabestab.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s1005_evttabestab_recibo_pdf.html', context)
            filename = "%s.xls" % s1005_evttabestab.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s1005_evttabestab_recibo_csv.html', context)
            filename = "%s.csv" % s1005_evttabestab.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s1005_evttabestab_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s1005_evttabestab_id, db_slug):
    import os
    ...
    if s1005_evttabestab.status in (0,1,2,11):
        s1005evtTabEstab.objects.using(db_slug).filter(id=s1005_evttabestab_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash):
    ...
    if s1005_evttabestab_id:
        s1005_evttabestab = get_object_or_404(s1005evtTabEstab.objects.using(db_slug), excluido=False, id=s1005_evttabestab_id)
        xml_assinado = gerar_xml_assinado(s1005_evttabestab_id, db_slug)
...
def duplicar(request, hash):
    ...
    s1005_evttabestab_id = int(dict_hash['id'])
    if s1005_evttabestab_id:
        s1005_evttabestab = get_object_or_404(s1005evtTabEstab.objects.using(db_slug), excluido=False,
                                                    id=s1005_evttabestab_id)

        arquivo = 'arquivos/Eventos/s1005_evttabestab/%s.xml' % s1005_evttabestab.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1005_evttabestab_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1005_evttabestab/%s.xml' % s1005_evttabestab.identidade)
        salvar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_duplicado_temp.xml' % s1005_evttabestab.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_duplicado_temp.xml' % s1005_evttabestab.identidade, request)
        from emensageriapro.esocial.views.s1005_evttabestab import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1005evtTabEstab.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1005_evttabestab.identidade),
            's1005_evttabestab', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1005_evttabestab_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_alteracao_temp.xml' % s1005_evttabestab.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_alteracao_temp.xml' % s1005_evttabestab.identidade, request)
        from emensageriapro.esocial.views.s1005_evttabestab import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1005evtTabEstab.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1005_evttabestab.identidade),
            's1005_evttabestab', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1005_evttabestab_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_exclusao_temp.xml' % s1005_evttabestab.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1005_evttabestab/%s_exclusao_temp.xml' % s1005_evttabestab.identidade, request)
        from emensageriapro.esocial.views.s1005_evttabestab import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1005evtTabEstab.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1005_evttabestab.identidade),
            's1005_evttabestab', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1005_evttabestab_id:
        ...
        if s1005_evttabestab.status == 0:
            from emensageriapro.esocial.views.s1005_evttabestab import identidade_evento
            dent = identidade_evento(s1005_evttabestab_id, db_slug)
            ...
            's1005_evttabestab', s1005_evttabestab_id, usuario_id, 1)
            return redirect('s1005_evttabestab_salvar', hash=url_hash)
        else:
    ...
    if s1005_evttabestab_id:
        ...
        if s1005_evttabestab.status in (0, 1, 2, 3, 4, 10, 11) or s1005_evttabestab.processamento_codigo_resposta in (401,402):
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s1005_evttabestab.identidade, BASE_DIR, s1005_evttabestab.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1005_evttabestab/%s.xml %s/arquivos/Eventos/s1005_evttabestab/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s1005_evttabestab/%s_backup_%s.xml' % (s1005_evttabestab.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s1005_evttabestab_id, db_slug):
    ...
    lista_validacoes = []
    s1005_evttabestab = get_object_or_404(s1005evtTabEstab.objects.using(db_slug), excluido=False, id=s1005_evttabestab_id)
    if s1005_evttabestab.transmissor_lote_esocial:
        if s1005_evttabestab.transmissor_lote_esocial.transmissor:
            if s1005_evttabestab.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's1005_evttabestab', s1005_evttabestab_id)
                if quant <= 0:
...
def validar_evento(request, hash):
    ...
    if s1005_evttabestab_id:
        lista_validacoes = []
        s1005_evttabestab = get_object_or_404(s1005evtTabEstab.objects.using(db_slug), excluido=False, id=s1005_evttabestab_id)
        if s1005_evttabestab.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1005_evttabestab_id, db_slug)
