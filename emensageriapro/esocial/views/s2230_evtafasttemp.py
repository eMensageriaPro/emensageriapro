...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2230_evtafasttemp.status == 0:
            ...
            s2230evtAfastTemp.objects.using( db_slug ).filter(id = s2230_evtafasttemp_id).delete()
...
def identidade_evento(s2230_evtafasttemp_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2230evtAfastTemp.objects.using( db_slug ).get(id=s2230_evtafasttemp_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2230evtAfastTemp.objects.using(db_slug).filter(id=s2230_evtafasttemp_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s2230_evtafasttemp_form.is_valid():
                ...
                if s2230_evtafasttemp_id:
                    if s2230_evtafasttemp.status == 0:
                        ...
                        #s2230_evtafasttemp_campos_multiple_passo1
                        s2230evtAfastTemp.objects.using(db_slug).filter(id=s2230_evtafasttemp_id).update(**dados)
                        obj = s2230evtAfastTemp.objects.using(db_slug).get(id=s2230_evtafasttemp_id)
                        #s2230_evtafasttemp_editar_custom
                        #s2230_evtafasttemp_campos_multiple_passo2
                    ...
                    #s2230_evtafasttemp_cadastrar_campos_multiple_passo1
                    obj = s2230evtAfastTemp(**dados)
                    obj.save(using = db_slug)
                    #s2230_evtafasttemp_cadastrar_custom
                    #s2230_evtafasttemp_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s2230_evtafasttemp_form = form_s2230_evtafasttemp(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s2230_evtafasttemp_form.fields['tpamb'].widget.attrs['disabled'] = True
            s2230_evtafasttemp_form.fields['tpamb'].widget.attrs['readonly'] = True
            s2230_evtafasttemp_form.fields['tpamb'].value = TP_AMB
            s2230_evtafasttemp_form.fields['procemi'].widget.attrs['disabled'] = True
            s2230_evtafasttemp_form.fields['procemi'].widget.attrs['readonly'] = True
            ...
            s2230_evtafasttemp_form.fields['status'].widget.attrs['disabled'] = True
            s2230_evtafasttemp_form.fields['status'].widget.attrs['readonly'] = True
            s2230_evtafasttemp_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
