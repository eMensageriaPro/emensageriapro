...
def validar_evento_funcao(request, s1295_evttotconting_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1295_evttotconting_id)
...
def validar_evento(request, hash):
    ...
    if s1295_evttotconting_id:
        ...
        if s1295_evttotconting.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1295_evttotconting_id)
            
            if s1295_evttotconting.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1295evtTotConting.objects.\
                    filter(id=s1295_evttotconting_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1295_evttotconting.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1295evtTotConting.objects.\
                    filter(id=s1295_evttotconting_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
