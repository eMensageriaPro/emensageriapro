...
def gerar_identidade(request, chave, evento_id):
    ...
    if chave == PASS_SCRIPT:
        db_slug = 'default'
        obj = get_object_or_404(s1035evtTabCarreira.objects.using( db_slug ), excluido = False, id = evento_id)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1035_evttabcarreira_id:
            ...
            s1035_evttabcarreira_form = form_s1035_evttabcarreira(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1035_evttabcarreira_form.is_valid():
                ...
                if s1035_evttabcarreira_id:
                    ...
                    s1035_evttabcarreira_form = form_s1035_evttabcarreira(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
