...
def validar_evento_funcao(request, s2221_evttoxic_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2221_evttoxic_id)
...
def validar_evento(request, hash):
    ...
    if s2221_evttoxic_id:
        ...
        if s2221_evttoxic.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2221_evttoxic_id)
            
            if s2221_evttoxic.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2221evtToxic.objects.\
                    filter(id=s2221_evttoxic_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2221_evttoxic.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2221evtToxic.objects.\
                    filter(id=s2221_evttoxic_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
