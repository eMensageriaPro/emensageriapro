...
def validacoes_s2240_evtexprisco(arquivo):
    ...
    if 'indRetif' in dir(evtExpRisco.ideEvento): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEvento.indRetif', evtExpRisco.ideEvento.indRetif.cdata, 1, u'1;2')
    if 'nrRecibo' in dir(evtExpRisco.ideEvento): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEvento.nrRecibo', evtExpRisco.ideEvento.nrRecibo.cdata, 0, u'')
    if 'tpAmb' in dir(evtExpRisco.ideEvento): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEvento.tpAmb', evtExpRisco.ideEvento.tpAmb.cdata, 1, u'1;2')
    if 'procEmi' in dir(evtExpRisco.ideEvento): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEvento.procEmi', evtExpRisco.ideEvento.procEmi.cdata, 1, u'1;2;3;4;5')
    if 'verProc' in dir(evtExpRisco.ideEvento): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEvento.verProc', evtExpRisco.ideEvento.verProc.cdata, 1, u'')
    if 'tpInsc' in dir(evtExpRisco.ideEmpregador): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEmpregador.tpInsc', evtExpRisco.ideEmpregador.tpInsc.cdata, 1, u'1;2;3;4')
    if 'nrInsc' in dir(evtExpRisco.ideEmpregador): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideEmpregador.nrInsc', evtExpRisco.ideEmpregador.nrInsc.cdata, 1, u'')
    if 'cpfTrab' in dir(evtExpRisco.ideVinculo): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideVinculo.cpfTrab', evtExpRisco.ideVinculo.cpfTrab.cdata, 1, u'')
    if 'nisTrab' in dir(evtExpRisco.ideVinculo): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideVinculo.nisTrab', evtExpRisco.ideVinculo.nisTrab.cdata, 0, u'')
    if 'matricula' in dir(evtExpRisco.ideVinculo): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideVinculo.matricula', evtExpRisco.ideVinculo.matricula.cdata, 0, u'')
    if 'codCateg' in dir(evtExpRisco.ideVinculo): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.ideVinculo.codCateg', evtExpRisco.ideVinculo.codCateg.cdata, 0, u'')
    if 'dtIniCondicao' in dir(evtExpRisco.infoExpRisco): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.infoExpRisco.dtIniCondicao', evtExpRisco.infoExpRisco.dtIniCondicao.cdata, 1, u'')
    if 'dscAtivDes' in dir(evtExpRisco.infoExpRisco.infoAtiv): validacoes_lista = validar_campo(validacoes_lista,'evtExpRisco.infoExpRisco.infoAtiv.dscAtivDes', evtExpRisco.infoExpRisco.infoAtiv.dscAtivDes.cdata, 1, u'')
    if 'infoAmb' in dir(evtExpRisco.infoExpRisco):
        for infoAmb in evtExpRisco.infoExpRisco.infoAmb:

            if 'codAmb' in dir(infoAmb): validacoes_lista = validar_campo(validacoes_lista,'infoAmb.codAmb', infoAmb.codAmb.cdata, 1, u'')
    ...
    if 'respReg' in dir(evtExpRisco.infoExpRisco):
        for respReg in evtExpRisco.infoExpRisco.respReg:
            ...
            if 'dtIni' in dir(respReg): validacoes_lista = validar_campo(validacoes_lista,'respReg.dtIni', respReg.dtIni.cdata, 1, u'')
            if 'dtFim' in dir(respReg): validacoes_lista = validar_campo(validacoes_lista,'respReg.dtFim', respReg.dtFim.cdata, 0, u'')
            if 'nisResp' in dir(respReg): validacoes_lista = validar_campo(validacoes_lista,'respReg.nisResp', respReg.nisResp.cdata, 1, u'')
            if 'nrOc' in dir(respReg): validacoes_lista = validar_campo(validacoes_lista,'respReg.nrOc', respReg.nrOc.cdata, 1, u'')
            if 'ufOC' in dir(respReg): validacoes_lista = validar_campo(validacoes_lista,'respReg.ufOC', respReg.ufOC.cdata, 0, u'')
