...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2206_evtaltcontratual_verificar.html' % s2206_evtaltcontratual.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2206_evtaltcontratual_verificar.html' % s2206_evtaltcontratual.versao,
            ...
            response =  render_to_response('%s/s2206_evtaltcontratual_verificar.html' % s2206_evtaltcontratual.versao, context)
...
def gerar_xml_assinado(s2206_evtaltcontratual_id, db_slug):
    ...
    if s2206_evtaltcontratual.status in (0,1,2,11):
        s2206evtAltContratual.objects.using(db_slug).filter(id=s2206_evtaltcontratual_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2206_evtaltcontratual_id = int(dict_hash['id'])
    if s2206_evtaltcontratual_id:
        s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual.objects.using(db_slug), excluido=False,
                                                    id=s2206_evtaltcontratual_id)

        arquivo = 'arquivos/Eventos/s2206_evtaltcontratual/%s.xml' % s2206_evtaltcontratual.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2206_evtaltcontratual_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s.xml' % s2206_evtaltcontratual.identidade)
        salvar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_duplicado_temp.xml' % s2206_evtaltcontratual.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_duplicado_temp.xml' % s2206_evtaltcontratual.identidade, request)
        from emensageriapro.esocial.views.s2206_evtaltcontratual import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2206evtAltContratual.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2206_evtaltcontratual.identidade),
            's2206_evtaltcontratual', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_alteracao_temp.xml' % s2206_evtaltcontratual.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_alteracao_temp.xml' % s2206_evtaltcontratual.identidade, request)
        from emensageriapro.esocial.views.s2206_evtaltcontratual import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2206evtAltContratual.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2206_evtaltcontratual.identidade),
            's2206_evtaltcontratual', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_exclusao_temp.xml' % s2206_evtaltcontratual.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2206_evtaltcontratual/%s_exclusao_temp.xml' % s2206_evtaltcontratual.identidade, request)
        from emensageriapro.esocial.views.s2206_evtaltcontratual import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2206evtAltContratual.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2206_evtaltcontratual.identidade),
            's2206_evtaltcontratual', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        ...
        if s2206_evtaltcontratual.status == 0:
            from emensageriapro.esocial.views.s2206_evtaltcontratual import identidade_evento
            dent = identidade_evento(s2206_evtaltcontratual_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual.objects.using(db_slug), excluido=False, id=s2206_evtaltcontratual_id)
        if s2206_evtaltcontratual.status in (0, 1, 2, 3, 4, 10, 11) or s2206_evtaltcontratual.processamento_codigo_resposta in (401,402):
            s2206evtAltContratual.objects.using(db_slug).filter(id=s2206_evtaltcontratual_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2206_evtaltcontratual/%s.xml' % (s2206_evtaltcontratual.identidade)
...
def validar_evento_funcao(s2206_evtaltcontratual_id, db_slug):
    ...
    lista_validacoes = []
    s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual.objects.using(db_slug), excluido=False, id=s2206_evtaltcontratual_id)
    if s2206_evtaltcontratual.transmissor_lote_esocial:
        if s2206_evtaltcontratual.transmissor_lote_esocial.transmissor:
            if s2206_evtaltcontratual.transmissor_lote_esocial.transmissor.verificar_predecessao:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s2206_evtaltcontratual SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s2206_evtaltcontratual_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s2206_evtaltcontratual/%s.xml' % (s2206_evtaltcontratual.identidade)
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2206_evtaltcontratual import validacoes_s2206_evtaltcontratual
                lista = validacoes_s2206_evtaltcontratual(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2206_evtaltcontratual import validacoes_s2206_evtaltcontratual
                lista = validacoes_s2206_evtaltcontratual(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s2206_evtaltcontratual SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s2206_evtaltcontratual_id), False)
    else:
        executar_sql("UPDATE public.s2206_evtaltcontratual SET validacoes='', status=4 WHERE id=%s;" % (s2206_evtaltcontratual_id), False)
...
def validar_evento(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        lista_validacoes = []
        s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual.objects.using(db_slug), excluido=False, id=s2206_evtaltcontratual_id)
        if s2206_evtaltcontratual.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2206_evtaltcontratual_id, db_slug)
