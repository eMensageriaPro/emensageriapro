...
class s1298evtReabreEvPerList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1298evtReabreEvPer.objects.using(db_slug).all()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if s1298_evtreabreevper_form.is_valid():
                ...
                if s1298_evtreabreevper_id:
                    ...
                    s1298_evtreabreevper_form = form_s1298_evtreabreevper(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        #             s1298_evtreabreevper_form.fields['status'].widget.attrs['disabled'] = True
        #             s1298_evtreabreevper_form.fields['status'].widget.attrs['readonly'] = True
        #             s1298_evtreabreevper_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1298_evtreabreevper_salvar.html',
                filename="s1298_evtreabreevper.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1298_evtreabreevper_salvar.html', context)
            filename = "s1298_evtreabreevper.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
