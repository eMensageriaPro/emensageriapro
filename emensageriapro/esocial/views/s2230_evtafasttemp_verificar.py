...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            return render(request, 's2230_evtafasttemp_verificar.html', context)
...
def gerar_xml_s2230(s2230_evtafasttemp_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2230_evtafasttemp_id, db_slug):
    ...
    if s2230_evtafasttemp.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2230evtAfastTemp.objects.using(db_slug).\
            filter(id=s2230_evtafasttemp_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2230_evtafasttemp_importar import read_s2230_evtafasttemp_string
    ...
    if s2230_evtafasttemp_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2230_evtafasttemp.identidade),
    ...
    if s2230_evtafasttemp_id:
        ...
        if s2230_evtafasttemp.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2230_evtafasttemp', s2230_evtafasttemp_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2230_evtafasttemp_id:
        ...
        if s2230_evtafasttemp.status in status_list:
            s2230evtAfastTemp.objects.using(db_slug).filter(id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2230_evtafasttemp/%s.xml' % (s2230_evtafasttemp.identidade)
...
def validar_evento_funcao(s2230_evtafasttemp_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2230_evtafasttemp_validar import validacoes_s2230_evtafasttemp
    ...
    if lista_validacoes:
        ...
        s2230evtAfastTemp.objects.using( db_slug ).\
            filter(id=s2230_evtafasttemp_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2230_evtafasttemp_id:
        ...
        if s2230_evtafasttemp.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2230_evtafasttemp_id, db_slug)

            if s2230_evtafasttemp.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2230evtAfastTemp.objects.using(db_slug).\
                    filter(excluido=False, id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2230_evtafasttemp.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2230evtAfastTemp.objects.using(db_slug).\
                    filter(excluido=False, id=s2230_evtafasttemp_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
