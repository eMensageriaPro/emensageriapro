...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2240_evtexprisco_verificar.html' % s2240_evtexprisco.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2240_evtexprisco_verificar.html' % s2240_evtexprisco.versao,
            ...
            response =  render_to_response('%s/s2240_evtexprisco_verificar.html' % s2240_evtexprisco.versao, context)
...
def gerar_xml_assinado(s2240_evtexprisco_id, db_slug):
    ...
    if s2240_evtexprisco.status in (0,1,2,11):
        s2240evtExpRisco.objects.using(db_slug).filter(id=s2240_evtexprisco_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2240_evtexprisco_id = int(dict_hash['id'])
    if s2240_evtexprisco_id:
        s2240_evtexprisco = get_object_or_404(s2240evtExpRisco.objects.using(db_slug), excluido=False,
                                                    id=s2240_evtexprisco_id)

        arquivo = 'arquivos/Eventos/s2240_evtexprisco/%s.xml' % s2240_evtexprisco.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2240_evtexprisco_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2240_evtexprisco/%s.xml' % s2240_evtexprisco.identidade)
        salvar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_duplicado_temp.xml' % s2240_evtexprisco.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_duplicado_temp.xml' % s2240_evtexprisco.identidade, request)
        from emensageriapro.esocial.views.s2240_evtexprisco import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2240evtExpRisco.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2240_evtexprisco.identidade),
            's2240_evtexprisco', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2240_evtexprisco_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_alteracao_temp.xml' % s2240_evtexprisco.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_alteracao_temp.xml' % s2240_evtexprisco.identidade, request)
        from emensageriapro.esocial.views.s2240_evtexprisco import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2240evtExpRisco.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2240_evtexprisco.identidade),
            's2240_evtexprisco', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2240_evtexprisco_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_exclusao_temp.xml' % s2240_evtexprisco.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2240_evtexprisco/%s_exclusao_temp.xml' % s2240_evtexprisco.identidade, request)
        from emensageriapro.esocial.views.s2240_evtexprisco import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2240evtExpRisco.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2240_evtexprisco.identidade),
            's2240_evtexprisco', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2240_evtexprisco_id:
        ...
        if s2240_evtexprisco.status == 0:
            from emensageriapro.esocial.views.s2240_evtexprisco import identidade_evento
            dent = identidade_evento(s2240_evtexprisco_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2240_evtexprisco_id:
        s2240_evtexprisco = get_object_or_404(s2240evtExpRisco.objects.using(db_slug), excluido=False, id=s2240_evtexprisco_id)
        if s2240_evtexprisco.status in (0, 1, 2, 3, 4, 10, 11) or s2240_evtexprisco.processamento_codigo_resposta in (401,402):
            s2240evtExpRisco.objects.using(db_slug).filter(id=s2240_evtexprisco_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2240_evtexprisco/%s.xml' % (s2240_evtexprisco.identidade)
...
def validar_evento_funcao(s2240_evtexprisco_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2240_evtexprisco import validacoes_s2240_evtexprisco
                lista = validacoes_s2240_evtexprisco(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2240_evtexprisco import validacoes_s2240_evtexprisco
                lista = validacoes_s2240_evtexprisco(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s2240_evtexprisco SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s2240_evtexprisco_id), False)
    else:
        executar_sql("UPDATE public.s2240_evtexprisco SET validacoes='', status=4 WHERE id=%s;" % (s2240_evtexprisco_id), False)
...
def validar_evento(request, hash):
    ...
    if s2240_evtexprisco_id:
        lista_validacoes = []
        s2240_evtexprisco = get_object_or_404(s2240evtExpRisco.objects.using(db_slug), excluido=False, id=s2240_evtexprisco_id)
        if s2240_evtexprisco.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2240_evtexprisco_id, db_slug)
