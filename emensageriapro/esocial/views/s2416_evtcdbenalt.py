...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2416_evtcdbenalt_id:
            ...
            s2416_evtcdbenalt_form = form_s2416_evtcdbenalt(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
            ...
            if s2416_evtcdbenalt_form.is_valid():
                ...
                if s2416_evtcdbenalt_id:
                    if s2416_evtcdbenalt.status == 0:
                        ...
                        #s2416_evtcdbenalt_campos_multiple_passo1
                        s2416evtCdBenAlt.objects.using(db_slug).filter(id=s2416_evtcdbenalt_id).update(**dados)
                        obj = s2416evtCdBenAlt.objects.using(db_slug).get(id=s2416_evtcdbenalt_id)
                        #s2416_evtcdbenalt_editar_custom
                        #s2416_evtcdbenalt_campos_multiple_passo2
                    ...
                    #s2416_evtcdbenalt_cadastrar_campos_multiple_passo1
                    obj = s2416evtCdBenAlt(**dados)
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2416_evtcdbenalt.status == 0:
            ...
            s2416evtCdBenAlt.objects.using( db_slug ).filter(id = s2416_evtcdbenalt_id).delete()
