    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s2260_evtconvinterm_verificar.html',
            ...
            response =  render_to_response('s2260_evtconvinterm_verificar.html', context)
            ...
            return render(request, 's2260_evtconvinterm_verificar.html', context)
...
def gerar_xml_s2260(s2260_evtconvinterm_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2260_evtconvinterm_id, db_slug):
    ...
    if s2260_evtconvinterm.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2260evtConvInterm.objects.using(db_slug).\
            filter(id=s2260_evtconvinterm_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        texto = gerar_xml_s2260(s2260_evtconvinterm_id, db_slug, versao="|")
        dados = read_s2260_evtconvinterm_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2260_evtconvinterm)

        s2260evtConvInterm.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2260_evtconvinterm.identidade),
            's2260_evtconvinterm', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2260_evtconvinterm.identidade),
            's2260_evtconvinterm', dados['id'], request.user.id, 1)
    ...
    if s2260_evtconvinterm_id:
        ...
        if s2260_evtconvinterm.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2260_evtconvinterm', s2260_evtconvinterm_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        if s2260_evtconvinterm.status in status_list:
            s2260evtConvInterm.objects.using(db_slug).filter(id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2260_evtconvinterm/%s.xml' % (s2260_evtconvinterm.identidade)
...
def validar_evento_funcao(s2260_evtconvinterm_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2260_evtconvinterm_validar import validacoes_s2260_evtconvinterm
    ...
    if lista_validacoes:
        ...
        s2260evtConvInterm.objects.using( db_slug ).\
            filter(id=s2260_evtconvinterm_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        if s2260_evtconvinterm.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2260_evtconvinterm_id, db_slug)

            if s2260_evtconvinterm.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2260evtConvInterm.objects.using(db_slug).\
                    filter(excluido=False, id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2260_evtconvinterm.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2260evtConvInterm.objects.using(db_slug).\
                    filter(excluido=False, id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
