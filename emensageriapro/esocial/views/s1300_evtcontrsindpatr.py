...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1300_evtcontrsindpatr')
    ...
    if request.method == 'POST':
        if s1300_evtcontrsindpatr.status == 0:
            ...
            s1300evtContrSindPatr.objects.using( db_slug ).filter(id = s1300_evtcontrsindpatr_id).delete()
        ...
        if request.session['retorno_pagina']== 's1300_evtcontrsindpatr_salvar':
            return redirect('s1300_evtcontrsindpatr', hash=request.session['retorno_hash'])
        else:
...
class s1300evtContrSindPatrList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1300evtContrSindPatr.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            post = True
            dict_fields = {
                'evtcontrsindpatr': 'evtcontrsindpatr',
        ...
        s1300_evtcontrsindpatr_lista = s1300evtContrSindPatr.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1300_evtcontrsindpatr_lista) > 100:
            filtrar = True
            s1300_evtcontrsindpatr_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        transmissor_lote_esocial_lista = TransmissorLoteEsocial.objects.using( db_slug ).filter(excluido = False).all()
        #s1300_evtcontrsindpatr_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 's1300_evtcontrsindpatr'
        context = {
        ...
        #return render(request, 's1300_evtcontrsindpatr_listar.html', context)
        if for_print in (0,1):
            return render(request, 's1300_evtcontrsindpatr_listar.html', context)
        elif for_print == 2:
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1300_evtcontrsindpatr_listar.html',
            ...
            response = render_to_response('s1300_evtcontrsindpatr_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1300_evtcontrsindpatr_csv.html', context)
            filename = "s1300_evtcontrsindpatr.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s1300_evtcontrsindpatr_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1300evtContrSindPatr.objects.using( db_slug ).get(id=s1300_evtcontrsindpatr_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1300evtContrSindPatr.objects.using(db_slug).filter(id=s1300_evtcontrsindpatr_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s1300_evtcontrsindpatr_form.is_valid():
                ...
                if s1300_evtcontrsindpatr_id:
                    if s1300_evtcontrsindpatr.status == 0:
                        ...
                        #s1300_evtcontrsindpatr_campos_multiple_passo1
                        s1300evtContrSindPatr.objects.using(db_slug).filter(id=s1300_evtcontrsindpatr_id).update(**dados)
                        obj = s1300evtContrSindPatr.objects.using(db_slug).get(id=s1300_evtcontrsindpatr_id)
                        #s1300_evtcontrsindpatr_editar_custom
                        #s1300_evtcontrsindpatr_campos_multiple_passo2
                    ...
                    #s1300_evtcontrsindpatr_cadastrar_campos_multiple_passo1
                    obj = s1300evtContrSindPatr(**dados)
                    obj.save(using = db_slug)
                    #s1300_evtcontrsindpatr_cadastrar_custom
                    #s1300_evtcontrsindpatr_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1300_evtcontrsindpatr_form = form_s1300_evtcontrsindpatr(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        s1300_evtcontrsindpatr_form = disabled_form_fields(s1300_evtcontrsindpatr_form, permissao.permite_editar)
        ...
        if s1300_evtcontrsindpatr_id:
            ...
            s1300_contribsind_form = form_s1300_contribsind(initial={ 's1300_evtcontrsindpatr': s1300_evtcontrsindpatr }, slug=db_slug)
            s1300_contribsind_form.fields['s1300_evtcontrsindpatr'].widget.attrs['readonly'] = True
            s1300_contribsind_lista = s1300contribSind.objects.using( db_slug ).filter(excluido = False, s1300_evtcontrsindpatr_id=s1300_evtcontrsindpatr.id).all()
        else:
        ...
        #             s1300_evtcontrsindpatr_form.fields['tpamb'].widget.attrs['disabled'] = True
        #             s1300_evtcontrsindpatr_form.fields['tpamb'].widget.attrs['readonly'] = True
        #             s1300_evtcontrsindpatr_form.fields['tpamb'].value = TP_AMB
        #             s1300_evtcontrsindpatr_form.fields['procemi'].widget.attrs['disabled'] = True
        #             s1300_evtcontrsindpatr_form.fields['procemi'].widget.attrs['readonly'] = True
        #             s1300_evtcontrsindpatr_form.fields['procemi'].value = 1
        #             s1300_evtcontrsindpatr_form.fields['verproc'].widget.attrs['readonly'] = True
        #             s1300_evtcontrsindpatr_form.fields['verproc'].value = VERSAO_EMENSAGERIA
        #             s1300_evtcontrsindpatr_form.fields['status'].widget.attrs['disabled'] = True
        #             s1300_evtcontrsindpatr_form.fields['status'].widget.attrs['readonly'] = True
        #             s1300_evtcontrsindpatr_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1300_evtcontrsindpatr_id, tabela='s1300_evtcontrsindpatr').all()
        context = {
