...
def validar_evento_funcao(request, s1200_evtremun_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1200_evtremun_id)
...
def validar_evento(request, hash):
    ...
    if s1200_evtremun_id:
        ...
        if s1200_evtremun.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1200_evtremun_id)
            
            if s1200_evtremun.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1200evtRemun.objects.\
                    filter(id=s1200_evtremun_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1200_evtremun.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1200evtRemun.objects.\
                    filter(id=s1200_evtremun_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
