...
def validar_evento_funcao(request, s1210_evtpgtos_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1210_evtpgtos_id)
...
def validar_evento(request, hash):
    ...
    if s1210_evtpgtos_id:
        ...
        if s1210_evtpgtos.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1210_evtpgtos_id)
            
            if s1210_evtpgtos.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1210evtPgtos.objects.\
                    filter(id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1210_evtpgtos.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1210evtPgtos.objects.\
                    filter(id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
