...
def read_s5011_evtcs(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s5011_evtcs_obj(request, doc, status, validar, arquivo)

    s5011evtCS.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s5011_evtcs_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s5011_evtcs_dados['arquivo'] = arquivo
