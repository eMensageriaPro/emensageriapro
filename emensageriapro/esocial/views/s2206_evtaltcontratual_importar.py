...
def read_s2206_evtaltcontratual(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s2206_evtaltcontratual_obj(request, doc, status, validar, arquivo)

    s2206evtAltContratual.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2206_evtaltcontratual_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s2206_evtaltcontratual_dados['arquivo'] = arquivo
