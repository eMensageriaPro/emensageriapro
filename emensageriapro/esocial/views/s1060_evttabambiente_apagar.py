...
def apagar(request, pk):
    ...
    if request.method == 'POST':
        ...
        if s1060_evttabambiente.status == STATUS_EVENTO_CADASTRADO:
            ...
            situacao_anterior = json.dumps(model_to_dict(s1060_evttabambiente), sort_keys=True, default=str)
            obj = s1060evtTabAmbiente.objects.get(id=pk)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1060_evttabambiente', pk, request.user.id, 3)
        else:

            messages.error(request, u'''Não foi possivel apagar o evento, somente é
                                        possível apagar os eventos com status "Cadastrado"!''')

        if 'return_page' in request.session and request.session['return_page'] and 's1060-evttabambiente' in request.session['return_page']:
            return redirect('s1060_evttabambiente')
