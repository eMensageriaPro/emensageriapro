    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s2405_evtcdbenefalt_verificar.html',
            ...
            response =  render_to_response('s2405_evtcdbenefalt_verificar.html', context)
            ...
            return render(request, 's2405_evtcdbenefalt_verificar.html', context)
...
def gerar_xml_s2405(s2405_evtcdbenefalt_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2405_evtcdbenefalt_id, db_slug):
    ...
    if s2405_evtcdbenefalt.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2405evtCdBenefAlt.objects.using(db_slug).\
            filter(id=s2405_evtcdbenefalt_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2405_evtcdbenefalt_importar import read_s2405_evtcdbenefalt_string
    ...
    if s2405_evtcdbenefalt_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2405_evtcdbenefalt.identidade),
    ...
    if s2405_evtcdbenefalt_id:
        ...
        if s2405_evtcdbenefalt.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2405_evtcdbenefalt', s2405_evtcdbenefalt_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2405_evtcdbenefalt_id:
        ...
        if s2405_evtcdbenefalt.status in status_list:
            s2405evtCdBenefAlt.objects.using(db_slug).filter(id=s2405_evtcdbenefalt_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2405_evtcdbenefalt/%s.xml' % (s2405_evtcdbenefalt.identidade)
...
def validar_evento_funcao(s2405_evtcdbenefalt_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2405_evtcdbenefalt_validar import validacoes_s2405_evtcdbenefalt
    ...
    if lista_validacoes:
        ...
        s2405evtCdBenefAlt.objects.using( db_slug ).\
            filter(id=s2405_evtcdbenefalt_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2405_evtcdbenefalt_id:
        ...
        if s2405_evtcdbenefalt.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2405_evtcdbenefalt_id, db_slug)

            if s2405_evtcdbenefalt.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2405evtCdBenefAlt.objects.using(db_slug).\
                    filter(excluido=False, id=s2405_evtcdbenefalt_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2405_evtcdbenefalt.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2405evtCdBenefAlt.objects.using(db_slug).\
                    filter(excluido=False, id=s2405_evtcdbenefalt_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
