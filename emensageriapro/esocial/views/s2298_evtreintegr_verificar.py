...
def gerar_xml_assinado(s2298_evtreintegr_id, db_slug):
    ...
    if s2298_evtreintegr.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2298evtReintegr.objects.using(db_slug).\
            filter(id=s2298_evtreintegr_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2298_evtreintegr_id:
        ...
        if s2298_evtreintegr.status in status_list:
...
def validar_evento(request, hash):
    ...
    if s2298_evtreintegr_id:
        ...
        if s2298_evtreintegr.versao in VERSOES_ESOCIAL:
            ...
            if s2298_evtreintegr.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2298evtReintegr.objects.using(db_slug).\
                    filter(excluido=False, id=s2298_evtreintegr_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2298_evtreintegr.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2298evtReintegr.objects.using(db_slug).\
                    filter(excluido=False, id=s2298_evtreintegr_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
