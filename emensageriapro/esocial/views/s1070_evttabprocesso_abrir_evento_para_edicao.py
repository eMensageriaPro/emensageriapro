...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1070evtTabProcesso'):

        if s1070_evttabprocesso_id:
            s1070_evttabprocesso = get_object_or_404(s1070evtTabProcesso, excluido=False, id=s1070_evttabprocesso_id)
            ...
            if s1070_evttabprocesso.status in status_list:
                s1070evtTabProcesso.objects.filter(id=s1070_evttabprocesso_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
