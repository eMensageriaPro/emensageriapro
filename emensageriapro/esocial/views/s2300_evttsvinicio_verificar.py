...
def gerar_xml_s2300(s2300_evttsvinicio_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2300_evttsvinicio_id, db_slug):
    ...
    if s2300_evttsvinicio.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2300evtTSVInicio.objects.using(db_slug).\
            filter(id=s2300_evttsvinicio_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2300_evttsvinicio_importar import read_s2300_evttsvinicio_string
    ...
    if s2300_evttsvinicio_id:
        ...
        texto = gerar_xml_s2300(s2300_evttsvinicio_id, db_slug, versao="|")
        dados = read_s2300_evttsvinicio_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2300_evttsvinicio)

        s2300evtTSVInicio.objects.using(db_slug).filter(id=dados['id']).\
            update(status=STATUS_EVENTO_CADASTRADO,
                   arquivo_original=0,
                   arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2300_evttsvinicio.identidade),
            's2300_evttsvinicio', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2300_evttsvinicio_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2300_evttsvinicio.identidade),
            's2300_evttsvinicio', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2300_evttsvinicio_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2300_evttsvinicio.identidade),
            's2300_evttsvinicio', dados['id'], request.user.id, 1)
    ...
    if s2300_evttsvinicio_id:
        ...
        if s2300_evttsvinicio.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2300_evttsvinicio', s2300_evttsvinicio_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2300_evttsvinicio_id:
        ...
        if s2300_evttsvinicio.status in status_list:
            s2300evtTSVInicio.objects.using(db_slug).filter(id=s2300_evttsvinicio_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2300_evttsvinicio/%s.xml' % (s2300_evttsvinicio.identidade)
...
def validar_evento_funcao(s2300_evttsvinicio_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2300_evttsvinicio_validar import validacoes_s2300_evttsvinicio
    ...
    if lista_validacoes:
        ...
        s2300evtTSVInicio.objects.using( db_slug ).\
            filter(id=s2300_evttsvinicio_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2300_evttsvinicio_id:
        ...
        if s2300_evttsvinicio.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2300_evttsvinicio_id, db_slug)

            if s2300_evttsvinicio.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2300evtTSVInicio.objects.using(db_slug).\
                    filter(excluido=False, id=s2300_evttsvinicio_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2300_evttsvinicio.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2300evtTSVInicio.objects.using(db_slug).\
                    filter(excluido=False, id=s2300_evttsvinicio_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
