...
def identidade_evento(s2245_evttreicap_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2245evtTreiCap.objects.using( db_slug ).get(id=s2245_evttreicap_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2245evtTreiCap.objects.using(db_slug).filter(id=s2245_evttreicap_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s2245_evttreicap_form.is_valid():
                ...
                if s2245_evttreicap_id:
                    if s2245_evttreicap.status == 0:
                        ...
                        #s2245_evttreicap_campos_multiple_passo1
                        s2245evtTreiCap.objects.using(db_slug).filter(id=s2245_evttreicap_id).update(**dados)
                        obj = s2245evtTreiCap.objects.using(db_slug).get(id=s2245_evttreicap_id)
                        #s2245_evttreicap_editar_custom
                        #s2245_evttreicap_campos_multiple_passo2
                    ...
                    #s2245_evttreicap_cadastrar_campos_multiple_passo1
                    obj = s2245evtTreiCap(**dados)
                    obj.save(using = db_slug)
                    #s2245_evttreicap_cadastrar_custom
                    #s2245_evttreicap_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s2245_evttreicap_form = form_s2245_evttreicap(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s2245_evttreicap_form.fields['tpamb'].widget.attrs['disabled'] = True
            s2245_evttreicap_form.fields['tpamb'].widget.attrs['readonly'] = True
            s2245_evttreicap_form.fields['tpamb'].value = TP_AMB
            s2245_evttreicap_form.fields['procemi'].widget.attrs['disabled'] = True
            s2245_evttreicap_form.fields['procemi'].widget.attrs['readonly'] = True
            ...
            s2245_evttreicap_form.fields['status'].widget.attrs['disabled'] = True
            s2245_evttreicap_form.fields['status'].widget.attrs['readonly'] = True
            s2245_evttreicap_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
