...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1202_evtrmnrpps.status == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s1202_evtrmnrpps), indent=4, sort_keys=True, default=str)
            s1202evtRmnRPPS.objects.using( db_slug ).filter(id = s1202_evtrmnrpps_id).delete()
            #s1202_evtrmnrpps_apagar_custom
            #s1202_evtrmnrpps_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1202_evtrmnrpps', s1202_evtrmnrpps_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 's1202_evtrmnrpps_salvar':
            return redirect('s1202_evtrmnrpps', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's1202_evtrmnrpps_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1202evtRmnRPPSList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1202evtRmnRPPS.objects.using(db_slug).all()
    serializer_class = s1202evtRmnRPPSSerializer
...
def identidade_evento(s1202_evtrmnrpps_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1202evtRmnRPPS.objects.using( db_slug ).get(id=s1202_evtrmnrpps_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1202evtRmnRPPS.objects.using(db_slug).filter(id=s1202_evtrmnrpps_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if not evento_totalizador:
            ...
            s1202_evtrmnrpps_form.fields['status'].widget.attrs['disabled'] = True
            s1202_evtrmnrpps_form.fields['status'].widget.attrs['readonly'] = True
            s1202_evtrmnrpps_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
