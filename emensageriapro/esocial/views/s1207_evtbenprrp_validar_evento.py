...
def validar_evento_funcao(request, s1207_evtbenprrp_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1207_evtbenprrp_id)
...
def validar_evento(request, hash):
    ...
    if s1207_evtbenprrp_id:
        ...
        if s1207_evtbenprrp.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1207_evtbenprrp_id)
            
            if s1207_evtbenprrp.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1207evtBenPrRP.objects.\
                    filter(id=s1207_evtbenprrp_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1207_evtbenprrp.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1207evtBenPrRP.objects.\
                    filter(id=s1207_evtbenprrp_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
