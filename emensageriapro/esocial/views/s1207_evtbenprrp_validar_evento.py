...
def validar_evento_funcao(request, pk):
    ...
    schema_filename = 'esocial/%s/evtBenPrRP.xsd' % (s1207_evtbenprrp.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1207_evtbenprrp' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1207_evtbenprrp', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1207_evtbenprrp.status in STATUS_VALIDACAO:
