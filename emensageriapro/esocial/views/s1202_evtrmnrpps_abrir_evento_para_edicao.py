...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1202evtRmnRPPS'):

        if s1202_evtrmnrpps_id:
            s1202_evtrmnrpps = get_object_or_404(s1202evtRmnRPPS, excluido=False, id=s1202_evtrmnrpps_id)
            ...
            if s1202_evtrmnrpps.status in status_list:
                s1202evtRmnRPPS.objects.filter(id=s1202_evtrmnrpps_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
