...
def gerar_xml_assinado(s2190_evtadmprelim_id, db_slug):
    ...
    if s2190_evtadmprelim.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2190evtAdmPrelim.objects.using(db_slug).\
            filter(id=s2190_evtadmprelim_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def alterar_identidade(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        if s2190_evtadmprelim.status == STATUS_EVENTO_CADASTRADO:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        if s2190_evtadmprelim.status in status_list:
            s2190evtAdmPrelim.objects.using(db_slug).filter(id=s2190_evtadmprelim_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2190_evtadmprelim/%s.xml' % (s2190_evtadmprelim.identidade)
...
def validar_evento(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        if s2190_evtadmprelim.versao in VERSOES_ESOCIAL:
            ...
            if s2190_evtadmprelim.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2190evtAdmPrelim.objects.using(db_slug).\
                    filter(excluido=False, id=s2190_evtadmprelim_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2190_evtadmprelim.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2190evtAdmPrelim.objects.using(db_slug).\
                    filter(excluido=False, id=s2190_evtadmprelim_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
