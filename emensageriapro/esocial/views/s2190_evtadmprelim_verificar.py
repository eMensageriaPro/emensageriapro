...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response =  render_to_response('s2190_evtadmprelim_verificar.html', context)
            ...
            return render(request, 's2190_evtadmprelim_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2190(s2190_evtadmprelim_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2190_evtadmprelim_id, db_slug):
    ...
    if s2190_evtadmprelim.status in (0,1,2,11):
        ...
        s2190evtAdmPrelim.objects.using(db_slug).\
            filter(id=s2190_evtadmprelim_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        texto = gerar_xml_s2190(s2190_evtadmprelim_id, db_slug, versao="|")
        dados = read_s2190_evtadmprelim_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s2190evtAdmPrelim.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2190_evtadmprelim.identidade),
...
def criar_alteracao(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2190_evtadmprelim.identidade),
            's2190_evtadmprelim', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        if s2190_evtadmprelim.status == 0:
            ...
            nova_identidade = identidade_evento(s2190_evtadmprelim_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2190_evtadmprelim', s2190_evtadmprelim_id, request.user.id, 1)
...
def validar_evento_funcao(s2190_evtadmprelim_id, db_slug):
    ...
    s2190evtAdmPrelim.objects.using( db_slug ).\
        filter(id=s2190_evtadmprelim_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2190_evtadmprelim_validar import validacoes_s2190_evtadmprelim
...
<<<<<<< HEAD
        if s2190_evtadmprelim.versao in VERSOES_ESOCIAL:
=======
        if s2190_evtadmprelim.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s2190_evtadmprelim_id, db_slug)
