...
def read_s1080_evttaboperport(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s1080_evttaboperport_obj(request, doc, status, validar, arquivo)

    s1080evtTabOperPort.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s1080_evttaboperport_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s1080_evttaboperport_dados['arquivo'] = arquivo
