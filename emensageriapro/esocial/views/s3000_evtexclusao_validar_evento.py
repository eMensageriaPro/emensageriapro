...
def validar_evento_funcao(request, pk):
    ...
    if not s3000_evtexclusao.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s3000_evtexclusao, 'esocial')
        s3000_evtexclusao = get_object_or_404(s3000evtExclusao, id=pk)
    ...
    schema_filename = 'esocial/%s/evtExclusao.xsd' % (s3000_evtexclusao.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's3000_evtexclusao' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s3000_evtexclusao', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s3000_evtexclusao.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s3000_evtexclusao_salvar', pk=pk)