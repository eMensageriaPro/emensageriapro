...
def validar_evento_funcao(request, s3000_evtexclusao_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s3000_evtexclusao_id)
...
def validar_evento(request, hash):
    ...
    if s3000_evtexclusao_id:
        ...
        if s3000_evtexclusao.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s3000_evtexclusao_id)
            
            if s3000_evtexclusao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s3000evtExclusao.objects.\
                    filter(id=s3000_evtexclusao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s3000_evtexclusao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s3000evtExclusao.objects.\
                    filter(id=s3000_evtexclusao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
