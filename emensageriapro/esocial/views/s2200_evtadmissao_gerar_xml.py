...
def gerar_xml_s2200(request, s2200_evtadmissao_id, versao=None):
    ...
    if s2200_evtadmissao_id:
        ...
        s2200_evtadmissao_lista = s2200evtAdmissao.objects. \
            filter(id=s2200_evtadmissao_id, excluido = False).all()
...
def gerar_xml_assinado(request, s2200_evtadmissao_id):
    ...
    if s2200_evtadmissao.arquivo_original:
        ...
        xml = gerar_xml_s2200(request, s2200_evtadmissao_id)
    ...
    if 'Signature' in xml:
        ...
        xml_assinado = assinar_esocial(request, xml, s2200_evtadmissao.transmissor_lote_esocial_id)
    ...
    if s2200_evtadmissao.status in (
        ...
        s2200evtAdmissao.objects.\
            filter(id=s2200_evtadmissao_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2200_evtadmissao_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s2200_evtadmissao_id)
