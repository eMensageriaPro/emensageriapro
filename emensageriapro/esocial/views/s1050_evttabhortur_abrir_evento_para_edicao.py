...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1050evtTabHorTur'):

        if s1050_evttabhortur_id:
            s1050_evttabhortur = get_object_or_404(s1050evtTabHorTur, excluido=False, id=s1050_evttabhortur_id)
            ...
            if s1050_evttabhortur.status in status_list:
                s1050evtTabHorTur.objects.filter(id=s1050_evttabhortur_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
