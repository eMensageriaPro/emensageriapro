...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5013evtFGTS'):

        if s5013_evtfgts_id:
            s5013_evtfgts = get_object_or_404(s5013evtFGTS, excluido=False, id=s5013_evtfgts_id)
            ...
            if s5013_evtfgts.status in status_list:
                s5013evtFGTS.objects.filter(id=s5013_evtfgts_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
