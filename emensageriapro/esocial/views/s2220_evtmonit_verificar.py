...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2220_evtmonit_lista = s2220evtMonit.objects.using( db_slug ).filter(id=s2220_evtmonit_id, excluido = False).all()
   

        s2220_exame_lista = s2220exame.objects.using(db_slug).filter(s2220_evtmonit_id__in = listar_ids(s2220_evtmonit_lista) ).filter(excluido=False).all()
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 's2220_evtmonit'
        context = {
            's2220_evtmonit_lista': s2220_evtmonit_lista,
            's2220_evtmonit_id': s2220_evtmonit_id,
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2220_evtmonit_verificar.html' % s2220_evtmonit.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2220_evtmonit_verificar.html' % s2220_evtmonit.versao,
            ...
            response =  render_to_response('%s/s2220_evtmonit_verificar.html' % s2220_evtmonit.versao, context)
            filename = "%s.xls" % s2220_evtmonit.identidade
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('%s/s2220_evtmonit_verificar.html' % s2220_evtmonit.versao, context)
            filename = "%s.csv" % s2220_evtmonit.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render(request, '%s/s2220_evtmonit_verificar.html' % s2220_evtmonit.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s2220_evtmonit = get_object_or_404(
            ...
            excluido = False, id = s2220_evtmonit_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s2220_evtmonit.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s2220_evtmonit_recibo_pdf.html', context)
            filename = "%s.xls" % s2220_evtmonit.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s2220_evtmonit_recibo_csv.html', context)
            filename = "%s.csv" % s2220_evtmonit.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s2220_evtmonit_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s2220_evtmonit_id, db_slug):
    import os
    ...
    if s2220_evtmonit.arquivo_original:
        ...
        xml = gerar_xml_s2220(s2220_evtmonit_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_esocial(xml)
    if s2220_evtmonit.status in (0,1,2,11):
        s2220evtMonit.objects.using(db_slug).filter(id=s2220_evtmonit_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash):
    ...
    if s2220_evtmonit_id:
        s2220_evtmonit = get_object_or_404(s2220evtMonit.objects.using(db_slug), excluido=False, id=s2220_evtmonit_id)
        xml_assinado = gerar_xml_assinado(s2220_evtmonit_id, db_slug)
...
def duplicar(request, hash):
    ...
    s2220_evtmonit_id = int(dict_hash['id'])
    if s2220_evtmonit_id:
        s2220_evtmonit = get_object_or_404(s2220evtMonit.objects.using(db_slug), excluido=False,
                                                    id=s2220_evtmonit_id)

        arquivo = 'arquivos/Eventos/s2220_evtmonit/%s.xml' % s2220_evtmonit.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2220_evtmonit_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2220_evtmonit/%s.xml' % s2220_evtmonit.identidade)
        salvar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_duplicado_temp.xml' % s2220_evtmonit.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_duplicado_temp.xml' % s2220_evtmonit.identidade, request)
        from emensageriapro.esocial.views.s2220_evtmonit import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2220evtMonit.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2220_evtmonit.identidade),
            's2220_evtmonit', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_alteracao_temp.xml' % s2220_evtmonit.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_alteracao_temp.xml' % s2220_evtmonit.identidade, request)
        from emensageriapro.esocial.views.s2220_evtmonit import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2220evtMonit.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2220_evtmonit.identidade),
            's2220_evtmonit', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_exclusao_temp.xml' % s2220_evtmonit.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2220_evtmonit/%s_exclusao_temp.xml' % s2220_evtmonit.identidade, request)
        from emensageriapro.esocial.views.s2220_evtmonit import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2220evtMonit.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2220_evtmonit.identidade),
            's2220_evtmonit', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        if s2220_evtmonit.status == 0:
            from emensageriapro.esocial.views.s2220_evtmonit import identidade_evento
            dent = identidade_evento(s2220_evtmonit_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2220_evtmonit_id:
        s2220_evtmonit = get_object_or_404(s2220evtMonit.objects.using(db_slug), excluido=False, id=s2220_evtmonit_id)
        if s2220_evtmonit.status in (0, 1, 2, 3, 4, 10, 11) or s2220_evtmonit.processamento_codigo_resposta in (401,402):
            s2220evtMonit.objects.using(db_slug).filter(id=s2220_evtmonit_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2220_evtmonit/%s.xml' % (s2220_evtmonit.identidade)
...
def validar_evento_funcao(s2220_evtmonit_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2220_evtmonit import validacoes_s2220_evtmonit
                lista = validacoes_s2220_evtmonit(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2220_evtmonit import validacoes_s2220_evtmonit
                lista = validacoes_s2220_evtmonit(arquivo)
    for a in lista:
        if a:
...
def validar_evento(request, hash):
    ...
    if s2220_evtmonit_id:
        lista_validacoes = []
        s2220_evtmonit = get_object_or_404(s2220evtMonit.objects.using(db_slug), excluido=False, id=s2220_evtmonit_id)
        if s2220_evtmonit.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2220_evtmonit_id, db_slug)
