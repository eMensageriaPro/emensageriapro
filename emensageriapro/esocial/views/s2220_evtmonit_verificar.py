    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s2220_evtmonit_verificar.html',
            ...
            response =  render_to_response('s2220_evtmonit_verificar.html', context)
            ...
            return render(request, 's2220_evtmonit_verificar.html', context)
...
def gerar_xml_s2220(s2220_evtmonit_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2220_evtmonit_id, db_slug):
    ...
    if s2220_evtmonit.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2220evtMonit.objects.using(db_slug).\
            filter(id=s2220_evtmonit_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        texto = gerar_xml_s2220(s2220_evtmonit_id, db_slug, versao="|")
        dados = read_s2220_evtmonit_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2220_evtmonit)

        s2220evtMonit.objects.using(db_slug).filter(id=dados['id']).\
            update(status=STATUS_EVENTO_CADASTRADO,
                   arquivo_original=0,
                   arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2220_evtmonit.identidade),
...
def criar_exclusao(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2220_evtmonit.identidade),
            's2220_evtmonit', dados['id'], request.user.id, 1)
    ...
    if s2220_evtmonit_id:
        ...
        if s2220_evtmonit.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2220_evtmonit', s2220_evtmonit_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        if s2220_evtmonit.status in status_list:
            s2220evtMonit.objects.using(db_slug).filter(id=s2220_evtmonit_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2220_evtmonit/%s.xml' % (s2220_evtmonit.identidade)
...
def validar_evento(request, hash):
    ...
    if s2220_evtmonit_id:
        ...
        if s2220_evtmonit.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2220_evtmonit_id, db_slug)

            if s2220_evtmonit.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2220evtMonit.objects.using(db_slug).\
                    filter(excluido=False, id=s2220_evtmonit_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2220_evtmonit.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2220evtMonit.objects.using(db_slug).\
                    filter(excluido=False, id=s2220_evtmonit_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
