...
def listar(request, hash):
    ...
    if request.user.has_perm('esocial.can_view_s2220evtMonit'):
        ...
        if request.method == 'POST':
            ...
            dict_fields = { 
                ...
                'exmedocup': 'exmedocup',
                'tpexameocup__icontains': 'tpexameocup__icontains',
                'aso': 'aso',
                'dtaso__range': 'dtaso__range',
                'tpaso__icontains': 'tpaso__icontains',
                'resaso__icontains': 'resaso__icontains',
                'medico': 'medico',
        ...
        s2220_evtmonit_lista = s2220evtMonit.objects.filter(**dict_qs).filter().exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            response = render_to_response('csv/s2220_evtmonit.csv', context)
