...
def validar_evento_funcao(request, s2250_evtavprevio_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2250_evtavprevio_id)
...
def validar_evento(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        if s2250_evtavprevio.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2250_evtavprevio_id)
            
            if s2250_evtavprevio.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2250evtAvPrevio.objects.\
                    filter(id=s2250_evtavprevio_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2250_evtavprevio.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2250evtAvPrevio.objects.\
                    filter(id=s2250_evtavprevio_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
