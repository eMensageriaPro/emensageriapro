...
def validar_evento_funcao(request, pk):
    ...
    schema_filename = 'esocial/%s/evtConvInterm.xsd' % (s2260_evtconvinterm.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2260_evtconvinterm' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2260_evtconvinterm', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2260_evtconvinterm.status in STATUS_VALIDACAO:
