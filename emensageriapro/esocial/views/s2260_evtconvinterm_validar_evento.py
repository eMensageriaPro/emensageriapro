...
def validar_evento_funcao(request, s2260_evtconvinterm_id):
    ...
    lista_validacoes = []
    s2260_evtconvinterm = get_object_or_404(s2260evtConvInterm, id=s2260_evtconvinterm_id)
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2260_evtconvinterm_id)
...
def validar_evento(request, hash):
    ...
    if s2260_evtconvinterm_id:
        ...
        if s2260_evtconvinterm.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2260_evtconvinterm_id)
            
            if s2260_evtconvinterm.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2260evtConvInterm.objects.\
                    filter(id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2260_evtconvinterm.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2260evtConvInterm.objects.\
                    filter(id=s2260_evtconvinterm_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
