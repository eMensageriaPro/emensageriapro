...
def validar_evento_funcao(request, s1270_evtcontratavnp_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1270_evtcontratavnp_id)
...
def validar_evento(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        if s1270_evtcontratavnp.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1270_evtcontratavnp_id)
            
            if s1270_evtcontratavnp.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1270evtContratAvNP.objects.\
                    filter(id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1270_evtcontratavnp.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1270evtContratAvNP.objects.\
                    filter(id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
