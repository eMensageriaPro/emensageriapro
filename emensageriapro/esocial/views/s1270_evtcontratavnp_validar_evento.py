...
def validar_evento_funcao(request, pk):
    ...
    if not s1270_evtcontratavnp.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1270_evtcontratavnp, 'esocial')
        s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP, id=pk)
    ...
    schema_filename = 'esocial/%s/evtContratAvNP.xsd' % (s1270_evtcontratavnp.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1270_evtcontratavnp' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1270_evtcontratavnp', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1270_evtcontratavnp.status in STATUS_VALIDACAO:
