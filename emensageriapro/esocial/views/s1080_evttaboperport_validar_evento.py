...
def validar_evento_funcao(request, s1080_evttaboperport_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1080_evttaboperport_id)
...
def validar_evento(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        if s1080_evttaboperport.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1080_evttaboperport_id)
            
            if s1080_evttaboperport.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1080evtTabOperPort.objects.\
                    filter(id=s1080_evttaboperport_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1080_evttaboperport.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1080evtTabOperPort.objects.\
                    filter(id=s1080_evttaboperport_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
