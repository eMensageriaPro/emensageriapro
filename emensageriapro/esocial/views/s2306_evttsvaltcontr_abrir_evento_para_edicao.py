...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2306evtTSVAltContr'):

        if s2306_evttsvaltcontr_id:
            s2306_evttsvaltcontr = get_object_or_404(s2306evtTSVAltContr, excluido=False, id=s2306_evttsvaltcontr_id)
            ...
            if s2306_evttsvaltcontr.status in status_list:
                s2306evtTSVAltContr.objects.filter(id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
