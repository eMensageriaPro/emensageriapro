...
def gerar_xml_assinado(s2416_evtcdbenalt_id, db_slug):
    ...
    if s2416_evtcdbenalt.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2416evtCdBenAlt.objects.using(db_slug).\
            filter(id=s2416_evtcdbenalt_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2416_evtcdbenalt_id:
        ...
        if s2416_evtcdbenalt.status in status_list:
            s2416evtCdBenAlt.objects.using(db_slug).filter(id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2416_evtcdbenalt/%s.xml' % (s2416_evtcdbenalt.identidade)
...
def validar_evento(request, hash):
    ...
    if s2416_evtcdbenalt_id:
        ...
        if s2416_evtcdbenalt.versao in VERSOES_ESOCIAL:
            ...
            if s2416_evtcdbenalt.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2416evtCdBenAlt.objects.using(db_slug).\
                    filter(excluido=False, id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2416_evtcdbenalt.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2416evtCdBenAlt.objects.using(db_slug).\
                    filter(excluido=False, id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
