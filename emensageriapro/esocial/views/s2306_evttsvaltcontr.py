...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2306_evttsvaltcontr_id:
            ...
            s2306_evttsvaltcontr_form = form_s2306_evttsvaltcontr(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s2306_evttsvaltcontr_form.is_valid():
                ...
                if s2306_evttsvaltcontr_id:
                    ...
                    s2306_evttsvaltcontr_form = form_s2306_evttsvaltcontr(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        #             s2306_evttsvaltcontr_form.fields['status'].widget.attrs['disabled'] = True
        #             s2306_evttsvaltcontr_form.fields['status'].widget.attrs['readonly'] = True
        #             s2306_evttsvaltcontr_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
