...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2231evtCessao'):

        if s2231_evtcessao_id:
            s2231_evtcessao = get_object_or_404(s2231evtCessao, excluido=False, id=s2231_evtcessao_id)
            ...
            if s2231_evtcessao.status in status_list:
                s2231evtCessao.objects.filter(id=s2231_evtcessao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
