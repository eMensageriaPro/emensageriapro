...
def read_s2245_evttreicap(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s2245_evttreicap_obj(request, doc, status, validar, arquivo)

    s2245evtTreiCap.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2245_evttreicap_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s2245_evttreicap_dados['arquivo'] = arquivo
