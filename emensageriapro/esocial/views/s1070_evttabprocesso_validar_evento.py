...
def validar_evento_funcao(request, pk):
    ...
    if not s1070_evttabprocesso.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1070_evttabprocesso, 'esocial')
        s1070_evttabprocesso = get_object_or_404(s1070evtTabProcesso, id=pk)
    ...
    schema_filename = 'esocial/%s/evtTabProcesso.xsd' % (s1070_evttabprocesso.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1070_evttabprocesso' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1070_evttabprocesso', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1070_evttabprocesso.status in STATUS_VALIDACAO:
