...
def validar_evento_funcao(request, s1070_evttabprocesso_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1070_evttabprocesso_id)
...
def validar_evento(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        if s1070_evttabprocesso.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1070_evttabprocesso_id)
            
            if s1070_evttabprocesso.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1070evtTabProcesso.objects.\
                    filter(id=s1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1070_evttabprocesso.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1070evtTabProcesso.objects.\
                    filter(id=s1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
