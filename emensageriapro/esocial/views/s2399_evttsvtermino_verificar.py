    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2399_evttsvtermino_verificar.html' % s2399_evttsvtermino.versao,
            ...
            response =  render_to_response('%s/s2399_evttsvtermino_verificar.html' % s2399_evttsvtermino.versao, context)
            ...
            return render(request, '%s/s2399_evttsvtermino_verificar.html' % s2399_evttsvtermino.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2399(s2399_evttsvtermino_id, db_slug):
    from django.template.loader import get_template
    if s2399_evttsvtermino_id:
        ...
        #return render(request, 'xml/%s/s2399_evttsvtermino.html' % s2399_evttsvtermino.versao, context, content_type='text/xml')
        t = get_template('%s/s2399_evttsvtermino_xml.html' % s2399_evttsvtermino.versao)
...
def gerar_xml_assinado(s2399_evttsvtermino_id, db_slug):
    ...
    if s2399_evttsvtermino.status in (0,1,2,11):
        s2399evtTSVTermino.objects.using(db_slug).filter(id=s2399_evttsvtermino_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2399_evttsvtermino_id = int(dict_hash['id'])
    if s2399_evttsvtermino_id:
        s2399_evttsvtermino = get_object_or_404(s2399evtTSVTermino.objects.using(db_slug), excluido=False,
                                                    id=s2399_evttsvtermino_id)

        arquivo = 'arquivos/Eventos/s2399_evttsvtermino/%s.xml' % s2399_evttsvtermino.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2399_evttsvtermino_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s.xml' % s2399_evttsvtermino.identidade)
        salvar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_duplicado_temp.xml' % s2399_evttsvtermino.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_duplicado_temp.xml' % s2399_evttsvtermino.identidade, request)
        from emensageriapro.esocial.views.s2399_evttsvtermino import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2399evtTSVTermino.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2399_evttsvtermino.identidade),
            's2399_evttsvtermino', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2399_evttsvtermino_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_alteracao_temp.xml' % s2399_evttsvtermino.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_alteracao_temp.xml' % s2399_evttsvtermino.identidade, request)
        from emensageriapro.esocial.views.s2399_evttsvtermino import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2399evtTSVTermino.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2399_evttsvtermino.identidade),
            's2399_evttsvtermino', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2399_evttsvtermino_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_exclusao_temp.xml' % s2399_evttsvtermino.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2399_evttsvtermino/%s_exclusao_temp.xml' % s2399_evttsvtermino.identidade, request)
        from emensageriapro.esocial.views.s2399_evttsvtermino import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2399evtTSVTermino.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2399_evttsvtermino.identidade),
            's2399_evttsvtermino', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2399_evttsvtermino_id:
        ...
        if s2399_evttsvtermino.status == 0:
            from emensageriapro.esocial.views.s2399_evttsvtermino import identidade_evento
            dent = identidade_evento(s2399_evttsvtermino_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2399_evttsvtermino_id:
        s2399_evttsvtermino = get_object_or_404(s2399evtTSVTermino.objects.using(db_slug), excluido=False, id=s2399_evttsvtermino_id)
        if s2399_evttsvtermino.status in (0, 1, 2, 3, 4, 10, 11) or s2399_evttsvtermino.processamento_codigo_resposta in (401,402):
            s2399evtTSVTermino.objects.using(db_slug).filter(id=s2399_evttsvtermino_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2399_evttsvtermino/%s.xml' % (s2399_evttsvtermino.identidade)
...
def validar_evento_funcao(s2399_evttsvtermino_id, db_slug):
    ...
    lista_validacoes = []
    s2399_evttsvtermino = get_object_or_404(s2399evtTSVTermino.objects.using(db_slug), excluido=False, id=s2399_evttsvtermino_id)
    if s2399_evttsvtermino.transmissor_lote_esocial:
        if s2399_evttsvtermino.transmissor_lote_esocial.transmissor:
            if s2399_evttsvtermino.transmissor_lote_esocial.transmissor.verificar_predecessao:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s2399_evttsvtermino SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s2399_evttsvtermino_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s2399_evttsvtermino/%s.xml' % (s2399_evttsvtermino.identidade)
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2399_evttsvtermino import validacoes_s2399_evttsvtermino
                lista = validacoes_s2399_evttsvtermino(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2399_evttsvtermino import validacoes_s2399_evttsvtermino
                lista = validacoes_s2399_evttsvtermino(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s2399_evttsvtermino SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s2399_evttsvtermino_id), False)
    else:
        executar_sql("UPDATE public.s2399_evttsvtermino SET validacoes='', status=4 WHERE id=%s;" % (s2399_evttsvtermino_id), False)
...
def validar_evento(request, hash):
    ...
    if s2399_evttsvtermino_id:
        lista_validacoes = []
        s2399_evttsvtermino = get_object_or_404(s2399evtTSVTermino.objects.using(db_slug), excluido=False, id=s2399_evttsvtermino_id)
        if s2399_evttsvtermino.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2399_evttsvtermino_id, db_slug)
