...
def validar_evento_funcao(request, s1300_evtcontrsindpatr_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1300_evtcontrsindpatr_id)
...
def validar_evento(request, hash):
    ...
    if s1300_evtcontrsindpatr_id:
        ...
        if s1300_evtcontrsindpatr.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1300_evtcontrsindpatr_id)
            
            if s1300_evtcontrsindpatr.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1300evtContrSindPatr.objects.\
                    filter(id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1300_evtcontrsindpatr.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1300evtContrSindPatr.objects.\
                    filter(id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
