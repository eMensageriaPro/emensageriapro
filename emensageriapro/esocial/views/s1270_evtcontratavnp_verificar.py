...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1270_evtcontratavnp_lista = s1270evtContratAvNP.objects.using( db_slug ).filter(id=s1270_evtcontratavnp_id, excluido = False).all()
   

        s1270_remunavnp_lista = s1270remunAvNP.objects.using(db_slug).filter(s1270_evtcontratavnp_id__in = listar_ids(s1270_evtcontratavnp_lista) ).filter(excluido=False).all()
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1270_evtcontratavnp_verificar.html' % s1270_evtcontratavnp.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1270_evtcontratavnp_verificar.html' % s1270_evtcontratavnp.versao,
            ...
            response =  render_to_response('%s/s1270_evtcontratavnp_verificar.html' % s1270_evtcontratavnp.versao, context)
            ...
            return render(request, '%s/s1270_evtcontratavnp_verificar.html' % s1270_evtcontratavnp.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1270(s1270_evtcontratavnp_id, db_slug):
    ...
    if s1270_evtcontratavnp_id:
        ...
        t = get_template('%s/s1270_evtcontratavnp_xml.html' % s1270_evtcontratavnp.versao)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s1270_evtcontratavnp = get_object_or_404(
            ...
            excluido = False, id = s1270_evtcontratavnp_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1270_evtcontratavnp.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s1270_evtcontratavnp_recibo_pdf.html', context)
            filename = "%s.xls" % s1270_evtcontratavnp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s1270_evtcontratavnp_recibo_csv.html', context)
            filename = "%s.csv" % s1270_evtcontratavnp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s1270_evtcontratavnp_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug):
    import os
    ...
    if s1270_evtcontratavnp.arquivo_original:
        ...
        xml = gerar_xml_s1270(s1270_evtcontratavnp_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_esocial(xml)
    if s1270_evtcontratavnp.status in (0,1,2,11):
        s1270evtContratAvNP.objects.using(db_slug).filter(id=s1270_evtcontratavnp_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP.objects.using(db_slug), excluido=False, id=s1270_evtcontratavnp_id)
        xml_assinado = gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug)
...
def duplicar(request, hash):
    ...
    s1270_evtcontratavnp_id = int(dict_hash['id'])
    if s1270_evtcontratavnp_id:
        s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP.objects.using(db_slug), excluido=False,
                                                    id=s1270_evtcontratavnp_id)

        arquivo = 'arquivos/Eventos/s1270_evtcontratavnp/%s.xml' % s1270_evtcontratavnp.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s.xml' % s1270_evtcontratavnp.identidade)
        salvar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_duplicado_temp.xml' % s1270_evtcontratavnp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_duplicado_temp.xml' % s1270_evtcontratavnp.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_alteracao_temp.xml' % s1270_evtcontratavnp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_alteracao_temp.xml' % s1270_evtcontratavnp.identidade, request)
        from emensageriapro.esocial.views.s1270_evtcontratavnp import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1270evtContratAvNP.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def criar_exclusao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_exclusao_temp.xml' % s1270_evtcontratavnp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1270_evtcontratavnp/%s_exclusao_temp.xml' % s1270_evtcontratavnp.identidade, request)
        from emensageriapro.esocial.views.s1270_evtcontratavnp import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1270evtContratAvNP.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP.objects.using(db_slug), excluido=False, id=s1270_evtcontratavnp_id)
        if s1270_evtcontratavnp.status in (0, 1, 2, 3, 4, 10, 11) or s1270_evtcontratavnp.processamento_codigo_resposta in (401,402):
            s1270evtContratAvNP.objects.using(db_slug).filter(id=s1270_evtcontratavnp_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1270_evtcontratavnp/%s.xml' % (s1270_evtcontratavnp.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s1270_evtcontratavnp.identidade, BASE_DIR, s1270_evtcontratavnp.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1270_evtcontratavnp/%s.xml %s/arquivos/Eventos/s1270_evtcontratavnp/%s_backup_%s.xml' % dad)
...
def validar_evento_funcao(s1270_evtcontratavnp_id, db_slug):
    ...
    lista_validacoes = []
    s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP.objects.using(db_slug), excluido=False, id=s1270_evtcontratavnp_id)
    if s1270_evtcontratavnp.transmissor_lote_esocial:
        if s1270_evtcontratavnp.transmissor_lote_esocial.transmissor:
            if s1270_evtcontratavnp.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's1270_evtcontratavnp', s1270_evtcontratavnp_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s1270_evtcontratavnp SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s1270_evtcontratavnp_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s1270_evtcontratavnp/%s.xml' % (s1270_evtcontratavnp.identidade)
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1270_evtcontratavnp import validacoes_s1270_evtcontratavnp
                lista = validacoes_s1270_evtcontratavnp(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1270_evtcontratavnp import validacoes_s1270_evtcontratavnp
                lista = validacoes_s1270_evtcontratavnp(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s1270_evtcontratavnp SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s1270_evtcontratavnp_id), False)
    else:
        executar_sql("UPDATE public.s1270_evtcontratavnp SET validacoes='', status=4 WHERE id=%s;" % (s1270_evtcontratavnp_id), False)
...
def validar_evento(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        lista_validacoes = []
        s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP.objects.using(db_slug), excluido=False, id=s1270_evtcontratavnp_id)
        if s1270_evtcontratavnp.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1270_evtcontratavnp_id, db_slug)
