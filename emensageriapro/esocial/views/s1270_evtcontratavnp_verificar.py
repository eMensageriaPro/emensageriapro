...
def gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug):
    ...
    if s1270_evtcontratavnp.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1270evtContratAvNP.objects.using(db_slug).\
            filter(id=s1270_evtcontratavnp_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        if s1270_evtcontratavnp.status in status_list:
            s1270evtContratAvNP.objects.using(db_slug).filter(id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1270_evtcontratavnp/%s.xml' % (s1270_evtcontratavnp.identidade)
...
def validar_evento(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        if s1270_evtcontratavnp.versao in VERSOES_ESOCIAL:
            ...
            if s1270_evtcontratavnp.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1270evtContratAvNP.objects.using(db_slug).\
                    filter(excluido=False, id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1270_evtcontratavnp.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1270evtContratAvNP.objects.using(db_slug).\
                    filter(excluido=False, id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
