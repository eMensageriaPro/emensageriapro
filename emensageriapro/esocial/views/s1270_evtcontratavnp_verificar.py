...
def gerar_xml_s1270(s1270_evtcontratavnp_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1270_evtcontratavnp_id, db_slug):
    ...
    if s1270_evtcontratavnp.status in (0,1,2,11):
        ...
        s1270evtContratAvNP.objects.using(db_slug).\
            filter(id=s1270_evtcontratavnp_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        texto = gerar_xml_s1270(s1270_evtcontratavnp_id, db_slug, versao="|")
        dados = read_s1270_evtcontratavnp_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1270evtContratAvNP.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1270_evtcontratavnp.identidade),
...
def criar_alteracao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1270_evtcontratavnp.identidade),
            's1270_evtcontratavnp', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1270_evtcontratavnp.identidade),
            's1270_evtcontratavnp', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1270_evtcontratavnp_id:
        ...
        if s1270_evtcontratavnp.status == 0:
            ...
            nova_identidade = identidade_evento(s1270_evtcontratavnp_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1270_evtcontratavnp', s1270_evtcontratavnp_id, request.user.id, 1)
...
def validar_evento_funcao(s1270_evtcontratavnp_id, db_slug):
    ...
    s1270evtContratAvNP.objects.using( db_slug ).\
        filter(id=s1270_evtcontratavnp_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1270_evtcontratavnp_validar import validacoes_s1270_evtcontratavnp
...
<<<<<<< HEAD
        if s1270_evtcontratavnp.versao in VERSOES_ESOCIAL:
=======
        if s1270_evtcontratavnp.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1270_evtcontratavnp_id, db_slug)
