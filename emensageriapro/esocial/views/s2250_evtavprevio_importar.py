...
def read_s2250_evtavprevio(request, dados, arquivo, validar=False):
    ...
    s2250evtAvPrevio.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2250_evtavprevio_obj(request, doc, status, validar=False):
    ...
    s2250_evtavprevio_dados['status'] = STATUS_EVENTO_IMPORTADO
