...
def validar_evento_funcao(request, s2400_evtcdbenefin_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2400_evtcdbenefin_id)
...
def validar_evento(request, hash):
    ...
    if s2400_evtcdbenefin_id:
        ...
        if s2400_evtcdbenefin.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2400_evtcdbenefin_id)
            
            if s2400_evtcdbenefin.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2400evtCdBenefIn.objects.\
                    filter(id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2400_evtcdbenefin.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2400evtCdBenefIn.objects.\
                    filter(id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
