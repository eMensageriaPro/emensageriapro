...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2205evtAltCadastral'):

        if s2205_evtaltcadastral_id:
            s2205_evtaltcadastral = get_object_or_404(s2205evtAltCadastral, excluido=False, id=s2205_evtaltcadastral_id)
            ...
            if s2205_evtaltcadastral.status in status_list:
                s2205evtAltCadastral.objects.filter(id=s2205_evtaltcadastral_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
