...
def gerar_xml_assinado(s1298_evtreabreevper_id, db_slug):
    ...
    if s1298_evtreabreevper.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1298evtReabreEvPer.objects.using(db_slug).\
            filter(id=s1298_evtreabreevper_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1298_evtreabreevper_id:
        ...
        if s1298_evtreabreevper.status in status_list:
...
def validar_evento(request, hash):
    ...
    if s1298_evtreabreevper_id:
        ...
        if s1298_evtreabreevper.versao in VERSOES_ESOCIAL:
            ...
            if s1298_evtreabreevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1298evtReabreEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1298_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1298_evtreabreevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1298evtReabreEvPer.objects.using(db_slug).\
                    filter(excluido=False, id=s1298_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
