...
def validar_evento_funcao(request, s2399_evttsvtermino_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2399_evttsvtermino_id)
...
def validar_evento(request, hash):
    ...
    if s2399_evttsvtermino_id:
        ...
        if s2399_evttsvtermino.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2399_evttsvtermino_id)
            
            if s2399_evttsvtermino.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2399evtTSVTermino.objects.\
                    filter(id=s2399_evttsvtermino_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2399_evttsvtermino.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2399evtTSVTermino.objects.\
                    filter(id=s2399_evttsvtermino_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
