...
def validar_evento_funcao(request, s1010_evttabrubrica_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1010_evttabrubrica_id)
...
def validar_evento(request, hash):
    ...
    if s1010_evttabrubrica_id:
        ...
        if s1010_evttabrubrica.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1010_evttabrubrica_id)
            
            if s1010_evttabrubrica.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1010evtTabRubrica.objects.\
                    filter(id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1010_evttabrubrica.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1010evtTabRubrica.objects.\
                    filter(id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
