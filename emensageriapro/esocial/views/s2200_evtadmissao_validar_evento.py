...
def validar_evento_funcao(request, s2200_evtadmissao_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2200_evtadmissao_id)
...
def validar_evento(request, hash):
    ...
    if s2200_evtadmissao_id:
        ...
        if s2200_evtadmissao.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2200_evtadmissao_id)
            
            if s2200_evtadmissao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2200evtAdmissao.objects.\
                    filter(id=s2200_evtadmissao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2200_evtadmissao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2200evtAdmissao.objects.\
                    filter(id=s2200_evtadmissao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
