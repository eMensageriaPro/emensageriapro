    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s2306_evttsvaltcontr_verificar.html',
            ...
            response =  render_to_response('s2306_evttsvaltcontr_verificar.html', context)
            ...
            return render(request, 's2306_evttsvaltcontr_verificar.html', context)
...
def gerar_xml_s2306(s2306_evttsvaltcontr_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2306_evttsvaltcontr_id, db_slug):
    ...
    if s2306_evttsvaltcontr.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2306evtTSVAltContr.objects.using(db_slug).\
            filter(id=s2306_evttsvaltcontr_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2306_evttsvaltcontr_importar import read_s2306_evttsvaltcontr_string
    ...
    if s2306_evttsvaltcontr_id:
        ...
        texto = gerar_xml_s2306(s2306_evttsvaltcontr_id, db_slug, versao="|")
        dados = read_s2306_evttsvaltcontr_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2306_evttsvaltcontr)

        s2306evtTSVAltContr.objects.using(db_slug).filter(id=dados['id']).\
            update(status=STATUS_EVENTO_CADASTRADO,
                   arquivo_original=0,
                   arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2306_evttsvaltcontr.identidade),
            's2306_evttsvaltcontr', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2306_evttsvaltcontr_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2306_evttsvaltcontr.identidade),
            's2306_evttsvaltcontr', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2306_evttsvaltcontr_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2306_evttsvaltcontr.identidade),
            's2306_evttsvaltcontr', dados['id'], request.user.id, 1)
    ...
    if s2306_evttsvaltcontr_id:
        ...
        if s2306_evttsvaltcontr.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2306_evttsvaltcontr', s2306_evttsvaltcontr_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2306_evttsvaltcontr_id:
        ...
        if s2306_evttsvaltcontr.status in status_list:
            s2306evtTSVAltContr.objects.using(db_slug).filter(id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2306_evttsvaltcontr/%s.xml' % (s2306_evttsvaltcontr.identidade)
...
def validar_evento_funcao(s2306_evttsvaltcontr_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2306_evttsvaltcontr_validar import validacoes_s2306_evttsvaltcontr
    ...
    if lista_validacoes:
        ...
        s2306evtTSVAltContr.objects.using( db_slug ).\
            filter(id=s2306_evttsvaltcontr_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2306_evttsvaltcontr_id:
        ...
        if s2306_evttsvaltcontr.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2306_evttsvaltcontr_id, db_slug)

            if s2306_evttsvaltcontr.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2306evtTSVAltContr.objects.using(db_slug).\
                    filter(excluido=False, id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2306_evttsvaltcontr.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2306evtTSVAltContr.objects.using(db_slug).\
                    filter(excluido=False, id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
