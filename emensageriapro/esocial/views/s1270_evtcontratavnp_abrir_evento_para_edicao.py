...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1270evtContratAvNP'):

        if s1270_evtcontratavnp_id:
            s1270_evtcontratavnp = get_object_or_404(s1270evtContratAvNP, excluido=False, id=s1270_evtcontratavnp_id)
            ...
            if s1270_evtcontratavnp.status in status_list:
                s1270evtContratAvNP.objects.filter(id=s1270_evtcontratavnp_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
