...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2299evtDeslig'):

        if s2299_evtdeslig_id:
            s2299_evtdeslig = get_object_or_404(s2299evtDeslig, excluido=False, id=s2299_evtdeslig_id)
            ...
            if s2299_evtdeslig.status in status_list:
                s2299evtDeslig.objects.filter(id=s2299_evtdeslig_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
