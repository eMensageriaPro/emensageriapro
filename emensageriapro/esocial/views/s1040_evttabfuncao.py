...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1040_evttabfuncao.status == 0:
            ...
            s1040evtTabFuncao.objects.using( db_slug ).filter(id = s1040_evttabfuncao_id).delete()
...
class s1040evtTabFuncaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1040evtTabFuncao.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1040_evttabfuncao_lista = s1040evtTabFuncao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            response = render_to_response('s1040_evttabfuncao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1040_evttabfuncao_csv.html', context)
...
def identidade_evento(s1040_evttabfuncao_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1040evtTabFuncao.objects.using( db_slug ).get(id=s1040_evttabfuncao_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1040evtTabFuncao.objects.using(db_slug).filter(id=s1040_evttabfuncao_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1040_evttabfuncao_id:
            s1040_evttabfuncao_form = form_s1040_evttabfuncao(request.POST or None, instance = s1040_evttabfuncao, slug = db_slug)
        else:
            s1040_evttabfuncao_form = form_s1040_evttabfuncao(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1040_evttabfuncao_form.is_valid():
                ...
                if s1040_evttabfuncao_id:
                    if s1040_evttabfuncao.status == 0:
                        ...
                        s1040evtTabFuncao.objects.using(db_slug).filter(id=s1040_evttabfuncao_id).update(**dados)
                        obj = s1040evtTabFuncao.objects.using(db_slug).get(id=s1040_evttabfuncao_id)
                        #s1040_evttabfuncao_editar_custom
                        #s1040_evttabfuncao_campos_multiple_passo2
                        messages.success(request, 'Alterado com sucesso!')
                        gravar_auditoria(json.dumps(model_to_dict(s1040_evttabfuncao), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1040_evttabfuncao', s1040_evttabfuncao_id, usuario_id, 2)
                    else:
                        obj = s1040evtTabFuncao.objects.using(db_slug).get(id=s1040_evttabfuncao_id)
                    ...
                    s1040_evttabfuncao_form = form_s1040_evttabfuncao(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1040_evttabfuncao', obj.id, usuario_id, 1)
                if request.session['retorno_pagina'] not in ('s1040_evttabfuncao_apagar', 's1040_evttabfuncao_salvar', 's1040_evttabfuncao'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
        ...
        if s1040_evttabfuncao_id:
            ...
            s1040_alteracao_form = form_s1040_alteracao(initial={ 's1040_evttabfuncao': s1040_evttabfuncao }, slug=db_slug)
            s1040_alteracao_form.fields['s1040_evttabfuncao'].widget.attrs['readonly'] = True
            s1040_alteracao_lista = s1040alteracao.objects.using( db_slug ).filter(excluido = False, s1040_evttabfuncao_id=s1040_evttabfuncao.id).all()
            s1040_exclusao_form = form_s1040_exclusao(initial={ 's1040_evttabfuncao': s1040_evttabfuncao }, slug=db_slug)
            s1040_exclusao_form.fields['s1040_evttabfuncao'].widget.attrs['readonly'] = True
            s1040_exclusao_lista = s1040exclusao.objects.using( db_slug ).filter(excluido = False, s1040_evttabfuncao_id=s1040_evttabfuncao.id).all()
        else:
        ...
        #             s1040_evttabfuncao_form.fields['tpamb'].widget.attrs['disabled'] = True
        #             s1040_evttabfuncao_form.fields['tpamb'].widget.attrs['readonly'] = True
        #             s1040_evttabfuncao_form.fields['tpamb'].value = TP_AMB
        #             s1040_evttabfuncao_form.fields['procemi'].widget.attrs['disabled'] = True
        #             s1040_evttabfuncao_form.fields['procemi'].widget.attrs['readonly'] = True
        ...
        #             s1040_evttabfuncao_form.fields['status'].widget.attrs['disabled'] = True
        #             s1040_evttabfuncao_form.fields['status'].widget.attrs['readonly'] = True
        #             s1040_evttabfuncao_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1040_evttabfuncao_id, tabela='s1040_evttabfuncao').all()
        context = {
