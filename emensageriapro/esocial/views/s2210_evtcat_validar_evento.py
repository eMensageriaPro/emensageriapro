...
def validar_evento_funcao(request, s2210_evtcat_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2210_evtcat_id)
...
def validar_evento(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        if s2210_evtcat.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2210_evtcat_id)
            
            if s2210_evtcat.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2210evtCAT.objects.\
                    filter(id=s2210_evtcat_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2210_evtcat.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2210evtCAT.objects.\
                    filter(id=s2210_evtcat_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
