...
def validar_evento_funcao(request, s2231_evtcessao_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2231_evtcessao_id)
...
def validar_evento(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        if s2231_evtcessao.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2231_evtcessao_id)
            
            if s2231_evtcessao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2231evtCessao.objects.\
                    filter(id=s2231_evtcessao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2231_evtcessao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2231evtCessao.objects.\
                    filter(id=s2231_evtcessao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
