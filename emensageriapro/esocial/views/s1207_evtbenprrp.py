...
def identidade_evento(s1207_evtbenprrp_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1207evtBenPrRP.objects.using( db_slug ).get(id=s1207_evtbenprrp_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1207evtBenPrRP.objects.using(db_slug).filter(id=s1207_evtbenprrp_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
            ...
            if s1207_evtbenprrp_form.is_valid():
                ...
                if s1207_evtbenprrp_id:
                    if s1207_evtbenprrp.status == 0:
                        ...
                        #s1207_evtbenprrp_campos_multiple_passo1
                        s1207evtBenPrRP.objects.using(db_slug).filter(id=s1207_evtbenprrp_id).update(**dados)
                        obj = s1207evtBenPrRP.objects.using(db_slug).get(id=s1207_evtbenprrp_id)
                        #s1207_evtbenprrp_editar_custom
                        #s1207_evtbenprrp_campos_multiple_passo2
                    ...
                    #s1207_evtbenprrp_cadastrar_campos_multiple_passo1
                    obj = s1207evtBenPrRP(**dados)
                    obj.save(using = db_slug)
                    #s1207_evtbenprrp_cadastrar_custom
                    #s1207_evtbenprrp_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1207_evtbenprrp_form = form_s1207_evtbenprrp(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s1207_evtbenprrp_form.fields['tpamb'].widget.attrs['disabled'] = True
            s1207_evtbenprrp_form.fields['tpamb'].widget.attrs['readonly'] = True
            s1207_evtbenprrp_form.fields['tpamb'].value = TP_AMB
            s1207_evtbenprrp_form.fields['procemi'].widget.attrs['disabled'] = True
            s1207_evtbenprrp_form.fields['procemi'].widget.attrs['readonly'] = True
            s1207_evtbenprrp_form.fields['procemi'].value = 1
            s1207_evtbenprrp_form.fields['verproc'].widget.attrs['readonly'] = True
            s1207_evtbenprrp_form.fields['verproc'].value = VERSAO_EMENSAGERIA
            s1207_evtbenprrp_form.fields['status'].widget.attrs['disabled'] = True
            s1207_evtbenprrp_form.fields['status'].widget.attrs['readonly'] = True
            s1207_evtbenprrp_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
