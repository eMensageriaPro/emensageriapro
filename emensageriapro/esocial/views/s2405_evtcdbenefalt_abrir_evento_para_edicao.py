...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2405evtCdBenefAlt'):

        if s2405_evtcdbenefalt_id:
            s2405_evtcdbenefalt = get_object_or_404(s2405evtCdBenefAlt, excluido=False, id=s2405_evtcdbenefalt_id)
            ...
            if s2405_evtcdbenefalt.status in status_list:
                s2405evtCdBenefAlt.objects.filter(id=s2405_evtcdbenefalt_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
