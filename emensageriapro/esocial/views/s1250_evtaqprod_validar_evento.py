...
def validar_evento_funcao(request, pk):
    ...
    if not s1250_evtaqprod.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1250_evtaqprod, 'esocial')
        s1250_evtaqprod = get_object_or_404(s1250evtAqProd, id=pk)
    ...
    schema_filename = 'esocial/%s/evtAqProd.xsd' % (s1250_evtaqprod.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1250_evtaqprod' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1250_evtaqprod', pk)
