...
def validar_evento_funcao(request, s1250_evtaqprod_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1250_evtaqprod_id)
...
def validar_evento(request, hash):
    ...
    if s1250_evtaqprod_id:
        ...
        if s1250_evtaqprod.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1250_evtaqprod_id)
            
            if s1250_evtaqprod.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1250evtAqProd.objects.\
                    filter(id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1250_evtaqprod.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1250evtAqProd.objects.\
                    filter(id=s1250_evtaqprod_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
