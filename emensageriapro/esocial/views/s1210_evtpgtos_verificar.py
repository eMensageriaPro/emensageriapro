...
def gerar_xml_s1210(s1210_evtpgtos_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1210_evtpgtos_id, db_slug):
    ...
    if s1210_evtpgtos.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1210evtPgtos.objects.using(db_slug).\
            filter(id=s1210_evtpgtos_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s1210_evtpgtos_importar import read_s1210_evtpgtos_string
    ...
    if s1210_evtpgtos_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1210_evtpgtos.identidade),
    ...
    if s1210_evtpgtos_id:
        ...
        if s1210_evtpgtos.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's1210_evtpgtos', s1210_evtpgtos_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1210_evtpgtos_id:
        ...
        if s1210_evtpgtos.status in status_list:
            s1210evtPgtos.objects.using(db_slug).filter(id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1210_evtpgtos/%s.xml' % (s1210_evtpgtos.identidade)
...
def validar_evento_funcao(s1210_evtpgtos_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1210_evtpgtos_validar import validacoes_s1210_evtpgtos
    ...
    if lista_validacoes:
        ...
        s1210evtPgtos.objects.using( db_slug ).\
            filter(id=s1210_evtpgtos_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s1210_evtpgtos_id:
        ...
        if s1210_evtpgtos.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s1210_evtpgtos_id, db_slug)

            if s1210_evtpgtos.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1210evtPgtos.objects.using(db_slug).\
                    filter(excluido=False, id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1210_evtpgtos.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1210evtPgtos.objects.using(db_slug).\
                    filter(excluido=False, id=s1210_evtpgtos_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
