...
def validar_evento_funcao(request, s2306_evttsvaltcontr_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2306_evttsvaltcontr_id)
...
def validar_evento(request, hash):
    ...
    if s2306_evttsvaltcontr_id:
        ...
        if s2306_evttsvaltcontr.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2306_evttsvaltcontr_id)
            
            if s2306_evttsvaltcontr.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2306evtTSVAltContr.objects.\
                    filter(id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2306_evttsvaltcontr.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2306evtTSVAltContr.objects.\
                    filter(id=s2306_evttsvaltcontr_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
