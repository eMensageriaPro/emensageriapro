...
def criar_alteracao(request, pk):
    ...
    from emensageriapro.esocial.views.s1020_evttablotacao_gerar_xml import gerar_xml_s1020
    from emensageriapro.functions import identidade_evento
    
    if request.user.has_perm('esocial.can_create_change_s1020evtTabLotacao'):

        s1020_evttablotacao = get_object_or_404(
            s1020evtTabLotacao,
            id=pk)

        texto = gerar_xml_s1020(request, pk, versao="|")
        texto = texto.replace('<inclusao>','<alteracao>').replace('</inclusao>','</alteracao>')
        dados = read_s1020_evttablotacao_string(request, {}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s1020_evttablotacao)

        s1020evtTabLotacao.objects.filter(id=dados['id']).\
        ...
        return redirect('s1020_evttablotacao_salvar', pk=return_pk, tab='master')
        ...
        return redirect('s1020_evttablotacao_salvar', pk=pk, tab='master')