...
def gerar_xml_assinado(s1295_evttotconting_id, db_slug):
    ...
    if s1295_evttotconting.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1295evtTotConting.objects.using(db_slug).\
            filter(id=s1295_evttotconting_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1295_evttotconting_id:
        ...
        if s1295_evttotconting.status in status_list:
            s1295evtTotConting.objects.using(db_slug).filter(id=s1295_evttotconting_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1295_evttotconting/%s.xml' % (s1295_evttotconting.identidade)
...
def validar_evento(request, hash):
    ...
    if s1295_evttotconting_id:
        ...
        if s1295_evttotconting.versao in VERSOES_ESOCIAL:
            ...
            if s1295_evttotconting.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1295evtTotConting.objects.using(db_slug).\
                    filter(excluido=False, id=s1295_evttotconting_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1295_evttotconting.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1295evtTotConting.objects.using(db_slug).\
                    filter(excluido=False, id=s1295_evttotconting_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
