...
def gerar_xml_s2420(s2420_evtcdbenterm_id, db_slug, versao=None):
    ...
    if s2420_evtcdbenterm_id:
        ...
        if not versao:
            ...
            versao = s2420_evtcdbenterm.versao
        ...
        t = get_template('s2420_evtcdbenterm.xml')
...
def gerar_xml_assinado(s2420_evtcdbenterm_id, db_slug):
    ...
    if s2420_evtcdbenterm.status in (0,1,2,11):
        ...
        s2420evtCdBenTerm.objects.using(db_slug).\
            filter(id=s2420_evtcdbenterm_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2420_evtcdbenterm_importar import read_s2420_evtcdbenterm_string
    ...
    if s2420_evtcdbenterm_id:
        ...
        texto = gerar_xml_s2420(s2420_evtcdbenterm_id, db_slug, versao="|")
        dados = read_s2420_evtcdbenterm_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s2420evtCdBenTerm.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2420_evtcdbenterm.identidade),
            's2420_evtcdbenterm', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2420_evtcdbenterm_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2420_evtcdbenterm.identidade),
            's2420_evtcdbenterm', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2420_evtcdbenterm_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2420_evtcdbenterm.identidade),
            's2420_evtcdbenterm', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2420_evtcdbenterm_id:
        ...
        if s2420_evtcdbenterm.status == 0:
            ...
            nova_identidade = identidade_evento(s2420_evtcdbenterm_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2420_evtcdbenterm', s2420_evtcdbenterm_id, request.user.id, 1)
...
def validar_evento_funcao(s2420_evtcdbenterm_id, db_slug):
    ...
    s2420evtCdBenTerm.objects.using( db_slug ).\
        filter(id=s2420_evtcdbenterm_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2420_evtcdbenterm_validar import validacoes_s2420_evtcdbenterm
...
<<<<<<< HEAD
        if s2420_evtcdbenterm.versao in VERSOES_ESOCIAL:
=======
        if s2420_evtcdbenterm.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s2420_evtcdbenterm_id, db_slug)
