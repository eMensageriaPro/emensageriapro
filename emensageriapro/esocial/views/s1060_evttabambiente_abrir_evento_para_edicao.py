...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1060evtTabAmbiente'):

        if s1060_evttabambiente_id:
            s1060_evttabambiente = get_object_or_404(s1060evtTabAmbiente, excluido=False, id=s1060_evttabambiente_id)
            ...
            if s1060_evttabambiente.status in status_list:
                s1060evtTabAmbiente.objects.filter(id=s1060_evttabambiente_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
