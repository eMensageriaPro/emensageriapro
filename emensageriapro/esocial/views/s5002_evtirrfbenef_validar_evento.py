...
def validar_evento_funcao(request, s5002_evtirrfbenef_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s5002_evtirrfbenef_id)
...
def validar_evento(request, hash):
    ...
    if s5002_evtirrfbenef_id:
        ...
        if s5002_evtirrfbenef.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s5002_evtirrfbenef_id)
            
            if s5002_evtirrfbenef.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5002evtIrrfBenef.objects.\
                    filter(id=s5002_evtirrfbenef_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5002_evtirrfbenef.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5002evtIrrfBenef.objects.\
                    filter(id=s5002_evtirrfbenef_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
