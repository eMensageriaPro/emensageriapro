...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2400_evtcdbenprrp_verificar.html' % s2400_evtcdbenprrp.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2400_evtcdbenprrp_verificar.html' % s2400_evtcdbenprrp.versao,
                                           filename="s2400_evtcdbenprrp.pdf",
                                           context=context,
            ...
            response =  render_to_response('%s/s2400_evtcdbenprrp_verificar.html' % s2400_evtcdbenprrp.versao, context)
            filename = "%s.xls" % s2400_evtcdbenprrp.identidade
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('%s/s2400_evtcdbenprrp_verificar.html' % s2400_evtcdbenprrp.versao, context)
            filename = "%s.csv" % s2400_evtcdbenprrp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s2400_evtcdbenprrp = get_object_or_404(
            s2400evtCdBenPrRP.objects.using( db_slug ),
            excluido = False, id = s2400_evtcdbenprrp_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s2400_evtcdbenprrp.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s2400_evtcdbenprrp_recibo_pdf.html', context)
            filename = "%s.xls" % s2400_evtcdbenprrp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s2400_evtcdbenprrp_recibo_csv.html', context)
            filename = "%s.csv" % s2400_evtcdbenprrp.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
        else:
            return render_to_pdf('s2400_evtcdbenprrp_recibo_pdf.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s2400_evtcdbenprrp_id, db_slug):
    import os
    ...
    s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False, id=s2400_evtcdbenprrp_id)
    if s2400_evtcdbenprrp.arquivo_original:
        xml = ler_arquivo(s2400_evtcdbenprrp.arquivo)
    else:
    ...
    if s2400_evtcdbenprrp.status in (0,1,2,11):
        s2400evtCdBenPrRP.objects.using(db_slug).filter(id=s2400_evtcdbenprrp_id,excluido=False).update(status=10)
    arquivo = 'arquivos/Eventos/s2400_evtcdbenprrp/%s.xml' % (s2400_evtcdbenprrp.identidade)
    os.system('mkdir -p %s/arquivos/Eventos/s2400_evtcdbenprrp/' % BASE_DIR)
...
def gerar_xml(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False, id=s2400_evtcdbenprrp_id)
        xml_assinado = gerar_xml_assinado(s2400_evtcdbenprrp_id, db_slug)
...
def duplicar(request, hash):
    ...
    s2400_evtcdbenprrp_id = int(dict_hash['id'])
    if s2400_evtcdbenprrp_id:
        s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False,
                                                    id=s2400_evtcdbenprrp_id)

        arquivo = 'arquivos/Eventos/s2400_evtcdbenprrp/%s.xml' % s2400_evtcdbenprrp.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s2400_evtcdbenprrp_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s.xml' % s2400_evtcdbenprrp.identidade)
        salvar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_duplicado_temp.xml' % s2400_evtcdbenprrp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_duplicado_temp.xml' % s2400_evtcdbenprrp.identidade, request)
        from emensageriapro.esocial.views.s2400_evtcdbenprrp import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2400evtCdBenPrRP.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2400_evtcdbenprrp.identidade),
            's2400_evtcdbenprrp', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_alteracao_temp.xml' % s2400_evtcdbenprrp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_alteracao_temp.xml' % s2400_evtcdbenprrp.identidade, request)
        from emensageriapro.esocial.views.s2400_evtcdbenprrp import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2400evtCdBenPrRP.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2400_evtcdbenprrp.identidade),
            's2400_evtcdbenprrp', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_exclusao_temp.xml' % s2400_evtcdbenprrp.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2400_evtcdbenprrp/%s_exclusao_temp.xml' % s2400_evtcdbenprrp.identidade, request)
        from emensageriapro.esocial.views.s2400_evtcdbenprrp import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2400evtCdBenPrRP.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s2400_evtcdbenprrp.identidade),
            's2400_evtcdbenprrp', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False,
                                                    id=s2400_evtcdbenprrp_id)
        if s2400_evtcdbenprrp.status == 0:
            from emensageriapro.esocial.views.s2400_evtcdbenprrp import identidade_evento
            dent = identidade_evento(s2400_evtcdbenprrp_id, db_slug)
            messages.success(request, 'Identidade do evento alterada com sucesso!')
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % s2400_evtcdbenprrp_id )
            usuario_id = request.session['usuario_id']
            gravar_auditoria(u'{}', u'{"funcao": "Identidade do evento foi alterada"}',
            's2400_evtcdbenprrp', s2400_evtcdbenprrp_id, usuario_id, 1)
            return redirect('s2400_evtcdbenprrp_salvar', hash=url_hash)
        else:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False, id=s2400_evtcdbenprrp_id)
        if s2400_evtcdbenprrp.status in (0, 1, 2, 3, 4, 10, 11) or s2400_evtcdbenprrp.processamento_codigo_resposta in (401,402):
            s2400evtCdBenPrRP.objects.using(db_slug).filter(id=s2400_evtcdbenprrp_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2400_evtcdbenprrp/%s.xml' % (s2400_evtcdbenprrp.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s2400_evtcdbenprrp.identidade, BASE_DIR, s2400_evtcdbenprrp.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s2400_evtcdbenprrp/%s.xml %s/arquivos/Eventos/s2400_evtcdbenprrp/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s2400_evtcdbenprrp/%s_backup_%s.xml' % (s2400_evtcdbenprrp.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s2400_evtcdbenprrp_id, db_slug):
    ...
    lista_validacoes = []
    s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False, id=s2400_evtcdbenprrp_id)
    quant = validar_precedencia('esocial', 's2400_evtcdbenprrp', s2400_evtcdbenprrp_id)
    if quant <= 0:
        #lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    else:
        precedencia = 1
    executar_sql("UPDATE public.s2400_evtcdbenprrp SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s2400_evtcdbenprrp_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s2400_evtcdbenprrp/%s.xml' % (s2400_evtcdbenprrp.identidade)
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s2400_evtcdbenprrp_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2400_evtcdbenprrp import validacoes_s2400_evtcdbenprrp
                lista = validacoes_s2400_evtcdbenprrp(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2400_evtcdbenprrp import validacoes_s2400_evtcdbenprrp
                lista = validacoes_s2400_evtcdbenprrp(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s2400_evtcdbenprrp SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s2400_evtcdbenprrp_id), False)
    else:
        executar_sql("UPDATE public.s2400_evtcdbenprrp SET validacoes='', status=4 WHERE id=%s;" % (s2400_evtcdbenprrp_id), False)
...
def validar_evento(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        lista_validacoes = []
        s2400_evtcdbenprrp = get_object_or_404(s2400evtCdBenPrRP.objects.using(db_slug), excluido=False, id=s2400_evtcdbenprrp_id)
        if s2400_evtcdbenprrp.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2400_evtcdbenprrp_id, db_slug)
