...
def gerar_xml_s2410(request, s2410_evtcdbenin_id, versao=None):
    ...
    if s2410_evtcdbenin_id:
        ...
        s2410_evtcdbenin_lista = s2410evtCdBenIn.objects. \
            filter(id=s2410_evtcdbenin_id, excluido = False).all()
...
def gerar_xml_assinado(request, s2410_evtcdbenin_id):
    ...
    if s2410_evtcdbenin.arquivo_original:
        ...
        xml = gerar_xml_s2410(request, s2410_evtcdbenin_id)
    ...
    if 'Signature' in xml:
        ...
        if not s2410_evtcdbenin.transmissor_lote_esocial:
            ...
            grupo = get_grupo(s2410evtCdBenIn)
        ...
        xml_assinado = assinar_esocial(request, xml, s2410_evtcdbenin.transmissor_lote_esocial_id)
    ...
    if s2410_evtcdbenin.status in (
        ...
        s2410evtCdBenIn.objects.\
            filter(id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s2410_evtcdbenin_id)
