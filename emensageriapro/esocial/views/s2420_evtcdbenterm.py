...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2420_evtcdbenterm.status == 0:
            ...
            s2420evtCdBenTerm.objects.using( db_slug ).filter(id = s2420_evtcdbenterm_id).delete()
            #s2420_evtcdbenterm_apagar_custom
            #s2420_evtcdbenterm_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2420_evtcdbenterm', s2420_evtcdbenterm_id, usuario_id, 3)
        else:
...
class s2420evtCdBenTermList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2420evtCdBenTerm.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2420_evtcdbenterm_lista = s2420evtCdBenTerm.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2420_evtcdbenterm_csv.html', context)
...
def identidade_evento(s2420_evtcdbenterm_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2420evtCdBenTerm.objects.using( db_slug ).get(id=s2420_evtcdbenterm_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2420evtCdBenTerm.objects.using(db_slug).filter(id=s2420_evtcdbenterm_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2420_evtcdbenterm_id:
            ...
            s2420_evtcdbenterm_form = form_s2420_evtcdbenterm(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s2420_evtcdbenterm_form.is_valid():
                ...
                if s2420_evtcdbenterm_id:
                    if s2420_evtcdbenterm.status == 0:
                        ...
                        #s2420_evtcdbenterm_campos_multiple_passo1
                        s2420evtCdBenTerm.objects.using(db_slug).filter(id=s2420_evtcdbenterm_id).update(**dados)
                        obj = s2420evtCdBenTerm.objects.using(db_slug).get(id=s2420_evtcdbenterm_id)
                        #s2420_evtcdbenterm_editar_custom
                        #s2420_evtcdbenterm_campos_multiple_passo2
                    ...
                    #s2420_evtcdbenterm_cadastrar_campos_multiple_passo1
                    obj = s2420evtCdBenTerm(**dados)
                    obj.save(using = db_slug)
                    #s2420_evtcdbenterm_cadastrar_custom
                    #s2420_evtcdbenterm_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s2420_evtcdbenterm_form = form_s2420_evtcdbenterm(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        s2420_evtcdbenterm_form = disabled_form_fields(s2420_evtcdbenterm_form, permissao.permite_editar)
        ...
        if not evento_totalizador:
            s2420_evtcdbenterm_form.fields['tpamb'].widget.attrs['disabled'] = True
            s2420_evtcdbenterm_form.fields['tpamb'].widget.attrs['readonly'] = True
            s2420_evtcdbenterm_form.fields['tpamb'].value = TP_AMB
            s2420_evtcdbenterm_form.fields['procemi'].widget.attrs['disabled'] = True
            s2420_evtcdbenterm_form.fields['procemi'].widget.attrs['readonly'] = True
            s2420_evtcdbenterm_form.fields['procemi'].value = 1
            s2420_evtcdbenterm_form.fields['verproc'].widget.attrs['readonly'] = True
            s2420_evtcdbenterm_form.fields['verproc'].value = VERSAO_EMENSAGERIA
            s2420_evtcdbenterm_form.fields['status'].widget.attrs['disabled'] = True
            s2420_evtcdbenterm_form.fields['status'].widget.attrs['readonly'] = True
            s2420_evtcdbenterm_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2420_evtcdbenterm_id, tabela='s2420_evtcdbenterm').all()
        context = {
