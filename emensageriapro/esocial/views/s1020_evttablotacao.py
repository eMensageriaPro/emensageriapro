...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1020_evttablotacao_lista = s1020evtTabLotacao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1020_evttablotacao_lista) > 100:
            filtrar = True
            s1020_evttablotacao_lista = None
            messages.warning(request, 'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')

        transmissor_lote_esocial_lista = TransmissorLoteEsocial.objects.using( db_slug ).filter(excluido = False).all()
        #s1020_evttablotacao_listar_custom
        ...
        #return render(request, 's1020_evttablotacao_listar.html', context)
        if for_print in (0,1):
            return render(request, 's1020_evttablotacao_listar.html', context)
        elif for_print == 2:
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1020_evttablotacao_listar.html',
            ...
            response = render_to_response('s1020_evttablotacao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1020_evttablotacao_csv.html', context)
            filename = "s1020_evttablotacao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s1020_evttablotacao_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1020evtTabLotacao.objects.using( db_slug ).get(id=s1020_evttablotacao_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1020evtTabLotacao.objects.using(db_slug).filter(id=s1020_evttablotacao_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1020_evttablotacao_id:
            s1020_evttablotacao_form = form_s1020_evttablotacao(request.POST or None, instance = s1020_evttablotacao, slug = db_slug)
        else:
            s1020_evttablotacao_form = form_s1020_evttablotacao(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1020_evttablotacao_form.is_valid():
                ...
                if s1020_evttablotacao_id:
                    if s1020_evttablotacao.status == 0:
                        ...
                        #s1020_evttablotacao_campos_multiple_passo1
                        s1020evtTabLotacao.objects.using(db_slug).filter(id=s1020_evttablotacao_id).update(**dados)
                        obj = s1020evtTabLotacao.objects.using(db_slug).get(id=s1020_evttablotacao_id)
                        #s1020_evttablotacao_editar_custom
                        #s1020_evttablotacao_campos_multiple_passo2
                    ...
                    #s1020_evttablotacao_cadastrar_campos_multiple_passo1
                    obj = s1020evtTabLotacao(**dados)
                    obj.save(using = db_slug)
                    #s1020_evttablotacao_cadastrar_custom
                    #s1020_evttablotacao_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1020_evttablotacao_form = form_s1020_evttablotacao(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        s1020_evttablotacao_form = disabled_form_fields(s1020_evttablotacao_form, permissao.permite_editar)
        ...
        if s1020_evttablotacao_id:
            ...
            s1020_inclusao_form = form_s1020_inclusao(initial={ 's1020_evttablotacao': s1020_evttablotacao }, slug=db_slug)
            s1020_inclusao_form.fields['s1020_evttablotacao'].widget.attrs['readonly'] = True
            s1020_inclusao_lista = s1020inclusao.objects.using( db_slug ).filter(excluido = False, s1020_evttablotacao_id=s1020_evttablotacao.id).all()
            s1020_alteracao_form = form_s1020_alteracao(initial={ 's1020_evttablotacao': s1020_evttablotacao }, slug=db_slug)
            s1020_alteracao_form.fields['s1020_evttablotacao'].widget.attrs['readonly'] = True
            s1020_alteracao_lista = s1020alteracao.objects.using( db_slug ).filter(excluido = False, s1020_evttablotacao_id=s1020_evttablotacao.id).all()
            s1020_exclusao_form = form_s1020_exclusao(initial={ 's1020_evttablotacao': s1020_evttablotacao }, slug=db_slug)
            s1020_exclusao_form.fields['s1020_evttablotacao'].widget.attrs['readonly'] = True
            s1020_exclusao_lista = s1020exclusao.objects.using( db_slug ).filter(excluido = False, s1020_evttablotacao_id=s1020_evttablotacao.id).all()
        else:
        ...
        if not evento_totalizador:
            s1020_evttablotacao_form.fields['tpamb'].widget.attrs['disabled'] = True
            s1020_evttablotacao_form.fields['tpamb'].widget.attrs['readonly'] = True
            s1020_evttablotacao_form.fields['tpamb'].value = TP_AMB
            s1020_evttablotacao_form.fields['procemi'].widget.attrs['disabled'] = True
            s1020_evttablotacao_form.fields['procemi'].widget.attrs['readonly'] = True
            s1020_evttablotacao_form.fields['procemi'].value = 1
            s1020_evttablotacao_form.fields['verproc'].widget.attrs['readonly'] = True
            s1020_evttablotacao_form.fields['verproc'].value = VERSAO_EMENSAGERIA
            s1020_evttablotacao_form.fields['status'].widget.attrs['disabled'] = True
            s1020_evttablotacao_form.fields['status'].widget.attrs['readonly'] = True
            s1020_evttablotacao_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1020_evttablotacao_id, tabela='s1020_evttablotacao').all()
        context = {
