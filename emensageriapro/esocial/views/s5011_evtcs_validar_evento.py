...
def validar_evento_funcao(request, s5011_evtcs_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s5011_evtcs_id)
...
def validar_evento(request, hash):
    ...
    if s5011_evtcs_id:
        ...
        if s5011_evtcs.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s5011_evtcs_id)
            
            if s5011_evtcs.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5011evtCS.objects.\
                    filter(id=s5011_evtcs_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5011_evtcs.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5011evtCS.objects.\
                    filter(id=s5011_evtcs_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
