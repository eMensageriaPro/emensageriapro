...
def validar_evento_funcao(request, s5001_evtbasestrab_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s5001_evtbasestrab_id)
...
def validar_evento(request, hash):
    ...
    if s5001_evtbasestrab_id:
        ...
        if s5001_evtbasestrab.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s5001_evtbasestrab_id)
            
            if s5001_evtbasestrab.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5001evtBasesTrab.objects.\
                    filter(id=s5001_evtbasestrab_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5001_evtbasestrab.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5001evtBasesTrab.objects.\
                    filter(id=s5001_evtbasestrab_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
