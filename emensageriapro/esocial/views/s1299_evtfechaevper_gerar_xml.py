...
def gerar_xml_s1299(request, s1299_evtfechaevper_id, versao=None):
    ...
    if s1299_evtfechaevper_id:
        ...
        s1299_evtfechaevper_lista = s1299evtFechaEvPer.objects. \
            filter(id=s1299_evtfechaevper_id, excluido = False).all()
...
def gerar_xml_assinado(request, s1299_evtfechaevper_id):
    ...
    if s1299_evtfechaevper.arquivo_original:
        ...
        xml = gerar_xml_s1299(request, s1299_evtfechaevper_id)
    ...
    if 'Signature' in xml:
        ...
        xml_assinado = assinar_esocial(request, xml, s1299_evtfechaevper.transmissor_lote_esocial_id)
    ...
    if s1299_evtfechaevper.status in (
        ...
        s1299evtFechaEvPer.objects.\
            filter(id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s1299_evtfechaevper_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s1299_evtfechaevper_id)
