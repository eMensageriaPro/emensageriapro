...
def gerar_xml_assinado(s2241_evtinsapo_id, db_slug):
    ...
    if s2241_evtinsapo.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2241evtInsApo.objects.using(db_slug).\
            filter(id=s2241_evtinsapo_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2241_evtinsapo_id:
        ...
        if s2241_evtinsapo.status in status_list:
            s2241evtInsApo.objects.using(db_slug).filter(id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2241_evtinsapo/%s.xml' % (s2241_evtinsapo.identidade)
...
def validar_evento(request, hash):
    ...
    if s2241_evtinsapo_id:
        ...
        if s2241_evtinsapo.versao in VERSOES_ESOCIAL:
            ...
            if s2241_evtinsapo.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2241evtInsApo.objects.using(db_slug).\
                    filter(excluido=False, id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2241_evtinsapo.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2241evtInsApo.objects.using(db_slug).\
                    filter(excluido=False, id=s2241_evtinsapo_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
