...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1030_evttabcargo_id:
            ...
            s1030_evttabcargo_form = form_s1030_evttabcargo(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1030_evttabcargo_form.is_valid():
                ...
                if s1030_evttabcargo_id:
                    ...
                    s1030_evttabcargo_form = form_s1030_evttabcargo(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            ...
            s1030_evttabcargo_form.fields['status'].widget.attrs['disabled'] = True
            s1030_evttabcargo_form.fields['status'].widget.attrs['readonly'] = True
            s1030_evttabcargo_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
