...
def gerar_xml_s1070(s1070_evttabprocesso_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1070_evttabprocesso_id, db_slug):
    ...
    if s1070_evttabprocesso.status in (0,1,2,11):
        ...
        s1070evtTabProcesso.objects.using(db_slug).\
            filter(id=s1070_evttabprocesso_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        texto = gerar_xml_s1070(s1070_evttabprocesso_id, db_slug, versao="|")
        dados = read_s1070_evttabprocesso_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1070evtTabProcesso.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1070_evttabprocesso.identidade),
            's1070_evttabprocesso', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1070_evttabprocesso.identidade),
            's1070_evttabprocesso', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        if s1070_evttabprocesso.status == 0:
            ...
            nova_identidade = identidade_evento(s1070_evttabprocesso_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1070_evttabprocesso', s1070_evttabprocesso_id, request.user.id, 1)
...
def validar_evento_funcao(s1070_evttabprocesso_id, db_slug):
    ...
    s1070evtTabProcesso.objects.using( db_slug ).\
        filter(id=s1070_evttabprocesso_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1070_evttabprocesso_validar import validacoes_s1070_evttabprocesso
...
<<<<<<< HEAD
        if s1070_evttabprocesso.versao in VERSOES_ESOCIAL:
=======
        if s1070_evttabprocesso.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1070_evttabprocesso_id, db_slug)
