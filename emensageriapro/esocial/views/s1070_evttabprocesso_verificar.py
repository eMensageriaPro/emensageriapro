...
def gerar_xml_assinado(s1070_evttabprocesso_id, db_slug):
    ...
    if s1070_evttabprocesso.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1070evtTabProcesso.objects.using(db_slug).\
            filter(id=s1070_evttabprocesso_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def validar_evento(request, hash):
    ...
    if s1070_evttabprocesso_id:
        ...
        if s1070_evttabprocesso.versao in VERSOES_ESOCIAL:
            ...
            if s1070_evttabprocesso.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1070evtTabProcesso.objects.using(db_slug).\
                    filter(excluido=False, id=s1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1070_evttabprocesso.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1070evtTabProcesso.objects.using(db_slug).\
                    filter(excluido=False, id=s1070_evttabprocesso_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
