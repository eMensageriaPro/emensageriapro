...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1010evtTabRubrica'):

        if s1010_evttabrubrica_id:
            s1010_evttabrubrica = get_object_or_404(s1010evtTabRubrica, excluido=False, id=s1010_evttabrubrica_id)
            ...
            if s1010_evttabrubrica.status in status_list:
                s1010evtTabRubrica.objects.filter(id=s1010_evttabrubrica_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
