...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1295evtTotConting'):

        if s1295_evttotconting_id:
            s1295_evttotconting = get_object_or_404(s1295evtTotConting, excluido=False, id=s1295_evttotconting_id)
            ...
            if s1295_evttotconting.status in status_list:
                s1295evtTotConting.objects.filter(id=s1295_evttotconting_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
