...
def gerar_xml_s1040(s1040_evttabfuncao_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1040_evttabfuncao_id, db_slug):
    ...
    if s1040_evttabfuncao.status in (0,1,2,11):
        ...
        s1040evtTabFuncao.objects.using(db_slug).\
            filter(id=s1040_evttabfuncao_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s1040_evttabfuncao_importar import read_s1040_evttabfuncao_string
    ...
    if s1040_evttabfuncao_id:
        ...
        texto = gerar_xml_s1040(s1040_evttabfuncao_id, db_slug, versao="|")
        dados = read_s1040_evttabfuncao_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1040evtTabFuncao.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1040_evttabfuncao.identidade),
            's1040_evttabfuncao', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1040_evttabfuncao.identidade),
            's1040_evttabfuncao', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1040_evttabfuncao.identidade),
            's1040_evttabfuncao', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        if s1040_evttabfuncao.status == 0:
            ...
            nova_identidade = identidade_evento(s1040_evttabfuncao_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1040_evttabfuncao', s1040_evttabfuncao_id, request.user.id, 1)
...
def validar_evento_funcao(s1040_evttabfuncao_id, db_slug):
    ...
    s1040evtTabFuncao.objects.using( db_slug ).\
        filter(id=s1040_evttabfuncao_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1040_evttabfuncao_validar import validacoes_s1040_evttabfuncao
...
<<<<<<< HEAD
        if s1040_evttabfuncao.versao in VERSOES_ESOCIAL:
=======
        if s1040_evttabfuncao.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1040_evttabfuncao_id, db_slug)
