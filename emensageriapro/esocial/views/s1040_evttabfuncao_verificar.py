...
def gerar_xml_assinado(s1040_evttabfuncao_id, db_slug):
    ...
    if s1040_evttabfuncao.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1040evtTabFuncao.objects.using(db_slug).\
            filter(id=s1040_evttabfuncao_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        if s1040_evttabfuncao.status in status_list:
            s1040evtTabFuncao.objects.using(db_slug).filter(id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1040_evttabfuncao/%s.xml' % (s1040_evttabfuncao.identidade)
...
def validar_evento(request, hash):
    ...
    if s1040_evttabfuncao_id:
        ...
        if s1040_evttabfuncao.versao in VERSOES_ESOCIAL:
            ...
            if s1040_evttabfuncao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1040evtTabFuncao.objects.using(db_slug).\
                    filter(excluido=False, id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1040_evttabfuncao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1040evtTabFuncao.objects.using(db_slug).\
                    filter(excluido=False, id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
