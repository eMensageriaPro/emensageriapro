...
def validar_evento_funcao(request, pk):
    ...
    if not s1298_evtreabreevper.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s1298_evtreabreevper, 'esocial')
        s1298_evtreabreevper = get_object_or_404(s1298evtReabreEvPer, id=pk)
    ...
    schema_filename = 'esocial/%s/evtReabreEvPer.xsd' % (s1298_evtreabreevper.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1298_evtreabreevper' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1298_evtreabreevper', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1298_evtreabreevper.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s1298_evtreabreevper_salvar', pk=pk)