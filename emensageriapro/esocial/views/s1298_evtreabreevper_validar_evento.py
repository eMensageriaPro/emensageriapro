...
def validar_evento_funcao(request, s1298_evtreabreevper_id):
    ...
    lista_validacoes = []
    s1298_evtreabreevper = get_object_or_404(s1298evtReabreEvPer, id=s1298_evtreabreevper_id)
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1298_evtreabreevper_id)
...
def validar_evento(request, hash):
    ...
    if s1298_evtreabreevper_id:
        ...
        if s1298_evtreabreevper.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1298_evtreabreevper_id)
            
            if s1298_evtreabreevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1298evtReabreEvPer.objects.\
                    filter(id=s1298_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1298_evtreabreevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1298evtReabreEvPer.objects.\
                    filter(id=s1298_evtreabreevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
