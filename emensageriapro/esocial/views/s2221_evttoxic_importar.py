...
def read_s2221_evttoxic(request, dados, arquivo, validar=False):
    ...
    s2221evtToxic.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2221_evttoxic_obj(request, doc, status, validar=False):
    ...
    s2221_evttoxic_dados['status'] = STATUS_EVENTO_IMPORTADO
