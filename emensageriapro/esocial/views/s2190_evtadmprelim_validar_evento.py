...
def validar_evento_funcao(request, s2190_evtadmprelim_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2190_evtadmprelim_id)
...
def validar_evento(request, hash):
    ...
    if s2190_evtadmprelim_id:
        ...
        if s2190_evtadmprelim.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2190_evtadmprelim_id)
            
            if s2190_evtadmprelim.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2190evtAdmPrelim.objects.\
                    filter(id=s2190_evtadmprelim_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2190_evtadmprelim.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2190evtAdmPrelim.objects.\
                    filter(id=s2190_evtadmprelim_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
