...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1260evtComProd'):

        if s1260_evtcomprod_id:
            s1260_evtcomprod = get_object_or_404(s1260evtComProd, excluido=False, id=s1260_evtcomprod_id)
            ...
            if s1260_evtcomprod.status in status_list:
                s1260evtComProd.objects.filter(id=s1260_evtcomprod_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
