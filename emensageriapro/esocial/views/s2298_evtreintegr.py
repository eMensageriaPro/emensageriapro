...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2298_evtreintegr.status == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s2298_evtreintegr), indent=4, sort_keys=True, default=str)
            s2298evtReintegr.objects.using( db_slug ).filter(id = s2298_evtreintegr_id).delete()
            #s2298_evtreintegr_apagar_custom
            #s2298_evtreintegr_apagar_custom
    ...
    return render(request, 's2298_evtreintegr_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s2298evtReintegrList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2298evtReintegr.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        show_fields = {
            ...
            'show_nrprocjud': 0,
            'show_nrleianistia': 0,
            'show_dtefetretorno': 1,
            'show_dtefeito': 1,
            'show_indpagtojuizo': 1, }
        post = False
        if request.method == 'POST':
            post = True
            dict_fields = {
                ...
                'inforeintegr': 'inforeintegr',
                'tpreint': 'tpreint',
                'nrprocjud__icontains': 'nrprocjud__icontains',
                'nrleianistia__icontains': 'nrleianistia__icontains',
                'dtefetretorno__range': 'dtefetretorno__range',
                'dtefeito__range': 'dtefeito__range',
                'indpagtojuizo__icontains': 'indpagtojuizo__icontains',}
        ...
        s2298_evtreintegr_lista = s2298evtReintegr.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2298_evtreintegr_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2298_evtreintegr_listar.html',
            ...
            response = render_to_response('s2298_evtreintegr_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2298_evtreintegr_csv.html', context)
            filename = "s2298_evtreintegr.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s2298_evtreintegr_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s2298evtReintegr.objects.using( db_slug ).get(id=s2298_evtreintegr_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s2298evtReintegr.objects.using(db_slug).filter(id=s2298_evtreintegr_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2298_evtreintegr_id:
            ...
            s2298_evtreintegr_form = form_s2298_evtreintegr(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s2298_evtreintegr_form.is_valid():
                ...
                if s2298_evtreintegr_id:
                    if s2298_evtreintegr.status == 0:
                        ...
                        #s2298_evtreintegr_campos_multiple_passo1
                        s2298evtReintegr.objects.using(db_slug).filter(id=s2298_evtreintegr_id).update(**dados)
                        obj = s2298evtReintegr.objects.using(db_slug).get(id=s2298_evtreintegr_id)
                        #s2298_evtreintegr_editar_custom
                        #s2298_evtreintegr_campos_multiple_passo2
                    ...
                    #s2298_evtreintegr_cadastrar_campos_multiple_passo1
                    obj = s2298evtReintegr(**dados)
                    obj.save(using = db_slug)
                    #s2298_evtreintegr_cadastrar_custom
                    #s2298_evtreintegr_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s2298_evtreintegr_form = form_s2298_evtreintegr(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s2298_evtreintegr_form.fields['tpamb'].widget.attrs['disabled'] = True
            s2298_evtreintegr_form.fields['tpamb'].widget.attrs['readonly'] = True
            s2298_evtreintegr_form.fields['tpamb'].value = TP_AMB
            s2298_evtreintegr_form.fields['procemi'].widget.attrs['disabled'] = True
            s2298_evtreintegr_form.fields['procemi'].widget.attrs['readonly'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2298_evtreintegr_id, tabela='s2298_evtreintegr').all()
        context = {
