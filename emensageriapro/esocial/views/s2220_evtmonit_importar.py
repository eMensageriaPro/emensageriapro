...
def read_s2220_evtmonit(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s2220_evtmonit_obj(request, doc, status, validar, arquivo)

    s2220evtMonit.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2220_evtmonit_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s2220_evtmonit_dados['arquivo'] = arquivo
