...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s5012_evtirrf_verificar.html' % s5012_evtirrf.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s5012_evtirrf_verificar.html' % s5012_evtirrf.versao,
            ...
            response =  render_to_response('%s/s5012_evtirrf_verificar.html' % s5012_evtirrf.versao, context)
            ...
            return render(request, '%s/s5012_evtirrf_verificar.html' % s5012_evtirrf.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_assinado(s5012_evtirrf_id, db_slug):
    ...
    if s5012_evtirrf.status in (0,1,2,11):
        s5012evtIrrf.objects.using(db_slug).filter(id=s5012_evtirrf_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s5012_evtirrf_id = int(dict_hash['id'])
    if s5012_evtirrf_id:
        s5012_evtirrf = get_object_or_404(s5012evtIrrf.objects.using(db_slug), excluido=False,
                                                    id=s5012_evtirrf_id)

        arquivo = 'arquivos/Eventos/s5012_evtirrf/%s.xml' % s5012_evtirrf.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s5012_evtirrf_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s5012_evtirrf/%s.xml' % s5012_evtirrf.identidade)
        salvar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_duplicado_temp.xml' % s5012_evtirrf.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_duplicado_temp.xml' % s5012_evtirrf.identidade, request)
        from emensageriapro.esocial.views.s5012_evtirrf import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s5012evtIrrf.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s5012_evtirrf.identidade),
            's5012_evtirrf', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s5012_evtirrf_id:
        ...
        salvar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_alteracao_temp.xml' % s5012_evtirrf.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_alteracao_temp.xml' % s5012_evtirrf.identidade, request)
        from emensageriapro.esocial.views.s5012_evtirrf import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s5012evtIrrf.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s5012_evtirrf.identidade),
            's5012_evtirrf', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s5012_evtirrf_id:
        ...
        salvar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_exclusao_temp.xml' % s5012_evtirrf.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s5012_evtirrf/%s_exclusao_temp.xml' % s5012_evtirrf.identidade, request)
        from emensageriapro.esocial.views.s5012_evtirrf import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s5012evtIrrf.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s5012_evtirrf.identidade),
            's5012_evtirrf', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s5012_evtirrf_id:
        ...
        if s5012_evtirrf.status == 0:
            from emensageriapro.esocial.views.s5012_evtirrf import identidade_evento
            dent = identidade_evento(s5012_evtirrf_id, db_slug)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5012_evtirrf_id:
        s5012_evtirrf = get_object_or_404(s5012evtIrrf.objects.using(db_slug), excluido=False, id=s5012_evtirrf_id)
        if s5012_evtirrf.status in (0, 1, 2, 3, 4, 10, 11) or s5012_evtirrf.processamento_codigo_resposta in (401,402):
            s5012evtIrrf.objects.using(db_slug).filter(id=s5012_evtirrf_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s5012_evtirrf/%s.xml' % (s5012_evtirrf.identidade)
...
def validar_evento_funcao(s5012_evtirrf_id, db_slug):
    ...
    lista_validacoes = []
    s5012_evtirrf = get_object_or_404(s5012evtIrrf.objects.using(db_slug), excluido=False, id=s5012_evtirrf_id)
    if s5012_evtirrf.transmissor_lote_esocial:
        if s5012_evtirrf.transmissor_lote_esocial.transmissor:
            if s5012_evtirrf.transmissor_lote_esocial.transmissor.verificar_predecessao:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s5012_evtirrf SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s5012_evtirrf_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s5012_evtirrf/%s.xml' % (s5012_evtirrf.identidade)
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s5012_evtirrf import validacoes_s5012_evtirrf
                lista = validacoes_s5012_evtirrf(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s5012_evtirrf import validacoes_s5012_evtirrf
                lista = validacoes_s5012_evtirrf(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s5012_evtirrf SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s5012_evtirrf_id), False)
    else:
        executar_sql("UPDATE public.s5012_evtirrf SET validacoes='', status=4 WHERE id=%s;" % (s5012_evtirrf_id), False)
...
def validar_evento(request, hash):
    ...
    if s5012_evtirrf_id:
        lista_validacoes = []
        s5012_evtirrf = get_object_or_404(s5012evtIrrf.objects.using(db_slug), excluido=False, id=s5012_evtirrf_id)
        if s5012_evtirrf.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s5012_evtirrf_id, db_slug)
