...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1260_evtcomprod.status == 0:
            ...
            s1260evtComProd.objects.using( db_slug ).filter(id = s1260_evtcomprod_id).delete()
...
class s1260evtComProdList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1260evtComProd.objects.using(db_slug).all()
    serializer_class = s1260evtComProdSerializer
    permission_classes = (IsAdminUser,)


class s1260evtComProdDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1260evtComProd.objects.using(db_slug).all()
    serializer_class = s1260evtComProdSerializer
    permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if request.method == 'POST':
            ...
            dict_fields = {
                ...
                'nrinscestabrural__icontains': 'nrinscestabrural__icontains',
        ...
        s1260_evtcomprod_lista = s1260evtComProd.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1260_evtcomprod_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1260_evtcomprod_listar.html',
            ...
            response = render_to_response('s1260_evtcomprod_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1260_evtcomprod_csv.html', context)
            filename = "s1260_evtcomprod.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s1260_evtcomprod_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1260evtComProd.objects.using( db_slug ).get(id=s1260_evtcomprod_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1260evtComProd.objects.using(db_slug).filter(id=s1260_evtcomprod_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1260_evtcomprod_id:
            s1260_evtcomprod_form = form_s1260_evtcomprod(request.POST or None, instance = s1260_evtcomprod, slug = db_slug)
        else:
            s1260_evtcomprod_form = form_s1260_evtcomprod(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1260_evtcomprod_form.is_valid():
                dados = s1260_evtcomprod_form.cleaned_data
                import json
                from django.forms.models import model_to_dict
                if s1260_evtcomprod_id:
                    if s1260_evtcomprod.status == 0:
                        ...
                        #s1260_evtcomprod_campos_multiple_passo1
                        s1260evtComProd.objects.using(db_slug).filter(id=s1260_evtcomprod_id).update(**dados)
                        obj = s1260evtComProd.objects.using(db_slug).get(id=s1260_evtcomprod_id)
                        #s1260_evtcomprod_editar_custom
                        #s1260_evtcomprod_campos_multiple_passo2
                        messages.success(request, 'Alterado com sucesso!')
                        gravar_auditoria(json.dumps(model_to_dict(s1260_evtcomprod), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1260_evtcomprod', s1260_evtcomprod_id, usuario_id, 2)
                    else:
                        obj = s1260evtComProd.objects.using(db_slug).get(id=s1260_evtcomprod_id)
                    ...
                    #s1260_evtcomprod_cadastrar_campos_multiple_passo1
                    obj = s1260evtComProd(**dados)
                    obj.save(using = db_slug)
                    #s1260_evtcomprod_cadastrar_custom
                    #s1260_evtcomprod_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1260_evtcomprod_form = form_s1260_evtcomprod(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1260_evtcomprod', obj.id, usuario_id, 1)
                if request.session['retorno_pagina'] not in ('s1260_evtcomprod_apagar', 's1260_evtcomprod_salvar', 's1260_evtcomprod'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                if s1260_evtcomprod_id != obj.id:
        ...
        #             s1260_evtcomprod_form.fields['tpamb'].widget.attrs['disabled'] = True
        #             s1260_evtcomprod_form.fields['tpamb'].widget.attrs['readonly'] = True
        #             s1260_evtcomprod_form.fields['tpamb'].value = TP_AMB
        #             s1260_evtcomprod_form.fields['procemi'].widget.attrs['disabled'] = True
        #             s1260_evtcomprod_form.fields['procemi'].widget.attrs['readonly'] = True
        ...
        #             s1260_evtcomprod_form.fields['status'].widget.attrs['disabled'] = True
        #             s1260_evtcomprod_form.fields['status'].widget.attrs['readonly'] = True
        #             s1260_evtcomprod_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1260_evtcomprod_id, tabela='s1260_evtcomprod').all()
        context = {
