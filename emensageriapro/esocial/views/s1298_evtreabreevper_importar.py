...
def read_s1298_evtreabreevper(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s1298_evtreabreevper_obj(request, doc, status, validar, arquivo)

    s1298evtReabreEvPer.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s1298_evtreabreevper_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s1298_evtreabreevper_dados['arquivo'] = arquivo
