    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s2245_evttreicap_verificar.html',
            ...
            response =  render_to_response('s2245_evttreicap_verificar.html', context)
            ...
            return render(request, 's2245_evttreicap_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2245(s2245_evttreicap_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2245_evttreicap_id, db_slug):
    ...
    if s2245_evttreicap.status in (0,1,2,11):
        ...
        s2245evtTreiCap.objects.using(db_slug).\
            filter(id=s2245_evttreicap_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        texto = gerar_xml_s2245(s2245_evttreicap_id, db_slug, versao="|")
        dados = read_s2245_evttreicap_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s2245evtTreiCap.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2245_evttreicap.identidade),
...
def criar_alteracao(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2245_evttreicap.identidade),
            's2245_evttreicap', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2245_evttreicap.identidade),
            's2245_evttreicap', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        if s2245_evttreicap.status == 0:
            ...
            nova_identidade = identidade_evento(s2245_evttreicap_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2245_evttreicap', s2245_evttreicap_id, request.user.id, 1)
...
def validar_evento_funcao(s2245_evttreicap_id, db_slug):
    ...
    s2245evtTreiCap.objects.using( db_slug ).\
        filter(id=s2245_evttreicap_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2245_evttreicap_validar import validacoes_s2245_evttreicap
...
<<<<<<< HEAD
        if s2245_evttreicap.versao in VERSOES_ESOCIAL:
=======
        if s2245_evttreicap.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s2245_evttreicap_id, db_slug)
