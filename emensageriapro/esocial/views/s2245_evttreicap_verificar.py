...
def gerar_xml_assinado(s2245_evttreicap_id, db_slug):
    ...
    if s2245_evttreicap.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2245evtTreiCap.objects.using(db_slug).\
            filter(id=s2245_evttreicap_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        if s2245_evttreicap.status in status_list:
            s2245evtTreiCap.objects.using(db_slug).filter(id=s2245_evttreicap_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2245_evttreicap/%s.xml' % (s2245_evttreicap.identidade)
...
def validar_evento(request, hash):
    ...
    if s2245_evttreicap_id:
        ...
        if s2245_evttreicap.versao in VERSOES_ESOCIAL:
            ...
            if s2245_evttreicap.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2245evtTreiCap.objects.using(db_slug).\
                    filter(excluido=False, id=s2245_evttreicap_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2245_evttreicap.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2245evtTreiCap.objects.using(db_slug).\
                    filter(excluido=False, id=s2245_evttreicap_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
