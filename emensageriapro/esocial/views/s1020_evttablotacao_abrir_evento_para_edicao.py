...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1020evtTabLotacao'):

        if s1020_evttablotacao_id:
            s1020_evttablotacao = get_object_or_404(s1020evtTabLotacao, excluido=False, id=s1020_evttablotacao_id)
            ...
            if s1020_evttablotacao.status in status_list:
                s1020evtTabLotacao.objects.filter(id=s1020_evttablotacao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
