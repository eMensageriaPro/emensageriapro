...
def read_s1210_evtpgtos(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s1210_evtpgtos_obj(request, doc, status, validar, arquivo)

    s1210evtPgtos.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s1210_evtpgtos_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s1210_evtpgtos_dados['arquivo'] = arquivo
