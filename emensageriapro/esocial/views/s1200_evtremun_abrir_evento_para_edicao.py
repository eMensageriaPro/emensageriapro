...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1200evtRemun'):

        if s1200_evtremun_id:
            s1200_evtremun = get_object_or_404(s1200evtRemun, excluido=False, id=s1200_evtremun_id)
            ...
            if s1200_evtremun.status in status_list:
                s1200evtRemun.objects.filter(id=s1200_evtremun_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
