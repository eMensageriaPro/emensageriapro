...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s3000evtExclusao'):

        if s3000_evtexclusao_id:
            s3000_evtexclusao = get_object_or_404(s3000evtExclusao, excluido=False, id=s3000_evtexclusao_id)
            ...
            if s3000_evtexclusao.status in status_list:
                s3000evtExclusao.objects.filter(id=s3000_evtexclusao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
