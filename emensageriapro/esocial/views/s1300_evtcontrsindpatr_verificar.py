...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1300_evtcontrsindpatr_id:
        ...
        if s1300_evtcontrsindpatr.status in status_list or s1300_evtcontrsindpatr.processamento_codigo_resposta in (401,402):
...
def validar_evento_funcao(s1300_evtcontrsindpatr_id, db_slug):
    ...
    lista_validacoes = []
    s1300_evtcontrsindpatr = get_object_or_404(s1300evtContrSindPatr.objects.using(db_slug), excluido=False, id=s1300_evtcontrsindpatr_id)
    if s1300_evtcontrsindpatr.transmissor_lote_esocial:
        if s1300_evtcontrsindpatr.transmissor_lote_esocial.transmissor:
