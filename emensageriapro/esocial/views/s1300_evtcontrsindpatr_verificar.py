...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s1300_evtcontrsindpatr = get_object_or_404(
            s1300evtContrSindPatr.objects.using( db_slug ),
            excluido = False, id = s1300_evtcontrsindpatr_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1300_evtcontrsindpatr.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            response =  render_to_response('s1300_evtcontrsindpatr_recibo_pdf.html', context)
            filename = "%s.xls" % s1300_evtcontrsindpatr.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            response =  render_to_response('s1300_evtcontrsindpatr_recibo_csv.html', context)
            filename = "%s.csv" % s1300_evtcontrsindpatr.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
            ...
            return render_to_pdf('s1300_evtcontrsindpatr_recibo_pdf.html', context)
...
def gerar_xml_assinado(s1300_evtcontrsindpatr_id, db_slug):
    ...
    if s1300_evtcontrsindpatr.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1300evtContrSindPatr.objects.using(db_slug).\
            filter(id=s1300_evtcontrsindpatr_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if s1300_evtcontrsindpatr_id:
        ...
        nova_identidade = identidade_evento(s1300_evtcontrsindpatr)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1300_evtcontrsindpatr_id:
        ...
        if s1300_evtcontrsindpatr.status in status_list:
            s1300evtContrSindPatr.objects.using(db_slug).filter(id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1300_evtcontrsindpatr/%s.xml' % (s1300_evtcontrsindpatr.identidade)
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s1300_evtcontrsindpatr.identidade, BASE_DIR, s1300_evtcontrsindpatr.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1300_evtcontrsindpatr/%s.xml %s/arquivos/Eventos/s1300_evtcontrsindpatr/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s1300_evtcontrsindpatr/%s_backup_%s.xml' % (s1300_evtcontrsindpatr.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s1300_evtcontrsindpatr_id, db_slug):
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s1300_evtcontrsindpatr_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
...
def validar_evento(request, hash):
    ...
    if s1300_evtcontrsindpatr_id:
        ...
        if s1300_evtcontrsindpatr.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s1300_evtcontrsindpatr_id, db_slug)

            if s1300_evtcontrsindpatr.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1300evtContrSindPatr.objects.using(db_slug).\
                    filter(excluido=False, id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1300_evtcontrsindpatr.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1300evtContrSindPatr.objects.using(db_slug).\
                    filter(excluido=False, id=s1300_evtcontrsindpatr_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
