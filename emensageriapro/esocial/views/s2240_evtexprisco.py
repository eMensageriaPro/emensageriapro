...
class s2240evtExpRiscoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2240evtExpRisco.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2240_evtexprisco_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if request.method == 'POST':
            if s2240_evtexprisco_form.is_valid():
                ...
                if s2240_evtexprisco_id:
                    ...
                    s2240_evtexprisco_form = form_s2240_evtexprisco(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            ...
            s2240_evtexprisco_form.fields['status'].widget.attrs['disabled'] = True
            s2240_evtexprisco_form.fields['status'].widget.attrs['readonly'] = True
            s2240_evtexprisco_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2240_evtexprisco_id, tabela='s2240_evtexprisco').all()
        context = {
