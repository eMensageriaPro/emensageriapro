...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1270_evtcontratavnp.status == 0:
            ...
            s1270evtContratAvNP.objects.using( db_slug ).filter(id = s1270_evtcontratavnp_id).delete()
...
def identidade_evento(s1270_evtcontratavnp_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1270evtContratAvNP.objects.using( db_slug ).get(id=s1270_evtcontratavnp_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1270evtContratAvNP.objects.using(db_slug).filter(id=s1270_evtcontratavnp_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1270_evtcontratavnp_id:
            s1270_evtcontratavnp_form = form_s1270_evtcontratavnp(request.POST or None, instance = s1270_evtcontratavnp, slug = db_slug)
        else:
            s1270_evtcontratavnp_form = form_s1270_evtcontratavnp(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1270_evtcontratavnp_form.is_valid():
                ...
                if s1270_evtcontratavnp_id:
                    if s1270_evtcontratavnp.status == 0:
                        ...
                        #s1270_evtcontratavnp_campos_multiple_passo1
                        s1270evtContratAvNP.objects.using(db_slug).filter(id=s1270_evtcontratavnp_id).update(**dados)
                        obj = s1270evtContratAvNP.objects.using(db_slug).get(id=s1270_evtcontratavnp_id)
                        #s1270_evtcontratavnp_editar_custom
                        #s1270_evtcontratavnp_campos_multiple_passo2
                    ...
                    #s1270_evtcontratavnp_cadastrar_campos_multiple_passo1
                    obj = s1270evtContratAvNP(**dados)
                    obj.save(using = db_slug)
                    #s1270_evtcontratavnp_cadastrar_custom
                    #s1270_evtcontratavnp_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1270_evtcontratavnp_form = form_s1270_evtcontratavnp(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            s1270_evtcontratavnp_form.fields['tpamb'].widget.attrs['disabled'] = True
            s1270_evtcontratavnp_form.fields['tpamb'].widget.attrs['readonly'] = True
            s1270_evtcontratavnp_form.fields['tpamb'].value = TP_AMB
            s1270_evtcontratavnp_form.fields['procemi'].widget.attrs['disabled'] = True
            s1270_evtcontratavnp_form.fields['procemi'].widget.attrs['readonly'] = True
            ...
            s1270_evtcontratavnp_form.fields['status'].widget.attrs['disabled'] = True
            s1270_evtcontratavnp_form.fields['status'].widget.attrs['readonly'] = True
            s1270_evtcontratavnp_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
