...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1050_evttabhortur.status == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s1050_evttabhortur), indent=4, sort_keys=True, default=str)
            s1050evtTabHorTur.objects.using( db_slug ).filter(id = s1050_evttabhortur_id).delete()
            #s1050_evttabhortur_apagar_custom
            #s1050_evttabhortur_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1050_evttabhortur', s1050_evttabhortur_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 's1050_evttabhortur_salvar':
            return redirect('s1050_evttabhortur', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's1050_evttabhortur_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1050evtTabHorTurList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1050evtTabHorTur.objects.using(db_slug).all()
    serializer_class = s1050evtTabHorTurSerializer
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1050_evttabhortur_lista = s1050evtTabHorTur.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1050_evttabhortur_csv.html', context)
...
def identidade_evento(s1050_evttabhortur_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1050evtTabHorTur.objects.using( db_slug ).get(id=s1050_evttabhortur_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1050evtTabHorTur.objects.using(db_slug).filter(id=s1050_evttabhortur_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1050_evttabhortur_id:
            s1050_evttabhortur_form = form_s1050_evttabhortur(request.POST or None, instance = s1050_evttabhortur, slug = db_slug)
        else:
            s1050_evttabhortur_form = form_s1050_evttabhortur(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1050_evttabhortur_form.is_valid():
                ...
                if s1050_evttabhortur_id:
                    if s1050_evttabhortur.status == 0:
                        ...
                        #s1050_evttabhortur_campos_multiple_passo1
                        s1050evtTabHorTur.objects.using(db_slug).filter(id=s1050_evttabhortur_id).update(**dados)
                        obj = s1050evtTabHorTur.objects.using(db_slug).get(id=s1050_evttabhortur_id)
                        #s1050_evttabhortur_editar_custom
                        #s1050_evttabhortur_campos_multiple_passo2
                    ...
                    #s1050_evttabhortur_cadastrar_campos_multiple_passo1
                    obj = s1050evtTabHorTur(**dados)
                    obj.save(using = db_slug)
                    #s1050_evttabhortur_cadastrar_custom
                    #s1050_evttabhortur_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1050_evttabhortur_form = form_s1050_evttabhortur(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if s1050_evttabhortur_id:
            ...
            s1050_inclusao_form = form_s1050_inclusao(initial={ 's1050_evttabhortur': s1050_evttabhortur }, slug=db_slug)
            s1050_inclusao_form.fields['s1050_evttabhortur'].widget.attrs['readonly'] = True
            s1050_inclusao_lista = s1050inclusao.objects.using( db_slug ).filter(excluido = False, s1050_evttabhortur_id=s1050_evttabhortur.id).all()
            s1050_alteracao_form = form_s1050_alteracao(initial={ 's1050_evttabhortur': s1050_evttabhortur }, slug=db_slug)
            s1050_alteracao_form.fields['s1050_evttabhortur'].widget.attrs['readonly'] = True
            s1050_alteracao_lista = s1050alteracao.objects.using( db_slug ).filter(excluido = False, s1050_evttabhortur_id=s1050_evttabhortur.id).all()
            s1050_exclusao_form = form_s1050_exclusao(initial={ 's1050_evttabhortur': s1050_evttabhortur }, slug=db_slug)
            s1050_exclusao_form.fields['s1050_evttabhortur'].widget.attrs['readonly'] = True
            s1050_exclusao_lista = s1050exclusao.objects.using( db_slug ).filter(excluido = False, s1050_evttabhortur_id=s1050_evttabhortur.id).all()
        else:
        ...
        #             s1050_evttabhortur_form.fields['tpamb'].widget.attrs['disabled'] = True
        #             s1050_evttabhortur_form.fields['tpamb'].widget.attrs['readonly'] = True
        #             s1050_evttabhortur_form.fields['tpamb'].value = TP_AMB
        #             s1050_evttabhortur_form.fields['procemi'].widget.attrs['disabled'] = True
        #             s1050_evttabhortur_form.fields['procemi'].widget.attrs['readonly'] = True
        #             s1050_evttabhortur_form.fields['procemi'].value = 1
        #             s1050_evttabhortur_form.fields['verproc'].widget.attrs['readonly'] = True
        #             s1050_evttabhortur_form.fields['verproc'].value = VERSAO_EMENSAGERIA
        #             s1050_evttabhortur_form.fields['status'].widget.attrs['disabled'] = True
        #             s1050_evttabhortur_form.fields['status'].widget.attrs['readonly'] = True
        #             s1050_evttabhortur_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1050_evttabhortur_id, tabela='s1050_evttabhortur').all()
        context = {
