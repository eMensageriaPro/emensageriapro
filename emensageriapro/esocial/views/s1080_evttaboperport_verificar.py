    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            response = PDFTemplateResponse(request=request,
                                           template='s1080_evttaboperport_verificar.html',
            ...
            return render(request, 's1080_evttaboperport_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s1080(s1080_evttaboperport_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1080_evttaboperport_id, db_slug):
    ...
    if s1080_evttaboperport.status in (0,1,2,11):
        ...
        s1080evtTabOperPort.objects.using(db_slug).\
            filter(id=s1080_evttaboperport_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s1080_evttaboperport_importar import read_s1080_evttaboperport_string
    ...
    if s1080_evttaboperport_id:
        ...
        texto = gerar_xml_s1080(s1080_evttaboperport_id, db_slug, versao="|")
        dados = read_s1080_evttaboperport_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1080evtTabOperPort.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        if s1080_evttaboperport.status == 0:
            ...
            nova_identidade = identidade_evento(s1080_evttaboperport_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1080_evttaboperport', s1080_evttaboperport_id, request.user.id, 1)
...
def validar_evento_funcao(s1080_evttaboperport_id, db_slug):
    ...
    s1080evtTabOperPort.objects.using( db_slug ).\
        filter(id=s1080_evttaboperport_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1080_evttaboperport_validar import validacoes_s1080_evttaboperport
...
<<<<<<< HEAD
        if s1080_evttaboperport.versao in VERSOES_ESOCIAL:
=======
        if s1080_evttaboperport.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1080_evttaboperport_id, db_slug)
