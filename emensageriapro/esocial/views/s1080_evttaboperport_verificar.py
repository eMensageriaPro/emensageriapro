...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s1080_evttaboperport_verificar.html' % s1080_evttaboperport.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s1080_evttaboperport_verificar.html' % s1080_evttaboperport.versao,
            ...
            response =  render_to_response('%s/s1080_evttaboperport_verificar.html' % s1080_evttaboperport.versao, context)
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s1080_evttaboperport = get_object_or_404(
            s1080evtTabOperPort.objects.using( db_slug ),
            excluido = False, id = s1080_evttaboperport_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s1080_evttaboperport.retornos_eventos_id, excluido=False)
...
def gerar_xml_assinado(s1080_evttaboperport_id, db_slug):
    import os
    ...
    if s1080_evttaboperport.status in (0,1,2,11):
        s1080evtTabOperPort.objects.using(db_slug).filter(id=s1080_evttaboperport_id,excluido=False).update(status=10)
...
def gerar_xml(request, hash):
    ...
    if s1080_evttaboperport_id:
        s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort.objects.using(db_slug), excluido=False, id=s1080_evttaboperport_id)
        xml_assinado = gerar_xml_assinado(s1080_evttaboperport_id, db_slug)
...
def duplicar(request, hash):
    ...
    s1080_evttaboperport_id = int(dict_hash['id'])
    if s1080_evttaboperport_id:
        s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort.objects.using(db_slug), excluido=False,
                                                    id=s1080_evttaboperport_id)

        arquivo = 'arquivos/Eventos/s1080_evttaboperport/%s.xml' % s1080_evttaboperport.identidade
        if not os.path.exists(BASE_DIR + '/' + arquivo):
            xml = gerar_xml_assinado(s1080_evttaboperport_id, db_slug)
   
        texto = ler_arquivo('arquivos/Eventos/s1080_evttaboperport/%s.xml' % s1080_evttaboperport.identidade)
        salvar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_duplicado_temp.xml' % s1080_evttaboperport.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_duplicado_temp.xml' % s1080_evttaboperport.identidade, request)
        from emensageriapro.esocial.views.s1080_evttaboperport import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1080evtTabOperPort.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        ...
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['identidade'], usuario_id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_alteracao_temp.xml' % s1080_evttaboperport.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_alteracao_temp.xml' % s1080_evttaboperport.identidade, request)
        from emensageriapro.esocial.views.s1080_evttaboperport import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1080evtTabOperPort.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['identidade'], usuario_id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        salvar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_exclusao_temp.xml' % s1080_evttaboperport.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s1080_evttaboperport/%s_exclusao_temp.xml' % s1080_evttaboperport.identidade, request)
        from emensageriapro.esocial.views.s1080_evttaboperport import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s1080evtTabOperPort.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.user.id
        gravar_auditoria(u'{}', u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (dent, s1080_evttaboperport.identidade),
            's1080_evttaboperport', dados['identidade'], usuario_id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1080_evttaboperport_id:
        ...
        if s1080_evttaboperport.status == 0:
            from emensageriapro.esocial.views.s1080_evttaboperport import identidade_evento
            dent = identidade_evento(s1080_evttaboperport_id, db_slug)
            messages.success(request, 'Identidade do evento alterada com sucesso!')
            url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % s1080_evttaboperport_id )
            usuario_id = request.user.id
            gravar_auditoria(u'{}', u'{"funcao": "Identidade do evento foi alterada"}',
            's1080_evttaboperport', s1080_evttaboperport_id, usuario_id, 1)
            return redirect('s1080_evttaboperport_salvar', hash=url_hash)
        else:
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1080_evttaboperport_id:
        s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort.objects.using(db_slug), excluido=False, id=s1080_evttaboperport_id)
        if s1080_evttaboperport.status in (0, 1, 2, 3, 4, 10, 11) or s1080_evttaboperport.processamento_codigo_resposta in (401,402):
            s1080evtTabOperPort.objects.using(db_slug).filter(id=s1080_evttaboperport_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1080_evttaboperport/%s.xml' % (s1080_evttaboperport.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s1080_evttaboperport.identidade, BASE_DIR, s1080_evttaboperport.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s1080_evttaboperport/%s.xml %s/arquivos/Eventos/s1080_evttaboperport/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s1080_evttaboperport/%s_backup_%s.xml' % (s1080_evttaboperport.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s1080_evttaboperport_id, db_slug):
    ...
    lista_validacoes = []
    s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort.objects.using(db_slug), excluido=False, id=s1080_evttaboperport_id)
    if s1080_evttaboperport.transmissor_lote_esocial:
        if s1080_evttaboperport.transmissor_lote_esocial.transmissor:
            if s1080_evttaboperport.transmissor_lote_esocial.transmissor.verificar_predecessao:
                quant = validar_precedencia('esocial', 's1080_evttaboperport', s1080_evttaboperport_id)
                if quant <= 0:
        ...
        lista_validacoes.append('Precedência não foi enviada!')
        precedencia = 0
    executar_sql("UPDATE public.s1080_evttaboperport SET validacao_precedencia=%s WHERE id=%s;" % (precedencia, s1080_evttaboperport_id), False)
    #
    # Validações internas
    #
    arquivo = 'arquivos/Eventos/s1080_evttaboperport/%s.xml' % (s1080_evttaboperport.identidade)
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s1080_evttaboperport_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s1080_evttaboperport import validacoes_s1080_evttaboperport
                lista = validacoes_s1080_evttaboperport(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s1080_evttaboperport import validacoes_s1080_evttaboperport
                lista = validacoes_s1080_evttaboperport(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s1080_evttaboperport SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s1080_evttaboperport_id), False)
    else:
        executar_sql("UPDATE public.s1080_evttaboperport SET validacoes='', status=4 WHERE id=%s;" % (s1080_evttaboperport_id), False)
...
def validar_evento(request, hash):
    ...
    if s1080_evttaboperport_id:
        lista_validacoes = []
        s1080_evttaboperport = get_object_or_404(s1080evtTabOperPort.objects.using(db_slug), excluido=False, id=s1080_evttaboperport_id)
        if s1080_evttaboperport.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s1080_evttaboperport_id, db_slug)
