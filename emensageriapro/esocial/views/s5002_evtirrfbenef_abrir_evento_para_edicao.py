...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5002evtIrrfBenef'):

        if s5002_evtirrfbenef_id:
            s5002_evtirrfbenef = get_object_or_404(s5002evtIrrfBenef, excluido=False, id=s5002_evtirrfbenef_id)
            ...
            if s5002_evtirrfbenef.status in status_list:
                s5002evtIrrfBenef.objects.filter(id=s5002_evtirrfbenef_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
