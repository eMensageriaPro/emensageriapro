...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            ...
            return render(request, 's2250_evtavprevio_verificar.html', context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2250(s2250_evtavprevio_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2250_evtavprevio_id, db_slug):
    ...
    if s2250_evtavprevio.status in (0,1,2,11):
        ...
        s2250evtAvPrevio.objects.using(db_slug).\
            filter(id=s2250_evtavprevio_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2250_evtavprevio_importar import read_s2250_evtavprevio_string
    ...
    if s2250_evtavprevio_id:
        ...
        texto = gerar_xml_s2250(s2250_evtavprevio_id, db_slug, versao="|")
        dados = read_s2250_evtavprevio_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s2250evtAvPrevio.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2250_evtavprevio.identidade),
            's2250_evtavprevio', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2250_evtavprevio.identidade),
            's2250_evtavprevio', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2250_evtavprevio.identidade),
            's2250_evtavprevio', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        if s2250_evtavprevio.status == 0:
            ...
            nova_identidade = identidade_evento(s2250_evtavprevio_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2250_evtavprevio', s2250_evtavprevio_id, request.user.id, 1)
...
def validar_evento_funcao(s2250_evtavprevio_id, db_slug):
    ...
    s2250evtAvPrevio.objects.using( db_slug ).\
        filter(id=s2250_evtavprevio_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2250_evtavprevio_validar import validacoes_s2250_evtavprevio
