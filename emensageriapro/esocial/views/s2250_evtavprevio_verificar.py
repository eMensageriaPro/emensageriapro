...
def gerar_xml_assinado(s2250_evtavprevio_id, db_slug):
    ...
    if s2250_evtavprevio.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2250evtAvPrevio.objects.using(db_slug).\
            filter(id=s2250_evtavprevio_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        if s2250_evtavprevio.status in status_list:
...
def validar_evento(request, hash):
    ...
    if s2250_evtavprevio_id:
        ...
        if s2250_evtavprevio.versao in VERSOES_ESOCIAL:
            ...
            if s2250_evtavprevio.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2250evtAvPrevio.objects.using(db_slug).\
                    filter(excluido=False, id=s2250_evtavprevio_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2250_evtavprevio.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2250evtAvPrevio.objects.using(db_slug).\
                    filter(excluido=False, id=s2250_evtavprevio_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
