...
def validar_evento_funcao(request, pk):
    ...
    if not s2410_evtcdbenin.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2410_evtcdbenin, 'esocial')
        s2410_evtcdbenin = get_object_or_404(s2410evtCdBenIn, id=pk)
    ...
    schema_filename = 'esocial/%s/evtCdBenIn.xsd' % (s2410_evtcdbenin.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2410_evtcdbenin' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2410_evtcdbenin', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2410_evtcdbenin.status in STATUS_VALIDACAO:
