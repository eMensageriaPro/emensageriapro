...
def validar_evento_funcao(request, s2410_evtcdbenin_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2410_evtcdbenin_id)
...
def validar_evento(request, hash):
    ...
    if s2410_evtcdbenin_id:
        ...
        if s2410_evtcdbenin.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2410_evtcdbenin_id)
            
            if s2410_evtcdbenin.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2410evtCdBenIn.objects.\
                    filter(id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2410_evtcdbenin.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2410evtCdBenIn.objects.\
                    filter(id=s2410_evtcdbenin_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
