...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1298evtReabreEvPer'):

        if s1298_evtreabreevper_id:
            s1298_evtreabreevper = get_object_or_404(s1298evtReabreEvPer, excluido=False, id=s1298_evtreabreevper_id)
            ...
            if s1298_evtreabreevper.status in status_list:
                s1298evtReabreEvPer.objects.filter(id=s1298_evtreabreevper_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
