...
def salvar(request, hash):
    ...
    if s2400_evtcdbenprrp_id:
        if s2400_evtcdbenprrp.status != 0:
            dict_permissoes['s2400_evtcdbenprrp_apagar'] = 0
            dict_permissoes['s2400_evtcdbenprrp_editar'] = 0
    ...
    if permissao.permite_visualizar:
            ...
            if s2400_evtcdbenprrp_form.is_valid():
                ...
                if s2400_evtcdbenprrp_id:
                    if s2400_evtcdbenprrp.status == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s2400_evtcdbenprrp), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's2400_evtcdbenprrp', s2400_evtcdbenprrp_id, usuario_id, 2)
                    else:
                    ...
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2400_evtcdbenprrp', obj.id, usuario_id, 1)
        ...
        if s2400_evtcdbenprrp_id:
            if s2400_evtcdbenprrp.status != 0:
                s2400_evtcdbenprrp_form = disabled_form_fields(s2400_evtcdbenprrp_form, False)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2400_evtcdbenprrp_id, tabela='s2400_evtcdbenprrp').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2400_evtcdbenprrp_salvar.html',
            ...
            response = render_to_response('s2400_evtcdbenprrp_salvar.html', context)
            filename = "s2400_evtcdbenprrp.xls"
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s2400_evtcdbenprrp.status == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2400_evtcdbenprrp), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2400_evtcdbenprrp', s2400_evtcdbenprrp_id, usuario_id, 3)
        else:
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2400_evtcdbenprrp_listar.html',
                filename="s2400_evtcdbenprrp.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2400_evtcdbenprrp_listar.html', context)
            filename = "s2400_evtcdbenprrp.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2400_evtcdbenprrp_csv.html', context)
            filename = "s2400_evtcdbenprrp.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
