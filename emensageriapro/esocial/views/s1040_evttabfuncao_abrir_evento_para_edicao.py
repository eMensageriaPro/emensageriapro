...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s1040evtTabFuncao'):

        if s1040_evttabfuncao_id:
            s1040_evttabfuncao = get_object_or_404(s1040evtTabFuncao, excluido=False, id=s1040_evttabfuncao_id)
            ...
            if s1040_evttabfuncao.status in status_list:
                s1040evtTabFuncao.objects.filter(id=s1040_evttabfuncao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
