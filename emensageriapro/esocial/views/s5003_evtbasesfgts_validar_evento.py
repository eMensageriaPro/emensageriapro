...
def validar_evento_funcao(request, s5003_evtbasesfgts_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s5003_evtbasesfgts_id)
...
def validar_evento(request, hash):
    ...
    if s5003_evtbasesfgts_id:
        ...
        if s5003_evtbasesfgts.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s5003_evtbasesfgts_id)
            
            if s5003_evtbasesfgts.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5003evtBasesFGTS.objects.\
                    filter(id=s5003_evtbasesfgts_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5003_evtbasesfgts.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5003evtBasesFGTS.objects.\
                    filter(id=s5003_evtbasesfgts_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
