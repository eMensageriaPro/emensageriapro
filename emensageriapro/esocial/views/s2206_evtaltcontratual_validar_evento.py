...
def validar_evento_funcao(request, s2206_evtaltcontratual_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2206_evtaltcontratual_id)
...
def validar_evento(request, hash):
    ...
    if s2206_evtaltcontratual_id:
        ...
        if s2206_evtaltcontratual.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2206_evtaltcontratual_id)
            
            if s2206_evtaltcontratual.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2206evtAltContratual.objects.\
                    filter(id=s2206_evtaltcontratual_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2206_evtaltcontratual.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2206evtAltContratual.objects.\
                    filter(id=s2206_evtaltcontratual_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
