...
def validar_evento_funcao(request, pk):
    ...
    if not s2206_evtaltcontratual.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2206_evtaltcontratual, 'esocial')
        s2206_evtaltcontratual = get_object_or_404(s2206evtAltContratual, id=pk)
    ...
    schema_filename = 'esocial/%s/evtAltContratual.xsd' % (s2206_evtaltcontratual.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2206_evtaltcontratual' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2206_evtaltcontratual', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2206_evtaltcontratual.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s2206_evtaltcontratual_salvar', pk=pk)