...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if s1010_evttabrubrica.status == 0:
            ...
            s1010evtTabRubrica.objects.using( db_slug ).filter(id = s1010_evttabrubrica_id).delete()
            #s1010_evttabrubrica_apagar_custom
            #s1010_evttabrubrica_apagar_custom
    ...
    return render(request, 's1010_evttabrubrica_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1010evtTabRubricaList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1010evtTabRubrica.objects.using(db_slug).all()
    serializer_class = s1010evtTabRubricaSerializer
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1010_evttabrubrica_lista = s1010evtTabRubrica.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1010_evttabrubrica_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1010_evttabrubrica_listar.html',
            ...
            response = render_to_response('s1010_evttabrubrica_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1010_evttabrubrica_csv.html', context)
            filename = "s1010_evttabrubrica.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def identidade_evento(s1010_evttabrubrica_id, db_slug):
    from emensageriapro.mensageiro.models import TransmissorEventosEsocial
    dados_evento = s1010evtTabRubrica.objects.using( db_slug ).get(id=s1010_evttabrubrica_id)
    identidade = 'ID'
    ...
    while existe:
        ...
        if not lista_eventos:
            s1010evtTabRubrica.objects.using(db_slug).filter(id=s1010_evttabrubrica_id).update(identidade=identidade_temp)
            existe = False
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1010_evttabrubrica_id:
            s1010_evttabrubrica_form = form_s1010_evttabrubrica(request.POST or None, instance = s1010_evttabrubrica, slug = db_slug)
        else:
            s1010_evttabrubrica_form = form_s1010_evttabrubrica(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1010_evttabrubrica_form.is_valid():
                dados = s1010_evttabrubrica_form.cleaned_data
                import json
                from django.forms.models import model_to_dict
                if s1010_evttabrubrica_id:
                    if s1010_evttabrubrica.status == 0:
                        ...
                        #s1010_evttabrubrica_campos_multiple_passo1
                        s1010evtTabRubrica.objects.using(db_slug).filter(id=s1010_evttabrubrica_id).update(**dados)
                        obj = s1010evtTabRubrica.objects.using(db_slug).get(id=s1010_evttabrubrica_id)
                        #s1010_evttabrubrica_editar_custom
                        #s1010_evttabrubrica_campos_multiple_passo2
                        messages.success(request, 'Alterado com sucesso!')
                        gravar_auditoria(json.dumps(model_to_dict(s1010_evttabrubrica), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1010_evttabrubrica', s1010_evttabrubrica_id, usuario_id, 2)
                    else:
                        obj = s1010evtTabRubrica.objects.using(db_slug).get(id=s1010_evttabrubrica_id)
                    ...
                    #s1010_evttabrubrica_cadastrar_campos_multiple_passo1
                    obj = s1010evtTabRubrica(**dados)
                    obj.save(using = db_slug)
                    #s1010_evttabrubrica_cadastrar_custom
                    #s1010_evttabrubrica_cadastrar_campos_multiple_passo2
                    identidade_evento(obj.id, db_slug)
                    messages.success(request, 'Cadastrado com sucesso!')
                    s1010_evttabrubrica_form = form_s1010_evttabrubrica(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1010_evttabrubrica', obj.id, usuario_id, 1)
                if request.session['retorno_pagina'] not in ('s1010_evttabrubrica_apagar', 's1010_evttabrubrica_salvar', 's1010_evttabrubrica'):
                    return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                if s1010_evttabrubrica_id != obj.id:
        ...
        if s1010_evttabrubrica_id:
            ...
            s1010_alteracao_form = form_s1010_alteracao(initial={ 's1010_evttabrubrica': s1010_evttabrubrica }, slug=db_slug)
            s1010_alteracao_form.fields['s1010_evttabrubrica'].widget.attrs['readonly'] = True
            s1010_alteracao_lista = s1010alteracao.objects.using( db_slug ).filter(excluido = False, s1010_evttabrubrica_id=s1010_evttabrubrica.id).all()
            s1010_exclusao_form = form_s1010_exclusao(initial={ 's1010_evttabrubrica': s1010_evttabrubrica }, slug=db_slug)
            s1010_exclusao_form.fields['s1010_evttabrubrica'].widget.attrs['readonly'] = True
            s1010_exclusao_lista = s1010exclusao.objects.using( db_slug ).filter(excluido = False, s1010_evttabrubrica_id=s1010_evttabrubrica.id).all()
        else:
        ...
        if not evento_totalizador:
            s1010_evttabrubrica_form.fields['tpamb'].widget.attrs['disabled'] = True
            s1010_evttabrubrica_form.fields['tpamb'].widget.attrs['readonly'] = True
            s1010_evttabrubrica_form.fields['tpamb'].value = TP_AMB
            s1010_evttabrubrica_form.fields['procemi'].widget.attrs['disabled'] = True
            s1010_evttabrubrica_form.fields['procemi'].widget.attrs['readonly'] = True
            ...
            s1010_evttabrubrica_form.fields['status'].widget.attrs['disabled'] = True
            s1010_evttabrubrica_form.fields['status'].widget.attrs['readonly'] = True
            s1010_evttabrubrica_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1010_evttabrubrica_id, tabela='s1010_evttabrubrica').all()
        context = {
