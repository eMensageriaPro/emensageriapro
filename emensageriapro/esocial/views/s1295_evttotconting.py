...
def apagar(request, hash):
    ...
    if s1295_evttotconting_id:
        if s1295_evttotconting.status != 0:
            dict_permissoes['s1295_evttotconting_apagar'] = 0
            dict_permissoes['s1295_evttotconting_editar'] = 0

    if request.method == 'POST':
        if s1295_evttotconting.status == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1295_evttotconting), indent=4, sort_keys=True, default=str)
...
class s1295evtTotContingList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1295evtTotConting.objects.using(db_slug).all()
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1295_evttotconting_csv.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1295_evttotconting_id:
            ...
            s1295_evttotconting_form = form_s1295_evttotconting(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1295_evttotconting_form.is_valid():
                ...
                if s1295_evttotconting_id:
                    if s1295_evttotconting.status == 0:
                        ...
                        gravar_auditoria(json.dumps(model_to_dict(s1295_evttotconting), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1295_evttotconting', s1295_evttotconting_id, usuario_id, 2)
                    else:
                        obj = s1295evtTotConting.objects.using(db_slug).get(id=s1295_evttotconting_id)
                    ...
                    s1295_evttotconting_form = form_s1295_evttotconting(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1295_evttotconting', obj.id, usuario_id, 1)
        ...
        #             s1295_evttotconting_form.fields['status'].widget.attrs['disabled'] = True
        #             s1295_evttotconting_form.fields['status'].widget.attrs['readonly'] = True
        #             s1295_evttotconting_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1295_evttotconting_id, tabela='s1295_evttotconting').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1295_evttotconting_salvar.html',
                filename="s1295_evttotconting.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1295_evttotconting_salvar.html', context)
            filename = "s1295_evttotconting.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
