...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5011evtCS'):

        if s5011_evtcs_id:
            s5011_evtcs = get_object_or_404(s5011evtCS, excluido=False, id=s5011_evtcs_id)
            ...
            if s5011_evtcs.status in status_list:
                s5011evtCS.objects.filter(id=s5011_evtcs_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
