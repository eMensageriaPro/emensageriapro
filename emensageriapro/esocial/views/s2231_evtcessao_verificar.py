...
def verificar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2231_evtcessao')
    ...
    if permissao.permite_listar:
        ...
        if for_print == 2:
            #return render_to_pdf('%s/s2231_evtcessao_verificar.html' % s2231_evtcessao.versao, context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(request=request,
                                           template='%s/s2231_evtcessao_verificar.html' % s2231_evtcessao.versao,
            ...
            response =  render_to_response('%s/s2231_evtcessao_verificar.html' % s2231_evtcessao.versao, context)
            ...
            return render(request, '%s/s2231_evtcessao_verificar.html' % s2231_evtcessao.versao, context)
    else:
        context = {
            'usuario': usuario,
...
def gerar_xml_s2231(s2231_evtcessao_id, db_slug):
    from django.template.loader import get_template
    if s2231_evtcessao_id:
        ...
        #return render(request, 'xml/%s/s2231_evtcessao.html' % s2231_evtcessao.versao, context, content_type='text/xml')
...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s2231_evtcessao.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            from django.shortcuts import render_to_response
            response =  render_to_response('s2231_evtcessao_recibo_pdf.html', context)
            filename = "%s.xls" % s2231_evtcessao.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response =  render_to_response('s2231_evtcessao_recibo_csv.html', context)
            filename = "%s.csv" % s2231_evtcessao.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
...
def gerar_xml_assinado(s2231_evtcessao_id, db_slug):
    import os
    ...
    if s2231_evtcessao.arquivo_original:
        ...
        xml = gerar_xml_s2231(s2231_evtcessao_id, db_slug)
    if 'Signature' in xml:
        xml_assinado = xml
    else:
        xml_assinado = assinar_esocial(xml)
    if s2231_evtcessao.status in (0,1,2,11):
        s2231evtCessao.objects.using(db_slug).filter(id=s2231_evtcessao_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    s2231_evtcessao_id = int(dict_hash['id'])
    if s2231_evtcessao_id:
        s2231_evtcessao = get_object_or_404(s2231evtCessao.objects.using(db_slug), excluido=False,
        ...
        dados = importar_arquivo('arquivos/Eventos/s2231_evtcessao/%s_duplicado_temp.xml' % s2231_evtcessao.identidade, request)
...
def criar_alteracao(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2231_evtcessao/%s_alteracao_temp.xml' % s2231_evtcessao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2231_evtcessao/%s_alteracao_temp.xml' % s2231_evtcessao.identidade, request)
        from emensageriapro.esocial.views.s2231_evtcessao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2231evtCessao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
...
def criar_exclusao(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        salvar_arquivo('arquivos/Eventos/s2231_evtcessao/%s_exclusao_temp.xml' % s2231_evtcessao.identidade, texto)
        from emensageriapro.funcoes_importacao import importar_arquivo
        dados = importar_arquivo('arquivos/Eventos/s2231_evtcessao/%s_exclusao_temp.xml' % s2231_evtcessao.identidade, request)
        from emensageriapro.esocial.views.s2231_evtcessao import identidade_evento
        dent = identidade_evento(dados['identidade'], db_slug)
        s2231evtCessao.objects.using(db_slug).filter(id=dados['identidade']).update(status=0, arquivo_original=0, arquivo='')
        usuario_id = request.session['usuario_id']
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2231_evtcessao_id:
        s2231_evtcessao = get_object_or_404(s2231evtCessao.objects.using(db_slug), excluido=False, id=s2231_evtcessao_id)
        if s2231_evtcessao.status in (0, 1, 2, 3, 4, 10, 11) or s2231_evtcessao.processamento_codigo_resposta in (401,402):
            s2231evtCessao.objects.using(db_slug).filter(id=s2231_evtcessao_id).update(status=0, arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2231_evtcessao/%s.xml' % (s2231_evtcessao.identidade)
            if os.path.exists(BASE_DIR + '/' + arquivo):
                from datetime import datetime
                data_hora_atual = str(datetime.now()).replace(':','_').replace(' ','_').replace('.','_')
                dad = (BASE_DIR, s2231_evtcessao.identidade, BASE_DIR, s2231_evtcessao.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s2231_evtcessao/%s.xml %s/arquivos/Eventos/s2231_evtcessao/%s_backup_%s.xml' % dad)
...
def validar_evento_funcao(s2231_evtcessao_id, db_slug):
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s2231_evtcessao_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
        texto_xml = ler_arquivo(arquivo).replace("s:", "")
        versao = get_versao_evento(texto_xml)
        if tipo == 'esocial':
            if versao == 'v02_04_02':
                from emensageriapro.esocial.validacoes.v02_04_02.s2231_evtcessao import validacoes_s2231_evtcessao
                lista = validacoes_s2231_evtcessao(arquivo)
        elif tipo == 'efdreinf':
            if versao == 'v1_03_02':
                from emensageriapro.efdreinf.validacoes.v1_03_02.s2231_evtcessao import validacoes_s2231_evtcessao
                lista = validacoes_s2231_evtcessao(arquivo)
    for a in lista:
        if a:
    ...
    #
    if lista_validacoes:
        executar_sql("UPDATE public.s2231_evtcessao SET validacoes='%s', status=3 WHERE id=%s;" % ('<br>'.join(lista_validacoes).replace("'","''"), s2231_evtcessao_id), False)
    else:
        executar_sql("UPDATE public.s2231_evtcessao SET validacoes='', status=4 WHERE id=%s;" % (s2231_evtcessao_id), False)
...
def validar_evento(request, hash):
    ...
    if s2231_evtcessao_id:
        lista_validacoes = []
        s2231_evtcessao = get_object_or_404(s2231evtCessao.objects.using(db_slug), excluido=False, id=s2231_evtcessao_id)
        if s2231_evtcessao.versao in VERSAO_ATUAL:
            lista_validacoes = validar_evento_funcao(s2231_evtcessao_id, db_slug)
