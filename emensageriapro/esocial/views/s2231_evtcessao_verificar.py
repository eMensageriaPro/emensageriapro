...
def gerar_xml_assinado(s2231_evtcessao_id, db_slug):
    ...
    if s2231_evtcessao.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2231evtCessao.objects.using(db_slug).\
            filter(id=s2231_evtcessao_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        if s2231_evtcessao.status in status_list:
            s2231evtCessao.objects.using(db_slug).filter(id=s2231_evtcessao_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2231_evtcessao/%s.xml' % (s2231_evtcessao.identidade)
...
def validar_evento(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        if s2231_evtcessao.versao in VERSOES_ESOCIAL:
            ...
            if s2231_evtcessao.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2231evtCessao.objects.using(db_slug).\
                    filter(excluido=False, id=s2231_evtcessao_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2231_evtcessao.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2231evtCessao.objects.using(db_slug).\
                    filter(excluido=False, id=s2231_evtcessao_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
