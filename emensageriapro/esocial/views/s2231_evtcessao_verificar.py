...
def gerar_xml_s2231(s2231_evtcessao_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2231_evtcessao_id, db_slug):
    ...
    if s2231_evtcessao.status in (0,1,2,11):
        ...
        s2231evtCessao.objects.using(db_slug).\
            filter(id=s2231_evtcessao_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2231_evtcessao_importar import read_s2231_evtcessao_string
    ...
    if s2231_evtcessao_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2231_evtcessao.identidade),
...
def alterar_identidade(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        if s2231_evtcessao.status == 0:
            ...
            nova_identidade = identidade_evento(s2231_evtcessao_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2231_evtcessao', s2231_evtcessao_id, request.user.id, 1)
...
def validar_evento_funcao(s2231_evtcessao_id, db_slug):
    ...
    s2231evtCessao.objects.using( db_slug ).\
        filter(id=s2231_evtcessao_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2231_evtcessao_validar import validacoes_s2231_evtcessao
...
<<<<<<< HEAD
        if s2231_evtcessao.versao in VERSOES_ESOCIAL:
=======
        if s2231_evtcessao.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s2231_evtcessao_id, db_slug)
