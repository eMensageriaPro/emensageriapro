...
def validar_evento_funcao(request, s5013_evtfgts_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s5013_evtfgts_id)
...
def validar_evento(request, hash):
    ...
    if s5013_evtfgts_id:
        ...
        if s5013_evtfgts.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s5013_evtfgts_id)
            
            if s5013_evtfgts.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5013evtFGTS.objects.\
                    filter(id=s5013_evtfgts_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5013_evtfgts.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5013evtFGTS.objects.\
                    filter(id=s5013_evtfgts_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
