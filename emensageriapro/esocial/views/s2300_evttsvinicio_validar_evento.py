...
def validar_evento_funcao(request, pk):
    ...
    if not s2300_evttsvinicio.identidade:
        from emensageriapro.functions import identidade_evento
        ident = identidade_evento(s2300_evttsvinicio, 'esocial')
        s2300_evttsvinicio = get_object_or_404(s2300evtTSVInicio, id=pk)
    ...
    schema_filename = 'esocial/%s/evtTSVInicio.xsd' % (s2300_evttsvinicio.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's2300_evttsvinicio' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s2300_evttsvinicio', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s2300_evttsvinicio.status in STATUS_VALIDACAO:
