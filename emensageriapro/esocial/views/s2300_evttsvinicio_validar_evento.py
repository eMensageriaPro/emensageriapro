...
def validar_evento_funcao(request, s2300_evttsvinicio_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s2300_evttsvinicio_id)
...
def validar_evento(request, hash):
    ...
    if s2300_evttsvinicio_id:
        ...
        if s2300_evttsvinicio.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s2300_evttsvinicio_id)
            
            if s2300_evttsvinicio.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2300evtTSVInicio.objects.\
                    filter(id=s2300_evttsvinicio_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2300_evttsvinicio.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2300evtTSVInicio.objects.\
                    filter(id=s2300_evttsvinicio_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
