...
def gerar_xml_s1000(s1000_evtinfoempregador_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1000_evtinfoempregador_id, db_slug):
    ...
    if s1000_evtinfoempregador.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1000evtInfoEmpregador.objects.using(db_slug).\
            filter(id=s1000_evtinfoempregador_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    if s1000_evtinfoempregador_id:
        ...
        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1000_evtinfoempregador.identidade),
    ...
    if s1000_evtinfoempregador_id:
        ...
        if s1000_evtinfoempregador.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's1000_evtinfoempregador', s1000_evtinfoempregador_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1000_evtinfoempregador_id:
        ...
        if s1000_evtinfoempregador.status in status_list:
...
def validar_evento_funcao(s1000_evtinfoempregador_id, db_slug):
    ...
    if lista_validacoes:
        ...
        s1000evtInfoEmpregador.objects.using( db_slug ).\
            filter(id=s1000_evtinfoempregador_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s1000_evtinfoempregador_id:
        ...
        if s1000_evtinfoempregador.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(s1000_evtinfoempregador_id, db_slug)

            if s1000_evtinfoempregador.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1000evtInfoEmpregador.objects.using(db_slug).\
                    filter(excluido=False, id=s1000_evtinfoempregador_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1000_evtinfoempregador.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1000evtInfoEmpregador.objects.using(db_slug).\
                    filter(excluido=False, id=s1000_evtinfoempregador_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
