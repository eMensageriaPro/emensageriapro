...
def gerar_xml_s2400(s2400_evtcdbenefin_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2400_evtcdbenefin_id, db_slug):
    ...
    if s2400_evtcdbenefin.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2400evtCdBenefIn.objects.using(db_slug).\
            filter(id=s2400_evtcdbenefin_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2400_evtcdbenefin_importar import read_s2400_evtcdbenefin_string
    ...
    if s2400_evtcdbenefin_id:
        ...
        texto = gerar_xml_s2400(s2400_evtcdbenefin_id, db_slug, versao="|")
        dados = read_s2400_evtcdbenefin_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(s2400_evtcdbenefin)

        s2400evtCdBenefIn.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if s2400_evtcdbenefin_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2400_evtcdbenefin.identidade),
            's2400_evtcdbenefin', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2400_evtcdbenefin_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2400_evtcdbenefin.identidade),
            's2400_evtcdbenefin', dados['id'], request.user.id, 1)
    ...
    if s2400_evtcdbenefin_id:
        ...
        if s2400_evtcdbenefin.status == STATUS_EVENTO_CADASTRADO:
            ...
            gravar_auditoria(u'{}',
                ...
                's2400_evtcdbenefin', s2400_evtcdbenefin_id, request.user.id, 1)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2400_evtcdbenefin_id:
        ...
        if s2400_evtcdbenefin.status in status_list:
            s2400evtCdBenefIn.objects.using(db_slug).filter(id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2400_evtcdbenefin/%s.xml' % (s2400_evtcdbenefin.identidade)
...
def validar_evento_funcao(s2400_evtcdbenefin_id, db_slug):
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2400_evtcdbenefin_validar import validacoes_s2400_evtcdbenefin
    ...
    if lista_validacoes:
        ...
        s2400evtCdBenefIn.objects.using( db_slug ).\
            filter(id=s2400_evtcdbenefin_id, excluido = False).\
...
def validar_evento(request, hash):
    ...
    if s2400_evtcdbenefin_id:
        ...
        if s2400_evtcdbenefin.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s2400_evtcdbenefin_id, db_slug)

            if s2400_evtcdbenefin.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2400evtCdBenefIn.objects.using(db_slug).\
                    filter(excluido=False, id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2400_evtcdbenefin.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2400evtCdBenefIn.objects.using(db_slug).\
                    filter(excluido=False, id=s2400_evtcdbenefin_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
