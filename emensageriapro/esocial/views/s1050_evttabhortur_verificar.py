...
def gerar_xml_s1050(s1050_evttabhortur_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1050_evttabhortur_id, db_slug):
    ...
    if s1050_evttabhortur.status in (0,1,2,11):
        ...
        s1050evtTabHorTur.objects.using(db_slug).\
            filter(id=s1050_evttabhortur_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s1050_evttabhortur_importar import read_s1050_evttabhortur_string
    ...
    if s1050_evttabhortur_id:
        ...
        texto = gerar_xml_s1050(s1050_evttabhortur_id, db_slug, versao="|")
        dados = read_s1050_evttabhortur_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1050evtTabHorTur.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1050_evttabhortur.identidade),
            's1050_evttabhortur', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s1050_evttabhortur_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1050_evttabhortur.identidade),
            's1050_evttabhortur', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1050_evttabhortur_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1050_evttabhortur.identidade),
            's1050_evttabhortur', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1050_evttabhortur_id:
        ...
        if s1050_evttabhortur.status == 0:
            ...
            nova_identidade = identidade_evento(s1050_evttabhortur_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1050_evttabhortur', s1050_evttabhortur_id, request.user.id, 1)
...
def validar_evento_funcao(s1050_evttabhortur_id, db_slug):
    ...
    s1050evtTabHorTur.objects.using( db_slug ).\
        filter(id=s1050_evttabhortur_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1050_evttabhortur_validar import validacoes_s1050_evttabhortur
...
<<<<<<< HEAD
        if s1050_evttabhortur.versao in VERSOES_ESOCIAL:
=======
        if s1050_evttabhortur.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1050_evttabhortur_id, db_slug)
