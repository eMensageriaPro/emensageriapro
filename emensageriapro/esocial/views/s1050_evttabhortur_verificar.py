...
def gerar_xml_assinado(s1050_evttabhortur_id, db_slug):
    ...
    if s1050_evttabhortur.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1050evtTabHorTur.objects.using(db_slug).\
            filter(id=s1050_evttabhortur_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def validar_evento(request, hash):
    ...
    if s1050_evttabhortur_id:
        ...
        if s1050_evttabhortur.versao in VERSOES_ESOCIAL:
            ...
            if s1050_evttabhortur.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1050evtTabHorTur.objects.using(db_slug).\
                    filter(excluido=False, id=s1050_evttabhortur_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1050_evttabhortur.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1050evtTabHorTur.objects.using(db_slug).\
                    filter(excluido=False, id=s1050_evttabhortur_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
