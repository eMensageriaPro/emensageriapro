...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if request.user.has_perm('esocial.can_see_s1080evtTabOperPort'):
            ...
            if s1080_evttaboperport_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria('{}',
                                 ...
                                 's1080_evttaboperport', obj.id, request.user.id, 1)
                else:
                ...
                if request.session['return_page'] not in (
                    's1080_evttaboperport_apagar', 
                    's1080_evttaboperport_salvar', 
        ...
        s1080_evttaboperport_form = disabled_form_fields(
             s1080_evttaboperport_form, 
             request.user.has_perm('esocial.change_s1080evtTabOperPort'))
        ...
        if tab or 's1080_evttaboperport' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1080_evttaboperport_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1080_evttaboperport').all()
