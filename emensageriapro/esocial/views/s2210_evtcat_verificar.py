...
def gerar_xml_assinado(s2210_evtcat_id, db_slug):
    ...
    if s2210_evtcat.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s2210evtCAT.objects.using(db_slug).\
            filter(id=s2210_evtcat_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        if s2210_evtcat.status in status_list:
            s2210evtCAT.objects.using(db_slug).filter(id=s2210_evtcat_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s2210_evtcat/%s.xml' % (s2210_evtcat.identidade)
...
def validar_evento(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        if s2210_evtcat.versao in VERSOES_ESOCIAL:
            ...
            if s2210_evtcat.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2210evtCAT.objects.using(db_slug).\
                    filter(excluido=False, id=s2210_evtcat_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s2210_evtcat.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s2210evtCAT.objects.using(db_slug).\
                    filter(excluido=False, id=s2210_evtcat_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
