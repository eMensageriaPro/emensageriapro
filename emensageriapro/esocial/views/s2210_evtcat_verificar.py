...
def verificar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2210_idelocalacid_lista = s2210ideLocalAcid.objects.using(db_slug).filter(s2210_evtcat_id__in = listar_ids(s2210_evtcat_lista) ).filter(excluido=False).all()
...
def gerar_xml_s2210(s2210_evtcat_id, db_slug, versao=None):
...
def gerar_xml_assinado(s2210_evtcat_id, db_slug):
    ...
    if s2210_evtcat.status in (0,1,2,11):
        ...
        s2210evtCAT.objects.using(db_slug).\
            filter(id=s2210_evtcat_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    from emensageriapro.esocial.views.s2210_evtcat_importar import read_s2210_evtcat_string
    ...
    if s2210_evtcat_id:
        ...
        texto = gerar_xml_s2210(s2210_evtcat_id, db_slug, versao="|")
        dados = read_s2210_evtcat_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s2210evtCAT.objects.using(db_slug).filter(id=dados['id']).\
            update(status=0, arquivo_original=0, arquivo='')

        gravar_auditoria(u'{}', u'{"funcao": "Evento de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2210_evtcat.identidade),
            's2210_evtcat', dados['id'], request.user.id, 1)
...
def criar_alteracao(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2210_evtcat.identidade),
            's2210_evtcat', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s2210_evtcat.identidade),
            's2210_evtcat', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s2210_evtcat_id:
        ...
        if s2210_evtcat.status == 0:
            ...
            nova_identidade = identidade_evento(s2210_evtcat_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's2210_evtcat', s2210_evtcat_id, request.user.id, 1)
...
def validar_evento_funcao(s2210_evtcat_id, db_slug):
    ...
    s2210evtCAT.objects.using( db_slug ).\
        filter(id=s2210_evtcat_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s2210_evtcat_validar import validacoes_s2210_evtcat
...
<<<<<<< HEAD
        if s2210_evtcat.versao in VERSOES_ESOCIAL:
=======
        if s2210_evtcat.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s2210_evtcat_id, db_slug)
