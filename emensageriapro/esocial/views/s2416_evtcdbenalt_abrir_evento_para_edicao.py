...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s2416evtCdBenAlt'):

        if s2416_evtcdbenalt_id:
            s2416_evtcdbenalt = get_object_or_404(s2416evtCdBenAlt, excluido=False, id=s2416_evtcdbenalt_id)
            ...
            if s2416_evtcdbenalt.status in status_list:
                s2416evtCdBenAlt.objects.filter(id=s2416_evtcdbenalt_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
