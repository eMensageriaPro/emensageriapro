...
def gerar_xml_assinado(s1030_evttabcargo_id, db_slug):
    ...
    if s1030_evttabcargo.status in (STATUS_EVENTO_CADASTRADO,
                           STATUS_EVENTO_IMPORTADO,
                           STATUS_EVENTO_DUPLICADO,
                           STATUS_EVENTO_GERADO):

        s1030evtTabCargo.objects.using(db_slug).\
            filter(id=s1030_evttabcargo_id,excluido=False).update(status=STATUS_EVENTO_ASSINADO)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        if s1030_evttabcargo.status in status_list:
            s1030evtTabCargo.objects.using(db_slug).filter(id=s1030_evttabcargo_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                          arquivo_original=0)
            arquivo = 'arquivos/Eventos/s1030_evttabcargo/%s.xml' % (s1030_evttabcargo.identidade)
...
def validar_evento(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        if s1030_evttabcargo.versao in VERSOES_ESOCIAL:
            ...
            if s1030_evttabcargo.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1030evtTabCargo.objects.using(db_slug).\
                    filter(excluido=False, id=s1030_evttabcargo_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1030_evttabcargo.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1030evtTabCargo.objects.using(db_slug).\
                    filter(excluido=False, id=s1030_evttabcargo_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
