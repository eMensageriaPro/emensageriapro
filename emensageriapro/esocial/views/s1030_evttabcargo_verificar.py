...
def gerar_xml_s1030(s1030_evttabcargo_id, db_slug, versao=None):
...
def gerar_xml_assinado(s1030_evttabcargo_id, db_slug):
    ...
    if s1030_evttabcargo.status in (0,1,2,11):
        ...
        s1030evtTabCargo.objects.using(db_slug).\
            filter(id=s1030_evttabcargo_id,excluido=False).update(status=10)
...
def duplicar(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        texto = gerar_xml_s1030(s1030_evttabcargo_id, db_slug, versao="|")
        dados = read_s1030_evttabcargo_string({}, texto.encode('utf-8'), 0)
        nova_identidade = identidade_evento(dados['id'], db_slug)

        s1030evtTabCargo.objects.using(db_slug).filter(id=dados['id']).\
...
def criar_alteracao(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de de alteração de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1030_evttabcargo.identidade),
            's1030_evttabcargo', dados['id'], request.user.id, 1)
...
def criar_exclusao(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        gravar_auditoria(u'{}',
            u'{"funcao": "Evento de exclusão de identidade %s criado a partir da duplicação do evento %s"}' % (nova_identidade, s1030_evttabcargo.identidade),
            's1030_evttabcargo', dados['id'], request.user.id, 1)
...
def alterar_identidade(request, hash):
    ...
    if s1030_evttabcargo_id:
        ...
        if s1030_evttabcargo.status == 0:
            ...
            nova_identidade = identidade_evento(s1030_evttabcargo_id, db_slug)
            ...
            gravar_auditoria(u'{}',
                ...
                's1030_evttabcargo', s1030_evttabcargo_id, request.user.id, 1)
...
def validar_evento_funcao(s1030_evttabcargo_id, db_slug):
    ...
    s1030evtTabCargo.objects.using( db_slug ).\
        filter(id=s1030_evttabcargo_id, excluido = False).\
    ...
    if os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        from emensageriapro.esocial.views.s1030_evttabcargo_validar import validacoes_s1030_evttabcargo
...
<<<<<<< HEAD
        if s1030_evttabcargo.versao in VERSOES_ESOCIAL:
=======
        if s1030_evttabcargo.versao in VERSAO_ATUAL:
>>>>>>> 3217f7abcc9a9c37261d88e43626ba3e9fb91ee3

            validar_evento_funcao(s1030_evttabcargo_id, db_slug)
