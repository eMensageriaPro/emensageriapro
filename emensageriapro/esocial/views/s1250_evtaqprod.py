    ...
    if request.method == 'POST':
        if s1250_evtaqprod.status == 0:
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1250_evtaqprod', s1250_evtaqprod_id, usuario_id, 3)
        else:
...
class s1250evtAqProdList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1250evtAqProd.objects.using(db_slug).all()
    serializer_class = s1250evtAqProdSerializer
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1250_evtaqprod_listar.html',
            ...
            response = render_to_response('s1250_evtaqprod_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1250_evtaqprod_csv.html', context)
            filename = "s1250_evtaqprod.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1250_evtaqprod_id:
            ...
            s1250_evtaqprod_form = form_s1250_evtaqprod(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s1250_evtaqprod_form.is_valid():
                ...
                if s1250_evtaqprod_id:
                    ...
                    s1250_evtaqprod_form = form_s1250_evtaqprod(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            ...
            s1250_evtaqprod_form.fields['status'].widget.attrs['disabled'] = True
            s1250_evtaqprod_form.fields['status'].widget.attrs['readonly'] = True
            s1250_evtaqprod_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1250_evtaqprod_id, tabela='s1250_evtaqprod').all()
        context = {
