...
def abrir_evento_para_edicao(request, hash):
    ...
    if request.user.has_perm('esocial.can_open_event_s5001evtBasesTrab'):

        if s5001_evtbasestrab_id:
            s5001_evtbasestrab = get_object_or_404(s5001evtBasesTrab, excluido=False, id=s5001_evtbasestrab_id)
            ...
            if s5001_evtbasestrab.status in status_list:
                s5001evtBasesTrab.objects.filter(id=s5001_evtbasestrab_id).update(status=STATUS_EVENTO_CADASTRADO,
                                                                              arquivo_original=0)
