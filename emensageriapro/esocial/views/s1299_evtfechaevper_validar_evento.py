...
def validar_evento_funcao(request, s1299_evtfechaevper_id):
    ...
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        ...
        gerar_xml_assinado(request, s1299_evtfechaevper_id)
...
def validar_evento(request, hash):
    ...
    if s1299_evtfechaevper_id:
        ...
        if s1299_evtfechaevper.versao in VERSOES_ESOCIAL:
            ...
            validar_evento_funcao(request, s1299_evtfechaevper_id)
            
            if s1299_evtfechaevper.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1299evtFechaEvPer.objects.\
                    filter(id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1299_evtfechaevper.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1299evtFechaEvPer.objects.\
                    filter(id=s1299_evtfechaevper_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
