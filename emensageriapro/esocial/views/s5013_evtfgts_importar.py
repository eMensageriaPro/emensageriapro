...
def read_s5013_evtfgts(request, dados, arquivo, validar=False):
    ...
    status = STATUS_EVENTO_IMPORTADO
    dados = read_s5013_evtfgts_obj(request, doc, status, validar, arquivo)

    s5013evtFGTS.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s5013_evtfgts_obj(request, doc, status, validar=False, arquivo=False):
    ...
    if arquivo:
        s5013_evtfgts_dados['arquivo'] = arquivo
