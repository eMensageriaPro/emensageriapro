...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5011_evtcs_id:
        ...
        if s5011_evtcs.status in status_list:
    ...
    if s5011_evtcs_id:
        ...
        if s5011_evtcs.versao in VERSOES_ESOCIAL:
            ...
            if s5011_evtcs.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5011evtCS.objects.using(db_slug).\
                    filter(excluido=False, id=s5011_evtcs_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5011_evtcs.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5011evtCS.objects.using(db_slug).\
                    filter(excluido=False, id=s5011_evtcs_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
