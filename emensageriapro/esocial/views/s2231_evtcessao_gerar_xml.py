...
def gerar_xml_s2231(request, s2231_evtcessao_id, versao=None):
    ...
    if s2231_evtcessao_id:
        ...
        s2231_evtcessao_lista = s2231evtCessao.objects. \
            filter(id=s2231_evtcessao_id, excluido = False).all()
...
def gerar_xml_assinado(request, s2231_evtcessao_id):
    ...
    if s2231_evtcessao.arquivo_original:
        ...
        xml = gerar_xml_s2231(request, s2231_evtcessao_id)
    ...
    if 'Signature' in xml:
        ...
        xml_assinado = assinar_esocial(request, xml, s2231_evtcessao.transmissor_lote_esocial_id)
    ...
    if s2231_evtcessao.status in (
        ...
        s2231evtCessao.objects.\
            filter(id=s2231_evtcessao_id).update(status=STATUS_EVENTO_ASSINADO)
...
def gerar_xml(request, hash):
    ...
    if s2231_evtcessao_id:
        ...
        xml_assinado = gerar_xml_assinado(request, s2231_evtcessao_id)
