...
def validar_evento_funcao(request, pk):
    ...
    if not s1050_evttabhortur.identidade:
        ...
        ident = identidade_evento(s1050_evttabhortur, 'esocial')
        s1050_evttabhortur = get_object_or_404(s1050evtTabHorTur, id=pk)
    ...
    schema_filename = 'esocial/%s/evtTabHorTur.xsd' % (s1050_evttabhortur.versao)
    ...
    if lista_validacoes:
        ...
        if VERIFICAR_PREDECESSAO_ANTES_ENVIO and 's1050_evttabhortur' not in EVENTOS_SEM_PREDECESSAO:

            quant = validar_precedencia('s1050_evttabhortur', pk)
...
def validar_evento(request, pk, tab=None):
    ...
    if s1050_evttabhortur.status in STATUS_VALIDACAO:
    ...
    if tab == 'mapa':
        ...
        return redirect('s1050_evttabhortur_salvar', pk=pk)