...
def read_s2241_evtinsapo(request, dados, arquivo, validar=False):
    ...
    s2241evtInsApo.objects.filter(id=dados['id']).update(arquivo=arquivo)
...
def read_s2241_evtinsapo_obj(request, doc, status, validar=False):
    ...
    s2241_evtinsapo_dados['status'] = STATUS_EVENTO_IMPORTADO
