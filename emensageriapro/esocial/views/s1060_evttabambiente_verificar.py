...
def abrir_evento_para_edicao(request, hash):
    ...
    if s1060_evttabambiente_id:
        ...
        if s1060_evttabambiente.status in status_list:
    ...
    if s1060_evttabambiente_id:
        ...
        if s1060_evttabambiente.versao in VERSOES_ESOCIAL:
            ...
            if s1060_evttabambiente.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1060evtTabAmbiente.objects.using(db_slug).\
                    filter(excluido=False, id=s1060_evttabambiente_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s1060_evttabambiente.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s1060evtTabAmbiente.objects.using(db_slug).\
                    filter(excluido=False, id=s1060_evttabambiente_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
