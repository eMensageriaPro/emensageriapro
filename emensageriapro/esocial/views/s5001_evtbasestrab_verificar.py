...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5001_evtbasestrab_id:
        ...
        if s5001_evtbasestrab.status in status_list or s5001_evtbasestrab.processamento_codigo_resposta in (401,402):
...
def validar_evento_funcao(s5001_evtbasestrab_id, db_slug):
    ...
    lista_validacoes = []
    s5001_evtbasestrab = get_object_or_404(s5001evtBasesTrab.objects.using(db_slug), excluido=False, id=s5001_evtbasestrab_id)
    if s5001_evtbasestrab.transmissor_lote_esocial:
        if s5001_evtbasestrab.transmissor_lote_esocial.transmissor:
