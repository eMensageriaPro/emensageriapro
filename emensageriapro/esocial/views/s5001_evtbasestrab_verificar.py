...
def recibo(request, hash, tipo):
    ...
    if permissao.permite_listar:
        ...
        s5001_evtbasestrab = get_object_or_404(
            s5001evtBasesTrab.objects.using( db_slug ),
            excluido = False, id = s5001_evtbasestrab_id)
        ...
        retorno = get_object_or_404( RetornosEventos.objects.using(db_slug),
            id=s5001_evtbasestrab.retornos_eventos_id, excluido=False)
        ...
        if tipo == 'XLS':
            response =  render_to_response('s5001_evtbasestrab_recibo_pdf.html', context)
            filename = "%s.xls" % s5001_evtbasestrab.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            response =  render_to_response('s5001_evtbasestrab_recibo_csv.html', context)
            filename = "%s.csv" % s5001_evtbasestrab.identidade
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'text/csv; charset=UTF-8'
            return response
            ...
            return render_to_pdf('s5001_evtbasestrab_recibo_pdf.html', context)
...
def abrir_evento_para_edicao(request, hash):
    ...
    if s5001_evtbasestrab_id:
        ...
        if s5001_evtbasestrab.status in status_list:
            ...
            if os.path.exists(BASE_DIR + '/' + arquivo):
                ...
                dad = (BASE_DIR, s5001_evtbasestrab.identidade, BASE_DIR, s5001_evtbasestrab.identidade, data_hora_atual)
                os.system('mv %s/arquivos/Eventos/s5001_evtbasestrab/%s.xml %s/arquivos/Eventos/s5001_evtbasestrab/%s_backup_%s.xml' % dad)
                gravar_nome_arquivo('/arquivos/Eventos/s5001_evtbasestrab/%s_backup_%s.xml' % (s5001_evtbasestrab.identidade, data_hora_atual),
                    1)
...
def validar_evento_funcao(s5001_evtbasestrab_id, db_slug):
    ...
    tipo = 'esocial'
    if not os.path.exists(BASE_DIR + '/' + arquivo):
        gerar_xml_assinado(s5001_evtbasestrab_id, db_slug)
    if os.path.exists(BASE_DIR + '/' + arquivo):
...
def validar_evento(request, hash):
    ...
    if s5001_evtbasestrab_id:
        ...
        if s5001_evtbasestrab.versao in VERSOES_ESOCIAL:

            validar_evento_funcao(s5001_evtbasestrab_id, db_slug)

            if s5001_evtbasestrab.transmissor_lote_esocial and not VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5001evtBasesTrab.objects.using(db_slug).\
                    filter(excluido=False, id=s5001_evtbasestrab_id).update(status=STATUS_EVENTO_AGUARD_ENVIO)

            elif s5001_evtbasestrab.transmissor_lote_esocial and VERIFICAR_PREDECESSAO_ANTES_ENVIO:
                s5001evtBasesTrab.objects.using(db_slug).\
                    filter(excluido=False, id=s5001_evtbasestrab_id).update(status=STATUS_EVENTO_AGUARD_PRECEDENCIA)
