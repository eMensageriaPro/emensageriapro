...
class s2220evtMonitList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2220evtMonit.objects.using(db_slug).all()
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2220_evtmonit_listar.html',
            ...
            response = render_to_response('s2220_evtmonit_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2220_evtmonit_csv.html', context)
            filename = "s2220_evtmonit.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2220_evtmonit_id:
            ...
            s2220_evtmonit_form = form_s2220_evtmonit(request.POST or None, slug = db_slug, initial={'versao': VERSAO_LAYOUT_ESOCIAL, 'status': 0, 'processamento_codigo_resposta': 0, 'tpamb': TP_AMB, 'procemi': 1, 'verproc': VERSAO_EMENSAGERIA})
        if request.method == 'POST':
            if s2220_evtmonit_form.is_valid():
                ...
                if s2220_evtmonit_id:
                    ...
                    s2220_evtmonit_form = form_s2220_evtmonit(request.POST or None, instance = obj, slug = db_slug)
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
        ...
        if not evento_totalizador:
            ...
            s2220_evtmonit_form.fields['status'].widget.attrs['disabled'] = True
            s2220_evtmonit_form.fields['status'].widget.attrs['readonly'] = True
            s2220_evtmonit_form.fields['transmissor_lote_esocial'].widget.attrs['disabled'] = True
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2220_evtmonit_id, tabela='s2220_evtmonit').all()
        context = {
