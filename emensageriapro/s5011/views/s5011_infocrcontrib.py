...
def apagar(request, hash):
    ...
    if s5011_infocrcontrib_id:
        dados_evento = s5011_infocrcontrib.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s5011_infocrcontrib_apagar'] = 0
            dict_permissoes['s5011_infocrcontrib_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5011_infocrcontrib), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's5011_infocrcontrib', s5011_infocrcontrib_id, usuario_id, 3)
        else:
...
class s5011infoCRContribList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s5011infoCRContrib.objects.using(db_slug).all()
    serializer_class = s5011infoCRContribSerializer
    # permission_classes = (IsAdminUser,)


class s5011infoCRContribDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s5011infoCRContrib.objects.using(db_slug).all()
    serializer_class = s5011infoCRContribSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_infocrcontrib_listar.html',
                filename="s5011_infocrcontrib.pdf",
                context=context,
            ...
            response = render_to_response('s5011_infocrcontrib_listar.html', context)
            filename = "s5011_infocrcontrib.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5011_infocrcontrib_csv.html', context)
            filename = "s5011_infocrcontrib.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5011_infocrcontrib_id:
            s5011_infocrcontrib_form = form_s5011_infocrcontrib(request.POST or None, instance = s5011_infocrcontrib,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s5011_infocrcontrib_form = form_s5011_infocrcontrib(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s5011_infocrcontrib_form.is_valid():

                dados = s5011_infocrcontrib_form.cleaned_data
                obj = s5011_infocrcontrib_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s5011_infocrcontrib_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's5011_infocrcontrib', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s5011_infocrcontrib), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's5011_infocrcontrib', s5011_infocrcontrib_id, usuario_id, 2)
        ...
        if s5011_infocrcontrib_id:
            if dados_evento['status'] != 0:
                s5011_infocrcontrib_form = disabled_form_fields(s5011_infocrcontrib_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5011_infocrcontrib_id, tabela='s5011_infocrcontrib').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_infocrcontrib_salvar.html',
                filename="s5011_infocrcontrib.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s5011_infocrcontrib_salvar.html', context)
            filename = "s5011_infocrcontrib.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
