...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s5011_infocomplobra = get_object_or_404(s5011infoComplObra, id=pk)
        evento_dados = s5011_infocomplobra.evento()
    ...
    if request.user.has_perm('s5011.can_see_s5011infoComplObra'):
        ...
        if request.method == 'POST':
            ...
            if s5011_infocomplobra_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s5011_infocomplobra), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's5011_infocomplobra_apagar', 
                    's5011_infocomplobra_salvar', 
                    's5011_infocomplobra'):
        ...
        s5011_infocomplobra_form = disabled_form_fields(
            s5011_infocomplobra_form, 
            request.user.has_perm('s5011.change_s5011infoComplObra'))
        ...
        if tab or 's5011_infocomplobra' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's5011_infocomplobra_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s5011_infocomplobra').all()
