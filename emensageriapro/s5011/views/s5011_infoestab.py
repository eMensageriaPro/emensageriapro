...
class s5011infoEstabList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s5011infoEstab.objects.using(db_slug).all()
    serializer_class = s5011infoEstabSerializer
    # permission_classes = (IsAdminUser,)


class s5011infoEstabDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s5011infoEstab.objects.using(db_slug).all()
    serializer_class = s5011infoEstabSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_infoestab_listar.html',
                filename="s5011_infoestab.pdf",
                context=context,
            ...
            response = render_to_response('s5011_infoestab_listar.html', context)
            filename = "s5011_infoestab.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5011_infoestab_csv.html', context)
            filename = "s5011_infoestab.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5011_infoestab_id:
            s5011_infoestab_form = form_s5011_infoestab(request.POST or None, instance = s5011_infoestab,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s5011_infoestab_form = form_s5011_infoestab(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s5011_infoestab_form.is_valid():

                dados = s5011_infoestab_form.cleaned_data
                obj = s5011_infoestab_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s5011_infoestab_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's5011_infoestab', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s5011_infoestab), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's5011_infoestab', s5011_infoestab_id, usuario_id, 2)
        ...
        if s5011_infoestab_id:
            if dados_evento['status'] != 0:
                s5011_infoestab_form = disabled_form_fields(s5011_infoestab_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5011_infoestab_id, tabela='s5011_infoestab').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_infoestab_salvar.html',
                filename="s5011_infoestab.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s5011_infoestab_salvar.html', context)
            filename = "s5011_infoestab.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
