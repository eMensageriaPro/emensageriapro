...
def apagar(request, hash):
    ...
    if s5011_basesavnport_id:
        dados_evento = s5011_basesavnport.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s5011_basesavnport_apagar'] = 0
            dict_permissoes['s5011_basesavnport_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5011_basesavnport), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's5011_basesavnport', s5011_basesavnport_id, usuario_id, 3)
        else:
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_basesavnport_listar.html',
                filename="s5011_basesavnport.pdf",
                context=context,
            ...
            response = render_to_response('s5011_basesavnport_listar.html', context)
            filename = "s5011_basesavnport.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5011_basesavnport_csv.html', context)
            filename = "s5011_basesavnport.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5011_basesavnport_id:
            s5011_basesavnport_form = form_s5011_basesavnport(request.POST or None, instance = s5011_basesavnport,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s5011_basesavnport_form = form_s5011_basesavnport(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s5011_basesavnport_form.is_valid():

                dados = s5011_basesavnport_form.cleaned_data
                obj = s5011_basesavnport_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s5011_basesavnport_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's5011_basesavnport', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s5011_basesavnport), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's5011_basesavnport', s5011_basesavnport_id, usuario_id, 2)
        ...
        if s5011_basesavnport_id:
            if dados_evento['status'] != 0:
                s5011_basesavnport_form = disabled_form_fields(s5011_basesavnport_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5011_basesavnport_id, tabela='s5011_basesavnport').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_basesavnport_salvar.html',
                filename="s5011_basesavnport.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s5011_basesavnport_salvar.html', context)
            filename = "s5011_basesavnport.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
