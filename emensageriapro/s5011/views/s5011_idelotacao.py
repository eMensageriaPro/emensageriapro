...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s5011_idelotacao')
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s5011_idelotacao), indent=4, sort_keys=True, default=str)
            s5011ideLotacao.objects.using( db_slug ).filter(id = s5011_idelotacao_id).delete()
            #s5011_idelotacao_apagar_custom
            #s5011_idelotacao_apagar_custom
    ...
    return render(request, 's5011_idelotacao_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s5011ideLotacaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s5011ideLotacao.objects.using(db_slug).all()
    serializer_class = s5011ideLotacaoSerializer
    permission_classes = (IsAdminUser,)


class s5011ideLotacaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s5011ideLotacao.objects.using(db_slug).all()
    serializer_class = s5011ideLotacaoSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s5011_idelotacao_lista = s5011ideLotacao.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s5011_idelotacao_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s5011_idelotacao_listar.html',
            ...
            response = render_to_response('s5011_idelotacao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s5011_idelotacao_csv.html', context)
            filename = "s5011_idelotacao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s5011_idelotacao_id:
            s5011_idelotacao_form = form_s5011_idelotacao(request.POST or None, instance = s5011_idelotacao, slug = db_slug)
        else:
            s5011_idelotacao_form = form_s5011_idelotacao(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s5011_idelotacao_form.is_valid():
                ...
                if s5011_idelotacao_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s5011_idelotacao_campos_multiple_passo1
                        s5011ideLotacao.objects.using(db_slug).filter(id=s5011_idelotacao_id).update(**dados)
                        obj = s5011ideLotacao.objects.using(db_slug).get(id=s5011_idelotacao_id)
                        #s5011_idelotacao_editar_custom
                        #s5011_idelotacao_campos_multiple_passo2
                    ...
                    #s5011_idelotacao_cadastrar_campos_multiple_passo1
                    obj = s5011ideLotacao(**dados)
                    obj.save(using = db_slug)
                    #s5011_idelotacao_cadastrar_custom
                    #s5011_idelotacao_cadastrar_campos_multiple_passo2
        ...
        for field in s5011_idelotacao_form.fields.keys():
        ...
        if s5011_idelotacao_id:
            ...
            s5011_infoemprparcial_form = form_s5011_infoemprparcial(initial={ 's5011_idelotacao': s5011_idelotacao }, slug=db_slug)
            s5011_infoemprparcial_form.fields['s5011_idelotacao'].widget.attrs['readonly'] = True
            s5011_infoemprparcial_lista = s5011infoEmprParcial.objects.using( db_slug ).filter(excluido = False, s5011_idelotacao_id=s5011_idelotacao.id).all()
            s5011_dadosopport_form = form_s5011_dadosopport(initial={ 's5011_idelotacao': s5011_idelotacao }, slug=db_slug)
            s5011_dadosopport_form.fields['s5011_idelotacao'].widget.attrs['readonly'] = True
            s5011_dadosopport_lista = s5011dadosOpPort.objects.using( db_slug ).filter(excluido = False, s5011_idelotacao_id=s5011_idelotacao.id).all()
            s5011_basesremun_form = form_s5011_basesremun(initial={ 's5011_idelotacao': s5011_idelotacao }, slug=db_slug)
            s5011_basesremun_form.fields['s5011_idelotacao'].widget.attrs['readonly'] = True
            s5011_basesremun_lista = s5011basesRemun.objects.using( db_slug ).filter(excluido = False, s5011_idelotacao_id=s5011_idelotacao.id).all()
            s5011_basesavnport_form = form_s5011_basesavnport(initial={ 's5011_idelotacao': s5011_idelotacao }, slug=db_slug)
            s5011_basesavnport_form.fields['s5011_idelotacao'].widget.attrs['readonly'] = True
            s5011_basesavnport_lista = s5011basesAvNPort.objects.using( db_slug ).filter(excluido = False, s5011_idelotacao_id=s5011_idelotacao.id).all()
            s5011_infosubstpatropport_form = form_s5011_infosubstpatropport(initial={ 's5011_idelotacao': s5011_idelotacao }, slug=db_slug)
            s5011_infosubstpatropport_form.fields['s5011_idelotacao'].widget.attrs['readonly'] = True
            s5011_infosubstpatropport_lista = s5011infoSubstPatrOpPort.objects.using( db_slug ).filter(excluido = False, s5011_idelotacao_id=s5011_idelotacao.id).all()
        else:
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s5011_idelotacao_id, tabela='s5011_idelotacao').all()
        context = {
