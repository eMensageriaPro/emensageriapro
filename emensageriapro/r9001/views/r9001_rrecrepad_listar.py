...
def listar(request, hash):
    ...
    pagina = ConfigPaginas.objects.get( endereco='r9001_rrecrepad')
    ...
    if permissao.permite_listar:
        ...
        r9001_rrecrepad_lista = r9001RRecRepAD.objects.filter(**dict_qs).filter().exclude(id=0).all()
        if not post and len(r9001_rrecrepad_lista) > 100:
            filtrar = True
            r9001_rrecrepad_lista = None
            messages.warning(request, u'Listagem com mais de 100 resultados! Filtre os resultados um melhor desempenho!')
        #[VARIAVEIS_LISTA_FILTRO_RELATORIO]
        #r9001_rrecrepad_listar_custom
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 'r9001_rrecrepad'
        context = {
        ...
        if for_print in (0,1):
            return render(request, 'r9001_rrecrepad_listar.html', context)
        elif for_print == 2:
            #return render_to_pdf('tables/r9001_rrecrepad_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(
                request=request,
                template='r9001_rrecrepad_listar.html',
            ...
            response = render_to_response('r9001_rrecrepad_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('csv/r9001_rrecrepad.csv', context)
            filename = "r9001_rrecrepad.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
