...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2231_fimcessao = get_object_or_404(s2231fimCessao, id=pk)
        evento_dados = s2231_fimcessao.evento()

    if request.user.has_perm('s2231.can_see_s2231fimCessao'):
        
        if pk:
        
            s2231_fimcessao_form = form_s2231_fimcessao(
                request.POST or None, 
                instance=s2231_fimcessao)
                                         
        else:
        
            s2231_fimcessao_form = form_s2231_fimcessao(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2231_fimcessao_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's2231_fimcessao_apagar', 
                    's2231_fimcessao_salvar', 
                    's2231_fimcessao'):
        ...
        s2231_fimcessao_form = disabled_form_fields(
            s2231_fimcessao_form, 
            request.user.has_perm('s2231.change_s2231fimCessao'))
        ...
        if tab or 's2231_fimcessao' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's2231_fimcessao_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2231_fimcessao').all()
