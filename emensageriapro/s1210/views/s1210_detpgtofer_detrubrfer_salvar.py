...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1210_detpgtofer_detrubrfer = get_object_or_404(s1210detPgtoFerdetRubrFer, id=pk)
        evento_dados = s1210_detpgtofer_detrubrfer.evento()
    ...
    if request.user.has_perm('s1210.can_see_s1210detPgtoFerdetRubrFer'):
        ...
        if request.method == 'POST':
            ...
            if s1210_detpgtofer_detrubrfer_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1210_detpgtofer_detrubrfer), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1210_detpgtofer_detrubrfer_apagar', 
                    's1210_detpgtofer_detrubrfer_salvar', 
                    's1210_detpgtofer_detrubrfer'):
        ...
        s1210_detpgtofer_detrubrfer_form = disabled_form_fields(
            s1210_detpgtofer_detrubrfer_form, 
            request.user.has_perm('s1210.change_s1210detPgtoFerdetRubrFer'))
        ...
        if tab or 's1210_detpgtofer_detrubrfer' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1210_detpgtofer_detrubrfer_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1210_detpgtofer_detrubrfer').all()
