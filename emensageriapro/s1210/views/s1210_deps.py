...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1210deps.objects.using( db_slug ).filter(id = s1210_deps_id).delete()
            ...
            if s1210_deps_form.is_valid():
                ...
                if s1210_deps_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1210_deps_campos_multiple_passo1
                        s1210deps.objects.using(db_slug).filter(id=s1210_deps_id).update(**dados)
                        obj = s1210deps.objects.using(db_slug).get(id=s1210_deps_id)
                        #s1210_deps_editar_custom
                        #s1210_deps_campos_multiple_passo2
                    ...
                    #s1210_deps_cadastrar_campos_multiple_passo1
                    obj = s1210deps(**dados)
                    obj.save(using = db_slug)
                    #s1210_deps_cadastrar_custom
                    #s1210_deps_cadastrar_campos_multiple_passo2
