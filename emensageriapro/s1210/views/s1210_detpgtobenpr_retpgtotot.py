...
def apagar(request, hash):
    ...
    if s1210_detpgtobenpr_retpgtotot_id:
        dados_evento = s1210_detpgtobenpr_retpgtotot.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s1210_detpgtobenpr_retpgtotot_apagar'] = 0
            dict_permissoes['s1210_detpgtobenpr_retpgtotot_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1210_detpgtobenpr_retpgtotot), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1210_detpgtobenpr_retpgtotot', s1210_detpgtobenpr_retpgtotot_id, usuario_id, 3)
        else:
...
class s1210detPgtoBenPrretPgtoTotList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoBenPrretPgtoTot.objects.using(db_slug).all()
    serializer_class = s1210detPgtoBenPrretPgtoTotSerializer
    # permission_classes = (IsAdminUser,)


class s1210detPgtoBenPrretPgtoTotDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoBenPrretPgtoTot.objects.using(db_slug).all()
    serializer_class = s1210detPgtoBenPrretPgtoTotSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1210_detpgtobenpr_retpgtotot_listar.html',
                filename="s1210_detpgtobenpr_retpgtotot.pdf",
                context=context,
            ...
            response = render_to_response('s1210_detpgtobenpr_retpgtotot_listar.html', context)
            filename = "s1210_detpgtobenpr_retpgtotot.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1210_detpgtobenpr_retpgtotot_csv.html', context)
            filename = "s1210_detpgtobenpr_retpgtotot.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1210_detpgtobenpr_retpgtotot_id:
            s1210_detpgtobenpr_retpgtotot_form = form_s1210_detpgtobenpr_retpgtotot(request.POST or None, instance = s1210_detpgtobenpr_retpgtotot,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1210_detpgtobenpr_retpgtotot_form = form_s1210_detpgtobenpr_retpgtotot(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s1210_detpgtobenpr_retpgtotot_form.is_valid():

                dados = s1210_detpgtobenpr_retpgtotot_form.cleaned_data
                obj = s1210_detpgtobenpr_retpgtotot_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s1210_detpgtobenpr_retpgtotot_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's1210_detpgtobenpr_retpgtotot', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s1210_detpgtobenpr_retpgtotot), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1210_detpgtobenpr_retpgtotot', s1210_detpgtobenpr_retpgtotot_id, usuario_id, 2)
        ...
        if s1210_detpgtobenpr_retpgtotot_id:
            if dados_evento['status'] != 0:
                s1210_detpgtobenpr_retpgtotot_form = disabled_form_fields(s1210_detpgtobenpr_retpgtotot_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1210_detpgtobenpr_retpgtotot_id, tabela='s1210_detpgtobenpr_retpgtotot').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1210_detpgtobenpr_retpgtotot_salvar.html',
                filename="s1210_detpgtobenpr_retpgtotot.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1210_detpgtobenpr_retpgtotot_salvar.html', context)
            filename = "s1210_detpgtobenpr_retpgtotot.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
