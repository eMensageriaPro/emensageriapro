...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1210_infopgto')
    ...
    if s1210_infopgto_id:
        dados_evento = s1210_infopgto.evento()
        if dados_evento['status'] != 0:
            dict_permissoes['s1210_infopgto_apagar'] = 0
            dict_permissoes['s1210_infopgto_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1210_infopgto), indent=4, sort_keys=True, default=str)
            s1210infoPgto.objects.using( db_slug ).filter(id = s1210_infopgto_id).delete()
            #s1210_infopgto_apagar_custom
            #s1210_infopgto_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1210_infopgto', s1210_infopgto_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's1210_infopgto_salvar':
            return redirect('s1210_infopgto', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's1210_infopgto_apagar.html', context)

from rest_framework import generics
from rest_framework.permissions import IsAdminUser


class s1210infoPgtoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1210infoPgto.objects.using(db_slug).all()
    serializer_class = s1210infoPgtoSerializer
    permission_classes = (IsAdminUser,)


class s1210infoPgtoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1210infoPgto.objects.using(db_slug).all()
    serializer_class = s1210infoPgtoSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1210_infopgto_lista = s1210infoPgto.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s1210_infopgto_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1210_infopgto_listar.html',
                filename="s1210_infopgto.pdf",
                context=context,
            ...
            response = render_to_response('s1210_infopgto_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1210_infopgto_csv.html', context)
            filename = "s1210_infopgto.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1210_infopgto_id:
            s1210_infopgto_form = form_s1210_infopgto(request.POST or None, instance = s1210_infopgto, slug = db_slug)
        else:
            s1210_infopgto_form = form_s1210_infopgto(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s1210_infopgto_form.is_valid():
                dados = s1210_infopgto_form.cleaned_data
                import json
                from django.forms.models import model_to_dict
                if s1210_infopgto_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1210_infopgto_campos_multiple_passo1
                        s1210infoPgto.objects.using(db_slug).filter(id=s1210_infopgto_id).update(**dados)
                        obj = s1210infoPgto.objects.using(db_slug).get(id=s1210_infopgto_id)
                        #s1210_infopgto_editar_custom
                        #s1210_infopgto_campos_multiple_passo2
                        messages.success(request, 'Alterado com sucesso!')
                        gravar_auditoria(json.dumps(model_to_dict(s1210_infopgto), indent=4, sort_keys=True, default=str),
                                         json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                         's1210_infopgto', s1210_infopgto_id, usuario_id, 2)
                    else:
                    ...
                    #s1210_infopgto_cadastrar_campos_multiple_passo1
                    obj = s1210infoPgto(**dados)
                    obj.save(using = db_slug)
                    #s1210_infopgto_cadastrar_custom
                    #s1210_infopgto_cadastrar_campos_multiple_passo2
                    messages.success(request, 'Cadastrado com sucesso!')
                    gravar_auditoria('{}',
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's1210_infopgto', obj.id, usuario_id, 1)
                    if request.session['retorno_pagina'] not in ('s1210_infopgto_apagar', 's1210_infopgto_salvar', 's1210_infopgto'):
                        return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
                    if s1210_infopgto_id != obj.id:
                        url_hash = base64.urlsafe_b64encode( '{"print": "0", "id": "%s"}' % (obj.id) )
                        return redirect('s1210_infopgto_salvar', hash=url_hash)
            else:
                messages.error(request, 'Erro ao salvar!')
        s1210_infopgto_form = disabled_form_fields(s1210_infopgto_form, permissao.permite_editar)
        if s1210_infopgto_id:
            if dados_evento['status'] != 0:
                s1210_infopgto_form = disabled_form_fields(s1210_infopgto_form, 0)
        #s1210_infopgto_campos_multiple_passo3

        for field in s1210_infopgto_form.fields.keys():
            s1210_infopgto_form.fields[field].widget.attrs['ng-model'] = 's1210_infopgto_'+field
        ...
        if s1210_infopgto_id:
            ...
            s1210_detpgtobenpr_form = form_s1210_detpgtobenpr(initial={ 's1210_infopgto': s1210_infopgto }, slug=db_slug)
            s1210_detpgtobenpr_form.fields['s1210_infopgto'].widget.attrs['readonly'] = True
            s1210_detpgtobenpr_lista = s1210detPgtoBenPr.objects.using( db_slug ).filter(excluido = False, s1210_infopgto_id=s1210_infopgto.id).all()
            ...
            s1210_infopgto = None
        #s1210_infopgto_salvar_custom_variaveis#
        tabelas_secundarias = []
        #[FUNCOES_ESPECIAIS_SALVAR]
        if dict_hash['tab'] or 's1210_infopgto' in request.session['retorno_pagina']:
            request.session["retorno_hash"] = hash
            request.session["retorno_pagina"] = 's1210_infopgto_salvar'
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1210_infopgto_id, tabela='s1210_infopgto').all()
        context = {
            ...
            'mensagem': mensagem,
            's1210_infopgto_id': int(s1210_infopgto_id),
            'usuario': usuario,
