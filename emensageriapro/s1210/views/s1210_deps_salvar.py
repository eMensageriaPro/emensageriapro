...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1210_deps = get_object_or_404(s1210deps, id=pk)
        evento_dados = s1210_deps.evento()

    if request.user.has_perm('s1210.can_see_s1210deps'):
        ...
        if pk:
            ...
            s1210_deps_form = form_s1210_deps(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s1210_deps_form.is_valid():
                ...
                if 's1210-deps' not in request.session['return']:
        ...
        s1210_deps_form = disabled_form_fields(
            s1210_deps_form,
            request.user.has_perm('s1210.change_s1210deps'))
        ...
        #if tab or 's1210_deps' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's1210_deps_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1210_deps').all()
