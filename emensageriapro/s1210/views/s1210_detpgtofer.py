...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1210detPgtoFer.objects.using( db_slug ).filter(id = s1210_detpgtofer_id).delete()
            ...
            if s1210_detpgtofer_form.is_valid():
                ...
                if s1210_detpgtofer_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1210_detpgtofer_campos_multiple_passo1
                        s1210detPgtoFer.objects.using(db_slug).filter(id=s1210_detpgtofer_id).update(**dados)
                        obj = s1210detPgtoFer.objects.using(db_slug).get(id=s1210_detpgtofer_id)
                        #s1210_detpgtofer_editar_custom
                        #s1210_detpgtofer_campos_multiple_passo2
                    ...
                    #s1210_detpgtofer_cadastrar_campos_multiple_passo1
                    obj = s1210detPgtoFer(**dados)
                    obj.save(using = db_slug)
                    #s1210_detpgtofer_cadastrar_custom
                    #s1210_detpgtofer_cadastrar_campos_multiple_passo2
