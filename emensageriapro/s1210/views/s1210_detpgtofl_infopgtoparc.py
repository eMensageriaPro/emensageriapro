...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s1210_detpgtofl_infopgtoparc')
    permissao = ConfigPermissoes.objects.using( db_slug ).get(excluido = False, config_paginas=pagina, config_perfis=usuario.config_perfis)
    if s1210_detpgtofl_infopgtoparc_id:
        s1210_detpgtofl_infopgtoparc = get_object_or_404(s1210detPgtoFlinfoPgtoParc.objects.using( db_slug ), excluido = False, id = s1210_detpgtofl_infopgtoparc_id)
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1210_detpgtofl_infopgtoparc_id:
            s1210_detpgtofl_infopgtoparc_form = form_s1210_detpgtofl_infopgtoparc(request.POST or None, instance = s1210_detpgtofl_infopgtoparc, slug = db_slug)
        else:
            s1210_detpgtofl_infopgtoparc_form = form_s1210_detpgtofl_infopgtoparc(request.POST or None, slug = db_slug, initial={})
            ...
            if s1210_detpgtofl_infopgtoparc_form.is_valid():
                ...
                if s1210_detpgtofl_infopgtoparc_id:
                    if dados_evento['status'] == 0:
                        ...
                        s1210detPgtoFlinfoPgtoParc.objects.using(db_slug).filter(id=s1210_detpgtofl_infopgtoparc_id).update(**dados)
                        obj = s1210detPgtoFlinfoPgtoParc.objects.using(db_slug).get(id=s1210_detpgtofl_infopgtoparc_id)
        ...
        for field in s1210_detpgtofl_infopgtoparc_form.fields.keys():
            s1210_detpgtofl_infopgtoparc_form.fields[field].widget.attrs['ng-model'] = 's1210_detpgtofl_infopgtoparc_'+field
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1210_detpgtofl_infopgtoparc_id, tabela='s1210_detpgtofl_infopgtoparc').all()
        context = {
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1210_detpgtofl_infopgtoparc), indent=4, sort_keys=True, default=str)
            s1210detPgtoFlinfoPgtoParc.objects.using( db_slug ).filter(id = s1210_detpgtofl_infopgtoparc_id).delete()
            #s1210_detpgtofl_infopgtoparc_apagar_custom
            #s1210_detpgtofl_infopgtoparc_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's1210_detpgtofl_infopgtoparc', s1210_detpgtofl_infopgtoparc_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')
   
        if request.session['retorno_pagina']== 's1210_detpgtofl_infopgtoparc_salvar':
            return redirect('s1210_detpgtofl_infopgtoparc', hash=request.session['retorno_hash'])
        else:
            return redirect(request.session['retorno_pagina'], hash=request.session['retorno_hash'])
    context = {
        'usuario': usuario,
...
class s1210detPgtoFlinfoPgtoParcList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoFlinfoPgtoParc.objects.using(db_slug).all()
    serializer_class = s1210detPgtoFlinfoPgtoParcSerializer
    permission_classes = (IsAdminUser,)


class s1210detPgtoFlinfoPgtoParcDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoFlinfoPgtoParc.objects.using(db_slug).all()
    serializer_class = s1210detPgtoFlinfoPgtoParcSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s1210_detpgtofl_infopgtoparc_lista = s1210detPgtoFlinfoPgtoParc.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        ...
        if for_print in (0,1):
            return render(request, 's1210_detpgtofl_infopgtoparc_listar.html', context)
        elif for_print == 2:
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s1210_detpgtofl_infopgtoparc_listar.html', context)
            filename = "s1210_detpgtofl_infopgtoparc.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1210_detpgtofl_infopgtoparc_csv.html', context)
