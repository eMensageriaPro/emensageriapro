...
class s1210detPgtoAntList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoAnt.objects.using(db_slug).all()
    serializer_class = s1210detPgtoAntSerializer
    # permission_classes = (IsAdminUser,)


class s1210detPgtoAntDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1210detPgtoAnt.objects.using(db_slug).all()
    serializer_class = s1210detPgtoAntSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1210_detpgtoant_id:
            s1210_detpgtoant_form = form_s1210_detpgtoant(request.POST or None, instance = s1210_detpgtoant,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s1210_detpgtoant_form = form_s1210_detpgtoant(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
