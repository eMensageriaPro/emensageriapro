...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1210_detpgtofer_penalim = get_object_or_404(s1210detPgtoFerpenAlim, id=pk)
        evento_dados = s1210_detpgtofer_penalim.evento()
    ...
    if request.user.has_perm('s1210.can_see_s1210detPgtoFerpenAlim'):
        ...
        if pk:
            ...
            s1210_detpgtofer_penalim_form = form_s1210_detpgtofer_penalim(
                ...
                instance=s1210_detpgtofer_penalim)
                     
        else:

            s1210_detpgtofer_penalim_form = form_s1210_detpgtofer_penalim(request.POST or None)
                     
        if request.method == 'POST':

            if s1210_detpgtofer_penalim_form.is_valid():

                obj = s1210_detpgtofer_penalim_form.save(request=request)
                ...
                #    gravar_auditoria(
                #        json.dumps(
                #            model_to_dict(s1210_detpgtofer_penalim),
                #            indent=4,
                #            sort_keys=True,
                #            default=str),
                #        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1210_detpgtofer_penalim_apagar',
                    's1210_detpgtofer_penalim_salvar',
                    's1210_detpgtofer_penalim'):
        ...
        s1210_detpgtofer_penalim_form = disabled_form_fields(
            s1210_detpgtofer_penalim_form,
            request.user.has_perm('s1210.change_s1210detPgtoFerpenAlim'))
        ...
        if tab or 's1210_detpgtofer_penalim' in request.session['return_page']:

            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1210_detpgtofer_penalim_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1210_detpgtofer_penalim').all()
