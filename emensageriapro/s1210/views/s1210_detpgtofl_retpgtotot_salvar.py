...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1210_detpgtofl_retpgtotot = get_object_or_404(s1210detPgtoFlretPgtoTot, id=pk)
        evento_dados = s1210_detpgtofl_retpgtotot.evento()
    ...
    if request.user.has_perm('s1210.can_see_s1210detPgtoFlretPgtoTot'):
        ...
        if request.method == 'POST':
            ...
            if s1210_detpgtofl_retpgtotot_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1210_detpgtofl_retpgtotot), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1210_detpgtofl_retpgtotot_apagar', 
                    's1210_detpgtofl_retpgtotot_salvar', 
                    's1210_detpgtofl_retpgtotot'):
        ...
        s1210_detpgtofl_retpgtotot_form = disabled_form_fields(
            s1210_detpgtofl_retpgtotot_form, 
            request.user.has_perm('s1210.change_s1210detPgtoFlretPgtoTot'))
        ...
        if tab or 's1210_detpgtofl_retpgtotot' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1210_detpgtofl_retpgtotot_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1210_detpgtofl_retpgtotot').all()
