...
def salvar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2410_infopenmorte')
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2410_infopenmorte_id:
            s2410_infopenmorte_form = form_s2410_infopenmorte(request.POST or None, instance = s2410_infopenmorte, slug = db_slug)
        else:
            s2410_infopenmorte_form = form_s2410_infopenmorte(request.POST or None, slug = db_slug, initial={})
            ...
            if s2410_infopenmorte_form.is_valid():
                ...
                if s2410_infopenmorte_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2410_infopenmorte_campos_multiple_passo1
                        s2410infoPenMorte.objects.using(db_slug).filter(id=s2410_infopenmorte_id).update(**dados)
                        obj = s2410infoPenMorte.objects.using(db_slug).get(id=s2410_infopenmorte_id)
                        #s2410_infopenmorte_editar_custom
                        #s2410_infopenmorte_campos_multiple_passo2
                    ...
                    #s2410_infopenmorte_cadastrar_campos_multiple_passo1
                    obj = s2410infoPenMorte(**dados)
                    obj.save(using = db_slug)
                    #s2410_infopenmorte_cadastrar_custom
                    #s2410_infopenmorte_cadastrar_campos_multiple_passo2
        ...
        for field in s2410_infopenmorte_form.fields.keys():
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2410_infopenmorte_id, tabela='s2410_infopenmorte').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                ...
                filename="s2410_infopenmorte.pdf",
                context=context,
...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2410_infopenmorte), indent=4, sort_keys=True, default=str)
            s2410infoPenMorte.objects.using( db_slug ).filter(id = s2410_infopenmorte_id).delete()
            #s2410_infopenmorte_apagar_custom
            #s2410_infopenmorte_apagar_custom
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2410_infopenmorte_lista = s2410infoPenMorte.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2410_infopenmorte_lista) > 100:
            filtrar = True
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2410_infopenmorte_listar.html',
                filename="s2410_infopenmorte.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2410_infopenmorte_listar.html', context)
            filename = "s2410_infopenmorte.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2410_infopenmorte_csv.html', context)
            filename = "s2410_infopenmorte.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
