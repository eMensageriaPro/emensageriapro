...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            s1080alteracaonovaValidade.objects.using( db_slug ).filter(id = s1080_alteracao_novavalidade_id).delete()
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s1080_alteracao_novavalidade_id:
            s1080_alteracao_novavalidade_form = form_s1080_alteracao_novavalidade(request.POST or None, instance = s1080_alteracao_novavalidade, slug = db_slug)
        else:
            s1080_alteracao_novavalidade_form = form_s1080_alteracao_novavalidade(request.POST or None, slug = db_slug, initial={})
        if request.method == 'POST':
            if s1080_alteracao_novavalidade_form.is_valid():
                ...
                if s1080_alteracao_novavalidade_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s1080_alteracao_novavalidade_campos_multiple_passo1
                        s1080alteracaonovaValidade.objects.using(db_slug).filter(id=s1080_alteracao_novavalidade_id).update(**dados)
                        obj = s1080alteracaonovaValidade.objects.using(db_slug).get(id=s1080_alteracao_novavalidade_id)
                        #s1080_alteracao_novavalidade_editar_custom
                        #s1080_alteracao_novavalidade_campos_multiple_passo2
                    ...
                    #s1080_alteracao_novavalidade_cadastrar_campos_multiple_passo1
                    obj = s1080alteracaonovaValidade(**dados)
                    obj.save(using = db_slug)
                    #s1080_alteracao_novavalidade_cadastrar_custom
                    #s1080_alteracao_novavalidade_cadastrar_campos_multiple_passo2
