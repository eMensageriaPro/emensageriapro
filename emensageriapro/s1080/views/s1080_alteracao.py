...
def apagar(request, hash):
    ...
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s1080_alteracao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's1080_alteracao', s1080_alteracao_id, usuario_id, 3)
        else:
...
class s1080alteracaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s1080alteracao.objects.using(db_slug).all()
    serializer_class = s1080alteracaoSerializer
    # permission_classes = (IsAdminUser,)


class s1080alteracaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s1080alteracao.objects.using(db_slug).all()
    serializer_class = s1080alteracaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s1080_alteracao_listar.html',
            ...
            response = render_to_response('s1080_alteracao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s1080_alteracao_csv.html', context)
            filename = "s1080_alteracao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s1080_alteracao_id:
            ...
            s1080_alteracao_form = form_s1080_alteracao(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s1080_alteracao_id, tabela='s1080_alteracao').all()
        context = {
