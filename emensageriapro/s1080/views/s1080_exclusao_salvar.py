...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1080_exclusao = get_object_or_404(s1080exclusao, id=pk)
        evento_dados = s1080_exclusao.evento()
    ...
    if request.user.has_perm('s1080.can_see_s1080exclusao'):
        ...
        if request.method == 'POST':
            ...
            if s1080_exclusao_form.is_valid():
                ...
                if request.session['return_page'] not in (
                    's1080_exclusao_apagar', 
                    's1080_exclusao_salvar', 
                    's1080_exclusao'):
        ...
        s1080_exclusao_form = disabled_form_fields(
            s1080_exclusao_form, 
            request.user.has_perm('s1080.change_s1080exclusao'))
        ...
        if tab or 's1080_exclusao' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1080_exclusao_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1080_exclusao').all()
