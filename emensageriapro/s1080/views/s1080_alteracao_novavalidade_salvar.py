...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s1080_alteracao_novavalidade = get_object_or_404(s1080alteracaonovaValidade, id=pk)
        evento_dados = s1080_alteracao_novavalidade.evento()

    if request.user.has_perm('s1080.can_see_s1080alteracaonovaValidade'):
        
        if pk:
        
            s1080_alteracao_novavalidade_form = form_s1080_alteracao_novavalidade(
                request.POST or None, 
                instance=s1080_alteracao_novavalidade)
                                         
        else:
        
            s1080_alteracao_novavalidade_form = form_s1080_alteracao_novavalidade(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s1080_alteracao_novavalidade_form.is_valid():
                ...
                if not pk:
                    ...
                    gravar_auditoria(
                        json.dumps(
                            model_to_dict(s1080_alteracao_novavalidade), 
                            indent=4, 
                            sort_keys=True, 
                            default=str),
                        json.dumps(
                ...
                if request.session['return_page'] not in (
                    's1080_alteracao_novavalidade_apagar', 
                    's1080_alteracao_novavalidade_salvar', 
                    's1080_alteracao_novavalidade'):
        ...
        s1080_alteracao_novavalidade_form = disabled_form_fields(
            s1080_alteracao_novavalidade_form, 
            request.user.has_perm('s1080.change_s1080alteracaonovaValidade'))
        ...
        if tab or 's1080_alteracao_novavalidade' in request.session['return_page']:
        
            request.session['return_pk'] = pk
            request.session['return_tab'] = tab
            request.session['return_page'] = 's1080_alteracao_novavalidade_salvar'
            
        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s1080_alteracao_novavalidade').all()
        ...
        context = {
            ...
            'modulos': ['s1080', ],
            'paginas': ['s1080_alteracao_novavalidade', ],
