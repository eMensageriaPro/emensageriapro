...
def apagar(request, hash):
    ...
    if s2205_exterior_id:
        dados_evento = s2205_exterior.evento()
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2205_exterior_apagar'] = 0
            dict_permissoes['s2205_exterior_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2205_exterior), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2205_exterior', s2205_exterior_id, usuario_id, 3)
        else:
...
class s2205exteriorList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2205exterior.objects.using(db_slug).all()
    serializer_class = s2205exteriorSerializer
    # permission_classes = (IsAdminUser,)


class s2205exteriorDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2205exterior.objects.using(db_slug).all()
    serializer_class = s2205exteriorSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2205_exterior_listar.html',
                filename="s2205_exterior.pdf",
                context=context,
            ...
            response = render_to_response('s2205_exterior_listar.html', context)
            filename = "s2205_exterior.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
        elif for_print == 4:
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2205_exterior_csv.html', context)
            filename = "s2205_exterior.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2205_exterior_id:
            s2205_exterior_form = form_s2205_exterior(request.POST or None, instance = s2205_exterior,
                                         slug = db_slug,
                                         initial={'excluido': False})
        else:
            s2205_exterior_form = form_s2205_exterior(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
            if s2205_exterior_form.is_valid():

                dados = s2205_exterior_form.cleaned_data
                obj = s2205_exterior_form.save(request=request)
                messages.success(request, u'Salvo com sucesso!')

                if not s2205_exterior_id:
                    gravar_auditoria('{}',
                                 json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                 's2205_exterior', obj.id, usuario_id, 1)
                else:

                    gravar_auditoria(json.dumps(model_to_dict(s2205_exterior), indent=4, sort_keys=True, default=str),
                                     json.dumps(model_to_dict(obj), indent=4, sort_keys=True, default=str),
                                     's2205_exterior', s2205_exterior_id, usuario_id, 2)
        ...
        if s2205_exterior_id:
            if dados_evento['status'] != 0:
                s2205_exterior_form = disabled_form_fields(s2205_exterior_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2205_exterior_id, tabela='s2205_exterior').all()
        context = {
        ...
        if for_print in (0,1 ):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2205_exterior_salvar.html',
                filename="s2205_exterior.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2205_exterior_salvar.html', context)
            filename = "s2205_exterior.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
