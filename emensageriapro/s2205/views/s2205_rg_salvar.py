...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2205_rg = get_object_or_404(s2205RG, id=pk)
        evento_dados = s2205_rg.evento()

    if request.user.has_perm('s2205.can_see_s2205RG'):
        ...
        if request.method == 'POST':
            ...
            if s2205_rg_form.is_valid():
                ...
                if 's2205-rg' not in request.session['return']:
        ...
        s2205_rg_form = disabled_form_fields(
            s2205_rg_form,
            request.user.has_perm('s2205.change_s2205RG'))
        ...
        #if tab or 's2205_rg' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2205_rg_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2205_rg').all()
