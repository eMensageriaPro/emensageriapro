...
class s2205contatoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2205contato.objects.using(db_slug).all()
    serializer_class = s2205contatoSerializer
    # permission_classes = (IsAdminUser,)


class s2205contatoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2205contato.objects.using(db_slug).all()
    serializer_class = s2205contatoSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2205_contato_id:
            ...
            s2205_contato_form = form_s2205_contato(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
