...
def salvar(request, pk=None, tab='master', output=None):
    ...
    if pk:
        ...
        s2205_ric = get_object_or_404(s2205RIC, id=pk)
        evento_dados = s2205_ric.evento()
    ...
    if request.user.has_perm('s2205.can_see_s2205RIC'):
        ...
        if pk:
            ...
            s2205_ric_form = form_s2205_ric(request.POST or None)
        ...
        if request.method == 'POST':
            ...
            if s2205_ric_form.is_valid():
                ...
                if 's2205-ric' not in request.session['return']:
        ...
        s2205_ric_form = disabled_form_fields(
            s2205_ric_form,
            request.user.has_perm('s2205.change_s2205RIC'))
        ...
        #if tab or 's2205_ric' in request.session['return_page']:
        #
        #    request.session['return_pk'] = pk
        #    request.session['return_tab'] = tab
        #    request.session['return_page'] = 's2205_ric_salvar'

        controle_alteracoes = Auditoria.objects.filter(identidade=pk, tabela='s2205_ric').all()
