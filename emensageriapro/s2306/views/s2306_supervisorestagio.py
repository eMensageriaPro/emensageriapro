...
class s2306supervisorEstagioList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2306supervisorEstagio.objects.using(db_slug).all()
    serializer_class = s2306supervisorEstagioSerializer
    # permission_classes = (IsAdminUser,)


class s2306supervisorEstagioDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2306supervisorEstagio.objects.using(db_slug).all()
    serializer_class = s2306supervisorEstagioSerializer
    # permission_classes = (IsAdminUser,)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        if s2306_supervisorestagio_id:
            ...
            s2306_supervisorestagio_form = form_s2306_supervisorestagio(request.POST or None, slug = db_slug,
                                         initial={'excluido': False})
        if request.method == 'POST':
