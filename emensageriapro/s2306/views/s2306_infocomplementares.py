...
def apagar(request, hash):
    ...
    pagina = ConfigPaginas.objects.using( db_slug ).get(excluido = False, endereco='s2306_infocomplementares')
    ...
    s2306_infocomplementares = get_object_or_404(s2306infoComplementares.objects.using( db_slug ), excluido = False, id = s2306_infocomplementares_id)
    dados_evento = {}
    ...
    if request.method == 'POST':
        if dados_evento['status'] == 0:
            ...
            situacao_anterior = json.dumps(model_to_dict(s2306_infocomplementares), indent=4, sort_keys=True, default=str)
            s2306infoComplementares.objects.using( db_slug ).filter(id = s2306_infocomplementares_id).delete()
            #s2306_infocomplementares_apagar_custom
            #s2306_infocomplementares_apagar_custom
            messages.success(request, 'Apagado com sucesso!')
            gravar_auditoria(situacao_anterior,
                             '',
                             's2306_infocomplementares', s2306_infocomplementares_id, usuario_id, 3)
        else:
            messages.error(request, 'Não foi possivel apagar o evento, somente é possível apagar os eventos com status "Cadastrado"!')

        if request.session['retorno_pagina']== 's2306_infocomplementares_salvar':
            return redirect('s2306_infocomplementares', hash=request.session['retorno_hash'])
        else:
    ...
    return render(request, 's2306_infocomplementares_apagar.html', context)
...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        mensagem = None
        if s2306_infocomplementares_id:
            s2306_infocomplementares_form = form_s2306_infocomplementares(request.POST or None, instance = s2306_infocomplementares, slug = db_slug)
        else:
            s2306_infocomplementares_form = form_s2306_infocomplementares(request.POST or None, slug = db_slug, initial={'s2306_evttsvaltcontr': '1'})
        if request.method == 'POST':
            if s2306_infocomplementares_form.is_valid():
                ...
                if s2306_infocomplementares_id:
                    if dados_evento['status'] == 0:
                        ...
                        #s2306_infocomplementares_campos_multiple_passo1
                        s2306infoComplementares.objects.using(db_slug).filter(id=s2306_infocomplementares_id).update(**dados)
                        obj = s2306infoComplementares.objects.using(db_slug).get(id=s2306_infocomplementares_id)
                        #s2306_infocomplementares_editar_custom
                        #s2306_infocomplementares_campos_multiple_passo2
                    ...
                    #s2306_infocomplementares_cadastrar_campos_multiple_passo1
                    obj = s2306infoComplementares(**dados)
                    obj.save(using = db_slug)
                    #s2306_infocomplementares_cadastrar_custom
                    #s2306_infocomplementares_cadastrar_campos_multiple_passo2
        ...
        for field in s2306_infocomplementares_form.fields.keys():
            s2306_infocomplementares_form.fields[field].widget.attrs['ng-model'] = 's2306_infocomplementares_'+field
        ...
        if s2306_infocomplementares_id:
            ...
            s2306_cargofuncao_form = form_s2306_cargofuncao(initial={ 's2306_infocomplementares': s2306_infocomplementares , 's2306_infocomplementares': '1'}, slug=db_slug)
            s2306_cargofuncao_form.fields['s2306_infocomplementares'].widget.attrs['readonly'] = True
            s2306_cargofuncao_lista = s2306cargoFuncao.objects.using( db_slug ).filter(excluido = False, s2306_infocomplementares_id=s2306_infocomplementares.id).all()
            s2306_remuneracao_form = form_s2306_remuneracao(initial={ 's2306_infocomplementares': s2306_infocomplementares , 's2306_infocomplementares': '1'}, slug=db_slug)
            ...
            s2306_infotrabcedido_form = form_s2306_infotrabcedido(initial={ 's2306_infocomplementares': s2306_infocomplementares , 's2306_infocomplementares': '1'}, slug=db_slug)
            ...
            s2306_infoestagiario_form = form_s2306_infoestagiario(initial={ 's2306_infocomplementares': s2306_infocomplementares , 's2306_infocomplementares': '1', 'instensino': '1'}, slug=db_slug)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2306_infocomplementares_id, tabela='s2306_infocomplementares').all()
        context = {
...
class s2306infoComplementaresList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2306infoComplementares.objects.using(db_slug).all()
    serializer_class = s2306infoComplementaresSerializer
    permission_classes = (IsAdminUser,)


class s2306infoComplementaresDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2306infoComplementares.objects.using(db_slug).all()
    serializer_class = s2306infoComplementaresSerializer
    permission_classes = (IsAdminUser,)
...
def listar(request, hash):
    ...
    if permissao.permite_listar:
        ...
        s2306_infocomplementares_lista = s2306infoComplementares.objects.using( db_slug ).filter(**dict_qs).filter(excluido = False).exclude(id=0).all()
        if not post and len(s2306_infocomplementares_lista) > 100:
            filtrar = True
        ...
        request.session["retorno_hash"] = hash
        request.session["retorno_pagina"] = 's2306_infocomplementares'
        context = {
        ...
        if for_print in (0,1):
            return render(request, 's2306_infocomplementares_listar.html', context)
        elif for_print == 2:
            #return render_to_pdf('tables/s1000_evtinfoempregador_pdf_xls.html', context)
            from wkhtmltopdf.views import PDFTemplateResponse
            response = PDFTemplateResponse(
                request=request,
                template='s2306_infocomplementares_listar.html',
                filename="s2306_infocomplementares.pdf",
                context=context,
                show_content_in_browser=True,
                cmd_options={'margin-top': 10,
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('s2306_infocomplementares_listar.html', context)
            filename = "s2306_infocomplementares.xls"
            response['Content-Disposition'] = 'attachment; filename=' + filename
            response['Content-Type'] = 'application/vnd.ms-excel; charset=UTF-8'
            return response
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2306_infocomplementares_csv.html', context)
            filename = "s2306_infocomplementares.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
