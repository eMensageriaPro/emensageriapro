...
def apagar(request, hash):
    ...
    if s2306_cargofuncao_id:
        ...
        if dados_evento['status'] != STATUS_EVENTO_CADASTRADO:
            dict_permissoes['s2306_cargofuncao_apagar'] = 0
            dict_permissoes['s2306_cargofuncao_editar'] = 0
    if request.method == 'POST':
        if dados_evento['status'] == STATUS_EVENTO_CADASTRADO:
            import json
            from django.forms.models import model_to_dict
            situacao_anterior = json.dumps(model_to_dict(s2306_cargofuncao), indent=4, sort_keys=True, default=str)
            ...
            gravar_auditoria(situacao_anterior,
                             '',
                             's2306_cargofuncao', s2306_cargofuncao_id, usuario_id, 3)
        else:
...
class s2306cargoFuncaoList(generics.ListCreateAPIView):
    db_slug = 'default'
    queryset = s2306cargoFuncao.objects.using(db_slug).all()
    serializer_class = s2306cargoFuncaoSerializer
    # permission_classes = (IsAdminUser,)


class s2306cargoFuncaoDetail(generics.RetrieveUpdateDestroyAPIView):
    db_slug = 'default'
    queryset = s2306cargoFuncao.objects.using(db_slug).all()
    serializer_class = s2306cargoFuncaoSerializer
    # permission_classes = (IsAdminUser,)
    ...
    if permissao.permite_listar:
        ...
        if for_print in (0,1):
            ...
            response = PDFTemplateResponse(
                request=request,
                template='s2306_cargofuncao_listar.html',
                filename="s2306_cargofuncao.pdf",
                context=context,
            ...
            response = render_to_response('s2306_cargofuncao_listar.html', context)
            ...
            from django.shortcuts import render_to_response
            response = render_to_response('tables/s2306_cargofuncao_csv.html', context)
            filename = "s2306_cargofuncao.csv"
            response['Content-Disposition'] = 'attachment; filename=' + filename
    ...
    if permissao.permite_visualizar:
        ...
        if s2306_cargofuncao_id:
            if dados_evento['status'] != 0:
                s2306_cargofuncao_form = disabled_form_fields(s2306_cargofuncao_form, 0)
        ...
        controle_alteracoes = Auditoria.objects.using( db_slug ).filter(identidade=s2306_cargofuncao_id, tabela='s2306_cargofuncao').all()
        context = {
